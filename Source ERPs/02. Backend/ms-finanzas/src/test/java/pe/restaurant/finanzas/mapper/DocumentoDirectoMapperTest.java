...
class DocumentoDirectoMapperTest {
    ...
    void setUp() {
        ...
        entityFull.setAno(2026);
        entityFull.setMes(5);
        entityFull.setCntblLibroId(1L);
        entityFull.setFechaRegistro(LocalDate.of(2026, 5, 1));
        ...
        requestFull.setAno(2026);
        requestFull.setMes(5);
        requestFull.setCntblLibroId(1L);
        requestFull.setFecRegistro(LocalDate.of(2026, 5, 1));
    }
    ...
    void toEntity_conRequestValido_retornaEntity() {
        ...
        assertThat(result.getTotal()).isEqualTo(new BigDecimal("500.00"));
        assertThat(result.getSaldo()).isEqualTo(new BigDecimal("500.00"));
        assertThat(result.getAno()).isEqualTo(2026);
        assertThat(result.getMes()).isEqualTo(5);
        assertThat(result.getCntblLibroId()).isEqualTo(1L);
        assertThat(result.getFechaRegistro()).isEqualTo(LocalDate.of(2026, 5, 1));
        assertThat(result.getCntblAsientoId()).isNull();
        assertThat(result.getTasaCambio()).isEqualTo(BigDecimal.ONE);
        assertThat(result.getFlagEstado()).isEqualTo("1");
    }

    @Test
    @DisplayName("toEntity() con tasaCambio explicito -> la usa")
    void toEntity_conTasaCambioExplicito_usaValor() {
        requestFull.setTasaCambio(new BigDecimal("3.8000"));

        CntasPagar result = mapper.toEntity(requestFull, 1L);

        assertThat(result.getTasaCambio()).isEqualTo(new BigDecimal("3.8000"));
    }

    @Test
    @DisplayName("toEntity() con tasaCambio null -> usa BigDecimal.ONE")
    void toEntity_conTasaCambioNull_usaDefault() {
        requestFull.setTasaCambio(null);
    ...
    @Test
    @DisplayName("toEntity() sin fecRegistro en request -> usa LocalDate.now()")
    void toEntity_sinFecRegistro_usaFechaActual() {
        requestFull.setFecRegistro(null);
