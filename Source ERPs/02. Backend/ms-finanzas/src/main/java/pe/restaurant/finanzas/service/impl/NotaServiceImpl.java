...
import pe.restaurant.finanzas.service.NotaService;
...
public class NotaServiceImpl implements NotaService {
    ...
    public NotaResponse crearNota(NotaRequest request) {
        ...
        // 6.2: Ajustar stock si es nota de crédito con OC vinculada
        if (TipoNota.CREDITO.equals(request.getTipoNota())) {
            for (NotaDetalleRequest detalle : request.getDetalles()) {
                if (detalle.getOrdenCompraDetId() != null) {
                    try {
                        AjusteStockRequest ajusteReq = new AjusteStockRequest(
                                detalle.getOrdenCompraDetId(),
                                detalle.getCantidad(),
                                "SALIDA", // NC = devolución = sale del stock
                                null);
                        comprasClient.ajustarStockPorNota(ajusteReq);
                        log.info("Stock ajustado por NC - OC detalle: {}, cantidad: {}",
                                detalle.getOrdenCompraDetId(), detalle.getCantidad());
                    } catch (Exception e) {
                        log.warn("No se pudo ajustar stock por NC: {}", e.getMessage());
                    }
                }
            }
        }
    ...
    /**
     * Convierte una nota a CntasPagarAsientoRequest para el endpoint de ms-contabilidad.
     * El asiento se genera automáticamente en ms-contabilidad basado en los datos de la nota.
     * 
     * @param cxp Cuenta por pagar ya creada
     * @param request Request original de la nota
     * @return CntasPagarAsientoRequest para enviar a ms-contabilidad
     */
