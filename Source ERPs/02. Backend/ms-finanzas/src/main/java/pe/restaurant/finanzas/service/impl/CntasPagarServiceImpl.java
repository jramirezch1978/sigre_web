...
import pe.restaurant.finanzas.client.ComprasClient;
...
public class CntasPagarServiceImpl implements CntasPagarService {
    ...
    public CntasPagar crear(CntasPagarRequest request) {
        ...
        try {
            ...
            // T6: Actualizar cant_facturada en OC si hay líneas vinculadas
            actualizarCantFacturadaOc(request);
    ...
    /**
     * Actualiza cant_facturada en las líneas de OC vinculadas a los detalles de esta CxP.
     * Si ningún detalle tiene ordenCompraDetId, no hace nada.
     */
    private void actualizarCantFacturadaOc(CntasPagarRequest request) {
        List<ActualizarCantFacturadaRequest.LineaCantFacturada> lineas = request.getDetalles().stream()
                .filter(d -> d.getOrdenCompraDetId() != null)
                .map(d -> new ActualizarCantFacturadaRequest.LineaCantFacturada(
                        d.getOrdenCompraDetId(), d.getCantidad()))
                .toList();

        if (lineas.isEmpty()) {
            log.debug("Ningún detalle de CxP tiene ordenCompraDetId, se omite actualización de cant_facturada");
            return;
        }

        try {
            var facturaRequest = new ActualizarCantFacturadaRequest(lineas);
            comprasClient.actualizarCantFacturada(facturaRequest);
            log.info("cant_facturada actualizada en {} líneas de OC", lineas.size());
        } catch (Exception e) {
            log.warn("No se pudo actualizar cant_facturada en OC: {}", e.getMessage());
            // No propagamos la excepción — la CxP ya está creada y el asiento generado.
            // La actualización de cant_facturada es informativa para validación 3 vías.
        }
    }
    ...
    public PendientesPagarResponse listarPendientesPorPagarAgrupado(
        ...
        List<CntasPagar> cuentasPagar = repository.findPendientesPorPagar(sucursalId, proveedorId, null, fechaDesde, fechaHasta);
    ...
    public PendientesPagarSimpleResponse listarPendientesPorPagarSimple(
            Long sucursalId, Long proveedorId, Long docTipoId, String tipoDocumento, LocalDate fechaDesde, LocalDate fechaHasta) {
        
        log.info("Listando pendientes por pagar simple - sucursal: {}, proveedor: {}, docTipoId: {}, tipoDocumento: {}, fechas: {} - {}", 
        ...
        boolean filtrar = tipoDocumento != null && !tipoDocumento.isEmpty();
        if (filtrar) {
            try {
                PendientesPagarSimpleResponse.TipoDocumentoPagar.valueOf(tipoDocumento.toUpperCase());
            } catch (IllegalArgumentException e) {
                throw new BusinessException(
                    "tipoDocumento inválido. Valores permitidos: CUENTA_PAGAR, ORDEN_GIRO, LIQUIDACION, RETENCION, DETRACCION",
                    HttpStatus.BAD_REQUEST, "FIN-VALIDACION");
            }
        }

        // Agregar cuentas por pagar
        if (!filtrar || "CUENTA_PAGAR".equalsIgnoreCase(tipoDocumento)) {
            List<CntasPagar> cuentasPagar = repository.findPendientesPorPagar(sucursalId, proveedorId, docTipoId, fechaDesde, fechaHasta);
            items.addAll(mapearCuentasPagarSimple(cuentasPagar));
        }

        // Agregar órdenes de giro
        if (!filtrar || "ORDEN_GIRO".equalsIgnoreCase(tipoDocumento)) {
            List<SolicitudGiro> ordenesGiro = solicitudGiroRepository.findPendientesPorPagar(sucursalId, null, fechaDesde, fechaHasta);
            items.addAll(mapearOrdenesGiroSimple(ordenesGiro));
        }

        // Agregar liquidaciones
        if (!filtrar || "LIQUIDACION".equalsIgnoreCase(tipoDocumento)) {
            List<Liquidacion> liquidaciones = liquidacionRepository.findPendientesPorPagar(sucursalId, proveedorId, fechaDesde, fechaHasta);
            items.addAll(mapearLiquidacionesSimple(liquidaciones));
        }

        // Agregar retenciones
        if (!filtrar || "RETENCION".equalsIgnoreCase(tipoDocumento)) {
            List<Retencion> retenciones = retencionRepository.findPendientes(sucursalId, proveedorId, fechaDesde, fechaHasta);
            items.addAll(mapearRetencionesSimple(retenciones));
        }

        // Agregar detracciones
        if (!filtrar || "DETRACCION".equalsIgnoreCase(tipoDocumento)) {
            List<Detraccion> detracciones = detraccionRepository.findPendientes(fechaDesde, fechaHasta);
