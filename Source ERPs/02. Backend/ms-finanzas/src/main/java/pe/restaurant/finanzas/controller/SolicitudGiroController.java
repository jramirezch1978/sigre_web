...
public class SolicitudGiroController {
    ...
    @PostMapping("/aprobar-lote")
    @Operation(summary = "Aprobar solicitudes en bloque",
               description = "Aprueba múltiples solicitudes de giro en una sola operación. Retorna resultado por cada ID.")
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Procesadas todas las solicitudes"),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "Error de validación")
    })
    public ApiResponse<List<BatchResultItem>> aprobarEnBloque(
            @Valid @RequestBody AprobacionEnBloqueRequest request) {
        List<BatchResultItem> resultados = service.aprobarEnBloque(request);
        return ApiResponse.ok(resultados, "Solicitudes procesadas");
    }
