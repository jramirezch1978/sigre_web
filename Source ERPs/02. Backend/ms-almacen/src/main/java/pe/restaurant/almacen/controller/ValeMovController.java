...
public class ValeMovController {
    ...
    /**
     * Endpoint contable (Ruta B): crea el movimiento y genera el pre-asiento en ms-contabilidad
     * por concepto financiero. Paralelo a {@link #crear}; no afecta el flujo legacy por matriz/subcategoría.
     */
    @PostMapping("/contable")
    @ResponseStatus(HttpStatus.CREATED)
    public ApiResponse<MovimientoDetalleResponse> crearContable(
            @Valid @RequestBody MovimientoContableCabeceraRequest request) {
        return ApiResponse.ok(valeMovService.crearContable(request), "Movimiento registrado y contabilizado");
    }
