...
public class PlanContableDetController {
    ...
    public ApiResponse<List<PlanContableDetResponse>> findAll(
        ...
        if (planContableId == null) {
            throw new BusinessException(
                    "El parámetro planContableId es obligatorio para listar cuentas contables",
                    HttpStatus.BAD_REQUEST,
                    ContabilidadErrorCodes.PLAN_CONTABLE_NO_ENCONTRADO
            );
        }
        return ApiResponse.ok(mapper.toResponseList(service.findAllActivos(planContableId, q, flagEstado)));
    }
