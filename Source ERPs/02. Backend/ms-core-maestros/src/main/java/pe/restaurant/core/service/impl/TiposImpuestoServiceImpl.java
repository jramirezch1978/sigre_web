...
public class TiposImpuestoServiceImpl implements TiposImpuestoService {
    ...
    /**
     * Valida que el plan_contable_det_id exista y esté activo.
     * Se consulta vía Feign a ms-contabilidad en lugar de SQL directo
     * para evitar acoplamiento al schema contabilidad.
     */
    private void validarPlanContableDetFk(Long planContableDetId) {
        ...
        try {
            var response = contabilidadClient.obtenerPlanContableDet(planContableDetId);
            ...
            if (!"1".equals(response.getData().getFlagEstado())) {
                throw new BusinessException(
            ...
            throw new BusinessException(
                    "Error al validar cuenta contable: " + e.getMessage(),
                    HttpStatus.SERVICE_UNAVAILABLE,
                    "CONTABILIDAD_NO_DISPONIBLE");
        }
    }
}
