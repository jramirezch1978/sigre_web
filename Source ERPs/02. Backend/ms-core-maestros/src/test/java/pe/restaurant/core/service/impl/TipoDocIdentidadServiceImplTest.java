...
class TipoDocIdentidadServiceImplTest {
    ...
    private static TipoDocIdentidad nuevoTipoDoc(String codigo, String nombre) {
    ...
    @BeforeEach
    void setUp() {
        tipoDoc = nuevoTipoDoc("DNI", "Documento Nacional de Identidad");
        tipoDoc.setId(1L);
    ...
    @Nested class Create {
        ...
        @Test void duplicateCodigoThrowsConflict() {
            TipoDocIdentidad existing = nuevoTipoDoc("DNI", "Otro");
            existing.setId(2L);
    ...
    @Nested class Update {
        ...
        @Test void duplicateCodigoInOtherRecordThrowsConflict() {
            TipoDocIdentidad other = nuevoTipoDoc("RUC", "RUC");
            other.setId(2L);
            when(repository.findById(1L)).thenReturn(Optional.of(tipoDoc));
            when(repository.findByCodigo("RUC")).thenReturn(Optional.of(other));
            TipoDocIdentidad upd = nuevoTipoDoc("RUC", "RUC update");
        ...
        @Test void codigoNotFoundInRepoSucceeds() {
            ...
            TipoDocIdentidad upd = nuevoTipoDoc("NEW", "Nuevo tipo");
        ...
        @Test void copiesFieldsFromEntity() {
            ...
            when(repository.save(any())).thenAnswer(inv -> inv.getArgument(0));
            TipoDocIdentidad upd = nuevoTipoDoc("CE", "Carnet Extranjeria");
            var result = service.update(1L, upd);
