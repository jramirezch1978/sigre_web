...
public class ActivosTestDataFactory {
    ...
    private static int ensureAfVentaFactory(JdbcTemplate jdbc) {
        ...
        Long cntasCobrarId = null;
        if (tableExists(jdbc, "ventas", "cntas_cobrar")) {
            cntasCobrarId = jdbc.query(
                    "SELECT id FROM ventas.cntas_cobrar WHERE flag_estado = '1' ORDER BY id LIMIT 1",
                    rs -> rs.next() ? rs.getLong(1) : null);
        }
        if (cntasCobrarId == null) {
            return 0;
        }
        return jdbc.update("""
                INSERT INTO activos.af_venta (af_maestro_id, cntas_cobrar_id, fecha_baja, motivo, valor_venta, comprador, flag_estado, created_by)
                ...
                """, maestro3, cntasCobrarId, FACTORY_USER_ID, maestro3);
    }
