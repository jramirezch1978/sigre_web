...
public class ProgramacionProduccionServiceImpl implements ProgramacionProduccionService {
    ...
    private static final Set<String> FRECUENCIAS_VALIDAS = Set.of("D", "S", "Q", "M", "A");

    @Override
    @Timed(value = "app.db.query", extraTags = {"table", "programacion_produccion", "operation", "findAll"})
    public Page<ProgramacionProduccion> findAll(Long recetaId, Long sucursalId, String flagFrecuencia,
                                                 LocalDate fechaDesde, LocalDate fechaHasta,
                                                 String flagEstado, Pageable pageable) {
        Specification<ProgramacionProduccion> spec = buildSpecification(
                recetaId, sucursalId, flagFrecuencia, fechaDesde, fechaHasta, flagEstado);
    ...
    public ProgramacionProduccion create(ProgramacionProduccion entity) {
        ...
        validarFrecuencia(entity.getFlagFrecuencia());
        validarOT(entity.getOrdenTrabajoId());
        if (entity.getSucursalId() != null) {
            validarSucursal(entity.getSucursalId());
        }
        entity.setFlagFrecuencia(entity.getFlagFrecuencia().trim().toUpperCase());
    ...
    public ProgramacionProduccion update(Long id, ProgramacionProduccion entity) {
        ...
        existing.setFlagFrecuencia(entity.getFlagFrecuencia().trim().toUpperCase());
        ...
        existing.setFlagTurno(entity.getFlagTurno());
    ...
    private void validarFrecuencia(String flagFrecuencia) {
        if (flagFrecuencia == null || flagFrecuencia.isBlank()) return;
        String normalizada = flagFrecuencia.trim().toUpperCase();
        if (!FRECUENCIAS_VALIDAS.contains(normalizada)) {
            throw new BusinessException("Frecuencia inválida: " + flagFrecuencia + ". Valores permitidos: D=Diaria, S=Semanal, Q=Quincenal, M=Mensual, A=Anual",
    ...
    private Specification<ProgramacionProduccion> buildSpecification(Long recetaId, Long sucursalId,
        ...
        return (root, query, cb) -> {
            ...
            if (flagFrecuencia != null && !flagFrecuencia.isBlank()) {
                predicates.add(cb.equal(cb.upper(root.get("flagFrecuencia")), flagFrecuencia.toUpperCase()));
            }
            if (fechaDesde != null) {
