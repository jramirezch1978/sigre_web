...
public class ProduccionTestDataFactory {
    ...
    public Map<String, Object> statusData() {
        ...
        out.put("programacionesExistentes", jdbc.queryForList(
                "SELECT id, receta_id, flag_frecuencia, flag_estado FROM produccion.programacion_produccion ORDER BY id"));
        ...
        out.put("controlesCalidad", jdbc.queryForList(
                "SELECT id, orden_trabajo_id, flag_estado AS resultado FROM produccion.control_calidad ORDER BY id"));
    ...
    private Map<String, Object> ensureProgramacion(JdbcTemplate jdbc) {
        ...
        return Map.of("id", progId, "recetaId", recetaId, "ordenTrabajoId", ordenTrabajoId, "flagFrecuencia", "D");
    }
    ...
    private Map<String, Object> ensureControlCalidad(JdbcTemplate jdbc) {
        ...
        Long id = jdbc.queryForObject(
                "INSERT INTO produccion.control_calidad (orden_trabajo_id, inspector_id, fecha, flag_estado," +
                        " observaciones, created_by, fec_creacion)" +
                        " VALUES (?, 1, ?, '1', 'Control de calidad de prueba - producto conforme'," +
                        " 1, NOW()) RETURNING id",
                Long.class, otId, LocalDate.now());
        return Map.of("id", id, "ordenTrabajoId", otId, "resultado", "1");
    }
