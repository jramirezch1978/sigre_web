...
public class ControlCalidadServiceImpl implements ControlCalidadService {
    ...
    private static final Set<String> RESULTADOS_VALIDOS = Set.of("1", "0", "9");
    ...
    public Page<ControlCalidad> findAll(Long ordenTrabajoId, String flagEstado, LocalDate fechaDesde,
                                        LocalDate fechaHasta, Long inspectorId, Pageable pageable) {
        Specification<ControlCalidad> spec = buildSpecification(ordenTrabajoId, flagEstado, fechaDesde, fechaHasta, inspectorId);
    ...
    public ControlCalidad create(ControlCalidad entity) {
        ...
        validarFKs(entity);
        validarResultado(entity.getFlagEstado());
    ...
    private void validarDatosObligatorios(ControlCalidad entity) {
        if (entity.getFecha() == null || entity.getFlagEstado() == null || entity.getFlagEstado().isBlank()) {
    ...
    private void validarResultado(String flagEstado) {
        if (flagEstado != null && !RESULTADOS_VALIDOS.contains(flagEstado)) {
            throw new BusinessException(
                    "El resultado debe ser uno de: " + String.join(", ", RESULTADOS_VALIDOS) + " (1=Aprobado, 0=Anulado, 9=Observado)",
                    HttpStatus.UNPROCESSABLE_ENTITY, ProduccionErrorCodes.CONTROL_CALIDAD_RESULTADO_INVALIDO);
        }
    }

    private Specification<ControlCalidad> buildSpecification(Long ordenTrabajoId, String flagEstado,
