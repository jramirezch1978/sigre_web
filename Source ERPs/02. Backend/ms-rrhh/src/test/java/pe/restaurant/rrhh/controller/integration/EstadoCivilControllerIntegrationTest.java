...
@DisplayName("Pruebas de Integración - EstadoCivilController")
class EstadoCivilControllerIntegrationTest {
    ...
    void flujoCRUD_completo() throws Exception {
        ...
        String response = mockMvc.perform(post("/api/rrhh/estados-civiles")
                        .header("Authorization", authToken)
                ...
                .andExpect(jsonPath("$.data.nombre").value("Estado Test"))
                .andReturn().getResponse().getContentAsString();

        Long id = objectMapper.readTree(response).get("data").get("id").asLong();

        mockMvc.perform(get("/api/rrhh/estados-civiles/{id}", id)
                        .header("Authorization", authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
        ...
        String actualizarJson = """
                {
                    "nombre": "Estado Test Actualizado"
                }
                """;

        mockMvc.perform(put("/api/rrhh/estados-civiles/{id}", id)
                        .header("Authorization", authToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(actualizarJson))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.nombre").value("Estado Test Actualizado"));

        mockMvc.perform(patch("/api/rrhh/estados-civiles/{id}/desactivar", id)
                        .header("Authorization", authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
    }
    ...
    @Test
    @DisplayName("POST /api/rrhh/estados-civiles con código duplicado -> 409")
    void crear_codigoDuplicado_retorna409() throws Exception {
        ...
        mockMvc.perform(post("/api/rrhh/estados-civiles")
                ...
                .andExpect(jsonPath("$.errorCode").value("RH-EC-001"));
    }

    @Test
    @DisplayName("GET /api/rrhh/estados-civiles/9999 -> 404")
    void obtenerPorId_idInexistente_retorna404() throws Exception {
        mockMvc.perform(get("/api/rrhh/estados-civiles/{id}", 9999)
                        .header("Authorization", authToken))
    ...
    @Test
    @DisplayName("PUT /api/rrhh/estados-civiles/9999 -> 404")
    void actualizar_idInexistente_retorna404() throws Exception {
        ...
        mockMvc.perform(put("/api/rrhh/estados-civiles/{id}", 9999)
                        .header("Authorization", authToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(json))
                .andExpect(status().isNotFound())
    ...
    @Test
    @DisplayName("DELETE /api/rrhh/estados-civiles/9999 -> 404")
    void eliminar_idInexistente_retorna404() throws Exception {
        mockMvc.perform(patch("/api/rrhh/estados-civiles/{id}/desactivar", 9999)
                        .header("Authorization", authToken))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.success").value(false));
    }
