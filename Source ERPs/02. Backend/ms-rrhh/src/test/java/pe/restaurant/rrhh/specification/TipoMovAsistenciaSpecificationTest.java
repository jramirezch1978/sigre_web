...
@DisplayName("TipoMovAsistenciaSpecification - Pruebas Unitarias")
class TipoMovAsistenciaSpecificationTest {

    @Mock private CriteriaBuilder cb;
    @Mock private CriteriaQuery<?> query;
    @Mock private Root<TipoMovAsistencia> root;

    @Test void filtros_todosNull_cbAndVacio() {
        Specification<TipoMovAsistencia> spec = TipoMovAsistenciaSpecification.filtros(null, null, null);
    ...
    @Test void filtros_conCodigo_cbLike() {
        Specification<TipoMovAsistencia> spec = TipoMovAsistenciaSpecification.filtros("COD", null, null);
    ...
    @Test void filtros_conNombre_cbLike() {
        Specification<TipoMovAsistencia> spec = TipoMovAsistenciaSpecification.filtros(null, "Asistencia", null);
        when(cb.lower(any())).thenReturn(mock(Expression.class));
        when(cb.like(any(), anyString())).thenReturn(mock(Predicate.class));
        when(cb.and(any(Predicate[].class))).thenReturn(mock(Predicate.class));
        assertThat(spec.toPredicate(root, query, cb)).isNotNull();
        verify(cb).lower(root.get("nombre"));
        verify(cb).like(any(), eq("%asistencia%"));
    }

    @Test void filtros_conFlagEstado_cbEqual() {
        Specification<TipoMovAsistencia> spec = TipoMovAsistenciaSpecification.filtros(null, null, "1");
    ...
    @Test void filtros_conTodos_todosPredicados() {
        Specification<TipoMovAsistencia> spec = TipoMovAsistenciaSpecification.filtros("COD", "Asistencia", "1");
        when(cb.lower(any())).thenReturn(mock(Expression.class));
        when(cb.like(any(), anyString())).thenReturn(mock(Predicate.class));
        when(cb.equal(root.get("flagEstado"), "1")).thenReturn(mock(Predicate.class));
        when(cb.and(any(Predicate[].class))).thenReturn(mock(Predicate.class));
    ...
    @Test void filtros_conFlagEstadoVacio_noAgrega() {
        Specification<TipoMovAsistencia> spec = TipoMovAsistenciaSpecification.filtros(null, null, "");
        when(cb.and(any(Predicate[].class))).thenReturn(mock(Predicate.class));
        assertThat(spec.toPredicate(root, query, cb)).isNotNull();
        verify(cb).and(any(Predicate[].class));
    }
