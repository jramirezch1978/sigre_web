...
@DisplayName("Pruebas de Integración - TipoConceptoCalculoController")
class TipoConceptoCalculoControllerIntegrationTest {
    ...
    void flujoCRUD_completo() throws Exception {
        String crearJson = """
                {
                    "codigo": "X",
                    "nombre": "TipoConceptoCalculo Test"
                }
                """;

        String response = mockMvc.perform(post("/api/rrhh/tipos-concepto-calculo")
                        .header("Authorization", authToken)
                ...
                .andExpect(jsonPath("$.data.nombre").value("TipoConceptoCalculo Test"))
                .andReturn().getResponse().getContentAsString();

        Long id = objectMapper.readTree(response).get("data").get("id").asLong();

        mockMvc.perform(get("/api/rrhh/tipos-concepto-calculo/{id}", id)
                        .header("Authorization", authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
        ...
        String actualizarJson = """
                {
                    "nombre": "TipoConceptoCalculo Test Actualizado"
                }
                """;

        mockMvc.perform(put("/api/rrhh/tipos-concepto-calculo/{id}", id)
                        .header("Authorization", authToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(actualizarJson))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.nombre").value("TipoConceptoCalculo Test Actualizado"));

        mockMvc.perform(patch("/api/rrhh/tipos-concepto-calculo/{id}/desactivar", id)
                        .header("Authorization", authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
    }
    ...
    @Test
    @DisplayName("POST /api/rrhh/tipos-concepto-calculo con código duplicado -> 409")
    void crear_codigoDuplicado_retorna409() throws Exception {
        String json = """
                {
                    "codigo": "DUP",
                    "nombre": "TipoConceptoCalculo Duplicado"
                }
                """;
        ...
        mockMvc.perform(post("/api/rrhh/tipos-concepto-calculo")
                ...
                .andExpect(jsonPath("$.errorCode").value("RH-TCC-001"));
    }

    @Test
    @DisplayName("GET /api/rrhh/tipos-concepto-calculo/9999 -> 404")
    void obtenerPorId_idInexistente_retorna404() throws Exception {
        mockMvc.perform(get("/api/rrhh/tipos-concepto-calculo/{id}", 9999)
                        .header("Authorization", authToken))
    ...
    @Test
    @DisplayName("PUT /api/rrhh/tipos-concepto-calculo/9999 -> 404")
    void actualizar_idInexistente_retorna404() throws Exception {
        ...
        mockMvc.perform(put("/api/rrhh/tipos-concepto-calculo/{id}", 9999)
                        .header("Authorization", authToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(json))
                .andExpect(status().isNotFound())
    ...
    @Test
    @DisplayName("DELETE /api/rrhh/tipos-concepto-calculo/9999 -> 404")
    void eliminar_idInexistente_retorna404() throws Exception {
        mockMvc.perform(patch("/api/rrhh/tipos-concepto-calculo/{id}/desactivar", 9999)
                        .header("Authorization", authToken))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.success").value(false));
    }
