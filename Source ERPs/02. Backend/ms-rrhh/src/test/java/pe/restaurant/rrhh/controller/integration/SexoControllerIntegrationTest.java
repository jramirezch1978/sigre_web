...
@DisplayName("Pruebas de Integración - SexoController")
class SexoControllerIntegrationTest {
    ...
    void flujoCRUD_completo() throws Exception {
        ...
        String response = mockMvc.perform(post("/api/rrhh/sexos")
                        .header("Authorization", authToken)
                ...
                .andExpect(jsonPath("$.data.nombre").value("Sexo Test"))
                .andReturn().getResponse().getContentAsString();

        Long id = objectMapper.readTree(response).get("data").get("id").asLong();

        mockMvc.perform(get("/api/rrhh/sexos/{id}", id)
                        .header("Authorization", authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
        ...
        String actualizarJson = """
                {
                    "nombre": "Sexo Test Actualizado"
                }
                """;

        mockMvc.perform(put("/api/rrhh/sexos/{id}", id)
                        .header("Authorization", authToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(actualizarJson))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.nombre").value("Sexo Test Actualizado"));

        mockMvc.perform(patch("/api/rrhh/sexos/{id}/desactivar", id)
                        .header("Authorization", authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
    }
    ...
    @Test
    @DisplayName("POST /api/rrhh/sexos con código duplicado -> 409")
    void crear_codigoDuplicado_retorna409() throws Exception {
        String json = """
                {
                    "codigo": "DUP",
                    "nombre": "Sexo Duplicado"
                }
                """;
        ...
        mockMvc.perform(post("/api/rrhh/sexos")
                ...
                .andExpect(jsonPath("$.errorCode").value("RH-SX-001"));
    }

    @Test
    @DisplayName("GET /api/rrhh/sexos/9999 -> 404")
    void obtenerPorId_idInexistente_retorna404() throws Exception {
        mockMvc.perform(get("/api/rrhh/sexos/{id}", 9999)
                        .header("Authorization", authToken))
    ...
    @Test
    @DisplayName("PUT /api/rrhh/sexos/9999 -> 404")
    void actualizar_idInexistente_retorna404() throws Exception {
        ...
        mockMvc.perform(put("/api/rrhh/sexos/{id}", 9999)
                        .header("Authorization", authToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(json))
                .andExpect(status().isNotFound())
    ...
    @Test
    @DisplayName("PATCH /api/rrhh/sexos/9999/desactivar -> 404")
    void desactivar_idInexistente_retorna404() throws Exception {
        mockMvc.perform(patch("/api/rrhh/sexos/{id}/desactivar", 9999)
                        .header("Authorization", authToken))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.success").value(false));
    }
