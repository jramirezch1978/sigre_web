...
@DisplayName("Pruebas de Integración - TipoSubsidioController")
class TipoSubsidioControllerIntegrationTest {
    ...
    void flujoCRUD_completo() throws Exception {
        String crearJson = """
                {
                    "codigo": "X",
                    "nombre": "Subsidio Test"
                }
                """;

        String response = mockMvc.perform(post("/api/rrhh/tipos-subsidio")
                        .header("Authorization", authToken)
                ...
                .andExpect(jsonPath("$.data.nombre").value("Subsidio Test"))
                .andReturn().getResponse().getContentAsString();

        Long id = objectMapper.readTree(response).get("data").get("id").asLong();

        mockMvc.perform(get("/api/rrhh/tipos-subsidio/{id}", id)
                        .header("Authorization", authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
        ...
        String actualizarJson = """
                {
                    "nombre": "Subsidio Actualizado"
                }
                """;

        mockMvc.perform(put("/api/rrhh/tipos-subsidio/{id}", id)
                        .header("Authorization", authToken)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        mockMvc.perform(patch("/api/rrhh/tipos-subsidio/{id}/desactivar", id)
                        .header("Authorization", authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
    }
    ...
    @Test
    @DisplayName("POST /api/rrhh/tipos-subsidio con código duplicado -> 409")
    void crear_codigoDuplicado_retorna409() throws Exception {
        String json = """
                {
                    "codigo": "DUP",
                    "nombre": "Subsidio Duplicado"
                }
                """;
        ...
        mockMvc.perform(post("/api/rrhh/tipos-subsidio")
                ...
                .andExpect(jsonPath("$.errorCode").value("RH-TS-001"));
    }

    @Test
    @DisplayName("GET /api/rrhh/tipos-subsidio/9999 -> 404")
    void obtenerPorId_idInexistente_retorna404() throws Exception {
        mockMvc.perform(get("/api/rrhh/tipos-subsidio/{id}", 9999)
                        .header("Authorization", authToken))
    ...
    @Test
    @DisplayName("PUT /api/rrhh/tipos-subsidio/9999 -> 404")
    void actualizar_idInexistente_retorna404() throws Exception {
        ...
        mockMvc.perform(put("/api/rrhh/tipos-subsidio/{id}", 9999)
                        .header("Authorization", authToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(json))
                .andExpect(status().isNotFound())
    ...
    @Test
    @DisplayName("PATCH /api/rrhh/tipos-subsidio/9999/desactivar -> 404")
    void desactivar_idInexistente_retorna404() throws Exception {
        mockMvc.perform(patch("/api/rrhh/tipos-subsidio/{id}/desactivar", 9999)
                        .header("Authorization", authToken))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.success").value(false));
    }
