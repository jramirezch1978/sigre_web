...
import pe.restaurant.auth.dto.ActualizarCredencialesBdRequest;
import pe.restaurant.auth.dto.ActualizarCredencialesBdResponse;
import pe.restaurant.auth.dto.DeleteEmpresaResponse;
import pe.restaurant.auth.dto.EmpresaLogoUploadResponse;
import pe.restaurant.auth.dto.ProvisionEmpresaRequest;
import pe.restaurant.auth.dto.ProvisionEmpresaResponse;
import pe.restaurant.auth.dto.RecreateEmpresaRequest;
import pe.restaurant.auth.dto.RecreateEmpresaResponse;
...
public class TenantProvisioningService {
    ...
    private static final Set<String> RESERVED_DB_NAMES = Set.of(
            ...
            "restaurant_pe_template", "restaurant_pe_security"
    );
    ...
    @Value("${app.tenant.template-database:restaurant_pe_template}")
    private String templateDatabase;

    @Value("${app.tenant.db-name-prefix:restaurant_pe_emp_}")
     ...
     * (equivalente a lo necesario para ms-auth-security y microservicios contra la BD tenant).
     * <p>
    ...
    private Long resolvePaisId(ProvisionEmpresaRequest req) {
        if (req.getPaisId() != null) {
            return paisRepository.findById(req.getPaisId())
                    .orElseThrow(() -> new BusinessException(
                            "País no encontrado", HttpStatus.NOT_FOUND, "PAIS_NO_ENCONTRADO"))
                    .getId();
        }
        String codigoPais = firstNonBlank(req.getCiuCodPais(), "PE");
        return paisRepository.findByCodigo(codigoPais)
                .or(() -> paisRepository.findByCodigo("PE"))
                .orElseThrow(() -> new BusinessException(
                        "País no encontrado en master.pais (código " + codigoPais + ")",
