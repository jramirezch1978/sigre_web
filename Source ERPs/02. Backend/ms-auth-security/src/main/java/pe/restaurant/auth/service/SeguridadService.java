...
import pe.restaurant.auth.dto.seguridad.*;
import pe.restaurant.auth.entity.Modulo;
import pe.restaurant.auth.entity.OpcionMenu;
import pe.restaurant.auth.repository.ModuloRepository;
import pe.restaurant.auth.repository.OpcionMenuRepository;
...
public class SeguridadService {
    ...
    public List<EmpresaAdminDto> listarEmpresasAdmin() {
        return jdbcTemplate.query(
                ...
                SELECT id, codigo, ruc, razon_social, nombre_comercial,
                       ...
                       db_name, pais_id, flag_estado
                FROM master.empresa
                ORDER BY razon_social
                """,
                (rs, i) -> mapEmpresaAdminDto(rs));
    }
    ...
    public EmpresaAdminDto actualizarEmpresaAdmin(long empresaId, EmpresaAdminUpdateRequest req) {
        ...
        if (req.getPaisId() != null) {
            requirePaisExistente(req.getPaisId());
        }
    ...
    private void requirePaisExistente(long paisId) {
        Integer n = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM master.pais WHERE id = ?",
                Integer.class,
                paisId);
        if (n == null || n == 0) {
            throw new BusinessException("País no encontrado.", HttpStatus.NOT_FOUND, "PAIS_NO_ENCONTRADO");
        }
    }

    private static EmpresaAdminDto mapEmpresaAdminDto(java.sql.ResultSet rs) throws java.sql.SQLException {
        return EmpresaAdminDto.builder()
                .id(rs.getLong("id"))
                ...
                .paisId(rs.getObject("pais_id") != null ? rs.getLong("pais_id") : null)
    ...
    private static ModuloDto toModuloDto(Modulo m) {
    ...
    public ModuloDto crearModulo(ModuloRequest req) {
        ...
        modulo.setCodigo(req.getCodigo());
        modulo.setNombre(req.getNombre());
        modulo.setActivo(req.getActivo());
        return toModuloDto(moduloRepository.save(modulo));
    }

    @Transactional(readOnly = true)
    public ModuloDto obtenerModulo(Long id) {
        return toModuloDto(requireModuloEntity(id));
    }

    @Transactional
    public ModuloDto actualizarModulo(long id, ModuloRequest req) {
        Modulo modulo = requireModuloEntity(id);
        modulo.setCodigo(req.getCodigo());
        modulo.setNombre(req.getNombre());
        modulo.setActivo(req.getActivo());
        return toModuloDto(moduloRepository.save(modulo));
    ...
    private static OpcionMenuDto toOpcionMenuDto(OpcionMenu o) {
        return OpcionMenuDto.builder()
                .id(o.getId())
                .moduloId(o.getModuloId())
                .codigo(o.getCodigo())
                .nombre(o.getNombre())
                .rutaFrontend(o.getRutaFrontend())
                .opcionPadreId(o.getOpcionPadreId())
                .orden(o.getOrden())
                .activo(o.getActivo())
                .build();
    }

    private OpcionMenu requireOpcionMenuEntity(Long id) {
        if (id == null) {
        ...
        return opcionMenuRepository.findById(id)
                .orElseThrow(() -> new BusinessException(
                        "Opción de menú no encontrada", HttpStatus.NOT_FOUND, "RESOURCE_NOT_FOUND"));
    }

    @Transactional(readOnly = true)
    public List<OpcionMenuDto> listarOpcionesMenu(Long moduloId) {
        List<OpcionMenu> rows = (moduloId == null)
                ? opcionMenuRepository.findAllByOrderByModuloIdAscOrdenAscNombreAsc()
                : opcionMenuRepository.findByModuloIdOrderByOrdenAscNombreAsc(moduloId);
        return rows.stream().map(SeguridadService::toOpcionMenuDto).toList();
    }

    @Transactional
    public OpcionMenuDto crearOpcionMenu(OpcionMenuRequest req) {
        OpcionMenu opcion = new OpcionMenu();
        opcion.setModuloId(req.getModuloId());
        opcion.setCodigo(req.getCodigo());
        opcion.setNombre(req.getNombre());
        opcion.setRutaFrontend(req.getRutaFrontend());
        opcion.setOpcionPadreId(req.getOpcionPadreId());
        opcion.setOrden(req.getOrden() != null ? req.getOrden() : 0);
        opcion.setActivo(req.getActivo());
        return toOpcionMenuDto(opcionMenuRepository.save(opcion));
    }

    @Transactional(readOnly = true)
    public OpcionMenuDto obtenerOpcionMenu(Long id) {
        return toOpcionMenuDto(requireOpcionMenuEntity(id));
    }

    @Transactional
    public OpcionMenuDto actualizarOpcionMenu(long id, OpcionMenuRequest req) {
        OpcionMenu opcion = requireOpcionMenuEntity(id);
        opcion.setModuloId(req.getModuloId());
        opcion.setCodigo(req.getCodigo());
        opcion.setNombre(req.getNombre());
        opcion.setRutaFrontend(req.getRutaFrontend());
