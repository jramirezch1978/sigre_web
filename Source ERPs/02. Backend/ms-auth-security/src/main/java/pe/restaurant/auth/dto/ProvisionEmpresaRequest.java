 ...
 * La sigla ({@code SIGLA}) es obligatoria y define {@code restaurant_pe_emp_<slug>}.
 * <p>
...
public class ProvisionEmpresaRequest {
    ...
    @NotBlank(message = "La sigla (SIGLA) es obligatoria: define el nombre de la BD tenant restaurant_pe_emp_<sigla normalizada>")
    @Size(max = 50)
    @JsonAlias({"SIGLA"})
    ...
    /** Opcional. Si no se envía, se resuelve desde {@code ciuCodPais} o por defecto Perú (PE). */
    private Long paisId;
}
