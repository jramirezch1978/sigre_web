...
public class TenantContextWebFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        try {
            ...
            boolean adminSistema = extractAdminSistemaFromJwt(request.getHeader("Authorization"));

            Long resolved = fromPath != null ? fromPath : (fromHeader != null ? fromHeader : fromJwt);

            // El admin de sistema opera el panel multi-empresa: puede usar una ruta
            // /empresas/{id} distinta a la empresa de su token sin que sea un conflicto.
            if (!adminSistema && fromPath != null && fromJwt != null && !fromPath.equals(fromJwt)) {
    ...
    private boolean extractAdminSistemaFromJwt(String authorization) {
        if (authorization == null || !authorization.startsWith("Bearer ")) {
            return false;
        }
        String token = authorization.substring(7).trim();
        ...
        return Boolean.TRUE.equals(jwtTokenProvider.getClaim(token, "adminSistema", Boolean.class));
    }
