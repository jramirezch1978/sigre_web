...
class ComprasFlowIntegrationTest {
    ...
    @Test
    @Order(12)
    @DisplayName("OC-03: Ajustar cantidad proyectada de línea OC")
    void oc_ajustarCantidadLinea() throws Exception {
        requiereArticuloMovProy();
        Long ocId = crearOrdenCompra();
        aprobarOc(ocId);

        Long lineaId = jdbc.queryForObject(
                "SELECT id FROM compras.orden_compra_det WHERE orden_compra_id = ? ORDER BY id LIMIT 1",
                Long.class, ocId);

        mockMvc.perform(patch("/api/compras/ordenes-compra/{id}/lineas/{lineaId}/ajustar-cantidad", ocId, lineaId)
                .header("Authorization", AUTH)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"ordenCompraDetId\": " + lineaId + ", \"nuevaCantidad\": 150.0000}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.lineas[0].cantProyectada").value(150.0));
    }

    @Test
    @Order(13)
    @DisplayName("OC-04: Cerrar OC con saldo pendiente")
    void oc_cerrarConSaldoPendiente() throws Exception {
        requiereArticuloMovProy();
        Long ocId = crearOrdenCompra();
    ...
    @Test
    @Order(33)
    @DisplayName("OS-03: Registrar conformidad por línea en OS")
    void os_registrarConformidadLinea() throws Exception {
        requiereNumeradorOrdenServicio();
        Long osId = crearOrdenServicio();
        ...
        Long lineaId = jdbc.queryForObject(
                "SELECT id FROM compras.orden_servicio_det WHERE orden_servicio_id = ? ORDER BY id LIMIT 1",
                Long.class, osId);

        mockMvc.perform(post("/api/compras/ordenes-servicio/{id}/lineas/{lineaId}/conformidad", osId, lineaId)
                .header("Authorization", AUTH)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"observacion\":\"Servicio conforme - test integracion\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.lineas[0].conformidadFecha").exists());

        // Verificar via JDBC que el log se creó (lo inserta directamente, no pasa por Hibernate)
        Integer logCount = jdbc.queryForObject(
                "SELECT COUNT(*) FROM compras.os_conformidad_log WHERE orden_servicio_det_id = ?",
                Integer.class, lineaId);
        assertThat(logCount).isGreaterThan(0);

        // Verificar el detalle de la OS vía endpoint GET (refleja datos committed/flusheados por Hibernate)
        mockMvc.perform(get("/api/compras/ordenes-servicio/{id}", osId)
                .header("Authorization", AUTH))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.lineas[0].conformidadFecha").isNotEmpty());
    }
