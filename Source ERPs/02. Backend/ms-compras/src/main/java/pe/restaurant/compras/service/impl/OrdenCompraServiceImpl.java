...
public class OrdenCompraServiceImpl implements OrdenCompraService {
    ...
    public OrdenCompraDetalleResponse enviarAprobacion(Long id) {
        ...
        if (!"3".equals(oc.getFlagEstado()) && !"0".equals(oc.getFlagEstado())) {
            throw new BusinessException(
    ...
    @Override
    @Transactional
    public OrdenCompraRecepcionResponse recepcionarEnAlmacenContable(Long id, OrdenCompraRecepcionContableRequest request) {
        validator.verificarCompradorActivo();
        OrdenCompra oc = buscar(id);
        ...
        MovimientoContableRequest movReq = new MovimientoContableRequest();
        movReq.setSucursalId(oc.getSucursalId());
        movReq.setAlmacenId(request.getAlmacenId());
        movReq.setArticuloMovTipoId(request.getArticuloMovTipoId());
        movReq.setFechaMov(request.getFechaMov() != null ? request.getFechaMov() : LocalDate.now());
        movReq.setProveedorId(oc.getProveedorId());
        movReq.setOrdenCompraId(id);
        movReq.setObservaciones(request.getObservaciones());
        movReq.setCntblLibroId(request.getCntblLibroId());
        movReq.setMonedaId(request.getMonedaId());
        movReq.setTipoCambio(request.getTipoCambio());
        movReq.setGlosa(request.getGlosa());

        List<MovimientoContableRequest.MovimientoLineaContable> lineas = new ArrayList<>();
        for (OrdenCompraRecepcionContableRequest.LineaConcepto lc : request.getLineas()) {
            MovimientoContableRequest.MovimientoLineaContable ml = new MovimientoContableRequest.MovimientoLineaContable();
            ml.setArticuloId(lc.getArticuloId());
            ml.setCantProcesada(lc.getCantidad());
            ml.setCostoUnitario(lc.getCostoUnitario());
            ml.setConceptoFinancieroId(lc.getConceptoFinancieroId());
            ml.setCentrosCostoId(lc.getCentrosCostoId());
            ml.setOcDetId(lc.getOcDetId());
            lineas.add(ml);
        }
        movReq.setLineas(lineas);

        MovimientoDetalleResponse recepcion;
        try {
            recepcion = almacenClient.recepcionOrdenCompraContable(movReq).getData();
        } catch (FeignException.NotFound e) {
            throw new BusinessException(
                    extraerMensajeDeError(e, "Orden de compra o almacén no encontrado en ms-almacen"),
                    HttpStatus.NOT_FOUND, "COM-038");
    ...
    private Map<Long, BigDecimal> obtenerRecepcionesRealesPorLinea(Long ordenCompraId) {
        try {
            String sql = """
                SELECT vmd.oc_det_id, COALESCE(SUM(vmd.cant_procesada), 0) AS total_recibido
    ...
    public DatosArticuloResponse datosArticulo(Long articuloId, Long proveedorId,
        ...
        if (articulo != null) {
            percepcionTasa = configParameterService.getDec("TASA_PERCEPCION", null);
    ...
    private boolean isAprobacionRequerida() {
        // ── La aprobación SIEMPRE es obligatoria por regla de negocio ──
        // Si en el futuro se requiere activar/desactivar por BD, cambiar a:
        // return configParameterService.isTextFlagOn("COMPRA_APROBACION_OC", "0");
        return true;
    }
    ...
    private void guardarImportacion(Long ocId, OrdenCompraCabeceraRequest request) {
        ...
        entity.setIncotermId(resolveIncotermId(imp.getIncoterm()));
    ...
    // El front opera con el código Incoterm (ej. "FOB"); en BD se persiste el FK incoterm_id.
    private Long resolveIncotermId(String codigo) {
        if (codigo == null || codigo.isBlank()) return null;
        return incotermRepository.findByCodigo(codigo.trim())
                .map(Incoterm::getId)
                .orElse(null);
    }

    private String resolveIncotermCodigo(Long incotermId) {
        if (incotermId == null) return null;
        return incotermRepository.findById(incotermId)
                .map(i -> i.getCodigo() != null ? i.getCodigo().trim() : null)
                .orElse(null);
    }
        ...
        if ("1".equals(oc.getFlagImportacion())) {
            impDto = ocImportacionRepository.findByOrdenCompraId(oc.getId())
                    .map(i -> OcImportacionDto.builder()
                            .id(i.getId())
                            .incoterm(resolveIncotermCodigo(i.getIncotermId()))
    ...
    @Override
    @Transactional
    public void actualizarCantFacturada(ActualizarCantFacturadaRequest request) {
        if (request.getLineas() == null || request.getLineas().isEmpty()) {
            return;
        }

        String sql = "UPDATE compras.orden_compra_det SET cant_facturada = cant_facturada + ? WHERE id = ?";
        jdbcTemplate.batchUpdate(sql,
                request.getLineas(),
                request.getLineas().size(),
                (ps, linea) -> {
                    ps.setBigDecimal(1, linea.getCantidad());
                    ps.setLong(2, linea.getOrdenCompraDetId());
                });

        log.info("Cantidades facturadas actualizadas para {} líneas de OC",
                request.getLineas().size());
    }

    // ── Ajuste de cantidad proyectada (sin DDL) ─────────────────────────────

    @Override
    @Transactional
    public OrdenCompraDetalleResponse ajustarCantidadLinea(Long id, Long lineaId, BigDecimal nuevaCantidad) {
        validator.verificarCompradorActivo();
        OrdenCompra oc = buscar(id);

        if (!"1".equals(oc.getFlagEstado())) {
            throw new BusinessException(
                    "Solo se puede ajustar cantidad en una OC activa",
                    HttpStatus.CONFLICT, "COM-017");
        }

        OrdenCompraDet linea = oc.getLineas().stream()
                .filter(l -> l.getId().equals(lineaId))
                .findFirst()
                .orElseThrow(() -> new BusinessException(
                        "Línea " + lineaId + " no pertenece a esta OC",
                        HttpStatus.BAD_REQUEST, "COM-018"));

        if (nuevaCantidad.compareTo(linea.getCantProcesada()) < 0) {
            throw new BusinessException(
                    "La nueva cantidad proyectada no puede ser menor a la cantidad ya procesada (" +
                    linea.getCantProcesada() + ")",
                    HttpStatus.UNPROCESSABLE_ENTITY, "COM-019");
        }

        BigDecimal anterior = linea.getCantProyectada();
        linea.setCantProyectada(nuevaCantidad);

        log.info("Cantidad proyectada ajustada: línea {} de OC {}: {} → {}",
                lineaId, id, anterior, nuevaCantidad);

        return toDetalle(ordenCompraRepository.save(oc));
    }

    // ── Ajuste de stock por nota de crédito/débito ─────────────────────────

    @Override
    @Transactional
    public void ajustarStockPorNota(AjusteStockRequest request) {
        // Verificar que la línea de OC existe
        Long ocId = jdbcTemplate.queryForObject(
                "SELECT orden_compra_id FROM compras.orden_compra_det WHERE id = ?",
                Long.class, request.getOrdenCompraDetId());

        // Obtener almacén de la OC si no se especificó
        if (request.getAlmacenId() == null) {
            Long almacenId = jdbcTemplate.queryForObject(
                    "SELECT COALESCE(almacen_id, 1) FROM compras.orden_compra WHERE id = ?",
                    Long.class, ocId);
            request.setAlmacenId(almacenId);
        }

        log.info("Ajuste de stock por nota {} - OC detalle: {}, cantidad: {}, almacen: {}",
                request.getTipoAjuste(), request.getOrdenCompraDetId(),
                request.getCantidad(), request.getAlmacenId());

        // Pendiente: crear ValeMov en ms-almacen para ajuste de stock real
        // Requiere extender AlmacenClient con método de ajuste
    }
