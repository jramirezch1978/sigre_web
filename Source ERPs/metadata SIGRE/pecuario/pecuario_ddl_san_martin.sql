...
-- ninguna sentencia -- se ejecuta conectado como el usuario/esquema destino
-- (ej. SAN_MARTIN, JARCH, o el que corresponda) y todos los objetos quedan
-- bajo ese usuario de forma implicita. No requiere buscar/reemplazar nada.
--
-- ============================================================================
...
--    tener FK a ORIGEN(cod_origen).
-- ============================================================================
--
-- Orden de creacion respeta dependencias de FK:
--   0) Especie: NO se crea tabla PC_ESPECIE -- se reutiliza TG_ESPECIES
...
--
-- NOTA SOBRE VENTANAS: Pecuario es un modulo independiente (no una extension
-- de Campo) con su propia libreria PowerBuilder (ws_objects/Pecuario/) y su
-- propio prefijo de ventana PC### (no CAM###), numeradas por TIPO de opcion
-- (igual regla que el resto del sistema): Tablas 000-299, Operaciones
-- 300-499, Consultas 500-699, Reportes 700-899, Procesos 900-999. Al ser
-- modulo propio, se numera secuencialmente desde el inicio de cada rango,
-- sin compartir espacio con Campo ni con ningun otro modulo:
--   Tablas      PC001-PC010 (10 opciones)
--   Operaciones PC301-PC321 (21 opciones)
--   Consultas   PC501-PC507 (7 opciones)
--   Reportes    PC701-PC707 (7 opciones)
--   Procesos    PC901-PC906 (6 opciones)
...
-- Usa USER_TABLES/USER_SEQUENCES (objetos del usuario/esquema conectado,
-- sin necesidad de indicar el nombre de esquema/owner de forma explicita --
-- funciona igual sea cual sea el usuario con el que se ejecute el script:
-- SAN_MARTIN, JARCH, o cualquier otro).
-- ============================================================================
begin
  for t in (
    select table_name from user_tables
     where table_name like 'PC\_%' escape '\'
  ) loop
    begin
...
begin
  for s in (
    select sequence_name from user_sequences
     where sequence_name like 'SEQ\_PC\_%' escape '\'
  ) loop
    begin
...
-- ============================================================================
-- 0) ESPECIE -- REUTILIZA TG_ESPECIES (catalogo real y ya existente
...
-- ============================================================================
begin
  execute immediate 'alter table TG_ESPECIES add periodo_gestacion_dias NUMBER(4)';
...
begin
  execute immediate 'alter table TG_ESPECIES add periodo_incubacion_dias NUMBER(4)';
exception when others then
  if sqlcode != -1430 then raise; end if;
end;
/

comment on column TG_ESPECIES.periodo_gestacion_dias is 'Pecuario (flag_tipo_matprim=P) - dias de gestacion (mamiferos): fec_prob_parto = fec_servicio + este valor';
comment on column TG_ESPECIES.periodo_incubacion_dias is 'Pecuario (flag_tipo_matprim=P) - dias de incubacion (oviparos): fec_eclosion_prevista = fec_carga + este valor';
...
create or replace trigger TIB_TG_ESPECIES
before insert on TG_ESPECIES
for each row
declare
...
begin
  if :new.especie is null then
    select count(*) into ln_count from NUM_TABLAS
     where tabla = 'TG_ESPECIES' and origen = lc_origen;
    if ln_count = 0 then
      insert into NUM_TABLAS(tabla, origen, ult_nro) values ('TG_ESPECIES', lc_origen, 1);
    end if;
    select ult_nro into ln_ult_nro from NUM_TABLAS
     where tabla = 'TG_ESPECIES' and origen = lc_origen for update;
    :new.especie := lc_origen || lpad(to_char(ln_ult_nro), 6, '0');
    update NUM_TABLAS set ult_nro = ln_ult_nro + 1
     where tabla = 'TG_ESPECIES' and origen = lc_origen;
  end if;
end;
/
...
-- del insert).
insert into TG_ESPECIES (descr_especie, nombre_cientifico, flag_tipo_matprim, flag_estado, periodo_gestacion_dias)
  select 'Bovino','Bos taurus','P','1',283 from dual
   where not exists (select 1 from TG_ESPECIES where descr_especie = 'Bovino' and flag_tipo_matprim = 'P');
insert into TG_ESPECIES (descr_especie, nombre_cientifico, flag_tipo_matprim, flag_estado, periodo_gestacion_dias)
  select 'Porcino','Sus scrofa domesticus','P','1',114 from dual
   where not exists (select 1 from TG_ESPECIES where descr_especie = 'Porcino' and flag_tipo_matprim = 'P');
insert into TG_ESPECIES (descr_especie, nombre_cientifico, flag_tipo_matprim, flag_estado, periodo_gestacion_dias)
  select 'Caprino','Capra aegagrus hircus','P','1',150 from dual
   where not exists (select 1 from TG_ESPECIES where descr_especie = 'Caprino' and flag_tipo_matprim = 'P');
insert into TG_ESPECIES (descr_especie, nombre_cientifico, flag_tipo_matprim, flag_estado, periodo_gestacion_dias)
  select 'Ovino','Ovis aries','P','1',150 from dual
   where not exists (select 1 from TG_ESPECIES where descr_especie = 'Ovino' and flag_tipo_matprim = 'P');
insert into TG_ESPECIES (descr_especie, nombre_cientifico, flag_tipo_matprim, flag_estado, periodo_gestacion_dias)
  select 'Equino','Equus ferus caballus','P','1',340 from dual
   where not exists (select 1 from TG_ESPECIES where descr_especie = 'Equino' and flag_tipo_matprim = 'P');
insert into TG_ESPECIES (descr_especie, nombre_cientifico, flag_tipo_matprim, flag_estado, periodo_incubacion_dias)
  select 'Avicola','Gallus gallus domesticus','P','1',21 from dual
   where not exists (select 1 from TG_ESPECIES where descr_especie = 'Avicola' and flag_tipo_matprim = 'P');
commit;
...
comment on table PC_RAZA is 'Pecuario - Catalogo de razas (de cualquier especie)';
comment on column PC_RAZA.reckey is 'PK autonumerica';
comment on column PC_RAZA.cod_especie is 'FK a TG_ESPECIES.especie (catalogo compartido de especies del modulo de pesca; Pecuario usa las filas con flag_tipo_matprim=P). El codigo (ej. AQ000001) lo genera el trigger de TG_ESPECIES, no es literal/mnemotecnico';
comment on column PC_RAZA.cod_raza is 'codigo de raza (unique, no PK)';
comment on column PC_RAZA.nom_raza is 'nombre de la raza';
comment on column PC_RAZA.flag_tipo is 'L=Lechera, C=Carne, M=Doble proposito, F=Fibra/lana, T=Trabajo/traccion, R=Reproduccion/genetica (aplica a cualquier especie, no solo bovinos)';
comment on column PC_RAZA.flag_estado is 'flag_estado';

create sequence SEQ_PC_RAZA start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_RAZA
before insert on PC_RAZA
for each row
begin
...
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'HOLS', 'Holstein', 'L' from TG_ESPECIES where descr_especie = 'Bovino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'JERS', 'Jersey', 'L' from TG_ESPECIES where descr_especie = 'Bovino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'BRSW', 'Brown Swiss', 'M' from TG_ESPECIES where descr_especie = 'Bovino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'GYR', 'Gyr', 'M' from TG_ESPECIES where descr_especie = 'Bovino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'BRAH', 'Brahman', 'C' from TG_ESPECIES where descr_especie = 'Bovino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'ANGU', 'Angus', 'C' from TG_ESPECIES where descr_especie = 'Bovino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'CRUC', 'Cruzado / mestizo', 'M' from TG_ESPECIES where descr_especie = 'Bovino' and flag_tipo_matprim = 'P';
-- Porcino
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'YORK', 'Yorkshire (Large White)', 'R' from TG_ESPECIES where descr_especie = 'Porcino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'LAND', 'Landrace', 'R' from TG_ESPECIES where descr_especie = 'Porcino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'DURO', 'Duroc', 'C' from TG_ESPECIES where descr_especie = 'Porcino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'HAMP', 'Hampshire', 'C' from TG_ESPECIES where descr_especie = 'Porcino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'PIET', 'Pietrain', 'C' from TG_ESPECIES where descr_especie = 'Porcino' and flag_tipo_matprim = 'P';
-- Caprino
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'SAAN', 'Saanen', 'L' from TG_ESPECIES where descr_especie = 'Caprino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'ALPI', 'Alpina', 'L' from TG_ESPECIES where descr_especie = 'Caprino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'BOER', 'Boer', 'C' from TG_ESPECIES where descr_especie = 'Caprino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'ANGO', 'Angora', 'F' from TG_ESPECIES where descr_especie = 'Caprino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'CRIC', 'Criollo', 'M' from TG_ESPECIES where descr_especie = 'Caprino' and flag_tipo_matprim = 'P';
-- Ovino
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'MERI', 'Merino', 'F' from TG_ESPECIES where descr_especie = 'Ovino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'CORR', 'Corriedale', 'M' from TG_ESPECIES where descr_especie = 'Ovino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'SUFF', 'Suffolk', 'C' from TG_ESPECIES where descr_especie = 'Ovino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'DORP', 'Dorper', 'C' from TG_ESPECIES where descr_especie = 'Ovino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'PELI', 'Pelibuey', 'C' from TG_ESPECIES where descr_especie = 'Ovino' and flag_tipo_matprim = 'P';
-- Equino
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'PERU', 'Peruano de Paso', 'T' from TG_ESPECIES where descr_especie = 'Equino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'CRIE', 'Criollo', 'T' from TG_ESPECIES where descr_especie = 'Equino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'PERC', 'Percheron', 'T' from TG_ESPECIES where descr_especie = 'Equino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'PASO', 'Paso Fino', 'T' from TG_ESPECIES where descr_especie = 'Equino' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'PSCR', 'Pura Sangre de Carrera', 'R' from TG_ESPECIES where descr_especie = 'Equino' and flag_tipo_matprim = 'P';
-- Avicola
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'ROSS', 'Ross 308 (pollo de engorde)', 'C' from TG_ESPECIES where descr_especie = 'Avicola' and flag_tipo_matprim = 'P';
insert into PC_RAZA (cod_especie, cod_raza, nom_raza, flag_tipo)
  select especie, 'HYLN', 'Hy-Line Brown (ponedora)', 'L' from TG_ESPECIES where descr_especie = 'Avicola' and flag_tipo_matprim = 'P';
commit;
...
comment on column PC_POTRERO.reckey is 'PK autonumerica';
comment on column PC_POTRERO.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_POTRERO.cod_potrero is 'codigo de potrero (unique junto a cod_origen, no PK)';
comment on column PC_POTRERO.nom_potrero is 'nombre del potrero';
comment on column PC_POTRERO.area_has is 'area en hectareas';
comment on column PC_POTRERO.tipo_pasto is 'tipo de pasto';
comment on column PC_POTRERO.capacidad_cab is 'capacidad de carga en numero de cabezas';
comment on column PC_POTRERO.flag_estado is 'flag_estado';

create sequence SEQ_PC_POTRERO start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_POTRERO
before insert on PC_POTRERO
for each row
begin
  if :new.reckey is null then
...
comment on table PC_ESTABLO is 'Pecuario - Establos/corrales (ubicacion cubierta) por fundo';
comment on column PC_ESTABLO.reckey is 'PK autonumerica';
comment on column PC_ESTABLO.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_ESTABLO.cod_establo is 'codigo de establo (unique junto a cod_origen, no PK)';
comment on column PC_ESTABLO.nom_establo is 'nombre del establo';
comment on column PC_ESTABLO.flag_tipo is 'O=Ordeno, M=Maternidad, C=Cuarentena, G=Corral de manejo (General), E=Engorde, I=Incubadora (avicola)';
comment on column PC_ESTABLO.capacidad_cab is 'capacidad de carga en numero de cabezas';
comment on column PC_ESTABLO.flag_estado is 'flag_estado';

create sequence SEQ_PC_ESTABLO start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_ESTABLO
before insert on PC_ESTABLO
for each row
begin
  if :new.reckey is null then
...
comment on column PC_CATEGORIA.reckey is 'PK autonumerica';
comment on column PC_CATEGORIA.cod_categoria is 'codigo de categoria (TER, VAQ, NOV, VPR, VSC, VDE, TOR, TDE) -- unique, no PK';
comment on column PC_CATEGORIA.nom_categoria is 'nombre de la categoria';
comment on column PC_CATEGORIA.flag_sexo is 'M/H/null=ambos';
comment on column PC_CATEGORIA.edad_min_meses is 'edad minima en meses';
comment on column PC_CATEGORIA.edad_max_meses is 'edad maxima en meses';
comment on column PC_CATEGORIA.flag_estado is 'flag_estado';

create sequence SEQ_PC_CATEGORIA start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_CATEGORIA
before insert on PC_CATEGORIA
for each row
begin
  if :new.reckey is null then
...
comment on column PC_SEMENTAL.reckey is 'PK autonumerica';
comment on column PC_SEMENTAL.cod_semental is 'codigo del semental/pajilla (unique, no PK)';
comment on column PC_SEMENTAL.nom_semental is 'nombre del semental';
comment on column PC_SEMENTAL.cod_raza is 'raza del semental';
comment on column PC_SEMENTAL.proveedor is 'proveedor / central de inseminacion';
comment on column PC_SEMENTAL.registro_genet is 'registro genealogico / codigo de catalogo del proveedor';
comment on column PC_SEMENTAL.flag_estado is 'flag_estado';

create sequence SEQ_PC_SEMENTAL start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_SEMENTAL
before insert on PC_SEMENTAL
for each row
begin
  if :new.reckey is null then
...
-- ----------------------------------------------------------------------------
create table PC_PRODUCTO_SANITARIO
(
  reckey          NUMBER(10)     not null,
...
comment on table PC_PRODUCTO_SANITARIO is 'Pecuario - Catalogo de vacunas, medicamentos e insumos veterinarios';
comment on column PC_PRODUCTO_SANITARIO.reckey is 'PK autonumerica';
comment on column PC_PRODUCTO_SANITARIO.cod_prod_san is 'codigo de producto sanitario (unique, no PK)';
comment on column PC_PRODUCTO_SANITARIO.nom_producto is 'nombre del producto';
comment on column PC_PRODUCTO_SANITARIO.flag_tipo is 'V=Vacuna, D=Desparasitante, M=Medicamento, S=Suplemento';
comment on column PC_PRODUCTO_SANITARIO.cod_art is 'FK a ARTICULO (Almacen) -- articulo real de stock que se descuenta al consumir este producto via una OT';
comment on column PC_PRODUCTO_SANITARIO.dias_refuerzo is 'dias hasta la proxima dosis de refuerzo (si aplica)';
comment on column PC_PRODUCTO_SANITARIO.periodo_retiro is 'dias de retiro de leche/carne tras aplicar';
comment on column PC_PRODUCTO_SANITARIO.flag_estado is 'flag_estado';

create sequence SEQ_PC_PRODUCTO_SANIT start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_PRODUCTO_SANIT
before insert on PC_PRODUCTO_SANITARIO
for each row
begin
  if :new.reckey is null then
...
insert into PC_PRODUCTO_SANITARIO (cod_prod_san, nom_producto, flag_tipo, dias_refuerzo, periodo_retiro) values ('VAC001','Vacuna Aftosa','V',180,21);
insert into PC_PRODUCTO_SANITARIO (cod_prod_san, nom_producto, flag_tipo, dias_refuerzo, periodo_retiro) values ('VAC002','Vacuna Brucelosis (Cepa 19)','V',null,0);
insert into PC_PRODUCTO_SANITARIO (cod_prod_san, nom_producto, flag_tipo, dias_refuerzo, periodo_retiro) values ('VAC003','Vacuna Carbunco sintomatico / Clostridiales','V',365,21);
insert into PC_PRODUCTO_SANITARIO (cod_prod_san, nom_producto, flag_tipo, dias_refuerzo, periodo_retiro) values ('VAC004','Vacuna Rabia bovina','V',365,21);
insert into PC_PRODUCTO_SANITARIO (cod_prod_san, nom_producto, flag_tipo, dias_refuerzo, periodo_retiro) values ('VAC005','Vacuna IBR-BVD','V',180,0);
insert into PC_PRODUCTO_SANITARIO (cod_prod_san, nom_producto, flag_tipo, dias_refuerzo, periodo_retiro) values ('VAC006','Vacuna Leptospirosis','V',180,0);
insert into PC_PRODUCTO_SANITARIO (cod_prod_san, nom_producto, flag_tipo, dias_refuerzo, periodo_retiro) values ('DES001','Desparasitante interno (Ivermectina)','D',90,35);
insert into PC_PRODUCTO_SANITARIO (cod_prod_san, nom_producto, flag_tipo, dias_refuerzo, periodo_retiro) values ('DES002','Desparasitante externo (bano garrapaticida)','D',30,0);
insert into PC_PRODUCTO_SANITARIO (cod_prod_san, nom_producto, flag_tipo, periodo_retiro) values ('MED001','Antibiotico intramamario (mastitis)','M',5);
insert into PC_PRODUCTO_SANITARIO (cod_prod_san, nom_producto, flag_tipo, periodo_retiro) values ('MED002','Antiinflamatorio','M',3);
insert into PC_PRODUCTO_SANITARIO (cod_prod_san, nom_producto, flag_tipo, periodo_retiro) values ('SUP001','Sales minerales','S',0);
commit;
...
comment on column PC_ENFERMEDAD.cod_enfermedad is 'codigo de enfermedad (unique, no PK)';
comment on column PC_ENFERMEDAD.nom_enfermedad is 'nombre de la enfermedad';
comment on column PC_ENFERMEDAD.flag_reproductiva is 'afecta el ciclo reproductivo (1=si)';
comment on column PC_ENFERMEDAD.flag_estado is 'flag_estado';

create sequence SEQ_PC_ENFERMEDAD start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_ENFERMEDAD
before insert on PC_ENFERMEDAD
for each row
begin
  if :new.reckey is null then
...
-- Datos iniciales sugeridos
insert into PC_ENFERMEDAD (cod_enfermedad, nom_enfermedad, flag_reproductiva) values ('MASTI1','Mastitis','0');
insert into PC_ENFERMEDAD (cod_enfermedad, nom_enfermedad, flag_reproductiva) values ('HIPOC1','Hipocalcemia / fiebre de leche','0');
insert into PC_ENFERMEDAD (cod_enfermedad, nom_enfermedad, flag_reproductiva) values ('CETOS1','Cetosis','0');
insert into PC_ENFERMEDAD (cod_enfermedad, nom_enfermedad, flag_reproductiva) values ('COJER1','Cojera / laminitis','0');
insert into PC_ENFERMEDAD (cod_enfermedad, nom_enfermedad, flag_reproductiva) values ('NEUMO1','Neumonia en terneros','0');
insert into PC_ENFERMEDAD (cod_enfermedad, nom_enfermedad, flag_reproductiva) values ('DIARR1','Diarrea neonatal','0');
insert into PC_ENFERMEDAD (cod_enfermedad, nom_enfermedad, flag_reproductiva) values ('BRUCE1','Brucelosis','1');
insert into PC_ENFERMEDAD (cod_enfermedad, nom_enfermedad, flag_reproductiva) values ('IBRBV1','IBR / BVD','1');
insert into PC_ENFERMEDAD (cod_enfermedad, nom_enfermedad, flag_reproductiva) values ('LEPTO1','Leptospirosis','1');
insert into PC_ENFERMEDAD (cod_enfermedad, nom_enfermedad, flag_reproductiva) values ('METRI1','Metritis','1');
insert into PC_ENFERMEDAD (cod_enfermedad, nom_enfermedad, flag_reproductiva) values ('RETPL1','Retencion de placenta','1');
insert into PC_ENFERMEDAD (cod_enfermedad, nom_enfermedad, flag_reproductiva) values ('DISTO1','Distocia','1');
commit;
...
comment on column PC_DIETA.reckey is 'PK autonumerica';
comment on column PC_DIETA.cod_dieta is 'codigo de dieta (unique, no PK)';
comment on column PC_DIETA.nom_dieta is 'nombre de la dieta';
comment on column PC_DIETA.cod_categoria is 'categoria de animal a la que aplica';
comment on column PC_DIETA.costo_kg_prom is 'costo promedio por kg, para costeo';
comment on column PC_DIETA.flag_estado is 'flag_estado';

create sequence SEQ_PC_DIETA start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_DIETA
before insert on PC_DIETA
for each row
begin
  if :new.reckey is null then
...
-- ----------------------------------------------------------------------------
create table PC_DIETA_COMPONENTE
(
  reckey          NUMBER(10)     not null,
  cod_dieta       CHAR(6)        not null,
...
comment on table PC_DIETA_COMPONENTE is 'Pecuario - Detalle de insumos que componen cada dieta';
comment on column PC_DIETA_COMPONENTE.reckey is 'PK autonumerica';
comment on column PC_DIETA_COMPONENTE.cod_dieta is 'dieta a la que pertenece';
comment on column PC_DIETA_COMPONENTE.item is 'item correlativo (unique junto a cod_dieta, no PK)';
comment on column PC_DIETA_COMPONENTE.cod_art is 'articulo de almacen (forraje/concentrado/mineral)';
comment on column PC_DIETA_COMPONENTE.cantidad_kg is 'cantidad en kg por animal/dia';
comment on column PC_DIETA_COMPONENTE.flag_estado is 'flag_estado';

create sequence SEQ_PC_DIETACOMP start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_DIETACOMP
before insert on PC_DIETA_COMPONENTE
for each row
begin
  if :new.reckey is null then
...
comment on column PC_RECETA.reckey is 'PK autonumerica';
comment on column PC_RECETA.cod_receta is 'codigo de receta (unique, no PK)';
comment on column PC_RECETA.nom_receta is 'nombre de la receta (ej. Concentrado vacas en produccion)';
comment on column PC_RECETA.cod_producto is 'FK a TIPO_PRODUCTO -- producto terminado que se fabrica (el concentrado)';
comment on column PC_RECETA.rendimiento_kg is 'kg de producto terminado que rinde un lote de esta receta';
comment on column PC_RECETA.flag_estado is 'flag_estado';

create sequence SEQ_PC_RECETA start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_RECETA
before insert on PC_RECETA
for each row
begin
  if :new.reckey is null then
...
comment on table PC_RECETA_DET is 'Pecuario - Materia prima (por kg) que consume un lote de la receta';
comment on column PC_RECETA_DET.reckey is 'PK autonumerica';
comment on column PC_RECETA_DET.cod_receta is 'receta a la que pertenece';
comment on column PC_RECETA_DET.item is 'item correlativo (unique junto a cod_receta, no PK)';
comment on column PC_RECETA_DET.cod_art is 'FK a ARTICULO -- materia prima consumida';
comment on column PC_RECETA_DET.cantidad_kg is 'cantidad de materia prima, en kg, por lote de rendimiento_kg';

create sequence SEQ_PC_RECETADET start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_RECETADET
before insert on PC_RECETA_DET
for each row
begin
  if :new.reckey is null then
...
comment on table PC_LOTE is 'Pecuario - Lote/cohorte de animales (agrupacion de manejo). Sirve tanto para agrupar cabezas individuales (PC_ANIMAL.cod_lote) como, en especies/operaciones sin registro individual, para llevar el conteo agregado directamente aqui';
comment on column PC_LOTE.reckey is 'PK autonumerica';
comment on column PC_LOTE.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_LOTE.cod_lote is 'codigo de lote (unique junto a cod_origen, no PK)';
comment on column PC_LOTE.nom_lote is 'nombre/descripcion del lote';
comment on column PC_LOTE.cod_potrero is 'ubicacion actual del lote';
comment on column PC_LOTE.cod_categoria is 'categoria homogenea del lote, si aplica';
comment on column PC_LOTE.fec_formacion is 'fecha en que se formo/inicio el lote';
comment on column PC_LOTE.fec_cierre is 'fecha de cierre/disolucion del lote';
comment on column PC_LOTE.cantidad_cabezas_inicial is 'cabezas al formar el lote (para especies sin registro individual)';
comment on column PC_LOTE.cantidad_cabezas_actual is 'cabezas vivas actuales (se descuenta por mortalidad/baja)';
comment on column PC_LOTE.flag_estado is '1=Activo, 0=Cerrado';
comment on column PC_LOTE.cod_usr is 'usuario que registro';
comment on column PC_LOTE.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_LOTE start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_LOTE
before insert on PC_LOTE
for each row
begin
  if :new.reckey is null then
...
comment on table PC_POSTURA is 'Pecuario (avicola) - Produccion diaria de huevos de un lote de reproductoras';
comment on column PC_POSTURA.reckey is 'PK autonumerica';
comment on column PC_POSTURA.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_POSTURA.cod_lote is 'FK a PC_LOTE -- lote de reproductoras que postura';
comment on column PC_POSTURA.fec_postura is 'fecha de la postura';
comment on column PC_POSTURA.cantidad_huevos is 'huevos recolectados ese dia';
comment on column PC_POSTURA.cantidad_descarte is 'huevos descartados (rotos, sucios, deformes) no aptos para incubar';
comment on column PC_POSTURA.cod_usr is 'usuario que registro';
comment on column PC_POSTURA.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_POSTURA start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_POSTURA
before insert on PC_POSTURA
for each row
begin
  if :new.reckey is null then
...
alter table PC_INCUBACION add constraint FK_PC_INCUBACION_LOTE_DEST foreign key (cod_origen, cod_lote_destino) references PC_LOTE(cod_origen, cod_lote);
alter table PC_INCUBACION add constraint FK_PC_INCUBACION_ESTABLO foreign key (cod_origen, cod_establo) references PC_ESTABLO(cod_origen, cod_establo);

comment on table PC_INCUBACION is 'Pecuario (avicola) - Carga de incubadora y resultado de eclosion, con trazabilidad lote-reproductoras -> lote-engorde';
comment on column PC_INCUBACION.reckey is 'PK autonumerica';
comment on column PC_INCUBACION.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_INCUBACION.cod_lote_origen is 'FK a PC_LOTE -- lote de reproductoras del que vinieron los huevos (trazabilidad)';
comment on column PC_INCUBACION.cod_establo is 'FK a PC_ESTABLO -- sala/nave de incubacion (flag_tipo=I)';
comment on column PC_INCUBACION.fec_carga is 'fecha de carga de la incubadora';
comment on column PC_INCUBACION.cantidad_huevos_cargados is 'huevos aptos cargados a la incubadora';
comment on column PC_INCUBACION.fec_eclosion_prevista is 'fecha esperada de nacimiento = fec_carga + TG_ESPECIES.periodo_incubacion_dias de la especie del lote (via PC_LOTE.cod_categoria/PC_ANIMAL o directamente conocido por la especie del lote, ej. 21 dias en aves)';
comment on column PC_INCUBACION.fec_eclosion_real is 'fecha real de nacimiento';
comment on column PC_INCUBACION.cantidad_nacidos is 'pollitos/crias nacidas -- junto a cantidad_huevos_cargados da el % de eclosion';
comment on column PC_INCUBACION.cantidad_mermas is 'huevos no eclosionados (infertiles, muerte embrionaria)';
comment on column PC_INCUBACION.cod_lote_destino is 'FK a PC_LOTE -- lote de engorde formado con los nacidos (se completa al registrar la eclosion)';
comment on column PC_INCUBACION.flag_estado is '0=Anulada, 1=En incubacion, 2=Eclosionada';
comment on column PC_INCUBACION.cod_usr is 'usuario que registro';
comment on column PC_INCUBACION.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_INCUBACION start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_INCUBACION
before insert on PC_INCUBACION
for each row
begin
  if :new.reckey is null then
...
-- ----------------------------------------------------------------------------
create table PC_LOTE_MORTALIDAD
(
  reckey            NUMBER(10) not null,
  cod_origen        CHAR(2)    not null,
  cod_lote          CHAR(6)    not null,
...
comment on table PC_LOTE_MORTALIDAD is 'Pecuario (avicola/engorde masivo) - Mortalidad agregada de un lote, sin registro individual por cabeza';
comment on column PC_LOTE_MORTALIDAD.reckey is 'PK autonumerica';
comment on column PC_LOTE_MORTALIDAD.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_LOTE_MORTALIDAD.cod_lote is 'FK a PC_LOTE';
comment on column PC_LOTE_MORTALIDAD.fec_evento is 'fecha del registro de mortalidad';
comment on column PC_LOTE_MORTALIDAD.cantidad_muertes is 'cantidad de cabezas muertas en el periodo';
comment on column PC_LOTE_MORTALIDAD.motivo is 'causa de la mortalidad, si se conoce';
comment on column PC_LOTE_MORTALIDAD.cod_usr is 'usuario que registro';
comment on column PC_LOTE_MORTALIDAD.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_LOTEMORT start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_LOTEMORT
before insert on PC_LOTE_MORTALIDAD
for each row
begin
  if :new.reckey is null then
...
create or replace trigger TRG_PC_LOTEMORT_AI
after insert on PC_LOTE_MORTALIDAD
for each row
begin
  update PC_LOTE
...
alter table PC_ANIMAL add constraint FK_PC_ANIMAL_ORIGEN foreign key (cod_origen) references ORIGEN(cod_origen);
alter table PC_ANIMAL add constraint FK_PC_ANIMAL_RAZA foreign key (cod_raza) references PC_RAZA(cod_raza);
alter table PC_ANIMAL add constraint FK_PC_ANIMAL_CATEG foreign key (cod_categoria) references PC_CATEGORIA(cod_categoria);
alter table PC_ANIMAL add constraint FK_PC_ANIMAL_POTRERO foreign key (cod_origen, cod_potrero) references PC_POTRERO(cod_origen, cod_potrero);
alter table PC_ANIMAL add constraint FK_PC_ANIMAL_LOTE foreign key (cod_origen, cod_lote) references PC_LOTE(cod_origen, cod_lote);
alter table PC_ANIMAL add constraint FK_PC_ANIMAL_ESTABLO foreign key (cod_origen, cod_establo) references PC_ESTABLO(cod_origen, cod_establo);
alter table PC_ANIMAL add constraint FK_PC_ANIMAL_SEMENTAL foreign key (cod_semental_padre) references PC_SEMENTAL(cod_semental);
-- Auto-referencias (genealogia): padre y madre son animales del mismo hato
alter table PC_ANIMAL add constraint FK_PC_ANIMAL_PADRE foreign key (cod_animal_padre) references PC_ANIMAL(cod_animal);
alter table PC_ANIMAL add constraint FK_PC_ANIMAL_MADRE foreign key (cod_animal_madre) references PC_ANIMAL(cod_animal);

comment on table PC_ANIMAL is 'Pecuario - Maestro de ganado (animal individual, de cualquier especie)';
comment on column PC_ANIMAL.cod_animal is 'PK -- numero de documento del animal: cod_origen(2) + correlativo(8), generado por trigger via NUM_TABLAS (ej. SU00000001). Es UNA SOLA COLUMNA; el origen ya queda embebido';
comment on column PC_ANIMAL.cod_origen is 'FK a ORIGEN -- fundo/sucursal (tambien va embebido en cod_animal)';
comment on column PC_ANIMAL.cod_interno is 'codigo interno/arete fisico que usa la empresa (libre, distinto del documento generado por el sistema)';
comment on column PC_ANIMAL.nom_animal is 'apodo, opcional';
comment on column PC_ANIMAL.cod_raza is 'raza del animal';
comment on column PC_ANIMAL.flag_sexo is 'M=Macho, H=Hembra';
comment on column PC_ANIMAL.fec_nacimiento is 'fecha de nacimiento';
comment on column PC_ANIMAL.cod_animal_padre is 'FK genealogia (auto-referencia) - padre, si es del propio hato';
comment on column PC_ANIMAL.cod_animal_madre is 'FK genealogia (auto-referencia) - madre, si es del propio hato';
comment on column PC_ANIMAL.cod_semental_padre is 'FK a PC_SEMENTAL si el padre fue por inseminacion artificial';
comment on column PC_ANIMAL.color is 'color/marcas distintivas';
comment on column PC_ANIMAL.cod_categoria is 'categoria/etapa actual (se recalcula por edad/estado)';
comment on column PC_ANIMAL.cod_potrero is 'ubicacion actual (potrero)';
comment on column PC_ANIMAL.cod_establo is 'FK opcional a PC_ESTABLO -- ubicacion cubierta actual (sala de ordeno, maternidad, cuarentena, etc.), independiente del potrero; historial completo en PC_MOVIMIENTO_ESTABLO';
comment on column PC_ANIMAL.cod_lote is 'FK opcional a PC_LOTE -- lote/cohorte de manejo al que pertenece esta cabeza (validado contra AGRI: cabezas y lotes son el mismo modelo, no ramas paralelas)';
comment on column PC_ANIMAL.flag_estado_repro is '0=vacia, 1=servida, 2=prenada confirmada, 3=recien parida';
comment on column PC_ANIMAL.peso_nacimiento is 'peso al nacer en kg';
comment on column PC_ANIMAL.peso_actual is 'ultimo peso registrado en kg';
comment on column PC_ANIMAL.fec_ult_pesaje is 'fecha del ultimo pesaje';
comment on column PC_ANIMAL.cod_procedencia is 'P=Nacido en el predio, C=Comprado';
comment on column PC_ANIMAL.fec_ingreso is 'fecha de alta al hato (nacimiento o compra)';
comment on column PC_ANIMAL.precio_compra is 'precio de compra si aplica, para costeo/activo biologico';
comment on column PC_ANIMAL.flag_estado is '1=activo en el hato, 0=de baja (ver PC_BAJA)';
comment on column PC_ANIMAL.cod_usr is 'usuario que registro';
comment on column PC_ANIMAL.fec_registro is 'fecha de registro en el sistema';
...
create or replace trigger TIB_PC_ANIMAL
before insert on PC_ANIMAL
for each row
declare
  ln_ult_nro  NUM_TABLAS.ult_nro%type;
  ln_count    number;
begin
  if :new.cod_animal is null then
    ...
    if ln_count = 0 then
      insert into NUM_TABLAS(tabla, origen, ult_nro) values ('PC_ANIMAL', :new.cod_origen, 1);
    end if;
    select ult_nro into ln_ult_nro from NUM_TABLAS
...
comment on table PC_OT_ANIMAL is 'Pecuario - Vinculo entre una Orden de Trabajo (OT Pecuaria, ot_adm=PECU en ORDEN_TRABAJO) y los animales que cubre';
comment on column PC_OT_ANIMAL.reckey is 'PK autonumerica';
comment on column PC_OT_ANIMAL.nro_orden is 'FK a ORDEN_TRABAJO';
comment on column PC_OT_ANIMAL.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_OT_ANIMAL.cod_animal is 'FK a PC_ANIMAL, animal cubierto por esta OT';
comment on column PC_OT_ANIMAL.cod_usr is 'usuario que registro';
comment on column PC_OT_ANIMAL.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_OT_ANIMAL start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_OT_ANIMAL
before insert on PC_OT_ANIMAL
for each row
begin
  if :new.reckey is null then
...
comment on table PC_CELO is 'Pecuario - Registro de detecciones de celo';
comment on column PC_CELO.reckey is 'PK autonumerica';
comment on column PC_CELO.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_CELO.cod_animal is 'animal (hembra) en celo';
comment on column PC_CELO.fec_celo is 'fecha de deteccion del celo';
comment on column PC_CELO.hora_deteccion is 'hora exacta de deteccion';
comment on column PC_CELO.metodo_deteccion is 'V=Visual, P=Podometro/collar, H=Hormonal';
comment on column PC_CELO.flag_servido is '1 si este celo derivo en un servicio';
comment on column PC_CELO.cod_usr is 'usuario que registro';
comment on column PC_CELO.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_CELO start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_CELO
before insert on PC_CELO
for each row
begin
  if :new.reckey is null then
...
alter table PC_SERVICIO add constraint FK_PC_SERVICIO_TORO foreign key (cod_animal_toro) references PC_ANIMAL(cod_animal);
alter table PC_SERVICIO add constraint FK_PC_SERVICIO_SEMENTAL foreign key (cod_semental) references PC_SEMENTAL(cod_semental);
alter table PC_SERVICIO add constraint FK_PC_SERVICIO_TECNICO foreign key (cod_tecnico) references PROVEEDOR(proveedor);

comment on table PC_SERVICIO is 'Pecuario - Registro de servicio (monta natural o inseminacion artificial). Es un HISTORIAL: un mismo animal tiene tantas filas como servicios reciba en su vida';
comment on column PC_SERVICIO.nro_servicio is 'PK -- numero de documento del servicio: cod_origen(2) + correlativo(8), generado por trigger via NUM_TABLAS (ej. SU00000001). UNA SOLA COLUMNA, el origen ya va embebido';
comment on column PC_SERVICIO.cod_origen is 'FK a ORIGEN -- fundo/sucursal (tambien va embebido en nro_servicio)';
comment on column PC_SERVICIO.cod_animal is 'FK a PC_ANIMAL -- hembra servida';
comment on column PC_SERVICIO.fec_servicio is 'fecha del servicio';
comment on column PC_SERVICIO.flag_tipo_servicio is 'N=Monta natural, I=Inseminacion artificial';
comment on column PC_SERVICIO.cod_animal_toro is 'toro del propio hato (si monta natural)';
comment on column PC_SERVICIO.cod_semental is 'semental/pajilla (si inseminacion artificial)';
comment on column PC_SERVICIO.cod_tecnico is 'FK a PROVEEDOR -- responsable de la inseminacion (generalmente externo, no personal de planilla)';
comment on column PC_SERVICIO.fec_prob_parto is 'fecha probable de parto = fec_servicio + TG_ESPECIES.periodo_gestacion_dias de la especie del animal (via PC_ANIMAL.cod_raza -> PC_RAZA.cod_especie -> TG_ESPECIES); NO hardcodear 283 dias, ese valor es solo el default bovino';
comment on column PC_SERVICIO.flag_estado is '1=vigente/en curso, 0=anulado (repitio celo, no prendio)';
comment on column PC_SERVICIO.cod_usr is 'usuario que registro';
comment on column PC_SERVICIO.fec_registro is 'fecha de registro en el sistema';

create or replace trigger TIB_PC_SERVICIO
before insert on PC_SERVICIO
for each row
declare
  ln_ult_nro  NUM_TABLAS.ult_nro%type;
  ln_count    number;
begin
  if :new.nro_servicio is null then
    ...
    if ln_count = 0 then
      insert into NUM_TABLAS(tabla, origen, ult_nro) values ('PC_SERVICIO', :new.cod_origen, 1);
    end if;
    select ult_nro into ln_ult_nro from NUM_TABLAS
...
-- ----------------------------------------------------------------------------
create table PC_DIAGNOSTICO_PRENEZ
(
  reckey            NUMBER(10) not null,
  cod_origen        CHAR(2)   not null,
  nro_servicio      CHAR(10)  not null,
...
alter table PC_DIAGNOSTICO_PRENEZ add constraint FK_PC_DXPRE_SERVICIO foreign key (nro_servicio) references PC_SERVICIO(nro_servicio);
alter table PC_DIAGNOSTICO_PRENEZ add constraint FK_PC_DXPRE_ANIMAL foreign key (cod_animal) references PC_ANIMAL(cod_animal);
alter table PC_DIAGNOSTICO_PRENEZ add constraint FK_PC_DXPRE_VET foreign key (cod_veterinario) references PROVEEDOR(proveedor);

comment on table PC_DIAGNOSTICO_PRENEZ is 'Pecuario - Diagnostico de prenez posterior al servicio';
comment on column PC_DIAGNOSTICO_PRENEZ.reckey is 'PK autonumerica';
comment on column PC_DIAGNOSTICO_PRENEZ.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_DIAGNOSTICO_PRENEZ.nro_servicio is 'FK a PC_SERVICIO que se esta confirmando';
comment on column PC_DIAGNOSTICO_PRENEZ.cod_animal is 'FK a PC_ANIMAL (denormalizado desde PC_SERVICIO para consulta directa)';
comment on column PC_DIAGNOSTICO_PRENEZ.fec_diagnostico is 'fecha del diagnostico';
comment on column PC_DIAGNOSTICO_PRENEZ.metodo is 'T=Tacto rectal, E=Ecografia';
comment on column PC_DIAGNOSTICO_PRENEZ.resultado is 'P=Prenada, V=Vacia';
comment on column PC_DIAGNOSTICO_PRENEZ.dias_gestacion is 'dias de gestacion calculados';
comment on column PC_DIAGNOSTICO_PRENEZ.cod_veterinario is 'FK a PROVEEDOR -- veterinario responsable (generalmente externo, no personal de planilla)';
comment on column PC_DIAGNOSTICO_PRENEZ.cod_usr is 'usuario que registro';
comment on column PC_DIAGNOSTICO_PRENEZ.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_DXPRE start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_DXPRE
before insert on PC_DIAGNOSTICO_PRENEZ
for each row
begin
  if :new.reckey is null then
...
alter table PC_PARTO add constraint FK_PC_PARTO_SERVICIO foreign key (nro_servicio) references PC_SERVICIO(nro_servicio);
alter table PC_PARTO add constraint FK_PC_PARTO_CRIA foreign key (cod_animal_cria) references PC_ANIMAL(cod_animal);
alter table PC_PARTO add constraint FK_PC_PARTO_VET foreign key (cod_veterinario) references PROVEEDOR(proveedor);

comment on table PC_PARTO is 'Pecuario - Registro de parto';
comment on column PC_PARTO.reckey is 'PK autonumerica';
comment on column PC_PARTO.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_PARTO.cod_animal is 'animal (madre) que pario';
comment on column PC_PARTO.fec_parto is 'fecha del parto';
comment on column PC_PARTO.nro_servicio is 'FK a PC_SERVICIO que origino este parto (si se conoce)';
comment on column PC_PARTO.flag_tipo_parto is 'E=Eutocico (normal), D=Distocico (con complicaciones)';
comment on column PC_PARTO.flag_asistido is 'parto asistido (1=si)';
comment on column PC_PARTO.cod_animal_cria is 'FK a PC_ANIMAL - la cria recien nacida';
comment on column PC_PARTO.sexo_cria is 'sexo de la cria';
comment on column PC_PARTO.peso_cria is 'peso de la cria al nacer en kg';
comment on column PC_PARTO.flag_cria_viva is '1=viva, 0=nacio muerta';
comment on column PC_PARTO.flag_retencion_placenta is '1=hubo retencion de placenta';
comment on column PC_PARTO.observaciones is 'observaciones del parto';
comment on column PC_PARTO.cod_veterinario is 'FK a PROVEEDOR -- veterinario responsable (generalmente externo, no personal de planilla)';
comment on column PC_PARTO.cod_usr is 'usuario que registro';
comment on column PC_PARTO.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_PARTO start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_PARTO
before insert on PC_PARTO
for each row
begin
  if :new.reckey is null then
...
comment on table PC_LACTANCIA is 'Pecuario - Periodos de lactancia (uno por parto)';
comment on column PC_LACTANCIA.reckey is 'PK autonumerica';
comment on column PC_LACTANCIA.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_LACTANCIA.cod_animal is 'vaca en lactancia';
comment on column PC_LACTANCIA.nro_lactancia is 'correlativo de lactancia de esta vaca (1ra, 2da, ...)';
comment on column PC_LACTANCIA.fec_parto is 'FK (junto a cod_animal) al parto que inicio esta lactancia';
comment on column PC_LACTANCIA.fec_secado is 'fecha de secado (cierre de la lactancia)';
comment on column PC_LACTANCIA.dias_lactancia is 'dias de lactancia, calculado al secar';
comment on column PC_LACTANCIA.litros_totales is 'litros acumulados, recalculado desde PC_ORDENO';
comment on column PC_LACTANCIA.flag_estado is '1=en curso, 0=cerrada (seca)';
comment on column PC_LACTANCIA.cod_usr is 'usuario que registro';
comment on column PC_LACTANCIA.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_LACTANCIA start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_LACTANCIA
before insert on PC_LACTANCIA
for each row
begin
  if :new.reckey is null then
...
alter table PC_ORDENO add constraint FK_PC_ORDENO_OT foreign key (nro_orden) references ORDEN_TRABAJO(nro_orden);
alter table PC_ORDENO add constraint FK_PC_ORDENO_ALMACEN foreign key (cod_almacen) references ALMACEN(almacen);
alter table PC_ORDENO add constraint FK_PC_ORDENO_PRODUCTO foreign key (cod_producto) references TIPO_PRODUCTO(cod_prod);

comment on table PC_ORDENO is 'Pecuario - Detalle diario de ordeno';
comment on column PC_ORDENO.reckey is 'PK autonumerica';
comment on column PC_ORDENO.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_ORDENO.cod_animal is 'vaca ordenada';
comment on column PC_ORDENO.fec_ordeno is 'fecha del ordeno';
comment on column PC_ORDENO.nro_turno is '1=manana, 2=tarde, 3=noche';
comment on column PC_ORDENO.litros is 'litros obtenidos en este ordeno';
comment on column PC_ORDENO.flag_descarte is '1 si la leche no se vende (periodo de retiro por medicamento)';
comment on column PC_ORDENO.nro_orden is 'FK a ORDEN_TRABAJO -- la misma OT usada para el consumo de alimento de ese potrero/dia; el cierre diario de la OT genera el ingreso real I09 en ARTICULO_MOV';
comment on column PC_ORDENO.cod_almacen is 'FK a ALMACEN -- almacen de destino (produccion) donde ingresa la leche';
comment on column PC_ORDENO.cod_producto is 'FK a TIPO_PRODUCTO -- producto terminado (ej. Leche cruda)';
comment on column PC_ORDENO.cod_usr is 'usuario que registro';
comment on column PC_ORDENO.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_ORDENO start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_ORDENO
before insert on PC_ORDENO
for each row
begin
  if :new.reckey is null then
...
-- PC_CONTROL_LECHERO
-- ----------------------------------------------------------------------------
create table PC_CONTROL_LECHERO
(
  reckey             NUMBER(10) not null,
  cod_origen         CHAR(2)   not null,
  cod_animal         CHAR(10)  not null,
...
comment on table PC_CONTROL_LECHERO is 'Pecuario - Control lechero mensual (calidad de leche)';
comment on column PC_CONTROL_LECHERO.reckey is 'PK autonumerica';
comment on column PC_CONTROL_LECHERO.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_CONTROL_LECHERO.cod_animal is 'vaca controlada';
comment on column PC_CONTROL_LECHERO.fec_control is 'fecha del muestreo';
comment on column PC_CONTROL_LECHERO.porc_grasa is 'porcentaje de grasa';
comment on column PC_CONTROL_LECHERO.porc_proteina is 'porcentaje de proteina';
comment on column PC_CONTROL_LECHERO.celulas_somaticas is 'conteo de celulas somaticas (CCS/SCC), celulas/ml';
comment on column PC_CONTROL_LECHERO.litros_dia_proy is 'litros/dia proyectados el dia del control';
comment on column PC_CONTROL_LECHERO.cod_usr is 'usuario que registro';
comment on column PC_CONTROL_LECHERO.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_CTRLLECHE start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_CTRLLECHE
before insert on PC_CONTROL_LECHERO
for each row
begin
  if :new.reckey is null then
...
-- ----------------------------------------------------------------------------
create table PC_CONDICION_CORPORAL
(
  reckey          NUMBER(10)  not null,
  cod_origen      CHAR(2)     not null,
  cod_animal      CHAR(10)    not null,
...
comment on table PC_CONDICION_CORPORAL is 'Pecuario - Historico de condicion corporal (BCS)';
comment on column PC_CONDICION_CORPORAL.reckey is 'PK autonumerica';
comment on column PC_CONDICION_CORPORAL.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_CONDICION_CORPORAL.cod_animal is 'animal evaluado';
comment on column PC_CONDICION_CORPORAL.fec_evaluacion is 'fecha de evaluacion';
comment on column PC_CONDICION_CORPORAL.puntaje_bcs is 'puntaje BCS, escala 1.0 a 5.0';
comment on column PC_CONDICION_CORPORAL.cod_usr is 'usuario que registro';
comment on column PC_CONDICION_CORPORAL.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_BCS start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_BCS
before insert on PC_CONDICION_CORPORAL
for each row
begin
  if :new.reckey is null then
...
-- ----------------------------------------------------------------------------
create table PC_ALIMENTACION_CONSUMO
(
  reckey          NUMBER(10)   not null,
  cod_origen      CHAR(2)      not null,
  cod_potrero     CHAR(6)      not null,
...
alter table PC_ALIMENTACION_CONSUMO add constraint FK_PC_ALIMCONS_OT foreign key (nro_orden) references ORDEN_TRABAJO(nro_orden);

comment on table PC_ALIMENTACION_CONSUMO is 'Pecuario - Planificacion de alimentacion diaria por potrero/lote (dieta + cabezas); el consumo real de insumos (cod_art, cantidad) queda registrado en ARTICULO_MOV contra la OT de esta fila';
comment on column PC_ALIMENTACION_CONSUMO.reckey is 'PK autonumerica';
comment on column PC_ALIMENTACION_CONSUMO.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_ALIMENTACION_CONSUMO.cod_potrero is 'potrero/lote donde se dio el alimento';
comment on column PC_ALIMENTACION_CONSUMO.fec_consumo is 'fecha del consumo';
comment on column PC_ALIMENTACION_CONSUMO.cod_dieta is 'dieta aplicada';
comment on column PC_ALIMENTACION_CONSUMO.cabezas_lote is 'cantidad de animales que comieron esta dieta ese dia';
comment on column PC_ALIMENTACION_CONSUMO.nro_orden is 'FK a ORDEN_TRABAJO -- OT (ot_adm=PECU) que agrupa el consumo real de insumos en ARTICULO_MOV para este potrero/dia; ver PC_OT_ANIMAL para saber que animales cubre';
comment on column PC_ALIMENTACION_CONSUMO.cod_usr is 'usuario que registro';
comment on column PC_ALIMENTACION_CONSUMO.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_ALIMCONS start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_ALIMCONS
before insert on PC_ALIMENTACION_CONSUMO
for each row
begin
  if :new.reckey is null then
...
-- 6) SANIDAD
-- ============================================================================
create table PC_SANIDAD_EVENTO
(
  reckey             NUMBER(10) not null,
  cod_origen         CHAR(2)   not null,
  cod_animal         CHAR(10)  not null,
...
alter table PC_SANIDAD_EVENTO add constraint FK_PC_SANIDAD_PROD foreign key (cod_prod_san) references PC_PRODUCTO_SANITARIO(cod_prod_san);
alter table PC_SANIDAD_EVENTO add constraint FK_PC_SANIDAD_ENFERM foreign key (cod_enfermedad) references PC_ENFERMEDAD(cod_enfermedad);
alter table PC_SANIDAD_EVENTO add constraint FK_PC_SANIDAD_OT foreign key (nro_orden) references ORDEN_TRABAJO(nro_orden);
alter table PC_SANIDAD_EVENTO add constraint FK_PC_SANIDAD_OS foreign key (cod_origen, nro_os) references ORDEN_SERVICIO(cod_origen, nro_os);
alter table PC_SANIDAD_EVENTO add constraint FK_PC_SANIDAD_VET foreign key (cod_veterinario) references PROVEEDOR(proveedor);

comment on table PC_SANIDAD_EVENTO is 'Pecuario - Eventos veterinarios (vacunas, desparasitaciones, tratamientos, diagnosticos)';
comment on column PC_SANIDAD_EVENTO.reckey is 'PK autonumerica';
comment on column PC_SANIDAD_EVENTO.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_SANIDAD_EVENTO.cod_animal is 'animal atendido';
comment on column PC_SANIDAD_EVENTO.nro_evento is 'correlativo de eventos sanitarios de este animal (unique junto a cod_animal, no PK)';
comment on column PC_SANIDAD_EVENTO.fec_evento is 'fecha del evento';
comment on column PC_SANIDAD_EVENTO.flag_tipo_evento is 'V=Vacuna, D=Desparasitacion, T=Tratamiento, X=Diagnostico';
comment on column PC_SANIDAD_EVENTO.cod_prod_san is 'producto aplicado (vacuna/medicamento)';
comment on column PC_SANIDAD_EVENTO.dosis is 'dosis aplicada';
comment on column PC_SANIDAD_EVENTO.cod_enfermedad is 'enfermedad diagnosticada/tratada, si aplica';
comment on column PC_SANIDAD_EVENTO.cod_veterinario is 'FK a PROVEEDOR -- veterinario responsable (generalmente externo, no personal de planilla)';
comment on column PC_SANIDAD_EVENTO.costo is 'costo de referencia rapida (el costo real y autorizado vive en ORDEN_SERVICIO.monto_total cuando nro_os esta presente)';
comment on column PC_SANIDAD_EVENTO.nro_orden is 'FK opcional a ORDEN_TRABAJO -- OT (ot_adm=PECU) que registra en ARTICULO_MOV el consumo real del producto sanitario (vacuna/desparasitante propio)';
comment on column PC_SANIDAD_EVENTO.nro_os is 'FK opcional a ORDEN_SERVICIO -- cuando el evento implica un servicio/costo externo (veterinario, laboratorio de terceros)';
comment on column PC_SANIDAD_EVENTO.fec_prox_refuerzo is 'fecha calculada de proximo refuerzo (segun producto)';
comment on column PC_SANIDAD_EVENTO.fec_fin_retiro is 'fecha calculada de fin de periodo de retiro (leche/venta)';
comment on column PC_SANIDAD_EVENTO.observaciones is 'observaciones del evento';
comment on column PC_SANIDAD_EVENTO.cod_usr is 'usuario que registro';
comment on column PC_SANIDAD_EVENTO.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_SANIDAD start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_SANIDAD
before insert on PC_SANIDAD_EVENTO
for each row
begin
  if :new.reckey is null then
...
alter table PC_LABORATORIO add constraint FK_PC_LAB_ORIGEN foreign key (cod_origen) references ORIGEN(cod_origen);
alter table PC_LABORATORIO add constraint FK_PC_LAB_ANIMAL foreign key (cod_animal) references PC_ANIMAL(cod_animal);
alter table PC_LABORATORIO add constraint FK_PC_LAB_SEMENTAL foreign key (cod_semental) references PC_SEMENTAL(cod_semental);
alter table PC_LABORATORIO add constraint FK_PC_LAB_SANIDAD foreign key (cod_animal, nro_evento) references PC_SANIDAD_EVENTO(cod_animal, nro_evento);
alter table PC_LABORATORIO add constraint FK_PC_LAB_VET foreign key (cod_veterinario) references PROVEEDOR(proveedor);
alter table PC_LABORATORIO add constraint FK_PC_LAB_CLIENTE foreign key (cod_cliente) references PROVEEDOR(proveedor);

comment on table PC_LABORATORIO is 'Pecuario - Cabecera de muestras enviadas a laboratorio (sangre, leche, fecal, semen, tejido)';
comment on column PC_LABORATORIO.reckey is 'PK autonumerica';
comment on column PC_LABORATORIO.nro_muestra is 'codigo de muestra del laboratorio externo (texto libre, no autogenerado, no PK)';
comment on column PC_LABORATORIO.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_LABORATORIO.cod_animal is 'animal muestreado (nulo si la muestra no es de un animal puntual, ej. forraje/agua)';
comment on column PC_LABORATORIO.cod_semental is 'semental analizado, si la muestra es de control de calidad de semen';
comment on column PC_LABORATORIO.fec_muestra is 'fecha de toma de la muestra';
comment on column PC_LABORATORIO.flag_tipo_muestra is 'S=Sangre, L=Leche, F=Fecal, M=Semen, T=Tejido/necropsia, O=Otro';
comment on column PC_LABORATORIO.laboratorio is 'laboratorio externo que proceso la muestra';
comment on column PC_LABORATORIO.cod_veterinario is 'FK a PROVEEDOR -- veterinario que tomo la muestra (generalmente externo, no personal de planilla)';
comment on column PC_LABORATORIO.nro_evento is 'FK opcional (junto a cod_animal) a PC_SANIDAD_EVENTO, si la muestra es parte de un evento sanitario ya registrado';
comment on column PC_LABORATORIO.fec_resultado is 'fecha en que el laboratorio entrego el resultado';
comment on column PC_LABORATORIO.flag_estado is '0=Anulada, 1=Pendiente de resultado, 2=Con resultado';
comment on column PC_LABORATORIO.flag_origen is 'P=Propio (laboratorio/veterinario nuestro), C=Cliente (lo realiza el comprador de la leche, ej. Laive, y descuenta el costo de la factura)';
comment on column PC_LABORATORIO.cod_cliente is 'FK a PROVEEDOR (unifica proveedor/cliente via flag_clie_prov) -- cliente que realizo/solicito el analisis, si flag_origen=C';
comment on column PC_LABORATORIO.costo_analisis is 'costo del analisis; si flag_origen=C, es el monto que el cliente descuenta de la factura de venta';
comment on column PC_LABORATORIO.flag_facturado is '1 si el costo del analisis ya fue descontado/facturado';
comment on column PC_LABORATORIO.observaciones is 'observaciones de la muestra';
comment on column PC_LABORATORIO.cod_usr is 'usuario que registro';
comment on column PC_LABORATORIO.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_LABORATORIO start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_LABORATORIO
before insert on PC_LABORATORIO
for each row
begin
  if :new.reckey is null then
...
-- PC_LABORATORIO_DET
-- ----------------------------------------------------------------------------
create table PC_LABORATORIO_DET
(
  reckey              NUMBER(10)    not null,
...
comment on table PC_LABORATORIO_DET is 'Pecuario - Detalle de parametros/analitos resultantes de una muestra de laboratorio';
comment on column PC_LABORATORIO_DET.reckey is 'PK autonumerica';
comment on column PC_LABORATORIO_DET.lab_reckey is 'FK a PC_LABORATORIO.reckey -- muestra a la que pertenece';
comment on column PC_LABORATORIO_DET.item is 'item correlativo (unique junto a lab_reckey, no PK)';
comment on column PC_LABORATORIO_DET.parametro is 'nombre del parametro/analito (ej. Brucelosis - ELISA, Motilidad espermatica, Huevos por gramo)';
comment on column PC_LABORATORIO_DET.valor_resultado is 'valor del resultado (texto, admite cualitativo Positivo/Negativo o numerico)';
comment on column PC_LABORATORIO_DET.unidad_medida is 'unidad de medida del resultado, si es numerico';
comment on column PC_LABORATORIO_DET.valor_ref_min is 'limite inferior del rango de referencia, si aplica';
comment on column PC_LABORATORIO_DET.valor_ref_max is 'limite superior del rango de referencia, si aplica';
comment on column PC_LABORATORIO_DET.flag_interpretacion is 'N=Normal, A=Alterado';

create sequence SEQ_PC_LABDET start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_LABDET
before insert on PC_LABORATORIO_DET
for each row
begin
  if :new.reckey is null then
...
-- ----------------------------------------------------------------------------
create table PC_MOVIMIENTO_POTRERO
(
  reckey               NUMBER(10) not null,
  cod_origen           CHAR(2)   not null,
  cod_animal           CHAR(10)  not null,
...
alter table PC_MOVIMIENTO_POTRERO add constraint FK_PC_MOVPOT_POT_DEST foreign key (cod_origen, cod_potrero_destino) references PC_POTRERO(cod_origen, cod_potrero);

comment on table PC_MOVIMIENTO_POTRERO is 'Pecuario - Historico de cambios de potrero (rotacion de pastoreo)';
comment on column PC_MOVIMIENTO_POTRERO.reckey is 'PK autonumerica';
comment on column PC_MOVIMIENTO_POTRERO.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_MOVIMIENTO_POTRERO.cod_animal is 'animal movido';
comment on column PC_MOVIMIENTO_POTRERO.fec_movimiento is 'fecha del movimiento';
comment on column PC_MOVIMIENTO_POTRERO.cod_potrero_origen is 'potrero de origen';
comment on column PC_MOVIMIENTO_POTRERO.cod_potrero_destino is 'potrero de destino';
comment on column PC_MOVIMIENTO_POTRERO.motivo is 'motivo del movimiento';
comment on column PC_MOVIMIENTO_POTRERO.cod_usr is 'usuario que registro';
comment on column PC_MOVIMIENTO_POTRERO.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_MOVPOT start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_MOVPOT
before insert on PC_MOVIMIENTO_POTRERO
for each row
begin
  if :new.reckey is null then
...
-- ----------------------------------------------------------------------------
create table PC_MOVIMIENTO_ESTABLO
(
  reckey               NUMBER(10) not null,
  cod_origen           CHAR(2)   not null,
  cod_animal           CHAR(10)  not null,
...
alter table PC_MOVIMIENTO_ESTABLO add constraint FK_PC_MOVEST_EST_DEST foreign key (cod_origen, cod_establo_destino) references PC_ESTABLO(cod_origen, cod_establo);

comment on table PC_MOVIMIENTO_ESTABLO is 'Pecuario - Historico de cambios de establo (ubicacion cubierta)';
comment on column PC_MOVIMIENTO_ESTABLO.reckey is 'PK autonumerica';
comment on column PC_MOVIMIENTO_ESTABLO.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_MOVIMIENTO_ESTABLO.cod_animal is 'animal movido';
comment on column PC_MOVIMIENTO_ESTABLO.fec_movimiento is 'fecha del movimiento';
comment on column PC_MOVIMIENTO_ESTABLO.cod_establo_origen is 'establo de origen (nulo si venia de pastoreo/sin establo)';
comment on column PC_MOVIMIENTO_ESTABLO.cod_establo_destino is 'establo de destino';
comment on column PC_MOVIMIENTO_ESTABLO.motivo is 'motivo del movimiento (ej. ordeno, parto, cuarentena por enfermedad)';
comment on column PC_MOVIMIENTO_ESTABLO.cod_usr is 'usuario que registro';
comment on column PC_MOVIMIENTO_ESTABLO.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_MOVEST start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_MOVEST
before insert on PC_MOVIMIENTO_ESTABLO
for each row
begin
  if :new.reckey is null then
...
comment on table PC_DTA is 'Pecuario - Documento de Transito Animal (trazabilidad SENASA)';
comment on column PC_DTA.reckey is 'PK autonumerica (interna)';
comment on column PC_DTA.nro_dta is 'numero de DTA asignado por SENASA -- documento EXTERNO, texto libre, no autogenerado por este sistema';
comment on column PC_DTA.fec_emision is 'fecha de emision';
comment on column PC_DTA.cod_origen_fundo is 'FK a ORIGEN -- fundo de origen';
comment on column PC_DTA.cod_destino_fundo is 'FK a ORIGEN -- fundo de destino (si es traslado interno)';
comment on column PC_DTA.razon_social_destino is 'razon social del destino (si es venta a un tercero)';
comment on column PC_DTA.motivo is 'V=Venta, T=Traslado interno, F=Feria/exposicion';
comment on column PC_DTA.flag_estado is 'flag_estado';
comment on column PC_DTA.cod_usr is 'usuario que registro';
comment on column PC_DTA.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_DTA start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_DTA
before insert on PC_DTA
for each row
begin
  if :new.reckey is null then
...
comment on table PC_DTA_DETALLE is 'Pecuario - Animales incluidos en cada DTA';
comment on column PC_DTA_DETALLE.reckey is 'PK autonumerica';
comment on column PC_DTA_DETALLE.dta_reckey is 'FK a PC_DTA.reckey -- DTA al que pertenece';
comment on column PC_DTA_DETALLE.item is 'item correlativo (unique junto a dta_reckey, no PK)';
comment on column PC_DTA_DETALLE.cod_animal is 'animal incluido en el traslado';

create sequence SEQ_PC_DTADET start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_DTADET
before insert on PC_DTA_DETALLE
for each row
begin
  if :new.reckey is null then
...
alter table PC_BAJA add constraint FK_PC_BAJA_OV foreign key (nro_ov) references ORDEN_VENTA(nro_ov);
alter table PC_BAJA add constraint FK_PC_BAJA_DTA foreign key (dta_reckey) references PC_DTA(reckey);

comment on table PC_BAJA is 'Pecuario - Bajas del hato (venta, muerte o descarte)';
comment on column PC_BAJA.reckey is 'PK autonumerica';
comment on column PC_BAJA.cod_origen is 'FK a ORIGEN -- fundo/sucursal';
comment on column PC_BAJA.cod_animal is 'animal dado de baja (unique, no PK -- una baja por animal)';
comment on column PC_BAJA.fec_baja is 'fecha de la baja';
comment on column PC_BAJA.flag_motivo is 'V=Venta, M=Muerte, D=Descarte (infertil/enfermo/vejez)';
comment on column PC_BAJA.causa_muerte is 'causa de muerte, si flag_motivo=M';
comment on column PC_BAJA.precio_venta is 'precio de venta, si flag_motivo=V';
comment on column PC_BAJA.nro_ov is 'FK a ORDEN_VENTA -- orden/factura de venta real (modulo Comercializacion), si flag_motivo=V';
comment on column PC_BAJA.dta_reckey is 'FK a PC_DTA.reckey si la baja implico traslado';
comment on column PC_BAJA.observaciones is 'observaciones de la baja';
comment on column PC_BAJA.cod_usr is 'usuario que registro';
comment on column PC_BAJA.fec_registro is 'fecha de registro en el sistema';

create sequence SEQ_PC_BAJA start with 1 increment by 1 nocache;
create or replace trigger TIB_PC_BAJA
before insert on PC_BAJA
for each row
begin
  if :new.reckey is null then
...
-- ----------------------------------------------------------------------------
create or replace trigger TRG_PC_BAJA_AI
after insert on PC_BAJA
for each row
begin
  update PC_ANIMAL
     set flag_estado = '0'
