...
10. **Decisión final de arquitectura y empresa destino (confirmado por el usuario)**:
    ...
    - **Error "ii_ck no tiene valor" — resuelto en el ancestro, no por ventana** — `u_dw_abc` (ancestro real de todo DataWindow control) postea `ue_val_param` desde su `constructor`, que exige `UpperBound(ii_ck) > 0` (columna(s) PK del dw) o muestra el MessageBox de error; peor aún, `of_create_log`/`of_save_log` (log diario de auditoría) están condicionados a `ib_log AND UpperBound(ii_ck) > 0` — sin `ii_ck`, el log de auditoría queda silenciosamente desactivado, no solo el mensaje. Primer intento: setear `ii_ck[1] = 1` a mano en el `constructor` de cada una de las 10 ventanas Pecuario (confirmado contra un ejemplo real, `w_rh083_via.srw` de RRHH) — funcionaba, pero el usuario aclaró que la idea es que **no sea tarea del desarrollador** especificar `ii_ck` ventana por ventana: el propio objeto DataWindow debe poder obtener su PK por código. **Fix definitivo**: en las **3 copias sincronizadas** de `u_dw_abc.sru` encontradas en el repo (`CoreLibrary_uo.pbl.src`, `BaseClassLib_uo.pbl.src` e `Integrador.pbl.src` — la parte tocada estaba verificada idéntica entre las tres antes del cambio, aunque el resto del archivo diverge entre copias) se agregó una **función nueva** `of_autodetect_ck()` (declarada en el bloque `forward prototypes` y con cuerpo propio, no código suelto dentro de un evento): recorre todas las columnas del dw (`DataWindow.Column.Count` + `#N.Name`, patrón ya usado en `of_protect` de la misma clase), marca en `ii_ck[]` cualquier columna con `key=yes` (via `Describe(columna + '.Key')`, el mismo nombre de atributo literal que aparece en el `table()` del `.srd` exportado) y devuelve cuántas encontró. El evento `ue_conversion` (que ya corre automático via `Post` desde `constructor`) quedó reducido a `IF UpperBound(ii_ck) = 0 THEN THIS.of_autodetect_ck() END IF` — es puramente aditivo, solo actúa si `ii_ck` sigue vacío, nunca pisa una ventana que ya lo setea a mano, por lo que no debería afectar ninguna de las ~250 ventanas existentes del ERP que ya referencian `ii_ck`. Con esto, las 10 ventanas Pecuario ya NO necesitan seteo manual de `ii_ck` (se revirtió la línea `ii_ck[1] = 1` agregada en el intento anterior); `ii_dk`/`ii_rk` de `PC007`/`PC009` sí siguen siendo manuales porque el vínculo maestro↔detalle es específico de cada ventana, no se puede inferir solo de `key=yes`. **Pendiente de verificar en PowerBuilder**: el atributo `.Key` vía `Describe()` no tiene un ejemplo literal ya existente en el repo (a diferencia de `.Protect`/`.ColType`/`.ID`/`.x`/`.width`, todos confirmados) — se infiere por el mismo patrón de nombre-de-atributo-tal-cual-en-el-.srd, pero conviene probarlo en una sola ventana antes de asumir que las ~250 pantallas que ya referencian `ii_ck` siguen funcionando igual.
