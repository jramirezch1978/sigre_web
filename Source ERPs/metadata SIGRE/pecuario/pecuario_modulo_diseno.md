...
Puntos clave confirmados en el sistema actual (ver también sección 5.6):

- Activo Fijo ya tiene el patrón de integración a replicar: `AF_MATRIZ_CONTABLE` (cuentas por `tipo_activo`+`cencos`), cálculo periódico que distribuye por centro de costos, y pre-asientos (`CNTBL_PRE_ASIENTO`/`CNTBL_PRE_ASIENTO_DET`) que se transfieren al libro diario (`CNTBL_ASIENTO`/`CNTBL_ASIENTO_DET`). Actualmente la generación automática está **deshabilitada** en Activo Fijo (transferencia manual) — decisión de negocio pendiente para Pecuario.
- Bajo NIC 41, el ganado **no se deprecia** — se revalúa periódicamente a valor razonable menos costos de venta, con la variación a resultados. La matriz contable de Pecuario necesitaría conceptos distintos: (activo biológico / ingreso o gasto por cambio en valor razonable), no (activo / depreciación acumulada / gasto).
- Hechos económicos que necesitan asiento: **Alta** (nacimiento o compra, CAM391), **Revaluación periódica** (proceso CAM904), **Baja por venta** (CAM405, con `ORDEN_VENTA`), **Baja por muerte/descarte** (CAM405). La leche es inventario (NIC 2), no activo biológico — se reconoce como producto agrícola al valor razonable en el punto de cosecha (el ordeño, CAM397).
...
| `cod_raza` | CHAR(4) | UNIQUE | Código de raza |
...
| `cod_art` | CHAR(12) | Sí | FK → `ARTICULO` — artículo real de stock |
| `dias_refuerzo` | NUMBER(4) | Sí | Días al refuerzo |
| `periodo_retiro` | NUMBER(3) | Sí | Días de retiro |
| `unidad_medida` | CHAR(3) | Sí | Unidad de dosis |
| `flag_estado` | CHAR(1) | No | 1=Activo, 0=Inactivo |
...
#### `PC_DIETA` / `PC_DIETA_COMPONENTE` — Raciones (PK reckey en ambas)
| Tabla | Columna | Tipo | Null | Descripción |
|---|---|---|---|---|
| `PC_DIETA` | `reckey` | NUMBER(10) | PK | Autonumérica |
| `PC_DIETA` | `cod_dieta` | CHAR(6) | UNIQUE | Código |
| `PC_DIETA` | `nom_dieta` | VARCHAR2(80) | No | Nombre |
| `PC_DIETA` | `cod_categoria` | CHAR(3) | No | FK → `PC_CATEGORIA` |
| `PC_DIETA` | `costo_kg_prom` | NUMBER(10,4) | Sí | Costo promedio/kg |
| `PC_DIETA_COMPONENTE` | `reckey` | NUMBER(10) | PK | Autonumérica |
| `PC_DIETA_COMPONENTE` | `cod_dieta`, `item` | — | UNIQUE | Dieta + correlativo |
| `PC_DIETA_COMPONENTE` | `cod_art` | CHAR(12) | No | FK → `ARTICULO` |
| `PC_DIETA_COMPONENTE` | `cantidad_kg` | NUMBER(8,3) | No | Kg por animal/día |

#### `PC_RECETA` / `PC_RECETA_DET` — Fabricación de concentrado (PK reckey en ambas)
| Tabla | Columna | Tipo | Null | Descripción |
|---|---|---|---|---|
| `PC_RECETA` | `reckey` | NUMBER(10) | PK | Autonumérica |
| `PC_RECETA` | `cod_receta` | CHAR(6) | UNIQUE | Código |
| `PC_RECETA` | `nom_receta` | VARCHAR2(80) | No | Nombre |
| `PC_RECETA` | `cod_producto` | CHAR(12) | No | FK → `TIPO_PRODUCTO` (el concentrado) |
| `PC_RECETA` | `rendimiento_kg` | NUMBER(10,3) | No | Kg de producto que rinde un lote |
| `PC_RECETA_DET` | `reckey` | NUMBER(10) | PK | Autonumérica |
| `PC_RECETA_DET` | `cod_receta`, `item` | — | UNIQUE | Receta + correlativo |
| `PC_RECETA_DET` | `cod_art` | CHAR(12) | No | FK → `ARTICULO` (materia prima) |
| `PC_RECETA_DET` | `cantidad_kg` | NUMBER(10,3) | No | Kg de materia prima por lote |
...
| `cod_tecnico` | CHAR(6) | Sí | Responsable |
...
| `cod_veterinario` | CHAR(6) | Sí | Responsable |
| `cod_usr` / `fec_registro` | — | Sí / No | Auditoría |
...
| `cod_veterinario`, `cod_usr` / `fec_registro` | — | Sí / No | Auditoría |
