...
10. **Decisión final de arquitectura y empresa destino (confirmado por el usuario)**:
    ...
    - **Bug real encontrado por el usuario: `reckey` desalineaba todas las columnas del grid** — los 11 DataWindows de catálogo (`PC001`, `PC003`-`PC010`, incluido el detalle de `PC007`/`PC009`) incluían `reckey` como primera columna del `table()` para tener una clave de update, pero los controles visuales `column(band=detail id=N ...)` seguían numerados como si `reckey` no existiera (offset de 1, o directamente mal en los campos ocultos de vínculo maestro-detalle) — el efecto real: la columna rotulada "Código" mostraba el valor de `reckey` (1,2,3...10) y cada columna siguiente mostraba el dato de la anterior, corriendo el desfase por toda la fila. **Fix**: se quitó `reckey` del `table()` y del `retrieve=` en los 11 `.srd`, se marcó `key=yes` en el código de negocio real (compuesto en `PC_POTRERO`/`PC_ESTABLO`/`PC_DIETA_COMPONENTE`/`PC_RECETA_DET`, simple en el resto), y se renumeraron todos los `id=` para que coincidan con la nueva posición real de columna. En `w_pc007_dietas.srw`/`w_pc009_recetas.srw` también se ajustó `ii_dk[1]`/`ii_rk[1]` de `2` a `1` (el índice del vínculo maestro-detalle apuntaba a la posición del código de negocio cuando `reckey` aún ocupaba la posición 1). `reckey` sigue existiendo como PK real en la tabla (sin cambios en el DDL); solo se dejó de traer/mostrar en el DataWindow. `d_pc_especie_tbl.srd` no tenía este bug (usa `especie` como única clave real, sin `reckey` agregado).
