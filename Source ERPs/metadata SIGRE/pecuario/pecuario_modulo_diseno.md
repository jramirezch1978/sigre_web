...
8. **Decisión pendiente del usuario**: cómo se genera `TG_ESPECIES.especie` para filas nuevas — 3 opciones planteadas y aún sin resolver: (a) mantener códigos legibles manuales como hoy (`BOVI`, `PORC`...); (b) autonumérico vía `NUM_TABLAS` con un prefijo fijo tipo `PE` (ya que es un catálogo global sin `cod_origen` real que aplique); (c) autonumérico con el `cod_origen` del fundo que registra la especie (con el riesgo de que la misma especie tenga códigos distintos según qué fundo la dio de alta, lo cual no calza con un catálogo global). Ninguna implementada aún — no cambiar el mecanismo de generación hasta que el usuario confirme.
