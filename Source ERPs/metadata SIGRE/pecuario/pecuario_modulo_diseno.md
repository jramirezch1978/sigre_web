...
| Operaciones | `391`–`411` (21 libres) | `CAM391`–`CAM408` (18) |
...
- **`PC_ESTABLO`** (CAM070, nuevo): ubicación **cubierta** (sala de ordeño, maternidad, cuarentena, corral de manejo, engorde), a diferencia de `PC_POTRERO` que es pastoreo a cielo abierto medido en hectáreas. Un animal puede estar en un potrero y en un establo a la vez — son dos dimensiones de ubicación independientes, no excluyentes (ver sección 8).
...
Para especies o operaciones donde no se registra cada cabeza individualmente (engorde masivo, aves — ver sección 18.2), `PC_LOTE` lleva además `cantidad_cabezas_inicial`/`cantidad_cabezas_actual` como conteo agregado, sin necesidad de una fila en `PC_ANIMAL` por cabeza.
...
También incluye: 2 potreros, **2 establos** (`PC_ESTABLO`: sala de ordeño y corral de cuarentena — Estrella pasa por el corral de cuarentena, con su historial en `PC_MOVIMIENTO_ESTABLO`, independiente de su historial de potreros), 1 semental, 1 dieta con su componente, **1 receta de concentrado** (con su propia OT de fabricación), **1 lote** (`PC_LOTE`, agrupa a Paloma y Luna en el Potrero Norte), 3 Órdenes de Trabajo Pecuarias (`ot_adm='PECU'`: alimentación, sanidad, fabricación de concentrado) vinculadas a sus animales vía `PC_OT_ANIMAL`, y filas mínimas en `ORIGEN`/`OT_TIPO`/`ALMACEN`/`TIPO_PRODUCTO`/`ORDEN_VENTA` para correr el escenario de punta a punta (en producción se crean desde sus pantallas estándar). Un tercer resultado de laboratorio ilustra el caso Laive.
...
**Gap parcialmente resuelto**: revisando la demo real de AGRI (sección 18.4) se confirma que "cabeza individual" y "lote" **no son dos ramas paralelas** sino dos vistas del mismo modelo — por eso se implementó `PC_LOTE` (sección 8) como una cohorte a la que cualquier `PC_ANIMAL` puede pertenecer (`cod_lote` opcional), con conteo agregado propio (`cantidad_cabezas_inicial`/`cantidad_cabezas_actual`) para cuando no se registra cada cabeza individualmente. Esto cubre bien el caso "bovino agrupado en lotes de manejo" (lo que muestra AGRI). **Lo que sigue sin cubrir** es el ciclo reproductivo específico de especies ovíparas (reproductoras → incubación → eclosión de huevos, con % de eclosión por lote) y el flujo de engorde masivo con conversión alimenticia — eso sí seguiría siendo una ampliación de alcance real (tablas específicas de incubación, no solo el campo `cod_lote`), a validar con el usuario antes de diseñar (sección 19).
...
6. **Decisión mayor pendiente**: tablas específicas de incubación/eclosión para especies ovíparas (aves), si el cliente llega a tener avicultura — `PC_LOTE` ya cubre el conteo agregado, pero el ciclo reproductivo por huevo (selección, carga de incubadora, % de eclosión) necesitaría tablas nuevas, análogas a `PC_SERVICIO`/`PC_PARTO` pero para huevos en vez de gestación.
...
5. `PC_LOTE` (control de lotes) ya está implementado (sección 8); decidir si se necesitan además las tablas específicas de incubación/eclosión para especies ovíparas (sección 18.2, punto 6) y cuáles de los ajustes de la sección 18.3 aplicar.
