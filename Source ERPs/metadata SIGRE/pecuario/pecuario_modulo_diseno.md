...
10. **Decisión final de arquitectura y empresa destino (confirmado por el usuario)**:
    ...
    - **Error "ii_ck no tiene valor" — resuelto en el ancestro, no por ventana** — `u_dw_abc` (ancestro real de todo DataWindow control) postea `ue_val_param` desde su `constructor`, que exige `UpperBound(ii_ck) > 0` (columna(s) PK del dw) o muestra el MessageBox de error; peor aún, `of_create_log`/`of_save_log` (log diario de auditoría) están condicionados a `ib_log AND UpperBound(ii_ck) > 0` — sin `ii_ck`, el log de auditoría queda silenciosamente desactivado, no solo el mensaje. Primer intento: setear `ii_ck[1] = 1` a mano en el `constructor` de cada una de las 10 ventanas Pecuario (confirmado contra un ejemplo real, `w_rh083_via.srw` de RRHH) — funcionaba, pero el usuario aclaró que la idea es que **no sea tarea del desarrollador** especificar `ii_ck` ventana por ventana: el propio objeto DataWindow debe poder obtener su PK por código. **Fix definitivo**: se agregó una **función nueva** `of_autodetect_ck()` (declarada en el bloque `forward prototypes` y con cuerpo propio, no código suelto dentro de un evento) que recorre todas las columnas del dw (`DataWindow.Column.Count` + `#N.Name`, patrón ya usado en `of_protect` de la misma clase), marca en `ii_ck[]` cualquier columna con `key=yes` (via `Describe(columna + '.Key')`, el mismo nombre de atributo literal que aparece en el `table()` del `.srd` exportado) y devuelve cuántas encontró. La llamada `IF UpperBound(ii_ck) = 0 THEN THIS.of_autodetect_ck() END IF` vive en el evento **`ue_val_param`** (no en `ue_conversion` como se había propuesto primero) — el usuario la colocó ahí directamente en PowerBuilder, justo antes del `MessageBox('ii_ck no tiene valor')` ya existente en ese mismo evento, lo cual funciona igual de bien (ambos eventos corren postergados desde `constructor`, en el mismo orden, y esta ubicación además garantiza que el chequeo que sigue inmediatamente después ya encuentra `ii_ck` lleno). Es puramente aditivo — solo actúa si `ii_ck` sigue vacío, nunca pisa una ventana que ya lo setea a mano. Con esto, las 10 ventanas Pecuario ya NO necesitan seteo manual de `ii_ck` (se revirtió la línea `ii_ck[1] = 1` agregada en el intento anterior); `ii_dk`/`ii_rk` de `PC007`/`PC009` sí siguen siendo manuales porque el vínculo maestro↔detalle es específico de cada ventana, no se puede inferir solo de `key=yes`.
    - **Aplicado solo en `CoreLibrary_uo.pbl.src`** — el repo tenía 3 copias de `u_dw_abc.sru` (`CoreLibrary`, `BaseClassLib`, `Integrador`); el usuario aclaró que `BaseClassLib` e `Integrador` son proyectos aparte, independientes de SIGRE, y los retiró (la carpeta `BaseClassLib` ya no existe en el repo; la copia de `Integrador` se revirtió a su estado original, sin `of_autodetect_ck`). El fix real y vigente para SIGRE queda únicamente en `CoreLibrary/CoreLibrary_uo.pbl.src/u_dw_abc.sru`. **Pendiente de verificar en PowerBuilder**: el atributo `.Key` vía `Describe()` no tiene un ejemplo literal ya existente en el repo (a diferencia de `.Protect`/`.ColType`/`.ID`/`.x`/`.width`, todos confirmados) — se infiere por el mismo patrón de nombre-de-atributo-tal-cual-en-el-.srd, pero conviene probarlo en una sola ventana antes de asumir que las ~250 pantallas del ERP que ya referencian `ii_ck` siguen funcionando igual.
    - **`gs_logo`/`gnvo_app.of_lista()` — patrón real confirmado, aplicado a las 10 ventanas de Tablas**: el usuario editó `w_pc001_razas.srw` a mano en PowerBuilder, reemplazando `f_lista(...)` por `gnvo_app.of_lista(...)` y la variable local `ls_null` (con `SetNull(ls_null)`) por `gnvo_app.is_null`. Verificado real: `gnvo_app` es `n_cst_app_obj`, creado en el `constructor` de `pecuario.sra` (el application object de Pecuario), con `of_lista()` (misma mecánica que `f_lista()`: abre `w_seleccionar`) e `is_null` (String, `SetNull` en el `constructor` de la clase) — ambos ya disponibles sin trabajo adicional. Se aplicó el mismo reemplazo a las 9 ventanas restantes con FK (`PC003`-`PC005`, `PC007`, `PC009`, `PC010`), y se redimensionaron proporcionalmente ventana + `dw_master`/`dw_detail` de las 10 (mismo ratio ancho-ventana/ancho-dw que usó el usuario en `PC001`) para que quepan las columnas ensanchadas en el fix anterior.
    - **Investigación del patrón de Operaciones (form + tabular, picker de referencias, reporte con logo)** — a pedido del usuario, se revisaron `w_al302_mov_almacen.srw` (4717 líneas), `w_cm311_orden_compra.srw` (4035 líneas) y `w_abc_seleccion(_md).srw` (1332/3275 líneas) del ERP real. Hallazgos clave, confirmados contra código real (no inventados):
      - **DataWindow "form" (cabecera) vs "tabular" (detalle)**: un dw *form* tiene `header(height=0)` (sin banda de encabezado repetida) y **todo** — etiquetas y controles — en una única banda `detail` alta (ej. `detail(height=1176)` en `d_abc_o_compra_cab_211.srd`), retrieved por argumento (un solo registro). Las etiquetas (`text(band=detail alignment="1" text="Campo:"...)`) van a la izquierda en `x` fijo, los controles a la derecha en otro `x` fijo, apiladas por `y` creciente (~80 unidades por fila). Un dw *tabular* (lo ya usado en Tablas) es el opuesto: `header` con los rótulos de columna y `detail` bajo (una fila) que se repite por cada registro.
      - **Logo en reportes**: `bitmap(band=detail filename="gs_logo" x=... y=... height=... width=... name=p_logo)`. `gs_logo` es una variable global `String` (ya declarada en `pecuario.sra`), seteada en `n_cst_app_obj` (`gnvo_app.of_open_system`) leyendo `ProfileString(inifile, "Logo", gs_empresa, "")` — **ya funciona para Pecuario sin trabajo adicional**, se resuelve automáticamente según la empresa logueada.
      - **Picker de referencias (`w_abc_seleccion`/`w_abc_seleccion_md`)**: se abren vía `OpenWithParm(w_abc_seleccion[_md], lstr_param)` donde `lstr_param` es una estructura `str_parametros` (campos `.titulo`, `.dw_master`, `.dw1`, `.tipo`, `.opcion`, `.tipo_doc`, `.nro_doc`, etc.) que le indica qué DataWindow usar como lista/maestro-detalle de selección; el resultado vuelve por `Message.PowerObjectParm`. Este mecanismo es mucho más pesado que `gnvo_app.of_lista()` (pensado para "elegir una Orden de Compra/Guía pendiente entre varias", no para un FK simple) — **aplica a las Operaciones que referencian una `ORDEN_TRABAJO` existente** (ej. futuras `PC307` Consumo de alimento, eventos de sanidad amarrados a OT), no a `PC301`.
      - **Decisión de alcance**: dado el tamaño real de las ventanas de referencia (~9000 líneas combinadas, lógica de Almacén/Compras no aplicable a Pecuario) y que son 21 pantallas de Operaciones por construir, se decidió NO generar las 21 de golpe sin verificar — mismo criterio que evitó el bug de `reckey`/`ii_ck`. Se construyó **`PC301` Maestro de Animal** como primera pantalla de Operaciones, completa y verificada, análoga a como `PC002` fue la prueba de concepto de Tablas.
    - **`PC301` Maestro de Animal — construido** (`PC_ANIMAL`, la primera de las 21 de Operaciones): `d_pc_animal_frm.srd` (dw estilo *form*, 32 columnas, un solo registro retrieved por `:as_cod_animal`) + `w_pc301_animal.srw`, agregada a `m_operaciones` en `m_master.srm` (hereda automáticamente en `m_abc_master_smpl.srm` vía herencia de menú de PowerBuilder — verificado que esa herencia NO requiere declarar los ítems de nuevo por ventana, a diferencia de lo que se documentó en una nota anterior). PK real = `cod_animal` (sin `reckey`, es tabla-documento per la regla de convención de PKs: código `cod_origen(2)+correlativo(8)` autogenerado por `TIB_PC_ANIMAL`), así que `of_autodetect_ck()` funciona sin ajuste. FKs con picker (`gnvo_app.of_lista()` + validación en `itemchanged`, igual patrón que Tablas): `cod_raza`→`PC_RAZA`, `cod_categoria`→`PC_CATEGORIA`, `cod_potrero`/`cod_establo`/`cod_lote`→`PC_POTRERO`/`PC_ESTABLO`/`PC_LOTE` (filtrados por `cod_origen` del propio animal, FK compuesta real en el DDL), `cod_semental_padre`→`PC_SEMENTAL`, `cod_animal_padre`/`cod_animal_madre`→auto-referencia a `PC_ANIMAL` (filtrado por sexo M/H respectivamente).
    - **`PC301`: primer layout rechazado por el usuario, rehecho como grilla real de 2 columnas** — la primera versión (campos en una sola columna, cada uno con su propio `x` "a ojo") generó desorden visual y hacía fallar de forma intermitente la herramienta de orden de tabulación del painter de PowerBuilder. El usuario mostró como referencia el layout real de `d_abc_o_compra_cab_211.srd`/pantalla de Orden de Compra: dos columnas bien definidas, posiciones de columna **fijas y repetidas** en cada fila (no una posición distinta por campo), con logo. Se reconstruyó `d_pc_animal_frm.srd` generándolo por script (no a mano) con geometría de grilla estricta: columna izquierda `x=20` (etiqueta) / `x=580` (código) / `x=940` (descripción FK, si aplica), columna derecha `x=1950` / `x=2510` / `x=2870`, fila de 90 unidades de alto reutilizada en las 12 filas — exactamente la misma posición de columna en cada fila, para que el painter (y la herramienta de orden) tengan una grilla consistente sobre la que operar. Se agregó `bitmap(band=detail filename="gs_logo" ...)` arriba a la izquierda (mismo mecanismo confirmado — variable global ya funcional) con el título "MAESTRO DE ANIMAL" al lado. `tabsequence` reasignado secuencial (10,20,30...) en orden de lectura fila por fila, izquierda-antes-que-derecha; campos protegidos/de solo lectura en `tabsequence=0`/`32766` (mismo criterio que el resto del proyecto). Verificado por script: sin solapamiento de coordenadas dentro de cada fila (se encontraron y corrigieron 3 solapamientos reales en la primera pasada — `cod_animal_padre`/`madre`/`cod_semental_padre` con el campo de código demasiado ancho invadiendo la columna de descripción), `id=` 1-32 completos y únicos, `key=yes` solo en `cod_animal`, paréntesis/comillas balanceados, BOM+CRLF correctos. `w_pc301_animal.srw` (`dw_master` y ventana) redimensionados para calzar con la nueva altura más compacta de la banda `detail` (1780, antes 2680).
    - **`PC301`: grilla aprobada, pero `tabsequence` roto — causa raíz encontrada** — el usuario confirmó que la grilla de 2 columnas se ve ordenada y que las herramientas de alineación del painter ya funcionan, pero reportó que el `tabsequence` "no funciona". Al leer el archivo (ya reimportado por el usuario — PowerBuilder lo reescribió a `release 25` con atributos modernos, confirmando que sí se reimportó) se encontró la causa exacta: los 10 campos que deben ser de solo lectura (`cod_animal` + las 9 `desc_xxx` unidas por el `JOIN`) tenían `protect="0~t1"` (editable) pero **seguían con `tabsequence=32766`** (la convención real de "sin parada de tabulación", confirmada contra `d_abc_vale_mov_ff.srd`). Esa combinación es contradictoria: al no estar protegidos deberían participar del tab order, pero al quedar en `32766` se saltan siempre — de ahí que "no funcionara". Hipótesis de cómo ocurrió: la herramienta de "Tab Order" del painter no permite asignar secuencia a un campo protegido (protect=1), así que al intentar usarla sobre los campos `desc_xxx`/`cod_animal`, algo (el usuario o el propio painter) los desprotegió sin llegar a reasignarles una secuencia real. **Fix**: se restauró `protect="1~t1"` en los 10 campos (deben seguir siendo de solo lectura — son datos calculados por el `JOIN` o llenados por el picker `gnvo_app.of_lista()`, nunca tecleados directamente), dejando `tabsequence=32766` como estaba (ya es el valor correcto). Verificado: los 22 campos editables reales quedan con `tabsequence` limpio y secuencial `10-220`, los 10 de solo lectura en `32766`, `key=yes` sigue solo en `cod_animal`, resto de la geometría/estructura intacta.
    - **`PC301`: numeración de `cod_animal` vía `of_set_numera()` + `num_tablas`** (a pedido del usuario, replicando el patrón real de `w_ve318_factura_popup.srw` en Comercializacion — "facturación simplificada"). Investigado y confirmado contra código real: `of_set_numera()` es una **función pública a nivel de VENTANA** (no del DataWindow), declarada en `forward prototypes` del `.srw`; se llama desde el evento **`ue_update_pre`** (hook declarado vacío en el ancestro base `w_abc.srw`, disparado por `w_abc_master::ue_update` vía `THIS.EVENT ue_update_pre()` justo antes del `dw_master.Update()` real). El patrón completo, replicado tal cual en `w_pc301_animal.srw`:
      - `is_action` (String, declarado en `w_abc.srw`) se pone en `'new'` al final de `dw_master::ue_insert_pre` — confirmado que NINGÚN ancestro lo setea automáticamente, cada ventana hoja lo hace en su propio `ue_insert_pre`.
      - `of_set_numera()`: si `is_action = 'new'`, lee `num_tablas.ult_nro` (`tabla` = `dw_master.object.DataWindow.Table.UpdateTable` = `pc_animal`, `origen` = el `cod_origen` **de la propia fila**, no `gs_origen` directo — más correcto para un modelo multi-fundo) con `for update` (lock), inserta la fila si no existe (`SQLCA.SQLCode = 100`), arma `cod_animal = origen + correlativo(8, zero-pad)`, e incrementa el contador. Envuelto en `try/catch(Exception ex)/finally/end try`, igual que la referencia.
      - `ue_update_pre` (evento de VENTANA, sin prefijo `dw_master::`): `ib_update_check = False` al inicio (asume fallo hasta el final — mismo patrón que la referencia), valida que haya fila, llama `if of_set_numera() = 0 then return` (aborta el guardado si falla), y solo si todo pasa, `ib_update_check = true` al final.
      - **No hay conflicto con el trigger `TIB_PC_ANIMAL`** (DDL, sin tocar): el trigger solo genera `cod_animal` cuando `:new.cod_animal is null`; como `of_set_numera()` ya lo asigna en cliente ANTES del `Update()`, el trigger simplemente no actúa — son dos mecanismos compatibles, el cliente tiene precedencia y el trigger queda como red de seguridad si algún flujo futuro insertara sin pasar por esta ventana.
    - **Pendiente explícito, no iniciado**: `PC302`-`PC321` (20 pantallas de Operaciones restantes) + sus reportes con logo (`PC701`-`PC707`). Se construirán incrementalmente, una o pocas a la vez, verificando cada una antes de continuar — mismo criterio que esta sección documenta como la lección de esta sesión.
