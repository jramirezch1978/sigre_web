...
10. **Decisión final de arquitectura y empresa destino (confirmado por el usuario)**:
    ...
    - ~~Pendiente de verificar el nombre de esquema/owner~~ — **resuelto, decisión del usuario**: se quitó el prefijo `CANTABRIA.` de las ~1004 referencias en todo el script (tablas, secuencias, triggers, inserts). El script ya **no fija ningún nombre de esquema**: se ejecuta conectado como el usuario que corresponda (`SAN_MARTIN`, `JARCH`, o cualquier otro) y los objetos quedan bajo ese usuario de forma implícita. El bloque de limpieza idempotente también se cambió de `ALL_TABLES`/`ALL_SEQUENCES` filtrado por `owner='CANTABRIA'` a `USER_TABLES`/`USER_SEQUENCES` (autodetecta el usuario conectado, sin hardcodear ningún nombre). Nota: `tablespace CANTABRIA` (el tablespace físico de almacenamiento, un concepto distinto al owner) **no se tocó** — si San Martín usa un tablespace con otro nombre, eso sigue pendiente de ajustar por separado.
