-- ============================================================
-- Restaurant.pe ERP - Tenant DB - core schema
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
         WHERE table_schema = 'core' AND table_name = 'credito_fiscal' AND column_name = 'descripcion'
           AND (character_maximum_length IS NULL OR character_maximum_length < 200)
    ) THEN
        ALTER TABLE core.credito_fiscal ALTER COLUMN descripcion TYPE VARCHAR(200);
    END IF;
END $$;
...
-- ============================================================
-- Parámetros del tenant: config.configuracion (DDL en security/02-config.sql).
-- Las funciones core.fn_get_parameter_* leen/crean filas; modulo por defecto 'CORE'.
...
BEGIN
    ...
    WHERE c.parametro = p_parameter AND c.activo = TRUE;

    IF NOT FOUND THEN
        INSERT INTO config.configuracion (modulo, parametro, tipo_dato, valor_texto, activo, modificado_en)
        VALUES ('CORE', p_parameter, 'TEXT', p_default, TRUE, NOW());
        RETURN p_default;
    END IF;
...
BEGIN
    ...
    IF NOT FOUND THEN
        INSERT INTO config.configuracion (modulo, parametro, tipo_dato, valor_entero, activo, modificado_en)
        VALUES ('CORE', p_parameter, 'INTEGER', p_default, TRUE, NOW());
        RETURN p_default;
    END IF;
...
BEGIN
    ...
    IF NOT FOUND THEN
        INSERT INTO config.configuracion (modulo, parametro, tipo_dato, valor_decimal, activo, modificado_en)
        VALUES ('CORE', p_parameter, 'DECIMAL', p_default, TRUE, NOW());
        RETURN p_default;
    END IF;
...
DECLARE
    ldat_valor DATE;
BEGIN
    SELECT c.valor_fecha INTO ldat_valor
    FROM config.configuracion c
    WHERE c.parametro = p_parameter AND c.activo = TRUE;

    IF NOT FOUND THEN
        INSERT INTO config.configuracion (modulo, parametro, tipo_dato, valor_fecha, activo, modificado_en)
        VALUES ('CORE', p_parameter, 'DATE', p_default, TRUE, NOW());
        RETURN p_default;
    END IF;
...
DECLARE
    lbol_valor BOOLEAN;
BEGIN
    SELECT c.valor_bool INTO lbol_valor
    FROM config.configuracion c
    WHERE c.parametro = p_parameter AND c.activo = TRUE;

    IF NOT FOUND THEN
        INSERT INTO config.configuracion (modulo, parametro, tipo_dato, valor_bool, activo, modificado_en)
        VALUES ('CORE', p_parameter, 'BOOLEAN', p_default, TRUE, NOW());
        RETURN p_default;
    END IF;
...
COMMENT ON FUNCTION core.fn_get_parameter_txt(VARCHAR, TEXT) IS 'Parámetro TEXT de config.configuracion; si no existe lo crea con el default.';
COMMENT ON FUNCTION core.fn_get_parameter_int(VARCHAR, INTEGER) IS 'Parámetro INTEGER de config.configuracion; si no existe lo crea con el default.';
COMMENT ON FUNCTION core.fn_get_parameter_dec(VARCHAR, NUMERIC) IS 'Parámetro DECIMAL de config.configuracion; si no existe lo crea con el default.';
COMMENT ON FUNCTION core.fn_get_parameter_date(VARCHAR, DATE) IS 'Parámetro DATE de config.configuracion; si no existe lo crea con el default.';
COMMENT ON FUNCTION core.fn_get_parameter_bool(VARCHAR, BOOLEAN) IS 'Parámetro BOOLEAN de config.configuracion; si no existe lo crea con el default.';
