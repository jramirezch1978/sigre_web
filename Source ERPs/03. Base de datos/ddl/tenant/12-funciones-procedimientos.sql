-- ============================================================
-- Restaurant.pe ERP - Tenant DB - Funciones y Procedimientos
...
-- ============================================================
-- RRHH — Motor de cálculo de planilla (MVP Restaurant.pe)
--
-- Contexto: régimen general EMP (empleados sueldo fijo).
-- Puerto a partir del análisis de SIGRE usp_rh_cal_calcula_planilla.
-- Sin pesca, sin tripulantes, sin destajeros.
--
-- Invocación desde ms-rrhh:
...
-- Resuelve el id de tipo_concepto_calculo por código ('INGRESO','DESCUENTO','APORTE')
...
-- Resuelve o crea un concepto_planilla por código.
-- p_codigo: código numérico string ('1001', '2201', etc.)
...
-- Código SIGRE (TEXT preferido; fallback valor_entero legacy) para concepto_planilla o grupo_calculo.
...
-- Código de grupo_calculo (VARCHAR(3)): lee config como TEXT y normaliza dígitos cortos a 3 posiciones.
...
-- Upsert en calculo_det (INSERT o UPDATE según UQ calculo_id + concepto_id + item).
...
-- ============================================================
-- Helpers motor planilla EMP (SIGRE → PostgreSQL)
-- ============================================================
...
-- Devuelve días a pagar en el período [p_fec_inicio, p_fec_final].
-- ============================================================
...
BEGIN
    ...
    -- Si el trabajador no aplica al período → 0
    ...
    -- Ajuste febrero: si fec_hasta es el último día del mes de feb → 30 días
...
-- Devuelve la RETENCIÓN MENSUAL de quinta categoría.
-- ============================================================
CREATE OR REPLACE FUNCTION rrhh.fn_calcular_impuesto_5ta(
    p_trabajador_id   BIGINT,
    p_fec_proceso     DATE,
    p_rem_mensual     NUMERIC,   -- remuneración computable del mes
    p_tipo_planilla_id BIGINT
) RETURNS NUMERIC
LANGUAGE plpgsql STABLE AS $$
DECLARE
...
BEGIN
    ...
    -- UIT vigente (última antes de la fecha de proceso)
    ...
    -- Remuneraciones acumuladas del año (meses anteriores al actual)
    SELECT COALESCE(SUM(cd.imp_soles), 0)
      INTO v_rem_acum
      FROM rrhh.calculo c
      JOIN rrhh.calculo_det cd ON cd.calculo_id = c.id
    ...
    -- Renta proyectada = acumulado + (rem_mensual × meses restantes)
    v_rem_proyectada := v_rem_acum + (p_rem_mensual * v_meses_rest);

    -- Renta neta anual (después de deducción 7 UIT)
    ...
    -- Retenciones previas del año (rem_retencion = retención mensual calculada)
    ...
    -- Retención del mes = (impuesto anual - ya retenido) / meses restantes
...
-- Limpia calculo + calculo_det del trabajador para el período,
-- permitiendo reprocesar sin duplicar líneas.
-- ============================================================
...
-- prorratea por días trabajados y escribe en calculo_det.
-- ============================================================
...
BEGIN
    -- Grupo ganancias fijas desde RRHHPARAM (GRC_GNN_FIJA = 10 → código '010')
    ...
    FOR rec IN
        SELECT gdf.concepto_id,
         ...
         ORDER BY cp.codigo
    LOOP
        -- Prorrateo: imp_fijo / dias_mes × dias_trabajados
...
-- Port: SIGRE usp_rh_cal_asig_familiar (EMP sueldo — sin prorrateo)
-- Lee el concepto asignación familiar de gan_desct_fijo; no prorratea.
-- ============================================================
...
-- Escribe en gan_desct_variable (promedio remuneración vacacional).
-- ============================================================
...
-- sp_cal_variables — DEPRECATED: usar sp_cal_ganancias_variables + sp_cal_descuento_variable
...
-- Descuento por días no laborados (falta injustificada).
-- ============================================================
...
BEGIN
    ...
    -- Sueldo diario = total ingresos fijos / dias_mes (aproximación)
    ...
    -- Concepto descuento inasistencia: grupo DIAS_INASIS_DSCCONT (value_int=8 → '008')
...
DECLARE
    ...
    v_monto         NUMERIC;
BEGIN
    -- Remuneración básica acumulada hasta ahora (ganancias fijas)
    SELECT COALESCE(SUM(cd.imp_soles), 0)
      INTO v_rem_basica
      FROM rrhh.calculo_det cd
      JOIN rrhh.tipo_concepto_calculo tcc ON tcc.id = cd.tipo_concepto_calculo_id
...
-- Totalizador ingresos (suma todos los conceptos INGRESO → concepto 1499).
-- ============================================================
...
-- ============================================================
-- sp_cal_onp
-- Port: SIGRE usp_rh_cal_snp — Descuento ONP/SNP 13%
...
BEGIN
    ...
    -- Tasa: leer factor_pago del primer concepto del grupo (típico 0.13)
    ...
    -- Concepto destino: primero del grupo SNP con código 2xxx
...
-- ============================================================
-- sp_cal_afp
-- Port: SIGRE usp_rh_cal_afp — 3 componentes AFP
...
DECLARE
    ...
    v_monto          NUMERIC;
BEGIN
    -- Grupo AFP (RRHHPARAM_CCONCEP.CONCEP_CALCULO_AFP = 23 → '023')
...
-- Calcula y registra retención mensual de quinta categoría.
-- ============================================================
...
BEGIN
    ...
    -- Remuneración computable = total ingresos afectos a quinta (flag_reporte_quinta='1')
    SELECT COALESCE(SUM(cd.imp_soles), 0)
    ...
    -- Concepto quinta categoría (RRHHPARAM_CCONCEP.QUINTA_CAT_PROYECTA = 24)
...
-- Totalizador descuentos (suma todos DESCUENTO → concepto 2398).
-- ============================================================
...
-- ============================================================
-- sp_cal_total_pagado
-- Neto = ingresos - descuentos → concepto 2399.
-- ============================================================
...
-- ============================================================
-- sp_cal_essalud
-- Port: SIGRE usp_rh_cal_apo_essalud — Aporte patronal EsSalud 9%
-- ============================================================
...
BEGIN
    ...
    -- Grupo EsSalud (RRHHPARAM_CCONCEP.CONCEP_ESSALUD = 77 → '077')
    ...
    -- RMV vigente (tope mínimo base EsSalud)
    ...
    -- Concepto EsSalud patronal (código 3xxx del grupo)
    SELECT cp.id INTO v_concepto_id
      FROM rrhh.grupo_calculo_det gcd
      JOIN rrhh.concepto_planilla cp ON cp.id = gcd.concepto_planilla_id
...
-- ============================================================
-- sp_cal_apo_total
-- Totalizador aportes (suma todos APORTE → concepto 3099).
-- ============================================================
...
-- ============================================================
-- sp_cal_feriado — Port SIGRE usp_rh_cal_feriado (no-op EMP sueldo fijo)
-- ============================================================
...
BEGIN
    ...
    -- Lógica jornal: pendiente tablas asistencia/calendario_feriado
END;
$$;

-- ============================================================
-- sp_cal_dso — Port SIGRE USP_RH_CAL_DSO (no-op EMP sueldo fijo)
-- ============================================================
...
-- ============================================================
-- sp_cal_vacaciones — Port SIGRE usp_rh_cal_vacaciones
-- ============================================================
...
-- ============================================================
-- sp_cal_enfermedad — Port SIGRE usp_rh_cal_enfermedad
-- ============================================================
...
-- ============================================================
-- sp_cal_maternidad — Port SIGRE usp_rh_cal_maternidad
-- ============================================================
...
-- ============================================================
-- sp_cal_reintegros — Port SIGRE usp_rh_cal_reintegros
-- ============================================================
...
-- ============================================================
-- sp_cal_descuentos_fijos — Port SIGRE usp_rh_cal_descuentos_fijos
-- ============================================================
...
-- ============================================================
-- sp_cal_desct_comedor — Port SIGRE usp_rh_cal_desct_comedor (no-op EMP oficina)
-- ============================================================
...
-- ============================================================
-- sp_cal_tardanzas — Port SIGRE usp_rh_cal_tardanzas
-- ============================================================
...
-- ============================================================
-- sp_cal_porcentaje_judicial — Port SIGRE usp_rh_cal_porcentaje_judicial
-- ============================================================
...
-- ============================================================
-- sp_cal_essalud_vida — Port SIGRE usp_rh_cal_essalud_vida
-- ============================================================
...
-- ============================================================
-- sp_cal_cuenta_corriente — Port SIGRE usp_rh_cal_cuenta_corriente
-- ============================================================
...
-- ============================================================
-- sp_cal_snp — alias SIGRE usp_rh_cal_snp (= sp_cal_onp)
-- ============================================================
...
-- ============================================================
-- sp_cal_apo_sctr_ipss — Port SIGRE usp_rh_cal_apo_sctr_ipss
-- ============================================================
...
-- ============================================================
-- sp_cal_apo_sctr_onp — Port SIGRE usp_rh_cal_apo_sctr_onp
-- ============================================================
...
-- ============================================================
-- sp_cal_apo_senati — Port SIGRE usp_rh_cal_apo_senati
-- ============================================================
...
-- ============================================================
-- sp_cal_otras_aport — Port SIGRE usp_rh_cal_otras_aport
-- ============================================================
...
-- ============================================================
-- sp_cal_cred_eps — Port SIGRE usp_rh_cal_cred_eps
-- ============================================================
...
-- ============================================================
-- sp_cal_apo_essalud — Port SIGRE usp_rh_cal_apo_essalud (reemplaza sp_cal_essalud)
-- ============================================================
...
-- ============================================================
-- sp_cal_limpia_ceros / sp_cal_limpia_fantasmas — Fases 1-3 SIGRE
-- ============================================================
...
-- ============================================================
-- sp_calcular_planilla  — ORQUESTADOR PRINCIPAL
--
-- Parámetros:
--   p_origen               — sucursal ('PI', 'LM', etc.)
--   p_fec_proceso          — fecha del proceso (1er día del mes)
--   p_tipo_planilla_codigo — código de rrhh.tipo_planilla ('N', 'Q', etc.)
--
...
--   7. Variables del período (propinas, HE-imp, otros)
...
--  12. Quinta categoría
--  13. Total descuentos
-- Orquestador principal — secuencia SIGRE usp_rh_cal_calcula_planilla (31 sub-SPs EMP)
-- ============================================================
...
BEGIN
    ...
    IF v_tipo_emp_id IS NULL THEN
        RAISE EXCEPTION 'No se encontró tipo_trabajador con codigo=%', v_tipo_emp_codigo;
    END IF;
...
-- ============================================================
-- fn_validar_calculo_referencia — compara motor vs seed 14 (CALCULO.json)
