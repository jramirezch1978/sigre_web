-- ============================================================
-- Restaurant.pe ERP - Tenant DB - contabilidad schema
...
-- FK diferida: compras.entidad_detraccion.detr_bien_serv_id → core.detr_bien_serv
ALTER TABLE compras.entidad_detraccion DROP CONSTRAINT IF EXISTS fk_entidad_detraccion_tipo;
ALTER TABLE compras.entidad_detraccion DROP CONSTRAINT IF EXISTS FK_ENTIDAD_DETRACCION_TIPO;
...
DO $$ BEGIN
    IF EXISTS (
        ...
        WHERE table_schema = 'compras'
          AND table_name = 'entidad_detraccion'
          AND column_name = 'cntbl_tipo_detraccion_id'
    ) THEN
        ALTER TABLE compras.entidad_detraccion
            RENAME COLUMN cntbl_tipo_detraccion_id TO detr_bien_serv_id;
    END IF;
END $$;

ALTER TABLE compras.entidad_detraccion ADD COLUMN IF NOT EXISTS detr_bien_serv_id BIGINT;

DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_entidad_detraccion_detr_bien_serv') THEN
        ALTER TABLE compras.entidad_detraccion
            ADD CONSTRAINT fk_entidad_detraccion_detr_bien_serv
            FOREIGN KEY (detr_bien_serv_id) REFERENCES core.detr_bien_serv(id);
    END IF;
END $$;

DROP TABLE IF EXISTS contabilidad.cntbl_tipo_detraccion CASCADE;
