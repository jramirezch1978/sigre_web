$PBExportHeader$w_ve322_listado_ce_fs.srw
forward
global type w_ve322_listado_ce_fs from w_abc
end type
type rb_todos from radiobutton within w_ve322_listado_ce_fs
end type
type rb_pendientes from radiobutton within w_ve322_listado_ce_fs
end type
type cbx_pdf from checkbox within w_ve322_listado_ce_fs
end type
type st_1 from statictext within w_ve322_listado_ce_fs
end type
type hpb_1 from hprogressbar within w_ve322_listado_ce_fs
end type
type cb_3 from commandbutton within w_ve322_listado_ce_fs
end type
type cbx_email from checkbox within w_ve322_listado_ce_fs
end type
type cbx_xml from checkbox within w_ve322_listado_ce_fs
end type
type uo_fechas from u_ingreso_rango_fechas within w_ve322_listado_ce_fs
end type
type uo_fecha from u_ingreso_fecha within w_ve322_listado_ce_fs
end type
type cb_1 from commandbutton within w_ve322_listado_ce_fs
end type
type dw_master from u_dw_abc within w_ve322_listado_ce_fs
end type
type cb_2 from commandbutton within w_ve322_listado_ce_fs
end type
type gb_1 from groupbox within w_ve322_listado_ce_fs
end type
type gb_2 from groupbox within w_ve322_listado_ce_fs
end type
type gb_3 from groupbox within w_ve322_listado_ce_fs
end type
end forward
...
integer width = 5051
integer height = 2552
string title = "[VE322] Envio de Comprobantes Electronicos a SUNAT"
...
event ue_generar ( )
event type boolean ue_procesar_comprobante ( string as_nro_registro,  string as_tipo_doc,  string as_nro_doc )
...
global w_ve322_listado_ce_fs w_ve322_listado_ce_fs
...
event ue_xml(long al_row);String ls_nro_registro, ls_tipo_doc, ls_nro_doc, ls_full_nro_doc, ls_nro_envio_id
...
try
	gnvo_app.ventas.of_create_only_xml(ls_nro_registro, ls_tipo_doc, ls_nro_doc)
catch(Exception ex)
finally
end try
...
u_ds_base 	ids_lista
Long			ll_row
String		ls_nro_envio_id, ls_nro_registro, ls_tipo_doc, ls_nro_doc
boolean		lb_return

try 
	
	ids_lista = create u_ds_base
	ids_lista.DataObject = 'd_lista_ce_envio_sunat_pendientes_tbl'
	...
	for ll_row = 1 to ids_lista.RowCount()
		...
		ls_nro_registro	= ids_lista.object.nro_registro 	[ll_row]
...
event type boolean ue_procesar_comprobante(string as_nro_registro, string as_tipo_doc, string as_nro_doc);boolean lb_Return

try
	if cbx_xml.checked then
		lb_return =  gnvo_app.ventas.of_create_only_xml(as_nro_registro, as_tipo_doc, as_nro_doc)
	end if
	
	if cbx_pdf.checked then
		if lb_return then
			gnvo_app.ventas.of_create_only_pdf(as_nro_registro, as_tipo_doc, as_nro_doc)
		end if
	end if
catch(Exception ex)
finally
...
event ue_enviar_email_rango();String 	ls_nro_registro, ls_tipo_doc, ls_nro_doc
...
event ue_pdf(long al_row);String ls_nro_registro, ls_tipo_doc, ls_nro_doc, ls_full_nro_doc, ls_nro_envio_id
...
gnvo_app.ventas.of_create_only_pdf(ls_nro_registro, ls_tipo_doc, ls_nro_doc)
...
on w_ve322_listado_ce_fs.create
int iCurrent
call super::create
...
on w_ve322_listado_ce_fs.destroy
call super::destroy
...
if upper(gs_empresa) = 'INNOVA' then
	dw_master.DataObject = 'd_lista_envio_sunat_innova_tbl'
else
	dw_master.DataObject = 'd_lista_ce_envio_sunat_tbl'
end if
...
event ue_refresh;call super::ue_refresh;String 	ls_nro_registro
...
if dw_master.RowCount() > 0 and not IsNull(ls_nro_registro) and trim(ls_nro_registro) <> '' then
	ll_find = dw_master.Find("nro_registro='" + ls_nro_registro + "'", 1, dw_master.RowCount())
...
integer width = 4475
integer height = 1848
string dataobject = "d_lista_ce_envio_sunat_tbl"
end type
