$PBExportHeader$w_seleccion_sub_canal.srw
forward
global type w_seleccion_sub_canal from w_abc_list
end type
type cb_1 from commandbutton within w_seleccion_sub_canal
end type
type st_1 from statictext within w_seleccion_sub_canal
end type
end forward
...
global w_seleccion_sub_canal w_seleccion_sub_canal

on w_seleccion_sub_canal.create
int iCurrent
call super::create
...
on w_seleccion_sub_canal.destroy
call super::destroy
destroy(this.cb_1)
destroy(this.st_1)
end on

event open;call super::open;str_parametros lstr_parametros
...
if isvalid(message.powerobjectparm) then
	...
	dw_1.Retrieve(lstr_parametros.str_array[])
...
type dw_1 from w_abc_list`dw_1 within w_seleccion_sub_canal
integer x = 37
integer y = 32
integer width = 1394
integer height = 1188
string dataobject = "d_lista_sub_canal"
...
type dw_2 from w_abc_list`dw_2 within w_seleccion_sub_canal
integer x = 1646
integer y = 32
integer width = 1394
integer height = 1188
...
type pb_1 from w_abc_list`pb_1 within w_seleccion_sub_canal
integer x = 1463
integer y = 396
integer width = 151
end type

type pb_2 from w_abc_list`pb_2 within w_seleccion_sub_canal
integer x = 1463
integer y = 532
integer width = 151
string text = " <"
...
integer i
string ls_cod,ls_desc, ls_canal
...
if ll_nbrow > 0 then
	...
	for i=1 to ll_nbrow
		...
		ls_canal  = trim(dw_2.object.canal[i])
		ls_cod    = trim(dw_2.object.sub_canal[i])
		ls_desc   = trim(dw_2.object.descripcion[i])
		
		insert into tt_canal_distribucion_det (canal, sub_canal, descripcion) values (:ls_canal, :ls_cod, :ls_desc);
