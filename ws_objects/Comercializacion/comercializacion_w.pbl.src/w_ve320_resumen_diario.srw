$PBExportHeader$w_ve320_resumen_diario.srw
forward
global type w_ve320_resumen_diario from w_abc
end type
type cb_3 from commandbutton within w_ve320_resumen_diario
end type
type hpb_1 from hprogressbar within w_ve320_resumen_diario
end type
type st_1 from statictext within w_ve320_resumen_diario
end type
type uo_fechas from u_ingreso_rango_fechas within w_ve320_resumen_diario
end type
type uo_fecha from u_ingreso_fecha within w_ve320_resumen_diario
end type
type cb_1 from commandbutton within w_ve320_resumen_diario
end type
type dw_master from u_dw_abc within w_ve320_resumen_diario
end type
type cb_2 from commandbutton within w_ve320_resumen_diario
end type
type gb_1 from groupbox within w_ve320_resumen_diario
end type
type gb_2 from groupbox within w_ve320_resumen_diario
end type
type gb_3 from groupbox within w_ve320_resumen_diario
end type
end forward
...
integer width = 4581
integer height = 2552
string title = "[VE320] Resumen Diario de Comprobantes Electronicos"
...
event type boolean ue_resumen ( date ad_fecha )
cb_3 cb_3
hpb_1 hpb_1
st_1 st_1
uo_fechas uo_fechas
...
global w_ve320_resumen_diario w_ve320_resumen_diario
...
if not this.event ue_resumen(ld_fecha) then 
	return
end if
...
Long			ll_row
String		ls_nro_rc_id, ls_nro_registro, ls_tipo_doc, ls_nro_doc
boolean		lb_return
...
try 
	...
	for ll_row = 1 to dw_master.RowCount()
		...
		yield()
		ls_nro_rc_id 		= dw_master.object.nro_rc_id 			[ll_row]
		
		if IsNull(ls_nro_rc_id) or trim(ls_nro_rc_id) = "" then
			
			//Obtengo ls fecha de emisión
			ld_fec_emision		= Date(dw_master.object.fec_emision	[ll_row])
			
			if IsNull(ld_fec_proceso) or ld_fec_proceso <> ld_fec_emision then
				ld_fec_proceso = ld_fec_emision
				if not this.event ue_resumen(ld_fec_proceso) then 
					return
				end if
			end if
...
event type boolean ue_resumen(date ad_fecha);if not gnvo_app.ventas.of_resumen_rc_fs( ad_fecha) then
	rollback;
	return false
end if
...
on w_ve320_resumen_diario.create
int iCurrent
call super::create
...
on w_ve320_resumen_diario.destroy
call super::destroy
...
integer width = 4475
integer height = 1848
string dataobject = "d_lista_rc_diario_tbl"
end type
