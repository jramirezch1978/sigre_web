$PBExportHeader$w_ve321_resumen_baja.srw
forward
global type w_ve321_resumen_baja from w_abc
end type
type st_1 from statictext within w_ve321_resumen_baja
end type
type hpb_1 from hprogressbar within w_ve321_resumen_baja
end type
type cb_3 from commandbutton within w_ve321_resumen_baja
end type
type uo_fechas from u_ingreso_rango_fechas within w_ve321_resumen_baja
end type
type uo_fecha from u_ingreso_fecha within w_ve321_resumen_baja
end type
type cb_1 from commandbutton within w_ve321_resumen_baja
end type
type dw_master from u_dw_abc within w_ve321_resumen_baja
end type
type cb_2 from commandbutton within w_ve321_resumen_baja
end type
type gb_1 from groupbox within w_ve321_resumen_baja
end type
type gb_2 from groupbox within w_ve321_resumen_baja
end type
type gb_3 from groupbox within w_ve321_resumen_baja
end type
end forward
...
integer width = 4581
integer height = 2552
string title = "[VE321] Resumen Diario de Baja de Comprobantes"
...
event type boolean ue_resumen ( date ad_fecha,  string as_tipo_doc )
...
global w_ve321_resumen_baja w_ve321_resumen_baja
...
try 
	...
	lds_data.DataObject = 'd_lista_ra_diario_tbl'
	lds_data.SetTransObject( SQLCA )
	
	ld_fecha = uo_fecha.of_get_fecha( )
	
	lds_data.retrieve(ld_fecha, ld_fecha)
	
	if lds_data.RowCount() = 0 then
		MessageBox('Error', 'No hay registros para procesar en la fecha ' + string(ld_fecha, 'dd/mm/yyyy'), StopSign!)
		return
	end if
	
	for ll_row = 1 to lds_data.RowCount()
		ls_tipo_doc = lds_data.object.tipo_doc [ll_row]
		if not this.event ue_resumen(ld_fecha, ls_tipo_doc) then 
			rollbacK;
			return
		end if
	next
	...
	gnvo_app.of_catch_exception(ex, 'Error en ue_generar()')
finally
	destroy lds_data
end try
...
event ue_generar_rango();date 			ld_Fecha, ld_fec_emision, ld_fec_proceso
Long			ll_row
String		ls_nro_ra_id, ls_nro_registro, ls_tipo_doc, ls_nro_doc
boolean		lb_return
...
try 
	...
	for ll_row = 1 to dw_master.RowCount()
		...
		yield()
		ls_nro_ra_id 		= dw_master.object.nro_ra_id 			[ll_row]
		ls_tipo_doc			= dw_master.object.tipo_doc			[ll_row]
		
		if IsNull(ls_nro_ra_id) or trim(ls_nro_ra_id) = "" then
			
			//Obtengo ls fecha de emisión
			ld_fec_emision		= Date(dw_master.object.fec_emision	[ll_row])
			
			if not this.event dynamic ue_resumen(ld_fec_emision, ls_tipo_doc) then 
				return
			end if
...
event type boolean ue_resumen(date ad_fecha, string as_tipo_doc);boolean	lb_Return1,lb_Return2

if as_tipo_doc = 'FAC' then
	if not gnvo_app.ventas.of_resumen_ra_fac( ad_fecha) then
		rollback;
		return false
	end if
end if

if as_tipo_doc = 'BOL' then
	if not gnvo_app.ventas.of_resumen_ra_bvc( ad_fecha) then
		rollback;
		return false
	end if
end if
...
event ue_xml(long al_row);date 			ld_Fecha
String		ls_tipo_doc

try 

	ld_fecha 	= Date(dw_master.object.fec_emision [al_row])
...
on w_ve321_resumen_baja.create
int iCurrent
call super::create
...
on w_ve321_resumen_baja.destroy
call super::destroy
...
integer width = 4475
integer height = 1848
string dataobject = "d_lista_ra_diario_tbl"
end type
...
choose case lower(dwo.name)
	...
	case "b_xml"
		parent.event dynamic ue_xml( row )
