...
type st_3 from statictext within w_cn958_genera_automaticas
end type
...
type ole_skin from w_prc`ole_skin within w_cn958_genera_automaticas
end type
...
event clicked;String ls_ano, ls_mes, ls_usuario, ls_msj
Long ll_ano, ll_mes
...
DECLARE PB_USP_CNT_GENERA_AUTOMATICAS PROCEDURE FOR 
	USP_CNT_GENERA_AUTOMATICAS( :ll_ano, :ll_mes ) ;
EXECUTE PB_USP_CNT_GENERA_AUTOMATICAS ;

IF sqlca.sqlcode = -1 THEN
	ls_msj = sqlca.sqlerrtext
	ROLLBACK ;
	MessageBox( 'Error', ls_msj, StopSign! )
	MessageBox ('Error',"Store Procedure USP_CNT_GENERA_AUTOMATICAS Falló", StopSign!)
ELSE
	COMMIT ;
	MessageBox ('Atención', "Proceso Ha Concluído Satisfactoriamente")
END IF

CLOSE PB_USP_CNT_GENERA_AUTOMATICAS ;
