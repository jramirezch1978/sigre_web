$PBExportHeader$w_ve721_rpt_reg_ventas.srw
forward
global type w_ve721_rpt_reg_ventas from w_rpt
end type
type rb_compras from radiobutton within w_ve721_rpt_reg_ventas
end type
type rb_ventas from radiobutton within w_ve721_rpt_reg_ventas
end type
type st_4 from statictext within w_ve721_rpt_reg_ventas
end type
type dw_origen from datawindow within w_ve721_rpt_reg_ventas
end type
type st_3 from statictext within w_ve721_rpt_reg_ventas
end type
type ddlb_mes from dropdownlistbox within w_ve721_rpt_reg_ventas
end type
type dw_reporte from u_dw_rpt within w_ve721_rpt_reg_ventas
end type
type cb_1 from commandbutton within w_ve721_rpt_reg_ventas
end type
type em_ano from editmask within w_ve721_rpt_reg_ventas
end type
type st_2 from statictext within w_ve721_rpt_reg_ventas
end type
type st_1 from statictext within w_ve721_rpt_reg_ventas
end type
type gb_1 from groupbox within w_ve721_rpt_reg_ventas
end type
end forward
...
global w_ve721_rpt_reg_ventas w_ve721_rpt_reg_ventas
...
IF ddlb_mes.text = '' or ddlb_mes.text = 'none' or IsNull(ddlb_mes.text) THEN
...
on w_ve721_rpt_reg_ventas.create
int iCurrent
call super::create
if this.MenuName = "m_impresion" then this.MenuID = create m_impresion
this.rb_compras=create rb_compras
...
this.Control[iCurrent+1]=this.rb_compras
this.Control[iCurrent+2]=this.rb_ventas
...
on w_ve721_rpt_reg_ventas.destroy
call super::destroy
if IsValid(MenuID) then destroy(MenuID)
destroy(this.rb_compras)
...
event ue_retrieve;call super::ue_retrieve;String 	ls_ano, ls_mes, ls_origen, ls_desc_origen, &
...
if rb_ventas.checked then
	ls_flag_sel = '1'
elseif rb_compras.checked then
	ls_flag_sel = '0'
end if
...
IF of_validacion_rpt() THEN
	...
	//create or replace procedure usp_fin_rpt_reg_ventas(
	//       as_ano              in string,
	//       as_mes              in string,
	//       as_origen           in origen.cod_origen%TYPE,
	//       ac_flag_sel_tcambio in char   
	//) is
	DECLARE usp_fin_rpt_reg_ventas PROCEDURE FOR 
		usp_fin_rpt_reg_ventas(:ls_ano, :ls_mes, :ls_origen,:ls_flag_sel);
	Execute usp_fin_rpt_reg_ventas ;
	
	IF SQLCA.SQLCode < 0 then
		ls_mensaje = SQLCA.SQLErrText
		ROLLBACK;
		MessageBox('Error en procedure usp_fin_rpt_reg_ventas', ls_mensaje)
		return
	end if
	
	CLOSE usp_fin_rpt_reg_ventas;
...
long textcolor = 33554432
long backcolor = 67108864
string text = "Seleccione Tipo de Cambio :"
