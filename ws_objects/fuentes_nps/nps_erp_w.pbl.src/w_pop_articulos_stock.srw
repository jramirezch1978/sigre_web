$PBExportHeader$w_pop_articulos_stock.srw
forward
global type w_pop_articulos_stock from w_cns
end type
type st_2 from statictext within w_pop_articulos_stock
end type
type dw_master from u_dw_cns within w_pop_articulos_stock
end type
type dw_1 from datawindow within w_pop_articulos_stock
end type
type cb_1 from commandbutton within w_pop_articulos_stock
end type
type dw_detail from u_dw_abc within w_pop_articulos_stock
end type
end forward
...
global w_pop_articulos_stock w_pop_articulos_stock
...
on w_pop_articulos_stock.create
int iCurrent
call super::create
...
on w_pop_articulos_stock.destroy
call super::destroy
destroy(this.st_2)
...
type p_pie from w_cns`p_pie within w_pop_articulos_stock
integer x = 0
integer y = 1496
end type

type ole_skin from w_cns`ole_skin within w_pop_articulos_stock
end type
...
string dataobject = "d_sel_articulo_busca_stock"
...
this.event ue_output( )
end event

event ue_output;call super::ue_output;string ls_codigo, ls_nro_serie

ls_codigo = this.object.cod_art 			[this.getRow()]
ls_nro_serie = this.object.numero_serie[this.GetRow()]

dw_detail.REtrieve( ls_codigo, ls_nro_serie)
...
dw_master.Retrieve(gnvo_app.invo_empresa.is_empresa, ls_texto)
...
integer taborder = 20
boolean bringtotop = true
string dataobject = "d_sel_articulo_stock"
end type

event doubleclicked;call super::doubleclicked;Integer li_pos, li_col, j
String  ls_column , ls_report, ls_color, ls_nro_serie
Long ll_row
...
if Row > 0 then		
	if Dec(this.object.sldo_total	[row]) <= 0 then
		MessageBox('Error', 'No hay saldo disponible en el almacen ' &
						+ string(this.object.almacen [row]))
		return
	end if
	...
	ist_datos.field_ret[4] = this.object.numero_serie	[row]
	ist_datos.field_ret[5] = this.object.almacen			[row]
