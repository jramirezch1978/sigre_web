...
ldw_master.object.fec_esp_ret		[ll_row] = Date(f_fecha_actual(1))
...
// prod_costos_diarios
select usf_fl_conv_mon(costo_produccion, cod_moneda, :is_soles, fecha)
  into :adc_costo_prod
...
if Sqlca.sqlcode = 100 then 
	select usf_fl_conv_mon(costo_produccion, cod_moneda, :is_soles, :ld_fecha)
	  into :adc_costo_prod
...
if IsNull(is_soles) or is_soles = '' then
	MessageBox('Aviso', 'No ha definido Parametro Soles en LogParam')
	return
end if
...
type p_pie from w_abc_list`p_pie within w_abc_seleccion
end type

type ole_skin from w_abc_list`ole_skin within w_abc_seleccion
end type
...
CHOOSE CASE ist_datos.opcion
	   ...
	   ls_almacen  = ldw_master.Object.Almacen [ldw_master.getrow()]
		...
		FOR j = 1 TO dw_2.RowCount()										 
			...
			IF ll_row > 0 THEN
				...
				ldw_detail.Object.cod_moneda	 [ll_row] = is_soles //dw_2.Object.cod_moneda     [j]
	...
	CASE 7 // Consumo de Articulos x Consignación
		...
		if of_art_Consig() then
			ist_datos.Titulo = 's'
			CloseWithReturn(parent, ist_mov_consig)
			return
		else
			return
		end if
