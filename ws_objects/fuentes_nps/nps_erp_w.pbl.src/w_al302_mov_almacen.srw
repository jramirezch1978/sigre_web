...
type sle_ori from singlelineedit within w_al302_mov_almacen
end type
type st_ori from statictext within w_al302_mov_almacen
end type
...
type variables
// Tipos de movimiento
String 	is_action, is_soles, is_dolares, is_rep_merc_dev, is_cencos, &
...
Decimal 		in_tipo_cambio, idc_cant_proy, idc_cant_proc, &
				idc_cant_fact, idc_cant_proc_und2
Long 			il_dias_holgura, il_afecta_pto, il_factor_lote, il_fac_total, &
				il_fac_total_dev
Boolean 		ib_log = FALSE, ib_control_lin = TRUE, ib_mod = false

dataStore 			ids_print
n_cst_log_diario	in_log_m, in_log_d
end variables
...
public function boolean of_buscar_serie (string as_numero_serie, long al_row)
end prototypes
...
of_retrieve(dw_master.object.cod_origen[ll_row], dw_master.object.nro_vale[ll_row])

IF MessageBox("Anulacion de Registro", "Esta Seguro ?", Question!, YesNo!, 2) <> 1 THEN
	RETURN
END IF

ls_tipo_ref 	= dw_master.object.tipo_refer 	[dw_master.GetRow()]
ls_nro_ref 		= dw_master.object.nro_refer 		[dw_master.GetRow()]
...
if ls_tipo_ref = is_doc_oc then
	...
	if ldc_cant_fact > 0 then
		MessageBox('Aviso', 'Imposible anular movimiento de almacen, ya ha sido facturado')
		return
	end if
end if
...
public subroutine of_retrieve (string as_origen, string as_nro);Long ll_row, ll_ano, ll_mes
...
if ll_row > 0 then
	...
	if ii_cerrado = 1 or SQLCA.SQLCode = 100 then
...
public function integer of_set_articulo (string as_articulo, string as_almacen);String 	ls_unidad, ls_desc_art, ls_und, ls_tipo_mov, ls_flag_und2, &
			...
			ls_cod_art, ls_codigo_serie, ls_mon_vta
Decimal 	ldc_prom_sol, ldc_saldo_total = 0, ldc_saldo_total_und2, &
			ldc_fac_conv, ldc_tipo_cambio, ldc_costo_prod, ldc_prom_dol
Integer	li_fac_total, li_garantia_cliente, li_year, li_mes
Long 		ll_row
Decimal	ldc_precio_vta
Date		ld_fecha

if dw_master.GetRow() = 0 then 
	MessageBox('Aviso', 'No existe cabecera de movimiento de almacen')
	return 0
end if
...
dw_detail.object.codigo_serie		[ll_row] = ls_codigo_serie
dw_detail.object.cant_procesada	[ll_row] = 1
dw_detail.object.moneda_vta		[ll_row] = ls_mon_vta
dw_detail.object.precio_venta		[ll_row] = ldc_precio_vta

//Calcular la fecha de caducidad
dw_detail.object.garantia_cliente[ll_row] = li_garantia_cliente
ld_fecha = date(dw_detail.object.fec_registro[ll_row])

li_year = Integer(string(ld_fecha, 'yyyy'))
li_mes = integer(string(ld_fecha, 'mm'))

if li_garantia_cliente > 12 then
	li_year += truncate(li_garantia_cliente/12, 0)
	li_mes += li_garantia_cliente - truncate(li_garantia_cliente/12, 0) * 12
else
	li_mes += li_garantia_cliente
end if
...
ld_fecha = date(string(li_year, '0000') + '/' + string(li_mes, '00') &
			+ '/' + string(ld_fecha, 'dd'))
				
dw_detail.object.fec_caducidad	[ll_row] = ld_fecha

if IsNull(ls_codigo_serie) or ls_codigo_serie= "" then
	dw_detail.object.codigo			[ll_row] = ls_cod_art
else
	dw_detail.object.codigo			[ll_row] = ls_codigo_serie
end if
...
if is_tipo_mov_dev <> '' and Not IsNull(is_tipo_mov_dev) then
	dw_detail.object.codigo.background.color = RGB(192,192,192)
	dw_detail.object.codigo.protect = 1
	dw_detail.object.codigo.edit.required = 'Yes'
end if
...
*/
String 	ls_almacen, ls_tipo_doc, ls_nro_doc, ls_opcion
Date 		ld_fecha
Long		ll_row_mas
...
ELSEIF is_clase_mov = 'T' then  // Orden de traslado
	...
	lstr_param.dw_master = 'd_sel_otr_transito'      
	lstr_param.dw1       = 'd_sel_otr_transito_det' 
	lstr_param.tipo		= 'OTR'     // con un parametro del tipo string
	lstr_param.opcion    = 5
...
public function integer of_evalua_mov (string as_accion);string 	ls_nro_vale, ls_origen_vale
long		ll_count
...
// Ahora verifico que el movimiento de almacén no haya sido generado por
// Almacén en tránsito
select count(*)
	into :ll_count
from articulo_mov am,
	  vale_mov_trans_det t
where am.cod_origen = t.org_am
  and am.nro_mov	  = t.nro_am
  and am.nro_vale	  = :ls_nro_vale;

if ll_count > 0 then
	MessageBox('Aviso', 'No puede ' + as_accion + ' este vale de almacen porque ha sido generado ' &
			 	+ 'desde MOVIMIENTOS EN TRÁNSITO', Exclamation!)
	return 0
end if 
...
public function boolean of_buscar_serie (string as_numero_serie, long al_row);long li_row
String ls_mensaje

if dw_detail.RowCount() = 0 then return false
if al_row <= 0 then return false

for li_row = 1 to dw_detail.RowCount()
	if li_row <> al_row then
		if dw_detail.object.numero_serie[li_row] = as_numero_serie then
			ls_mensaje = "El numero de serie ya esta asignado al articulo " &
						  + "~r~nCódigo: " + dw_detail.object.codigo  [li_row] + " " &
						  + "~r~nDescripcion: " +dw_detail.object.desc_art[li_row]
...
this.Control[iCurrent+10]=this.em_saldo_und2
this.Control[iCurrent+11]=this.em_saldo
this.Control[iCurrent+12]=this.em_prom_dol
this.Control[iCurrent+13]=this.em_prom_sol
...
in_tipo_cambio = of_get_tipo_cambio( DATE(id_fecha_proc) )
...
li_protect = integer(dw_detail.Object.codigo.Protect)
...
IF ib_log THEN
	...
	in_log_m.of_create_log(dw_master, lds_log_m, is_colname_m, is_coltype_m, gnvo_app.is_user, is_tabla_m, gnvo_app.invo_empresa.is_empresa)
	
	lds_log_d = Create u_ds_base
	lds_log_d.DataObject = 'd_log_diario_tbl'
	lds_log_d.SetTransObject(SQLCA)
	in_log_d.of_create_log(dw_detail, lds_log_d, is_colname_d, is_coltype_d, gnvo_app.is_user, is_tabla_d, gnvo_app.invo_empresa.is_empresa)
...
IF lbo_ok THEN
	...
	of_retrieve( ls_origen_vale, ls_nro_vale)
	of_set_status_doc( dw_master)
...
gb_1.width  = p_pie.width  - (gb_1.x - p_pie.x)
gb_1.Y		= p_pie.y + (p_pie.height - gb_1.height)/2 + this.cii_Windowborder
	
st_1.y = gb_1.y + (gb_1.height - st_1.height) / 2
em_saldo. y = gb_1.y + (gb_1.height - em_saldo.height) / 2
	
st_2.y  = gb_1.y + (gb_1.height - st_2.height) / 2
em_prom_sol.y = gb_1.y + (gb_1.height - em_prom_sol.height) / 2
	
st_3.y = gb_1.y + (gb_1.height - st_3.height) / 2
em_prom_dol.y= gb_1.y + (gb_1.height - em_prom_dol.height) / 2

st_4.y = gb_1.y + (gb_1.height - st_4.height) / 2
em_saldo_und2.y= gb_1.y + (gb_1.height - em_saldo_und2.height) / 2
...
If IsNull(ids_print) then return

if IsValid(ids_print) then
	DESTROY ids_print
end if
end event
...
event ue_open_pos;call super::ue_open_pos;IF ib_log THEN											
	in_log_m = Create n_cst_log_diario
	in_log_m.of_dw_map(dw_master, is_colname_m, is_coltype_m)
...
type p_pie from w_abc`p_pie within w_al302_mov_almacen
end type

type ole_skin from w_abc`ole_skin within w_al302_mov_almacen
integer y = 40
end type

type uo_h from w_abc`uo_h within w_al302_mov_almacen
end type

type st_box from w_abc`st_box within w_al302_mov_almacen
end type

type phl_logonps from w_abc`phl_logonps within w_al302_mov_almacen
end type

type p_mundi from w_abc`p_mundi within w_al302_mov_almacen
end type

type p_logo from w_abc`p_logo within w_al302_mov_almacen
end type
...
event itemchanged;call super::itemchanged;String 	ls_tipo_mov, ls_cod_art, ls_almacen, ls_consig, ls_pide_cen, ls_null, &
...
Decimal {4} ld_cant, ld_precio, ldc_fac_conv, ldc_cant_proc_und2, &
			ldc_precio_venta, ldc_Null
Double  	ld_saldo_act = 0, ln_cant_asig
long 		ll_count, ll_null, ll_row_mas, ll_mes, ll_ano
...
ls_almacen  	= dw_master.Object.almacen[dw_master.getrow()]
ls_tipo_mov 	= dw_master.Object.tipo_mov[dw_master.getrow()]
ls_tipo_refer	= dw_master.object.tipo_refer[dw_master.GetRow()]
...
CHOOSE CASE dwo.name
	CASE 'codigo'		
		if of_set_articulo( data, ls_almacen) = 0 then
			this.object.cod_art			[row] = ls_null
			this.object.desc_art			[row] = ls_null
			this.object.und				[row] = ls_null
			this.object.codigo			[row] = ls_null
			this.object.codigo_serie	[row] = ls_null
			this.object.precio_venta	[row] = 0.00
	...
	CASE 'precio_unit'		
		...
		ldc_precio_venta = dec(this.object.precio_venta[row])
		if IsNull(ldc_precio_venta) then ldc_precio_venta = 0
		
		if double( data) > ldc_precio_venta then
			Messagebox("Aviso", "El precio de costo no puede ser mayor que el precio de venta") 
			this.object.precio_unit[row] = ldc_null
			return 1
		end if
	...
	CASE 'cant_procesada'
		...
		ldc_cant_proc_und2 	= this.object.cant_proc_und2[row]
		If IsNull(ldc_cant_proc_und2) then ldc_cant_proc_und2 = 0

		if dec(data) <= 0 then  // no acepta negativos
			Messagebox("Aviso", "No puede ingresar esta cantidad") 
			this.object.cant_procesada[row] = ll_null
			return 1
		end if
		
		if is_tipo_mov_dev <> '' and Not ISNull(is_tipo_mov_dev) then
			if dec(data) > idc_cant_proc and Not IsNull(idc_cant_proc) then
				MessageBox('Aviso', 'no puede digitar mas que la cantidad que proceso anteriormente')
				this.object.cant_procesada[row] = idc_cant_proc
				return 1
			end if
		end if
		
		if of_evalua_saldos(row) = 0 then 
			this.object.cant_procesada[row] = ll_null
			return 1
		end if	
		...
		if ls_flag_und2 = '1' then
			...
			if is_tipo_mov_dev <> '' and Not ISNull(is_tipo_mov_dev) then
				if dec(this.object.cant_proc_und2[row]) > idc_cant_proc_und2 then
					MessageBox('Aviso', 'no puede digitar mas que la cantidad que proceso anteriormente (Und2)')
					this.object.cant_proc_und2[row] = idc_cant_proc_und2
					return 1
				end if
			end if
	...
	CASE 'cant_proc_und2'
		...
		if dec(data) < 0 then  // no acepta negativos
			Messagebox("Aviso", "No puede ingresar esta cantidad") 
			this.object.cant_proc_und2[row] = ll_null
			return 1
		end if

		if is_tipo_mov_dev <> '' and Not ISNull(is_tipo_mov_dev) then
			if dec(data) > idc_cant_proc_und2 then
				MessageBox('Aviso', 'no puede digitar mas que la cantidad que proceso anteriormente (Und2)')
				this.object.cant_proc_und2[row] = idc_cant_proc_und2
				return 1
			end if
	...
	CASE "cod_maquina"
		...
		if ll_count = 0 then			
			Messagebox( "Atencion", "Maquina no existe o no esta activo")
	...
	CASE "matriz"
		...
		ELSEIF is_flag_matriz_contab = 'S' THEN
			if IsNull(ls_cod_art) or trim(ls_cod_art) = '' then
				MessageBox('Error', 'Codigo de Articulo esta nulo o vacio')
				return 1
			end if
			...
			if SQLCA.SQLCode = 100 then
				MessageBox('Error', 'Codigo de Articulo ' + ls_cod_art + ' no existe')
				return 1
			end if
		...
		ELSE
			MessageBox('Aviso', 'Valor de FLAG_MATRIZ_CONTAB no valido')
			return 1
		end if
	...
	case "numero_serie"
		if of_buscar_serie(data, row) then
			this.object.numero_serie[row] = ls_null
			this.setColumn("numero_serie")
			return 1
		end if
END CHOOSE
...
this.object.flag_obsequio	[al_row] = '0'
this.object.fec_registro	[al_row] = f_fecha_actual(0) //dw_master.object.fec_registro[dw_master.getRow()]
...
// Si es un movimiento de devolucion no puedo modificar el articulo
if is_tipo_mov_dev <> '' and Not IsNull(is_tipo_mov_dev) then
	this.object.codigo.background.color = RGB(192,192,192)
	this.object.codigo.protect = 1
	this.object.codigo.edit.required = 'Yes'
end if
...
String 	ls_name, ls_prot, ls_tipo_mov, ls_cencos, ls_cnta_prsp, &
    		ls_grp_cntbl, ls_matriz, ls_grupo_cntbl, ls_sql, ls_codigo, &
			ls_data, ls_null, ls_almacen, ls_sub_cat, ls_cod_art
Long 		ll_count, ll_mes, ll_year
boolean	lb_Ret
str_parametros sl_param
...
ll_year = Long(string(dw_master.object.fec_registro[dw_master.GetRow()], 'yyyy'))

choose case lower(as_columna)
	case "codigo"
		ls_almacen = dw_master.object.almacen[dw_master.getRow()]
		
		sl_param.almacen = ls_almacen
		
		OpenWithParm (w_pop_articulos, sl_param)
		sl_param = MESSAGE.POWEROBJECTPARM
		IF sl_param.titulo <> 'n' then
			if of_set_articulo( sl_param.field_ret[1], dw_master.Object.almacen[dw_master.getrow()]) = 1 then
				...
				this.object.flag_numero_serie	[al_row] = sl_param.field_ret[8]	
				
				if ls_tipo_mov <> is_oper_ing_oc then
					this.object.numero_serie	[al_row] = sl_param.field_ret[7]	
				end if
			end if
			this.ii_update = 1
		END IF
	...
	case "cnta_prsp"
		...
		if is_contab = '1' then
			if of_set_matriz() = 0 then
				this.object.matriz 	[al_row] = ls_null
				this.setfocus()
				this.setcolumn('cnta_prsp')
				return 
			end if	
	...
	case "nro_lote"
		...
		if ls_almacen = '' or IsNull(ls_almacen) then
			MEssageBox('Aviso', 'Defina el almacen primero')
			return
		end if
		
		if il_fac_total = 1 then
			sl_param.dw1 	 = "d_sel_templas"
			sl_param.titulo = "Templas disponibles"
		else
			sl_param.dw1 = "d_sel_saldo_templa_x_codigo"
			sl_param.titulo = "Templas disponibles"
			sl_param.tipo = '2S'
			sl_param.string1 = this.object.cod_art[al_row]
	...
	case "matriz"
		...
		if is_flag_matriz_contab = 'P' then
			...
			OpenWithParm( w_search_almacen, sl_param)		
			sl_param = MESSAGE.POWEROBJECTPARM
			if sl_param.titulo <> 'n' then		
				this.object.matriz[al_row] = sl_param.field_ret[1]		
				ls_matriz 				   = sl_param.field_ret[1]
...
choose case lower(as_columna)
	...
	case "tipo_mov"
		...
		lb_ret = f_lista_3ret(ls_sql, ls_codigo, ls_data, is_tipo_mov_dev, '2')
...
if ls_boton = 'b_ref' then	
	...
	IF is_solicita_ref = '0' then
		...
		Return
	END IF	
	of_verifica_mov(ls_tipo_mov)	
	
elseif ls_boton = 'b_ingreso' then	
	...
	OpenWithParm(w_al302_ingreso_masivo, sl_param)
...
elseif ls_boton = 'b_fecha' then
	...
	of_retrieve(ls_origen, ls_nro_vale)
