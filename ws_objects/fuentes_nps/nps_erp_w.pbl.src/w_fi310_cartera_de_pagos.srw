...
type rb_crp from radiobutton within w_fi310_cartera_de_pagos
end type
type cb_3 from commandbutton within w_fi310_cartera_de_pagos
end type
type cb_2 from commandbutton within w_fi310_cartera_de_pagos
end type
...
this.Control[iCurrent+3]=this.rb_cv
this.Control[iCurrent+4]=this.rb_v
this.Control[iCurrent+5]=this.rb_cr
this.Control[iCurrent+6]=this.rb_crp
this.Control[iCurrent+7]=this.cb_3
this.Control[iCurrent+8]=this.cb_2
this.Control[iCurrent+9]=this.cbx_ref
...
IF ls_flag_retencion = '1' THEN /*flag de retencion de cabecera*/
	FOR ll_inicio = 1 TO ids_crelacion_ext_tbl.Rowcount()
	 	 ls_cod_relacion = ids_crelacion_ext_tbl.object.cod_relacion [ll_inicio]
	 	 ls_nro_ret	     = ids_crelacion_ext_tbl.object.nro_cr       [ll_inicio]
	 
		 ls_expresion_cri =	'cod_relacion = '+"'"+ls_cod_relacion+"' and tipo_docref1 = "+"'"+is_doc_ret+"'"
		 ll_found_cri = tab_1.tabpage_2.dw_asiento_det.find(ls_expresion_cri, 1,tab_1.tabpage_2.dw_asiento_det.rowcount( ) )
		
		 if ll_found_cri > 0 then
			 tab_1.tabpage_2.dw_asiento_det.object.nro_docref1 [ll_found_cri] = ls_nro_ret
		 end if
...
FOR ll_inicio = 1 TO tab_1.tabpage_1.dw_detail.Rowcount()
	 ...
	 ls_flag_provisionado = tab_1.tabpage_1.dw_detail.object.flag_provisionado [ll_inicio]	 
	 ls_flag_referencia   = tab_1.tabpage_1.dw_detail.object.flag_referencia   [ll_inicio]	 	 
	 ls_referencia   		 = tab_1.tabpage_1.dw_detail.object.referencia   		[ll_inicio]	 	 
	 ll_item			       = tab_1.tabpage_1.dw_detail.object.item			      [ll_inicio]	
...
// Si no tiene registros rrhhparam entonces inserto uno el blanco
select count(*)
	into :ll_count
	from rrhhparam;

if ll_count = 0 then
	insert into rrhhparam(reckey) values('1');
	
	if not gnvo_app.of_valida_transaccion( "Error al insertar en RHHPARAM", SQLCA) then 
		ib_update_check = false
		return
	end if
end if
...
/*Datos de Retencion*/
if wf_genera_doc_retencion(ls_flag_retencion,ldc_tasa_cambio,ldc_imp_ret_sol,ldc_imp_ret_dol,ldt_fecha_doc,ls_origen,ll_nro_registro) = false then
	lbo_ok = FALSE
	GOTO SALIDA
end if
...
ELSE
	...
	IF tab_1.tabpage_1.dw_detail.ii_protect = 0 THEN //MODIFICABLE DETALLE DOCUMENTOS
		...
		tab_1.tabpage_1.dw_detail.Modify("flag_referencia.Protect ='1~tIf(IsNull(flag_doc),1,0)'")		
		...
		tab_1.tabpage_1.dw_detail.Modify("tasa_cambio.Protect ='1~tIf(IsNull(t_detrac),0,1)'")		
	END IF
...
event ue_print;call super::ue_print;String ls_origen,ls_nro_certificado,ls_flag_ts,ls_print
Long   ll_nro_registro,ll_row_master,ll_count,ll_tiempo,ll_i
Double	ldb_rc
...
IF rb_cv.checked THEN
	...
	lnvo_impresion.of_voucher_cheque ('d_rpt_fomato_chq_voucher_tbl',ls_origen,ll_nro_registro,gnvo_app.invo_empresa.is_empresa,ls_flag_ts,ls_print,'P')
...
ELSEIF rb_cr.checked THEN
	...
	elseif ll_count = 1 then
		select nro_certificado into :ls_nro_certificado from retencion_igv_crt 
		 where (origen				 = :ls_origen      ) and
		 		 (nro_reg_caja_ban = :ll_nro_registro) ;
   ...
   /*asignacion de dw*/
	if cbx_ts.checked then
		ids_rpt.dataobject =  'd_rpt_formato_retencion_igv_ts_tbl'
		
	else
		/*IMPRESION LOCAL */
		ids_rpt.dataobject =  'd_rpt_formato_retencion_igv_tbl'		
	end if
	
	ids_rpt.Settransobject(sqlca)
	ids_rpt.Retrieve(ls_nro_certificado)
	ids_rpt.Print(True)	
...
ELSEIF rb_crp.checked THEN
	...
	if cbx_ts.checked then
		...
		ids_rpt.dataobject = 'd_rpt_formato_carta_ret_igv_terminal_tbl'
	else
		ids_rpt.dataobject = 'd_rpt_formato_paramong_retencion_igv_tbl'		
	end if
...
For ll_i = 1 to ll_tiempo 
	 w_fi310_cartera_de_pagos.SetMicrohelp( String( ll_i ))
End For 
...
w_fi310_cartera_de_pagos.SetMicrohelp( String( 0 ))
end event
...
type ole_skin from w_abc`ole_skin within w_fi310_cartera_de_pagos
integer x = 3698
integer y = 56
end type
...
IF SQLCA.SQLCode = -1 THEN 
	ls_mensaje = 'Error en Procedure USP_FIN_GENER_CHEQUE_X_DOC: ' &
				  + SQLCA.SQLErrText
	ROLLBACK;
...
Decimal {2} ldc_importe,ldc_imp_retencion
Decimal {3} ldc_tasa_cambio
dwItemStatus ldis_status
...
String ls_name,ls_prot,ls_flag_provisionado,ls_cencos,ls_cnta_prsp,ls_null,ls_accion,&
		 ls_cod_relacion	
...
IF f_nro_libro_pagos(ll_nro_libro) = FALSE THEN Messagebox('Aviso','No Existe Nro de Libro , Verifique Tabla de Parametros Finparam , Campo libro_pagos')
