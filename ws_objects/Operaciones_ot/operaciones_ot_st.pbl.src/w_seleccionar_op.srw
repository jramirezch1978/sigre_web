$PBExportHeader$w_seleccionar_op.srw
forward
global type w_seleccionar_op from window
end type
type dw_new from uo_datawindow within w_seleccionar_op
end type
type st_2 from statictext within w_seleccionar_op
end type
type st_1 from statictext within w_seleccionar_op
end type
type cbx_1 from checkbox within w_seleccionar_op
end type
type st_3 from statictext within w_seleccionar_op
end type
type cb_cancelar from commandbutton within w_seleccionar_op
end type
type cb_detener from commandbutton within w_seleccionar_op
end type
type cb_iniciar from commandbutton within w_seleccionar_op
end type
type cb_aceptar from commandbutton within w_seleccionar_op
end type
type sle_descripcion from singlelineedit within w_seleccionar_op
end type
type ddlb_seleccion from dropdownlistbox within w_seleccionar_op
end type
type mle_1 from multilineedit within w_seleccionar_op
end type
end forward
...
global w_seleccionar_op w_seleccionar_op
...
event ue_aceptar;Integer  li_i,li_j=0
...
FOR li_i = 1 TO Integer(dw_new.Object.DataWindow.Column.Count)
...
event ue_busqueda();Integer ll_i
String  ls_sql_old,ls_cadena,ls_argumento,ls_sql_new
...
ls_cadena    = ddlb_seleccion.Text 
ls_argumento = wf_condicion_sql(ls_cadena)
ls_sql_new   = ls_sql_old + ls_argumento
...
event ue_cancel;str_seleccionar lstr_seleccionar
...
as_cadena   = UPPER(as_cadena)
ll_pos      = Pos(ls_sql,' '+as_cadena,1) 
...
IF lb_where THEN
   as_cadena = ' WHERE ( '+ls_sql+' LIKE '+ls_descripcion+' )'    
ELSE
	as_cadena = ' AND ( '+ls_sql+' LIKE '+ls_descripcion+' )'
END IF
...
String  ls_report_type,ls_style,ls_sql_syntax,ls_dw_err,ls_dw_syntax
Integer i
...
event doubleclicked;call super::doubleclicked;IF cb_aceptar.enabled = true AND dw_new.rowcount() > 0 THEN
