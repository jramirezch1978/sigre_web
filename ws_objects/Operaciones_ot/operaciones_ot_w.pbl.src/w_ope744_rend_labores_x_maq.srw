$PBExportHeader$w_ope744_rend_labores_x_maq.srw
forward
global type w_ope744_rend_labores_x_maq from w_report_smpl
end type
type sle_codigo from singlelineedit within w_ope744_rend_labores_x_maq
end type
type rb_cc from radiobutton within w_ope744_rend_labores_x_maq
end type
type rb_ej from radiobutton within w_ope744_rend_labores_x_maq
end type
type sle_nombre from singlelineedit within w_ope744_rend_labores_x_maq
end type
type uo_1 from u_ingreso_rango_fechas within w_ope744_rend_labores_x_maq
end type
type cb_3 from commandbutton within w_ope744_rend_labores_x_maq
end type
type pb_1 from picturebutton within w_ope744_rend_labores_x_maq
end type
type gb_1 from groupbox within w_ope744_rend_labores_x_maq
end type
end forward
...
integer width = 3269
integer height = 2040
string title = "Rendimiento de labores mecanizadas x campo (ope744)"
...
global w_ope744_rend_labores_x_maq w_ope744_rend_labores_x_maq

on w_ope744_rend_labores_x_maq.create
int iCurrent
call super::create
...
on w_ope744_rend_labores_x_maq.destroy
call super::destroy
...
type dw_report from w_report_smpl`dw_report within w_ope744_rend_labores_x_maq
integer x = 50
integer y = 428
integer width = 3145
integer height = 1240
...
event dw_report::doubleclicked;call super::doubleclicked;Date ld_fec_ini, ld_fec_fin
...
IF rb_cc.checked=true then
	idw_1.DataObject='d_rpt_rend_labores_maquina_campo_tbl'
	idw_1.SetTransObject(sqlca)
	idw_1.retrieve(ld_fec_ini, ld_fec_fin)
	ls_texto = ''
ELSEIF rb_ej.checked=true then
	idw_1.DataObject='d_rpt_rend_labores_maq_campo_ejec_tbl'
	idw_1.SetTransObject(sqlca)
	idw_1.retrieve(ls_codigo, ld_fec_ini, ld_fec_fin)
	ls_texto = 'Ejecutor: ' + ls_codigo
...
IF rb_ej.checked = true then
	...
	lstr_seleccionar.s_sql = 'SELECT EJECUTOR.COD_EJECUTOR  AS EJECUTOR,'&
									 +'EJECUTOR.DESCRIPCION AS DESCRIPCION '&     	
		   						 +'FROM EJECUTOR ' &
									 +'WHERE EJECUTOR.FLAG_ESTADO <> ' + "'"+ls_inactivo+"'"
