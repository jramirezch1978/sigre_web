$PBExportHeader$w_lista_aprob.srw
forward
global type w_lista_aprob from w_abc_master_smpl
end type
type st_campo from statictext within w_lista_aprob
end type
type dw_1 from datawindow within w_lista_aprob
end type
type cb_3 from commandbutton within w_lista_aprob
end type
type pb_1 from picturebutton within w_lista_aprob
end type
type pb_2 from picturebutton within w_lista_aprob
end type
type pb_3 from picturebutton within w_lista_aprob
end type
end forward
...
global w_lista_aprob w_lista_aprob
...
String  	ls_sql_old, ls_sql_new, ls_descripcion, ls_campo, ls_cad_a, ls_where, ls_select, ls_alias
...
ls_sql_new = invo_utility.of_replace(ls_sql_old, '"', '')
ls_sql_new = invo_utility.of_replace(ls_sql_new, char(9), ' ')
ls_sql_new = invo_utility.of_replace(ls_sql_new, char(13), ' ')
ls_sql_new = invo_utility.of_replace(ls_sql_new, char(10), ' ')
...
if Pos(ls_select, upper(is_col)) > 0 then
	...
	if Pos(ls_select, "." + upper(is_col)) > 0 then
		...
		if is_col0 <> "" then
			if Pos(ls_alias, is_col0) > 0 then
...
IF dw_master.SetSQLSelect(ls_sql_new) = 1 THEN
	...
	if li_return = -1 then
		MessageBox(this.ClassName(), 'NO SE PUDO EJECUTAR CORRECTAMENTE ' &
							+ 'LA SENTENCIA SQL: ~r~n' + ls_sql_new, StopSign!)
...
event ue_open_pre;String ls_null
Long ll_row
...
THIS.EVENT ue_retrieve()
//invo_utility = create n_cst_utilitario
end event

event close;call super::close;//destroy invo_utility
end event

event ue_retrieve;call super::ue_retrieve;string ls_ot
ls_ot = ist_inp.string1
idw_1.retrieve (ls_ot)
end event

type dw_master from w_abc_master_smpl`dw_master within w_lista_aprob
integer y = 44
integer width = 2034
integer height = 1100
...
ELSE
	ist_ret.string1 = idw_1.object.nro_aprob[row]
	ist_ret.titulo = "s"		
