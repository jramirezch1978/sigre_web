	...
	ELSE		// caso contrario hace un retrieve con parametros
		CHOOSE CASE is_param.tipo
			    CASE '1NVP' //Nota de Ventas Por Pagar
				 ...
				 CASE 'OT'	  //ORDEN DE TRABAJO	

						
						dw_1.Retrieve(is_param.string2,is_param.string3,is_param.string4,is_param.string5)	
						
						//filtrado						
						if is_param.bret = TRUE then
							dw_1.setfilter('cant_pend <= '+Trim(String(is_param.dc1)))
						else
							dw_1.setfilter('cant_pend = '+Trim(String(is_param.dc1)))							
						end if 
...
CHOOSE CASE is_param.opcion 
      ...
      CASE  11
		...
		CASE	13 ,15 ,25 //Cheques a Conciliar,Documentos a Conciliar ,Orden de Trabajo
		...
		CASE	14,16,17,18,19,20,21,22,23,24//Articulo,Concepto Finanaciero,centro de costo,maquina,cliente,ot_adm
				ii_dk [1] = 1 //Codigo
				ii_dk [2] = 2 //Nombre
...
CHOOSE CASE is_param.opcion 
      ...
      CASE  12 //Comprobantes de Egresos
		...
		CASE	13 ,15 ,25 //Cheques a Conciliar,Documentos a Conciliar ,ordenes de trabajo
				ii_rk [1] = 1 //Nro Reg cheque
				ii_rk [2] = 2 //Cuenta de Banco	
				ii_rk [3] = 3 //Nro de Cheque
				ii_rk [4] = 4 //Importe
		...
		CASE	14,16,17,18,19,20,21,22,23,24 //Articulo,Concepto financiero,centro de costo,maquina,cliente,ot_adm
				ii_rk [1] = 1 //Codigo
				ii_rk [2] = 2 //Nombre
...
//								 is_param.dw_c.Object.flag_hab			[ll_inicio] = 'S'
       ...
       CASE	12 /*COMPROBANTE DE EGRESOS (EGRESOS DIRECTOS)*/
		 ...
		 CASE	25	
				 IF dw_2.rowcount() > 1 THEN
					 Messagebox('Aviso','Solo Seleccione un Movimiento')
					 RETURN
				 END IF	 
				 
				//CAMBIAR DW DESTINO
				for ll_inicio = 1  to dw_2.rowcount( )
					 is_param.dw_m.object.org_amp_ref  [is_param.dw_m.GetRow ()] = dw_2.object.cod_origen [ll_inicio] 
					 is_param.dw_m.object.nro_amp_ref  [is_param.dw_m.GetRow ()] = dw_2.object.nro_mov    [ll_inicio] 		
					 is_param.dw_m.object.cant_proyect [is_param.dw_m.GetRow ()] = dw_2.object.cant_pend  [ll_inicio] 	
					 is_param.dw_m.object.origen_ref   [is_param.dw_m.GetRow ()] = dw_2.object.cod_origen [ll_inicio]					 
 					 is_param.dw_m.object.tipo_ref     [is_param.dw_m.GetRow ()] = dw_2.object.tipo_doc   [ll_inicio]					 
					 is_param.dw_m.object.referencia   [is_param.dw_m.GetRow ()] = dw_2.object.nro_doc    [ll_inicio]					 					  
