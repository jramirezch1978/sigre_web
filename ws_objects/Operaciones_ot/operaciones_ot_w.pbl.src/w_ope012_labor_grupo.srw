$PBExportHeader$w_ope012_labor_grupo.srw
forward
global type w_ope012_labor_grupo from w_abc_mastdet_smpl
end type
type dw_detdet from u_dw_abc within w_ope012_labor_grupo
end type
type st_grupos from statictext within w_ope012_labor_grupo
end type
type st_detdet from statictext within w_ope012_labor_grupo
end type
type st_detail from statictext within w_ope012_labor_grupo
end type
end forward
...
integer width = 2848
integer height = 1892
string title = "Labor Grupo - OT Adm  (OPE012)"
...
st_detdet.width  = newwidth  - st_detdet.x - 10
...
type dw_master from w_abc_mastdet_smpl`dw_master within w_ope012_labor_grupo
integer x = 0
integer y = 84
integer width = 1285
integer height = 768
string dataobject = "d_abc_labor_grupo_tbl"
end type
...
type dw_detail from w_abc_mastdet_smpl`dw_detail within w_ope012_labor_grupo
event ue_display ( string as_columna,  long al_row )
integer x = 0
integer y = 944
integer width = 2665
integer height = 716
string dataobject = "d_abc_labor_grupo_ot_adm_tbl"
end type
...
event dw_detail::itemchanged;call super::itemchanged;Long   ll_count
...
choose case dwo.name
		 ...
		 case 'ot_adm'	
				...
				if ll_count = 0 then
					...
					select descripcion into :ls_descrip from ot_administracion where ot_adm = :data ;
					This.object.descripcion [row] = ls_descrip
				end if
...
string dataobject = "d_abc_labor_grupo_labor_rel_tbl"
...
choose case dwo.name
		 ...
		 case 'cod_labor'	
				select count(*) into :ll_count from labor where cod_labor = :data ;
				...
				if ll_count = 0 then
					...
					Return 1
				ELSE
					select DESC_LABOR into :lS_DESCRIP from labor where cod_labor = :data ;
					
					This.object.DESC_LABOR [row] = ls_DESCRIP
				end if
