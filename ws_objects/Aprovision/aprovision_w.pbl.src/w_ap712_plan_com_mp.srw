$PBExportHeader$w_ap712_plan_com_mp.srw
forward
global type w_ap712_plan_com_mp from w_rpt
end type
type st_desc_precio from statictext within w_ap712_plan_com_mp
end type
type sle_cod_precio from singlelineedit within w_ap712_plan_com_mp
end type
type st_descripcion from statictext within w_ap712_plan_com_mp
end type
type pb_1 from picturebutton within w_ap712_plan_com_mp
end type
type rb_3 from radiobutton within w_ap712_plan_com_mp
end type
type rb_2 from radiobutton within w_ap712_plan_com_mp
end type
type rb_1 from radiobutton within w_ap712_plan_com_mp
end type
type dw_origen from u_dw_abc within w_ap712_plan_com_mp
end type
type cbx_especie from checkbox within w_ap712_plan_com_mp
end type
type uo_fecha from u_ingreso_rango_fechas_v within w_ap712_plan_com_mp
end type
type sle_especie from singlelineedit within w_ap712_plan_com_mp
end type
type dw_report from u_dw_rpt within w_ap712_plan_com_mp
end type
type gb_1 from groupbox within w_ap712_plan_com_mp
end type
end forward

global type w_ap712_plan_com_mp from w_rpt
integer width = 4352
integer height = 2156
string title = "Planilla de Compra de Materia Prima con Diferentes Precios   (AP712)"
string menuname = "m_rpt"
windowstate windowstate = maximized!
long backcolor = 67108864
event ue_query_retrieve ( )
st_desc_precio st_desc_precio
sle_cod_precio sle_cod_precio
st_descripcion st_descripcion
pb_1 pb_1
rb_3 rb_3
rb_2 rb_2
rb_1 rb_1
dw_origen dw_origen
cbx_especie cbx_especie
uo_fecha uo_fecha
sle_especie sle_especie
dw_report dw_report
gb_1 gb_1
end type
global w_ap712_plan_com_mp w_ap712_plan_com_mp

type variables
string is_cod_origen
end variables

forward prototypes
public function decimal of_tipo_cambio (date ad_fecha)
public function boolean of_verificar ()
end prototypes

event ue_query_retrieve();this.event dynamic ue_retrieve()
end event

public function decimal of_tipo_cambio (date ad_fecha);Dec 		ld_cambio
long 		ll_count
String	ls_cod_soles

select count(*)
  	into :ll_count
from calendario
where to_char(fecha, 'yyyymmdd') = to_char(:ad_fecha, 'yyyymmdd');

if ll_count = 0 then
   select l.ult_tipo_cam
      into :ld_cambio
      from logparam l
      where reckey = '1';
      
     if ld_cambio = 0 then
        messagebox('Aprovisionamiento', 'No ha definido el tipo de cambio para'+&
		  				'el dia ' + string(ad_fecha, 'dd/mm/yyyy') )
        return 0
     end if
end if

if ll_count <> 0 then
    	select nvl(c.vta_dol_prom,0)
    		into :ld_cambio
	      from calendario c
 	    where to_char(c.fecha, 'yyyymmdd') = to_char(:ad_fecha, 'yyyymmdd');
   end if
	
Return ld_cambio
end function

public function boolean of_verificar ();// Verifica que no falten parametros para el reporte

long 		ll_i
String 	ls_separador
boolean	lb_ok

is_cod_origen = ''
ls_separador  = ''
lb_ok			  = True

IF sle_especie.Text = '' and Not cbx_especie.Checked THEN
	messagebox('Aprovisionamiento', 'Por favor seleccione una Especie')
	RETURN lb_ok = False
END IF

IF sle_cod_precio.Text = '' and rb_3.Checked THEN
	messagebox('Aprovisionamiento', 'Por favor seleccione un codigo de precio')
	RETURN lb_ok = False
END IF

// leer el dw_origen con los origenes seleccionados

For ll_i = 1 To dw_origen.RowCount()
	If dw_origen.Object.Chec[ll_i] = '1' Then
		if is_cod_origen <>'' THEN ls_separador = ', '
		is_cod_origen = is_cod_origen + ls_separador + dw_origen.Object.cod_origen[ll_i]
	end if
Next

IF LEN(is_cod_origen) = 0 THEN
	messagebox('Aprovisionamiento', 'Debe seleccionar al menos un origen para el Reporte')
	return lb_ok = False
END IF

RETURN lb_ok
end function

on w_ap712_plan_com_mp.create
int iCurrent
call super::create
if this.MenuName = "m_rpt" then this.MenuID = create m_rpt
this.st_desc_precio=create st_desc_precio
this.sle_cod_precio=create sle_cod_precio
this.st_descripcion=create st_descripcion
this.pb_1=create pb_1
this.rb_3=create rb_3
this.rb_2=create rb_2
this.rb_1=create rb_1
this.dw_origen=create dw_origen
this.cbx_especie=create cbx_especie
this.uo_fecha=create uo_fecha
this.sle_especie=create sle_especie
this.dw_report=create dw_report
this.gb_1=create gb_1
iCurrent=UpperBound(this.Control)
this.Control[iCurrent+1]=this.st_desc_precio
this.Control[iCurrent+2]=this.sle_cod_precio
this.Control[iCurrent+3]=this.st_descripcion
this.Control[iCurrent+4]=this.pb_1
this.Control[iCurrent+5]=this.rb_3
this.Control[iCurrent+6]=this.rb_2
this.Control[iCurrent+7]=this.rb_1
this.Control[iCurrent+8]=this.dw_origen
this.Control[iCurrent+9]=this.cbx_especie
this.Control[iCurrent+10]=this.uo_fecha
this.Control[iCurrent+11]=this.sle_especie
this.Control[iCurrent+12]=this.dw_report
this.Control[iCurrent+13]=this.gb_1
end on

on w_ap712_plan_com_mp.destroy
call super::destroy
if IsValid(MenuID) then destroy(MenuID)
destroy(this.st_desc_precio)
destroy(this.sle_cod_precio)
destroy(this.st_descripcion)
destroy(this.pb_1)
destroy(this.rb_3)
destroy(this.rb_2)
destroy(this.rb_1)
destroy(this.dw_origen)
destroy(this.cbx_especie)
destroy(this.uo_fecha)
destroy(this.sle_especie)
destroy(this.dw_report)
destroy(this.gb_1)
end on

event ue_preview;call super::ue_preview;IF ib_preview THEN
	idw_1.Modify("DataWindow.Print.Preview=No")
	idw_1.Modify("datawindow.print.preview.zoom = " + String(idw_1.ii_zoom_actual))
	idw_1.title = "Reporte " + " (Zoom: " + String(idw_1.ii_zoom_actual) + "%)"
	SetPointer(hourglass!)
	ib_preview = FALSE
ELSE
	idw_1.Modify("DataWindow.Print.Preview=Yes")
	idw_1.Modify("datawindow.print.preview.zoom = " + String(idw_1.ii_zoom_actual))
	idw_1.title = "Reporte " + " (Zoom: " + String(idw_1.ii_zoom_actual) + "%)"
	SetPointer(hourglass!)
	ib_preview = TRUE
END IF
end event

event ue_open_pre;call super::ue_open_pre;long ll_row

idw_1 = dw_report
idw_1.Visible = False
idw_1.SetTransObject(sqlca)

// Para mostrar los origenes
dw_origen.SetTransObject(sqlca)
dw_origen.Retrieve()
  
ll_row = dw_origen.Find ("cod_origen = '" + gs_origen +"'", 1, dw_origen.RowCount())

dw_origen.object.chec[ll_row] = '1'



THIS.Event ue_preview()


end event

event resize;call super::resize;dw_report.width  = newwidth - dw_report.x
dw_report.height = newheight - dw_report.y
end event

event ue_print;call super::ue_print;idw_1.EVENT ue_print()
end event

event ue_retrieve;call super::ue_retrieve;string ls_rango_fecha, ls_razsoc, ls_ruc, ls_direccion, ls_lote, &
		 ls_mnz, ls_especie, ls_cod_precio
date 	 ld_fecha_ini, ld_fecha_fin
dec 	 ld_tipo_cambio 

// Llama a la función para verificar
IF NOT of_verificar() THEN RETURN

ld_fecha_ini = date(uo_fecha.of_get_fecha1( ))
ld_fecha_fin = date(uo_fecha.of_get_fecha2( ))

idw_1.SetRedraw(false)

//Recupera los datos del reporte

IF cbx_especie.Checked THEN
	ls_especie = '%%'
ELSE
	ls_especie = Trim(sle_especie.Text)
END IF

ld_tipo_cambio = of_tipo_cambio(ld_fecha_fin)

ls_cod_precio = Trim(sle_cod_precio.text)

IF rb_2.Checked = True Then
	idw_1.Retrieve(is_cod_origen, ls_especie, ld_fecha_ini, ld_fecha_fin, ld_tipo_cambio)
	idw_1.object.tipo_cambio.text		= string(ld_tipo_cambio)
	idw_1.object.tc_t.text				= 'Tipo de Cambio: '+String(ld_fecha_fin,'dd/mm/yyyy')
Elseif rb_1.Checked = True Then
	idw_1.Retrieve(ld_fecha_ini, ld_fecha_fin, ls_especie, is_cod_origen )	
	dw_report.Object.DataWindow.Print.Orientation = 1
	dw_report.Object.dw_1.x 		= 41
	dw_report.Object.dw_1.width 	= 4841
ELSEIF rb_3.Checked = True Then
	idw_1.Retrieve(ls_cod_precio, ls_especie, is_cod_origen, ld_fecha_ini, ld_fecha_fin)	
	dw_report.Object.DataWindow.Print.Orientation = 1
	dw_report.Object.dw_1.x 		= 41
	dw_report.Object.dw_1.width 	= 4841
	
End IF

ls_rango_fecha = 'Del ' + string(ld_fecha_ini) + ' Al ' + string (ld_fecha_fin)

select nombre, ruc
	into :ls_razsoc, :ls_ruc
from empresa
where cod_empresa = :gs_empresa;

select dir_calle, dir_lote, dir_mnz
	into :ls_direccion, :ls_lote, :ls_mnz
from origen
where cod_origen = :gs_origen;

if ls_direccion = '' or IsNull(ls_direccion) then
	ls_direccion = 'NO DISPONIBLE'
else
	ls_direccion = trim(ls_direccion)
end if

if ls_mnz <> '' and not IsNull(ls_mnz) then
	ls_direccion = ls_direccion + ' MZ ' + ls_mnz
end if

if ls_lote <> '' and not IsNull(ls_lote) then
	ls_direccion = ls_direccion + ' LOTE ' + ls_lote
end if

idw_1.object.usuario_t.text 		= gs_user
idw_1.object.p_logo.filename 		= gs_logo
idw_1.object.rango_1.text			= ls_rango_fecha

// para printpreview

idw_1.Visible = True
idw_1.SetRedraw(true)

ib_preview = False			// Para Print Preview del DW
This.Event ue_preview()
end event

event ue_saveas;//OVERRIDING
	
date ld_fecha_ini, ld_fecha_fin
string 		ls_separador, ls_origen, ls_file, ls_path, ls_especie
long			ll_i, ll_rc
u_ds_base	lds_datos

IF rb_1.checked THEN

	IF sle_especie.Text = '' and Not cbx_especie.Checked THEN
		messagebox('Aprovisionamiento', 'Por favor seleccione una Especie')
		return
	END IF
	
	// leer el dw_origen con los origenes seleccionados
	
	For ll_i = 1 To dw_origen.RowCount()
		If dw_origen.Object.Chec[ll_i] = '1' Then
			if ls_origen <>'' THEN ls_separador = ', '
			ls_origen = ls_origen + ls_separador + dw_origen.Object.cod_origen[ll_i]
		end if
	Next
	
	IF LEN(ls_origen) = 0 THEN
		messagebox('Error', 'Debe seleccionar al menos un origen para el Reporte')
		return
	END IF
	
	ld_fecha_ini = date(uo_fecha.of_get_fecha1( ))
	ld_fecha_fin = date(uo_fecha.of_get_fecha2( ))

	IF cbx_especie.Checked THEN
		ls_especie = '%%'
	ELSE
		ls_especie = Trim(sle_especie.Text)
	END IF

	lds_datos = create u_ds_base
	lds_datos.Dataobject = 'd_ap_pd_compra_matprim_s_ct'
	lds_datos.SetTransObject(SQLCA)
	lds_datos.Retrieve(ld_fecha_ini, ld_fecha_fin, ls_especie, is_cod_origen )	
	
	IF lds_datos.RowCount() = 0 THEN
		MessageBox('Error', 'No hay datos para exportar')
		destroy lds_datos
		return
	END IF
	
	ll_rc = GetFileSaveName ( "Grabar Archivo", ls_path, ls_file, 'XLS', &
	    'XLS Files (*.XLS), *.XLS', "C:\",  32770)
		 
	IF ll_rc <> 1 Then
		return
	End If
	
	ll_rc = lds_datos.SaveAs(ls_path, Excel!, TRUE)
	
	if ll_rc = -1 then
		MessageBox('Error', 'Ha ocurrido un error al exportar los datos a excel')
		destroy lds_datos
		return
	end if
	destroy lds_datos
ELSE
	idw_1.EVENT ue_saveas()
END IF
end event

type st_desc_precio from statictext within w_ap712_plan_com_mp
integer x = 3045
integer y = 176
integer width = 777
integer height = 80
integer textsize = -7
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long backcolor = 134217739
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type sle_cod_precio from singlelineedit within w_ap712_plan_com_mp
event dobleclick pbm_lbuttondblclk
integer x = 2830
integer y = 176
integer width = 206
integer height = 76
integer taborder = 40
boolean bringtotop = true
integer textsize = -8
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
string pointer = "H:\Source\CUR\taladro.cur"
long textcolor = 33554432
boolean enabled = false
textcase textcase = upper!
integer limit = 8
borderstyle borderstyle = stylelowered!
end type

event dobleclick;boolean lb_ret
string ls_codigo, ls_data, ls_sql



ls_sql = "SELECT DISTINCT COD_PRECIO AS COD_PRECIO, " &
		 + "DESC_PRECIO AS DESCRIPCION_PRECIO " &
		 + "FROM ap_precios_rep " &
  	    + "WHERE FLAG_ESTADO = '1'"
				  
lb_ret = f_lista(ls_sql, ls_codigo, ls_data, '1')
	
IF ls_codigo <> '' THEN
	This.text 				= ls_codigo
	st_desc_precio.text  = ls_data
END IF

end event

event modified;String 	ls_precio, ls_desc

ls_precio = sle_cod_precio.text
if ls_precio = '' or IsNull(ls_precio) then
	MessageBox('Aviso', 'Debe Ingresar un codigo de precio')
	return
end if

SELECT desc_precio
	INTO :ls_desc
FROM ap_precios_rep
WHERE cod_precio = :ls_precio;

IF SQLCA.SQLCode = 100 THEN
	Messagebox('Aviso', 'Codigo de Precio no existe')
	This.text			  = ''
	st_desc_precio.text = ''
	return
end if

st_desc_precio.text = ls_desc

end event

type st_descripcion from statictext within w_ap712_plan_com_mp
integer x = 951
integer y = 140
integer width = 818
integer height = 88
integer textsize = -8
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long backcolor = 134217739
boolean border = true
borderstyle borderstyle = stylelowered!
boolean focusrectangle = false
end type

type pb_1 from picturebutton within w_ap712_plan_com_mp
integer x = 3858
integer y = 140
integer width = 306
integer height = 148
integer taborder = 50
integer textsize = -9
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
string picturename = "H:\source\BMP\procesar_enb.bmp"
alignment htextalign = left!
end type

event clicked;parent.event ue_retrieve()
end event

type rb_3 from radiobutton within w_ap712_plan_com_mp
integer x = 3607
integer y = 52
integer width = 416
integer height = 76
integer textsize = -9
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long backcolor = 67108864
string text = "P.Represent."
borderstyle borderstyle = styleraised!
end type

event clicked;// Para planilla de precio de Representacion
// asociar al dw respectivo
dw_report.DataObject = 'd_ap_precios_representacion_cpst'
idw_1.SetTransObject(sqlca)

sle_cod_precio.enabled = True
sle_cod_precio.setfocus()
end event

type rb_2 from radiobutton within w_ap712_plan_com_mp
integer x = 2839
integer y = 52
integer width = 361
integer height = 76
integer textsize = -9
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long backcolor = 67108864
string text = "P.Castigo"
boolean checked = true
borderstyle borderstyle = styleraised!
end type

event clicked;// asociar al dw respectivo
dw_report.DataObject = 'd_ap_pd_compra_matprim_tbl'
idw_1.SetTransObject(sqlca)

//Bloquear el control para ingresar el codigo de precio
sle_cod_precio.text = ''
st_desc_precio.text = ''
sle_cod_precio.enabled = False





end event

type rb_1 from radiobutton within w_ap712_plan_com_mp
integer x = 3223
integer y = 52
integer width = 370
integer height = 76
integer textsize = -9
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long backcolor = 67108864
string text = "P.Unitario"
borderstyle borderstyle = styleraised!
end type

event clicked;// Para planilla de precio unitario

// asociar al dw respectivo
dw_report.DataObject = 'd_ap_pd_compra_matprim_sd_cpst'
idw_1.SetTransObject(sqlca)

//Bloquear el control para ingresar el codigo de precio
sle_cod_precio.text = ''
st_desc_precio.text = ''
sle_cod_precio.enabled = False

end event

type dw_origen from u_dw_abc within w_ap712_plan_com_mp
integer x = 1806
integer y = 12
integer width = 1001
integer height = 260
integer taborder = 40
string dataobject = "d_ap_origen_liq_pesca_tbl"
boolean vscrollbar = true
end type

event constructor;call super::constructor;is_dwform = 'form'	// tabular, form (default)

ii_ck[1] = 1				// columnas de lectrua de este dw
end event

type cbx_especie from checkbox within w_ap712_plan_com_mp
integer x = 649
integer y = 68
integer width = 238
integer height = 80
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long backcolor = 67108864
string text = "Todas"
boolean checked = true
end type

event clicked;IF THIS.CHECKED THEN
	sle_especie.Enabled 	= False
	sle_especie.Text 		= ''
	st_descripcion.Text = ''
ELSE
	sle_especie.Enabled = True
END IF
end event

type uo_fecha from u_ingreso_rango_fechas_v within w_ap712_plan_com_mp
event destroy ( )
integer x = 5
integer y = 36
integer width = 594
integer taborder = 10
end type

on uo_fecha.destroy
call u_ingreso_rango_fechas_v::destroy
end on

event constructor;call super::constructor;of_set_label('Desde:','Hasta:') // para seatear el titulo del boton
of_set_fecha(today(), today()) //para setear la fecha inicial
of_set_rango_inicio(date('01/01/1900')) // rango inicial
of_set_rango_fin(date('31/12/9999')) // rango final
// of_get_fecha1(), of_get_fecha2()  para leer las fechas

end event

type sle_especie from singlelineedit within w_ap712_plan_com_mp
event dobleclick pbm_lbuttondblclk
integer x = 649
integer y = 140
integer width = 293
integer height = 88
integer taborder = 30
boolean bringtotop = true
integer textsize = -8
integer weight = 400
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
string pointer = "H:\Source\CUR\taladro.cur"
long textcolor = 33554432
boolean enabled = false
textcase textcase = upper!
integer limit = 8
borderstyle borderstyle = stylelowered!
end type

event dobleclick;boolean lb_ret
string ls_codigo, ls_data, ls_sql

ls_sql = "SELECT ESPECIE AS COD_ESPECIE, " &
		  + "DESCR_ESPECIE AS DESCRIPCION_ESPECIE " &
		  + "FROM TG_ESPECIES " &
		  + "WHERE FLAG_ESTADO = '1'"
				  
lb_ret = f_lista(ls_sql, ls_codigo, ls_data, '1')
	
if ls_codigo <> '' then
	this.text 				= ls_codigo
	st_descripcion.text  = ls_data
end if

end event

event modified;String 	ls_especie, ls_desc

ls_especie = sle_especie.text
if ls_especie = '' or IsNull(ls_especie) then
	MessageBox('Aviso', 'Debe Ingresar un codigo de especie')
	return
end if

SELECT descr_especie
	INTO :ls_desc
FROM tg_especies
WHERE especie = :ls_especie;

IF SQLCA.SQLCode = 100 THEN
	Messagebox('Aviso', 'Codigo de Especie no existe')
	This.text			  = ''
	st_descripcion.text = ''
	return
end if

st_descripcion.text = ls_desc

end event

type dw_report from u_dw_rpt within w_ap712_plan_com_mp
integer x = 18
integer y = 424
integer width = 3867
integer height = 1148
integer taborder = 20
string dataobject = "d_ap_pd_compra_matprim_tbl"
boolean hscrollbar = true
boolean vscrollbar = true
end type

type gb_1 from groupbox within w_ap712_plan_com_mp
integer x = 622
integer y = 16
integer width = 1166
integer height = 228
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long backcolor = 67108864
string text = "Especie:"
end type

