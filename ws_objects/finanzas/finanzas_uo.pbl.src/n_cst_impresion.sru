...
public function boolean of_cheque_voucher_bcp (string as_dw, string as_origen, long al_nro, string as_empresa, string as_flag_ts, string as_print, string as_tipo_imp)
public function integer of_imprimir_ts (string as_dir, string as_archivo)
public function boolean of_chq_voucher_bcp_dol (string as_dw, string as_origen, long al_nro, string as_empresa, string as_flag_ts, string as_print, string as_tipo_imp)
end prototypes
...
if as_flag_ts = '0' then
	...
	this.of_imprimir_ts(ls_dir_ts, ls_file)
end if
...
// Edg Begin 04Dic2007
do 
	ls_file = mid(gs_user + string( rand(32767) ), 1, 8) 
	ls_file = ls_dir_ts + ls_file + '.PRN'
loop while FileExists(ls_File)
// Edg End   04Dic2007
...
if isnull( as_archivo ) or as_archivo = "" then
	MessageBox('Error', 'No hay archivo especifico')
	return 0
end if

integer li_FileNum
String ls_text
li_FileNum = FileOpen("i:\pb_exe\PRINT.BAT", &
   LineMode!, Write!, LockWrite!, Replace!)
	ls_text = "COPY /B " + as_archivo + " \\127.0.0.1\EPSON"
FileWrite(li_FileNum,ls_text)
//FileSeek(li_FileNum,2,FromBeginning!)
//FileWrite(li_FileNum,"Del "+as_archivo)
FileClose(li_FileNum)
ls_run = "i:\pb_exe\PRINT.BAT" 
//ls_run = "COPY /B " + as_archivo + " \\127.0.0.1\EPSON"
...
public function boolean of_cheque_voucher_bcp (string as_dw, string as_origen, long al_nro, string as_empresa, string as_flag_ts, string as_print, string as_tipo_imp);//Variables cabecera del voucher
Date				ld_fecha_emision
string			ls_string , ls_run, ls_dir_ts, ls_file, ls_cadena, ls_mensaje 

Long				ll_long, ll_i
Decimal			ldc_decimal
integer 			li_FileNum,li_linea_max,li_count_ch,li_count_blc,&
					li_pos
u_ds_base	 	lds_data
...
if as_flag_ts = '1' then
	//ls_dir_ts = '\\tsclient\c\'
	ls_dir_ts = 'i:\pb_exe\FilePRN\' + gs_user + '\'
	
	if not DirectoryExists(ls_dir_ts) then
		CreateDirectory(ls_dir_ts)
	end if
else
	ls_dir_ts = 'c:\'
end if
...
If Not DirectoryExists ( ls_dir_ts ) Then
	MessageBox ("File Mgr", "Directory " + ls_dir_ts + " no existe, por favor verifique" )
	destroy lds_data
	return false
End If
...
// Activo el nivel de paginas (Cheque Voucher)
// Activo a 12 CPI
// eL VOUCHER ES DE 69 LINEAS
ls_cadena = is_InitialitePrinter + this.of_setpagelines(69) &
			 + is_SetMode12CPI + is_CancelCondensado 
...
// Primera Linea = Empresa + Fecha Emision
ls_cadena = ls_cadena + fill(" ", 17) + left(as_empresa,50) + Fill(" ", 58 - len(as_empresa))
ld_fecha_emision = Date(lds_data.object.fecha_emision[1])
if Not IsNUll(ld_fecha_emision ) then 
	ls_cadena += string(ld_fecha_emision, 'dd/mm/yyyy')
end if

FileWrite(li_FileNum, ls_cadena  + is_CancelCondensado)  
//
//FileWrite(li_FileNum, '')  
...
//
if IsNull(ls_string) then ls_string = fill(" ", 49)
ls_cadena = fill(" ", 11) + ls_string + fill(" ", 60 - len(ls_string))
...
// Tercera Linea = Obs + Tasa Cambio + Total
ls_string = left(lds_data.object.caja_bancos_obs [1], 42)
ls_cadena = fill(" ", 8) + ls_string + fill(" ",42 - len(ls_string)) + is_SetNegrita + "T.CAMBIO    TOTAL" + is_CancelNegrita
FileWrite(li_FileNum, ls_cadena )  

// Tercera Linea = Tipo Cambio + Total
ls_string = fill(" ", 50) + string(lds_data.object.tasa_cambio[1], "###,##0.000") &
			 + fill(" ", 4) + lds_data.object.cod_moneda[1] &
			 + string(lds_data.object.imp_total[1], "###,##0.00")
...
for ll_i = 1 to li_linea_max 
	...
	ls_string = left(lds_data.object.cencos[ll_i],7)
	if ISNull(ls_string) then ls_string = fill(' ', 7)
	ls_cadena = ls_string + fill(' ', 14 - len(ls_string))

	//Cuenta Contable
	ls_string = lds_data.object.cnta_ctbl [ll_i]
	if ISNull(ls_string) then ls_string = fill(" ", 10)
	ls_cadena = ls_cadena + ls_string + fill(' ', 15 - len(ls_string))
	...
	ls_string = left(lds_data.object.nro_docref1[ll_i],15)
	if ISNull(ls_string) then ls_string = fill(" ", 15)
	ls_cadena = ls_cadena + ls_string + fill(' ', 25 - len(ls_string))
...
ls_string = f_llena_caracteres(' ', string(lds_data.object.total_soles_cargo[1], '###,###0.00'), 10)
if ISNull(ls_string) then ls_string = fill(" ", 17)
ls_cadena = ls_cadena + ls_string + fill(' ', 24 - len(ls_string))
...
//Primero el lugar
ls_cadena = lds_data.object.nom_origen[1]
if IsNull(ls_cadena) then ls_cadena = ""
ls_cadena += "   ";

ls_cadena = Fill(" ", 63 - len(ls_cadena)) + ls_cadena
// Luego le junto la fecha
ls_string = string(lds_data.object.dia[1] ) + " " &
			 + string(lds_data.object.mes_c[1]) + "  " &
			 + string(lds_data.object.ano_c[1]) 
...
ls_cadena = fill( " ", 32) + is_SetCondensado + ls_string + this.is_CancelCondensado
FileWrite(li_FileNum, ls_cadena + is_lineFeed )

// Total de Letras
ls_string = string(lds_data.object.total_letras[1])
ls_cadena = fill( " ", 25) + ls_string 

FileWrite(li_FileNum, ls_cadena + fill(is_LineFeed, 2))

// NO Transferible
ls_string = 'NO NEGOCIABLE'
ls_cadena = fill( " ", 25) + is_SetNegrita + is_SetDoubleHeightMode &
			 + ls_string + is_CancelDoubleHeightMode &
			 + is_CancelNegrita
...
public function integer of_imprimir_ts (string as_dir, string as_archivo);string ls_run
...
ls_run = "i:\pb_exe\PRINTTS.BAT" + " " + as_archivo
...
public function boolean of_chq_voucher_bcp_dol (string as_dw, string as_origen, long al_nro, string as_empresa, string as_flag_ts, string as_print, string as_tipo_imp);if gs_empresa = 'CEPIBO'  then
	Datastore ids_print
	ids_print = Create Datastore
	ids_print.DataObject = 'd_chqvoucher_cepibo'
	ids_print.SettransObject(sqlca)
	ids_print.Retrieve(al_nro)
	ids_print.Object.DataWindow.Print.Paper.Size = 256 
	ids_print.Object.DataWindow.Print.CustomPage.Width = 229
	ids_print.Object.DataWindow.Print.CustomPage.Length = 72
	ids_print.object.fecha_2.text = string(day(gd_fecimp),"00") + ' / ' + String(month(gd_fecimp),"00") + ' / ' + string(year(gd_fecimp))
	ids_print.print()
	return true
else 
		...
		//Variables cabecera del voucher
		Date				ld_fecha_emision
		string			ls_string , ls_run, ls_dir_ts, ls_file, ls_cadena, ls_mensaje 
		...
		if lds_data.RowCount() > li_count_blc then
			MessageBox('Error', 'No se permite un Cheque Voucher con mas de 20 líneas')
			return false
		end if
		...
		// Activo a 12 CPI
		// eL VOUCHER ES DE 69 LINEAS
		ls_cadena = is_InitialitePrinter + this.of_setpagelines(46) &
					 + is_SetMode12CPI + is_CancelCondensado 
		...
		FileWrite(li_FileNum, ls_cadena + is_LineFeed + is_LineFeed)  
		...
		ls_cadena = Fill(" ", 65 - len(ls_cadena)) + ls_cadena
		// Luego le junto la fecha
		ls_string = string(lds_data.object.dia[1] ) + " " &
					 + string(lds_data.object.mes_c[1]) + " " &
					 + string(lds_data.object.ano_c[1]) 
		...
		if ISNull(ls_string) then ls_string = fill(" ", 12)
		//12
		ls_cadena = ls_cadena + ls_string + fill(' ', 14 - len(ls_string))
		//23
		...
		ls_cadena = fill( " ", 34) + is_SetCondensado + ls_string + this.is_CancelCondensado
		FileWrite(li_FileNum, ls_cadena + is_lineFeed )
		
		// Total de Letras
		ls_string = string(lds_data.object.total_letras[1])
		ls_cadena = fill( " ", 28) + ls_string 
		
		FileWrite(li_FileNum, ls_cadena + fill(is_LineFeed, 2))
		
		// NO Transferible
		ls_string = 'NO NEGOCIABLE'
		ls_cadena = fill( " ", 30) + is_SetNegrita + is_SetDoubleHeightMode &
					 + ls_string + is_CancelDoubleHeightMode &
					 + is_CancelNegrita
