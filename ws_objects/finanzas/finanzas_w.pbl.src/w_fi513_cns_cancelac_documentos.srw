$PBExportHeader$w_fi513_cns_cancelac_documentos.srw
forward
global type w_fi513_cns_cancelac_documentos from w_cns
end type
type st_1 from statictext within w_fi513_cns_cancelac_documentos
end type
type cbx_doc from checkbox within w_fi513_cns_cancelac_documentos
end type
type cbx_prov from checkbox within w_fi513_cns_cancelac_documentos
end type
type cb_2 from commandbutton within w_fi513_cns_cancelac_documentos
end type
type cb_1 from commandbutton within w_fi513_cns_cancelac_documentos
end type
type dw_1 from datawindow within w_fi513_cns_cancelac_documentos
end type
type gb_1 from groupbox within w_fi513_cns_cancelac_documentos
end type
type dw_cns from u_dw_cns within w_fi513_cns_cancelac_documentos
end type
end forward
...
global w_fi513_cns_cancelac_documentos w_fi513_cns_cancelac_documentos

on w_fi513_cns_cancelac_documentos.create
int iCurrent
call super::create
...
this.Control[iCurrent+2]=this.cbx_doc
this.Control[iCurrent+3]=this.cbx_prov
...
on w_fi513_cns_cancelac_documentos.destroy
call super::destroy
...
long backcolor = 67108864
string text = "Por defecto el documento factura por pagar (FAP) es el seleccionado"
...
event clicked;String ls_tip_rep, ls_proveedor, ls_tipo_doc
...
// Selecciona tipos de documentos y proveedores
IF cbx_doc.checked THEN
	ls_tipo_doc = '%%'
ELSE
	SELECT doc_fact_pagar INTO :ls_tipo_doc FROM finparam WHERE reckey='1' ;
END IF

IF cbx_prov.checked THEN 
	INSERT INTO tt_fin_proveedor 
	SELECT distinct(cp.cod_relacion) FROM cntas_pagar cp 
	 WHERE cp.tipo_doc like :ls_tipo_doc and 
      	 trunc(cp.fecha_emision) BETWEEN :ld_fec_inicial and :ld_fec_final ;
END IF 
