$PBExportHeader$w_fi358_cntas_pagar_sin_control.srw
forward
global type w_fi358_cntas_pagar_sin_control from w_abc
end type
type sle_matriz from singlelineedit within w_fi358_cntas_pagar_sin_control
end type
type sle_confin from singlelineedit within w_fi358_cntas_pagar_sin_control
end type
type rb_ff from radiobutton within w_fi358_cntas_pagar_sin_control
end type
type rb_og from radiobutton within w_fi358_cntas_pagar_sin_control
end type
type sle_2 from singlelineedit within w_fi358_cntas_pagar_sin_control
end type
type st_3 from statictext within w_fi358_cntas_pagar_sin_control
end type
type sle_1 from singlelineedit within w_fi358_cntas_pagar_sin_control
end type
type pb_1 from picturebutton within w_fi358_cntas_pagar_sin_control
end type
type st_2 from statictext within w_fi358_cntas_pagar_sin_control
end type
type st_1 from statictext within w_fi358_cntas_pagar_sin_control
end type
type pb_os from picturebutton within w_fi358_cntas_pagar_sin_control
end type
type pb_oc from picturebutton within w_fi358_cntas_pagar_sin_control
end type
type dw_master from u_dw_abc within w_fi358_cntas_pagar_sin_control
end type
type tab_1 from tab within w_fi358_cntas_pagar_sin_control
end type
...
type gb_1 from groupbox within w_fi358_cntas_pagar_sin_control
end type
type gb_2 from groupbox within w_fi358_cntas_pagar_sin_control
end type
type gb_3 from groupbox within w_fi358_cntas_pagar_sin_control
end type
end forward
...
integer width = 3799
integer height = 2468
string title = "Proveedores (FI358)"
...
global w_fi358_cntas_pagar_sin_control w_fi358_cntas_pagar_sin_control

type variables
String  			 is_accion ,is_orden_compra,is_orden_serv,is_tip_cred //Variable de Estado de Transación del Documento
...
public function boolean wf_verifica_data_oc (ref string as_cod_relacion, ref decimal adc_tasa_cambio)
...
public function boolean wf_verifica_flag_cntas (string as_cta_cntbl, string as_tipo_doc, string as_nro_doc, string as_cod_relacion, long al_row);String  ls_flag_ctabco,ls_flag_cencos,ls_flag_doc_ref,ls_flag_cod_rel,ls_tip_ref,ls_nro_ref,&
		  ls_flag_tip_ref,ls_flag_cbenef	
Boolean lb_retorno = TRUE
...
lb_retorno  = f_generacion_ctas_cxc_cxp (dw_master,tab_1.tabpage_1.dw_ctas_pag_det,tab_1.tabpage_2.dw_ref_x_pagar,tab_1.tabpage_3.dw_imp_x_pagar,tab_1.tabpage_5.dw_pre_asiento_det,ids_matriz_cntbl_det,ids_data_glosa,ls_moneda,ldc_tasa_cambio,ls_campo_formula,tab_1,'P',ls_cencos,ls_cbenef)
...
on w_fi358_cntas_pagar_sin_control.create
int iCurrent
call super::create
...
this.Control[iCurrent+1]=this.sle_matriz
this.Control[iCurrent+2]=this.sle_confin
this.Control[iCurrent+3]=this.rb_ff
this.Control[iCurrent+4]=this.rb_og
...
this.Control[iCurrent+11]=this.pb_os
this.Control[iCurrent+12]=this.pb_oc
...
on w_fi358_cntas_pagar_sin_control.destroy
call super::destroy
...
tab_1.tabpage_1.dw_ctas_pag_det.width     = newwidth   - tab_1.tabpage_1.dw_ctas_pag_det.x - 75
tab_1.tabpage_1.dw_ctas_pag_det.height    = newheight  - tab_1.tabpage_1.dw_ctas_pag_det.y - 1080
tab_1.tabpage_2.dw_ref_x_pagar.height     = newheight  - tab_1.tabpage_2.dw_ref_x_pagar.y  - 1080
tab_1.tabpage_3.dw_imp_x_pagar.height     =  newheight - tab_1.tabpage_3.dw_imp_x_pagar.y  - 1080
tab_1.tabpage_4.dw_totales.width     		= newwidth   - tab_1.tabpage_4.dw_totales.x - 75
tab_1.tabpage_4.dw_totales.height         = newheight  - tab_1.tabpage_4.dw_totales.y - 1080
tab_1.tabpage_5.dw_pre_asiento_det.width  = newwidth   - tab_1.tabpage_5.dw_pre_asiento_det.x - 75
tab_1.tabpage_5.dw_pre_asiento_det.height = newheight  - tab_1.tabpage_5.dw_pre_asiento_det.y - 1080
tab_1.tabpage_6.dw_asiento_aux.height     = newheight  - tab_1.tabpage_6.dw_asiento_aux.y     - 1080
end event
...
Decimal {2} ldc_totsoldeb   = 0.00,ldc_totdoldeb    = 0.00,ldc_totsolhab     = 0.00,ldc_totdolhab     = 0.00,ldc_importe_imp = 0.00,&
				...
				ldc_porc_detrac		 ,ldc_saldo_sol          ,ldc_saldo_dol
...
/****/
SELECT cod_soles,cod_dolares INTO :ls_cod_soles,:ls_cod_dolares FROM logparam WHERE reckey = '1' ;
/****/
...
/*Datos de Documentos*/
select flag_separ_serie into :ls_flag_serie from doc_tipo where tipo_doc = :ls_tipo_doc ;
...
SELECT porc_ret_igv,flag_retencion,imp_min_ret_igv INTO :ldc_porc_ret_igv,:ls_flag_retencion,:ldc_imp_min_igv FROM finparam           WHERE reckey = '1' ;
...
IF ls_flag_retencion = '1' OR ls_flag_detraccion = '0' THEN /*RETIENE Y NO TIENE DETRACCION*/
	...
	IF ldc_monto_fac > ldc_imp_min_igv THEN
		IF tab_1.tabpage_3.dw_imp_x_pagar.Rowcount() > 0 THEN
			...
			SELECT Count(*) INTO :ll_count FROM doc_grupo_relacion WHERE grupo = '07' AND tipo_doc = :ls_tipo_doc ;
			IF ll_count > 0 THEN 
...
lb_retorno  = invo_cntbl_asiento.of_validar_asiento(tab_1.tabpage_5.dw_pre_asiento_det)
...
event modified;String ls_data,ls_matriz
Long   ll_count
...
if ll_count > 0 then
	select matriz_cntbl into :ls_matriz from concepto_financiero where confin = :ls_data ;
	
	sle_matriz.Text =ls_matriz
else
	SetNull(ls_matriz)
	sle_matriz.Text =ls_matriz
end if
...
fontfamily fontfamily = swiss!
string facename = "Arial"
string picturename = "H:\Source\Gif\arrowl.gif"
...
event clicked;String  		ls_tipo_doc_oc, ls_tipo_ref,ls_cod_cliente,ls_cod_moneda,ls_result,ls_mensaje,&
				ls_flag_os
Decimal {3} ldc_tasa_cambio
Decimal {2} ldc_total
Long        ll_row,ll_ano,ll_mes
...
IF wf_verifica_data_oc (ls_cod_cliente,ldc_tasa_cambio) = FALSE THEN Return
...
sl_param.tipo		 = '11'
sl_param.opcion	 = 10         //Ordenes de Compra
sl_param.titulo 	 = 'Selección de Ordenes de Compra'
sl_param.dw_master = 'd_abc_lista_oc_pendientes_no_control_tbl'
sl_param.dw1		 = 'd_abc_art_mov_oc_pend_no_control_tbl'
sl_param.dw_m		 = dw_master
sl_param.dw_d		 = tab_1.tabpage_1.dw_ctas_pag_det
sl_param.dw_c		 = tab_1.tabpage_2.dw_ref_x_pagar
sl_param.string1	 = ls_cod_cliente
...
event itemchanged;call super::itemchanged;String      ls_nom_proveedor  , ls_forma_pago , ls_mes,ls_flag_detraccion,&
				ls_nro_detraccion , ls_desc_fpago , ls_null
...
This.object.flag_cntr_almacen[al_row] = '0'
		 ...
		 CASE	'forma_pago'	
				...
				IF lstr_seleccionar.s_action = "aceptar" THEN
					...
					IF il_dias_vencimiento > 0 THEN
						ld_fecha_vencimiento = RelativeDate ( date(this.object.fecha_emision [row]) , il_dias_vencimiento )
						This.Object.vencimiento [row] = ld_fecha_vencimiento
					END IF
...
event itemchanged;call super::itemchanged;String  ls_matriz_cntbl, ls_item   , ls_cod_art , ls_tipo_doc , ls_nro_doc , ls_nro_oc ,&
		  ls_flag_cant   , ls_cencos , ls_cnta_prsp
Long    ll_count
