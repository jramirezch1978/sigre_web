$PBExportHeader$w_fi303_letras_x_cobrar_renovacion.srw
forward
global type w_fi303_letras_x_cobrar_renovacion from w_abc
end type
type cb_1 from commandbutton within w_fi303_letras_x_cobrar_renovacion
end type
type tab_1 from tab within w_fi303_letras_x_cobrar_renovacion
end type
...
type dw_master from u_dw_abc within w_fi303_letras_x_cobrar_renovacion
end type
end forward
...
integer width = 3643
integer height = 2156
string title = "Renovación de Letras x Cobrar (FI303)"
...
global w_fi303_letras_x_cobrar_renovacion w_fi303_letras_x_cobrar_renovacion
...
public function boolean wf_genera_letra (ref string as_mensaje, ref long al_nro_renovacion, ref string as_nro_letra)
...
public function boolean wf_verifica_flag_cntas (string as_cta_cntbl, string as_tipo_doc, string as_nro_doc, string as_cod_relacion, long al_row);String  ls_flag_ctabco,ls_flag_cencos,ls_flag_doc_ref,ls_flag_cod_rel,&
		  ls_tipo_doc_ref,ls_nro_doc_ref,ls_cod_relacion,ls_flag_cbenef
...
public function boolean wf_genera_letra (ref string as_mensaje, ref long al_nro_renovacion, ref string as_nro_letra);Long    ll_found
String  ls_tipo_doc,ls_expresion,ls_cod_relacion,ls_nro_doc_ref
Boolean lb_retorno = TRUE

/*recupero doc.letra x pagar*/
select doc_letra_cobrar into :ls_tipo_doc from finparam where reckey = '1' ;



IF Isnull(ls_tipo_doc) OR Trim(ls_tipo_doc) = '' THEN
	as_mensaje = 'Tipo de Documentos Letras x Cobrar No existe, Verifique!'
	lb_retorno =  FALSE
	GOTO SALIDA
END IF
...
ls_expresion = 'tipo_ref = '+"'"+ls_tipo_doc+"'"
ll_found     = tab_1.tabpage_1.dw_detail.Find(ls_expresion,1,tab_1.tabpage_1.dw_detail.Rowcount())

IF ll_found > 0 THEN 
	ls_cod_relacion = tab_1.tabpage_1.dw_detail.object.cod_relacion [ll_found]
	ls_nro_doc_ref  = tab_1.tabpage_1.dw_detail.object.nro_ref      [ll_found]
	
	  
	/*Recuperar Nro de Renovacion*/	  
	 SELECT Nvl(nro_ren_ltr,0) INTO :al_nro_renovacion
	   FROM cntas_cobrar
	  WHERE ((cod_relacion = :ls_cod_relacion ) AND
	  			(tipo_doc     = :ls_tipo_doc     ) AND
	         (nro_doc      = :ls_nro_doc_ref  )) ;
				
	/*******************************/			
	/*incrementa numerador de letra*/
	/*******************************/
	
	al_nro_renovacion = al_nro_renovacion + 1

	/*Actuliza Letra Original*/
	UPDATE cntas_cobrar  SET nro_renovacion = :al_nro_renovacion
    WHERE ((cod_relacion = :ls_cod_relacion ) AND
	 		  (tipo_doc     = :ls_tipo_doc     ) AND
	        (nro_doc      = :ls_nro_doc_ref  )) ;			  
	
	as_nro_letra = Mid(ls_nro_doc_ref,1,8) + f_llena_caracteres('0',Trim(String(al_nro_renovacion)),2)
...
on w_fi303_letras_x_cobrar_renovacion.create
int iCurrent
call super::create
...
on w_fi303_letras_x_cobrar_renovacion.destroy
call super::destroy
...
tab_1.tabpage_1.dw_detail.height = newheight - tab_1.tabpage_1.dw_detail.y - 1025
tab_1.tabpage_2.dw_asiento_det.width  = newwidth  - tab_1.tabpage_2.dw_asiento_det.x - 75
tab_1.tabpage_2.dw_asiento_det.height = newheight - tab_1.tabpage_2.dw_asiento_det.y - 1025
end event
...
event ue_update_pre;call super::ue_update_pre;Long   ll_row_master    ,ll_ano   ,ll_mes,ll_nro_libro,ll_nro_asiento,ll_count_doc_ref,&
		 ll_count_asientos,ll_inicio,ll_nro_ren
String   ls_doc_lt_cob    ,ls_soles      ,ls_dolares   ,ls_cod_origen,ls_cod_relacion,ls_tipo_doc,ls_nro_doc,&
...
//tabla de parametros.....
select doc_letra_cobrar into :ls_doc_lt_cob from finparam where reckey = '1' ;

f_monedas(ls_soles,ls_dolares)


IF Isnull(ls_doc_lt_cob) OR Trim(ls_doc_lt_cob) = '' THEN
	Messagebox('Aviso','Documento de Parametro de Letra x Cobrar No Existe , Verifique!')
	ib_update_check = False	
	Return
END IF
...
IF is_accion = 'new' THEN
	...
	IF wf_genera_letra (ls_mensaje,ll_nro_ren,ls_nro_doc) = FALSE THEN
		Messagebox('Aviso',ls_mensaje)
		ib_update_check = FALSE
		RETURN
	ELSE
		dw_master.object.nro_doc     [ll_row_master] = ls_nro_doc
		dw_master.object.nro_ren_ltr [ll_row_master] = ll_nro_ren
	END IF
	
	/******/
	ll_nro_libro = f_nro_libro(ls_doc_lt_cob)	 
...
//lb_retorno  = invo_asiento_contable.f_validar_asiento(tab_1.tabpage_2.dw_asiento_det)
...
sl_param.tipo    = '1SQL'
sl_param.string1 =  ' AND ("CNTAS_COBRAR"."FLAG_TIPO_LTR" =  '+"'"+'R'+"'"+')    '&
						 +' AND ("CNTAS_COBRAR"."FLAG_ESTADO"   IN '+'('+"'"+'1'+"',"+"'"+'2'+"',"+"'"+'3'+"')"+')   '&
...
IF sl_param.titulo <> 'n' THEN
	dw_master.Retrieve('R',sl_param.field_ret[1],sl_param.field_ret[2],sl_param.field_ret[3])
	tab_1.tabpage_1.dw_detail.Retrieve(sl_param.field_ret[1],sl_param.field_ret[2],sl_param.field_ret[3],'R')
...
This.Object.flag_tipo_ltr		 [al_row] = 'R' //RENOVACION DE LETRAS
