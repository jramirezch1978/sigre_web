...
string   ls_planta, ls_graphname="gr_1", ls_find, &
...
if len(trim(ls_nave)) > 0 then
	if li_count > 1 then
		OpenSheetWithParm( w_fl907_grafica_ppto_var, ls_nave, w_main )
	else
		...
		OpenSheetWithParm( w_fl908_grafica_ppto_compos, ls_parametro, w_main )
	end if
end if
end event
