...
FETCH usp_fl_revierte_presupuesto INTO :ls_mensaje, :li_ok;
	
CLOSE usp_fl_revierte_presupuesto;

if li_ok <> 1 then
	MessageBox('Error PROCEDURE usp_fl_revierte_presupuesto', ls_mensaje, StopSign!)	
	return false
end if

MessageBox('Aviso','Se ha revertido el presupuesto para el año ' &
...
FETCH usp_fl_genera_presupuesto INTO :ls_mensaje, :li_ok;
...
MessageBox('Aviso','Se ha generado el presupuesto para el año ' &
	+trim(string(li_ano)) + ' satisfactoriamente', Exclamation! )
...
event ue_open_pre;call super::ue_open_pre;em_ano.text = string(year(Today()) + 1)
