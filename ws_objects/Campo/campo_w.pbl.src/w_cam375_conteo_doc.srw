$PBExportHeader$w_cam375_conteo_doc.srw
forward
global type w_cam375_conteo_doc from w_abc_master
end type
type dw_detalle from u_dw_abc within w_cam375_conteo_doc
end type
end forward
...
integer width = 3337
integer height = 3280
string title = "[CAM375] RG-44 Conteo de Documentos Obsoletos"
string menuname = "m_abc_anular_lista"
dw_detalle dw_detalle
end type
global w_cam375_conteo_doc w_cam375_conteo_doc
...
if is_action = 'new' then
	...
	ls_table = 'LOCK TABLE num_sic_conteo_doc IN EXCLUSIVE MODE'
	EXECUTE IMMEDIATE :ls_table ;
	
	Select ult_nro 
		into :ll_ult_nro 
	from num_sic_conteo_doc
	where cod_origen = :gs_origen;
	
	IF SQLCA.SQLCode = 100 then
		Insert into num_sic_conteo_doc (cod_origen, ult_nro)
			values( :gs_origen, 1);
...
on w_cam375_conteo_doc.destroy
call super::destroy
...
sl_param.dw1    = 'd_list_sic_conteo_doc_tbl'
sl_param.titulo = 'Registro de Conteo de Documentos'
...
type dw_master from w_abc_master`dw_master within w_cam375_conteo_doc
integer width = 3150
integer height = 652
string dataobject = "d_abc_sic_conteo_doc_cab_ff"
end type
...
string dataobject = "d_abc_sic_conteo_doc_det_tbl"
