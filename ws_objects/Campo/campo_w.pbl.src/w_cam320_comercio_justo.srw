$PBExportHeader$w_cam320_comercio_justo.srw
forward
global type w_cam320_comercio_justo from w_abc_mastdet_smpl
end type
end forward
...
integer width = 2629
integer height = 2268
string title = "[CM320] Inspección Comercio Justo"
string menuname = "m_abc_anular_lista"
end type
global w_cam320_comercio_justo w_cam320_comercio_justo
...
if is_action = 'new' then
	...
	Select count(*) 
		into :ll_count
	from num_sic_comercio_justo_test 
	where cod_origen = :gs_origen;
	
	IF ll_count = 0 then
		Insert into num_sic_comercio_justo_test (cod_origen, ult_nro)
			values( :gs_origen, 1);
	end if
	...
	// Incrementa contador
	Update num_sic_comercio_justo_test
...
for ll_j = 1 to dw_detail.RowCount()	
	dw_detail.object.nro_test[ll_j] = ls_nro	
next
return 1
end function
...
sl_param.dw1    = 'd_list_sic_comercio_justo_tbl'
sl_param.titulo = 'Inspecciones Comercio Justo'
...
type dw_master from w_abc_mastdet_smpl`dw_master within w_cam320_comercio_justo
integer x = 0
integer y = 0
integer width = 2533
integer height = 896
string dataobject = "d_abc_sic_comercio_justo_cab_ff"
...
this.object.fecha_auditoria		[al_row] = Date(f_fecha_Actual())
...
event dw_master::clicked;call super::clicked;String 		ls_cuadrilla, ls_ot_adm, ls_trabajador, ls_labor, ls_desc_labor, ls_und
Date 			ld_fec_parte
...
if dwo.name = 'b_imp' then
		...
		ds_imp.DataObject = 'd_list_sic_preguntas_cj_tbl'
		...
		for li_i=1 to ds_imp.RowCount()
				...
				if li_row > 0 then
					dw_detail.object.id_req 						[li_row] = ds_imp.object.id_req [li_i]
					dw_detail.object.cod_req 					[li_row] = ds_imp.object.cod_req [li_i]
					dw_detail.object.desc_nivel_pregunta		[li_row] = ds_imp.object.desc_nivel_pregunta [li_i]
					dw_detail.object.referencia 					[li_row] = ds_imp.object.referencia [li_i]
					dw_detail.object.flag_preg 					[li_row] = ds_imp.object.flag_preg [li_i]
				end if
		next
...
type dw_detail from w_abc_mastdet_smpl`dw_detail within w_cam320_comercio_justo
integer x = 0
integer y = 916
integer width = 2542
integer height = 1084
string dataobject = "d_abc_sic_comercio_justo_det_tbl"
end type
