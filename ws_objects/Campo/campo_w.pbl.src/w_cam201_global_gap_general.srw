$PBExportHeader$w_cam201_global_gap_general.srw
forward
global type w_cam201_global_gap_general from w_abc_mastdet_smpl
end type
end forward
...
integer width = 2629
integer height = 2268
string title = "[CM201] Inspección Global GAP General"
string menuname = "m_abc_anular_lista"
end type
global w_cam201_global_gap_general w_cam201_global_gap_general
...
if is_action = 'new' then
	...
	Select count(*) 
		into :ll_count
	from NUM_SIC_GLOBAL_BAP_TEST 
	where cod_origen = :gs_origen;
	
	IF ll_count = 0 then
		Insert into NUM_SIC_GLOBAL_BAP_TEST (cod_origen, ult_nro)
			values( :gs_origen, 1);
	end if
	...
	// Incrementa contador
	Update NUM_SIC_GLOBAL_BAP_TEST
...
sl_param.dw1    = 'd_list_sic_insp_gg_tbl'
sl_param.titulo = 'Inspecciones Global GAP'
...
type dw_master from w_abc_mastdet_smpl`dw_master within w_cam201_global_gap_general
integer x = 0
integer y = 0
integer width = 2533
integer height = 936
string dataobject = "d_abc_sic_insp_gg_cab_ff"
boolean hscrollbar = false
boolean vscrollbar = false
end type

event dw_master::ue_insert_pre;call super::ue_insert_pre;this.object.fec_registro 	[al_row] = f_fecha_Actual()
this.object.fec_realizado 	[al_row] = Date(f_fecha_Actual())
this.object.flag_estado		[al_row] = '1'
this.setcolumn('nro_certificacion')
...
CHOOSE CASE dwo.name
	...
	case "nro_certificacion"
		...
		SELECT a.nro_certificacion, p.nom_proveedor
		...
		if SQLCA.SQlCode = 100 then
			...
			this.object.nro_certificacion		[row] = ls_desc1
...
event dw_master::clicked;call super::clicked;// Asigna valores a structura
...
choose case lower(dwo.name)
	case 'b_imp'
			...
			lstr_param.dw_master 				= "d_list_sic_gg_mod_tbl"
			lstr_param.dw1       					= "d_list_sic_gg_preg_tbl"	
			lstr_param.titulo 						= "Módulos y Preguntas"
...
type dw_detail from w_abc_mastdet_smpl`dw_detail within w_cam201_global_gap_general
integer x = 0
integer y = 956
integer width = 2542
integer height = 1044
string dataobject = "d_abc_sic_global_gap_det_tbl"
end type
