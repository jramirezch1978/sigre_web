$PBExportHeader$w_cam363_limpieza_aplicacion.srw
forward
global type w_cam363_limpieza_aplicacion from w_abc_master
end type
type tab_1 from tab within w_cam363_limpieza_aplicacion
end type
...
type dw_lavado from u_dw_abc within tabpage_4
end type
type tabpage_4 from userobject within tab_1
dw_lavado dw_lavado
end type
...
type dw_preparacion from u_dw_abc within tabpage_5
end type
type tabpage_5 from userobject within tab_1
...
type dw_medicion from u_dw_abc within tabpage_6
end type
type tabpage_6 from userobject within tab_1
dw_medicion dw_medicion
end type
type tab_1 from tab within w_cam363_limpieza_aplicacion
tabpage_1 tabpage_1
tabpage_2 tabpage_2
...
integer width = 3337
integer height = 3280
string title = "[CAM363] RG-20 Limpieza y aplicación insumos empacadora"
string menuname = "m_abc_anular_lista"
tab_1 tab_1
end type
global w_cam363_limpieza_aplicacion w_cam363_limpieza_aplicacion

type variables
u_dw_abc idw_detalle, idw_insumos, idw_registro, idw_lavado, idw_preparacion, idw_medicion
end variables
...
idw_lavado 	= tab_1.tabpage_4.dw_lavado
idw_preparacion 	= tab_1.tabpage_5.dw_preparacion
idw_medicion 	= tab_1.tabpage_6.dw_medicion
end subroutine
...
if is_action = 'new' then
	...
	ls_table = 'LOCK TABLE NUM_SIC_REG_LIMP_INS_EMP IN EXCLUSIVE MODE'
	EXECUTE IMMEDIATE :ls_table ;
	
	Select ult_nro 
		into :ll_ult_nro 
	from NUM_SIC_REG_LIMP_INS_EMP
	where cod_origen = :gs_origen;
	
	IF SQLCA.SQLCode = 100 then
		Insert into NUM_SIC_REG_LIMP_INS_EMP (cod_origen, ult_nro)
			values( :gs_origen, 1);
	...
	// Incrementa contador
	Update NUM_SIC_REG_LIMP_INS_EMP 
...
for ll_j = 1 to idw_insumos.RowCount()	
	idw_insumos.object.nro_registro		[ll_j] = ls_nro	
next
for ll_j = 1 to idw_registro.RowCount()	
	idw_registro.object.nro_registro		[ll_j] = ls_nro	
next
for ll_j = 1 to idw_lavado.RowCount()	
	idw_lavado.object.nro_registro		[ll_j] = ls_nro	
next
for ll_j = 1 to idw_preparacion.RowCount()	
	idw_preparacion.object.nro_registro		[ll_j] = ls_nro	
next
for ll_j = 1 to idw_medicion.RowCount()	
	idw_medicion.object.nro_registro		[ll_j] = ls_nro	
next
...
idw_lavado.width  = tab_1.tabpage_4.width  - tab_1.tabpage_3.x - 10
idw_lavado.height = tab_1.tabpage_4.height  - tab_1.tabpage_3.y - 10

idw_preparacion.width  = tab_1.tabpage_5.width  - tab_1.tabpage_3.x - 10
idw_preparacion.height = tab_1.tabpage_5.height  - tab_1.tabpage_3.y - 10

idw_medicion.width  = tab_1.tabpage_6.width  - tab_1.tabpage_3.x - 10
idw_medicion.height = tab_1.tabpage_6.height  - tab_1.tabpage_3.y - 10
end event

on w_cam363_limpieza_aplicacion.create
int iCurrent
call super::create
...
on w_cam363_limpieza_aplicacion.destroy
call super::destroy
...
idw_lavado.AcceptText()
idw_preparacion.AcceptText()
idw_medicion.AcceptText()
...
IF ib_log THEN
	...
	idw_lavado.of_create_log()
	idw_preparacion.of_create_log()
	idw_medicion.of_create_log()
END IF
...
IF idw_lavado.ii_update = 1 AND lbo_ok = TRUE THEN
	IF idw_lavado.Update(true, false) = -1 then		// Grabacion del detalle
		lbo_ok = FALSE
		ROLLBACK USING SQLCA;
		ls_msg = "Se ha procedido al rollback"
		messagebox("Error en Grabacion registro de idw_lavado", ls_msg, StopSign!)
	END IF
END IF

IF idw_preparacion.ii_update = 1 AND lbo_ok = TRUE THEN
	IF idw_preparacion.Update(true, false) = -1 then		// Grabacion del detalle
		lbo_ok = FALSE
		ROLLBACK USING SQLCA;
		ls_msg = "Se ha procedido al rollback"
		messagebox("Error en Grabacion registro de idw_preparacion", ls_msg, StopSign!)
	END IF
END IF

IF idw_medicion.ii_update = 1 AND lbo_ok = TRUE THEN
	IF idw_medicion.Update(true, false) = -1 then		// Grabacion del detalle
		lbo_ok = FALSE
		ROLLBACK USING SQLCA;
		ls_msg = "Se ha procedido al rollback"
		messagebox("Error en Grabacion registro de idw_medicion", ls_msg, StopSign!)
	END IF
END IF
...
IF ib_log THEN
	...
	END IF
	IF lbo_ok THEN
		lbo_ok = idw_lavado.of_save_log()
	END IF
	IF lbo_ok THEN
		lbo_ok = idw_preparacion.of_save_log()
	END IF
	IF lbo_ok THEN
		lbo_ok = idw_medicion.of_save_log()
	END IF
END IF
...
IF lbo_ok THEN
	...
	idw_insumos.ii_update = 0
	idw_registro.ii_update = 0
	idw_lavado.ii_update = 0
	idw_preparacion.ii_update = 0
	idw_medicion.ii_update = 0
	
	dw_master.il_totdel = 0
	idw_detalle.il_totdel = 0
	idw_insumos.il_totdel = 0
	idw_registro.il_totdel = 0
	idw_lavado.il_totdel = 0
	idw_preparacion.il_totdel = 0
	idw_medicion.il_totdel = 0
		
	dw_master.ResetUpdate()
	idw_detalle.ResetUpdate()
	idw_insumos.ResetUpdate()
	idw_registro.ResetUpdate()
	idw_lavado.ResetUpdate()
	idw_preparacion.ResetUpdate()
	idw_medicion.ResetUpdate()
...
// Verifica que campos son requeridos y tengan valores
if f_row_Processing( idw_lavado, idw_lavado.is_dwform) <> true then	return

// Verifica que campos son requeridos y tengan valores
if f_row_Processing( idw_preparacion, idw_preparacion.is_dwform) <> true then	return

// Verifica que campos son requeridos y tengan valores
if f_row_Processing( idw_medicion, idw_medicion.is_dwform) <> true then	return
...
sl_param.dw1    = 'd_list_sic_reg_limp_ins_emp_tbl'
sl_param.titulo = 'Registro de Limpieza y Aplicación de Insumos'
...
type dw_master from w_abc_master`dw_master within w_cam363_limpieza_aplicacion
integer width = 3150
integer height = 684
string dataobject = "d_abc_sic_limp_ins_emp_cab_ff"
end type
...
this.object.fecha	[al_row] = Date(ldt_fec_oracle)
...
choose case lower(as_columna)
	...
	case "cod_empacadora"
		ls_sql = "SELECT a.COD_EMPACADORA AS COD_EMPACADORA, " &
				 + "a.DESC_EMPACADORA as DESC_EMPACADORA, " &
				 + "a.TIPO_EMPACADORA AS TIPO_EMPACADORA, " &
				 + "a.FLAG_ESTADO AS FLAG_ESTADO " &
...
integer height = 2224
long backcolor = 79741120
string text = "Equipamiento y Limpieza ~r~nantes del Proceso"
...
integer height = 1360
integer taborder = 20
string dataobject = "d_abc_sic_reg_equip_tbl"
...
integer height = 2224
long backcolor = 79741120
string text = "Desinfección de Empacadora"
long tabtextcolor = 33554432
long tabbackcolor = 79741120
long picturemaskcolor = 536870912
dw_insumos dw_insumos
...
integer width = 2162
integer height = 1224
string dataobject = "d_abc_sic_reg_desinf_tbl"
boolean hscrollbar = true
boolean vscrollbar = true
borderstyle borderstyle = styleraised!
end type
...
integer height = 2224
long backcolor = 79741120
string text = "Aplicación de Agroquimicos"
long tabtextcolor = 33554432
long tabbackcolor = 79741120
long picturemaskcolor = 536870912
dw_registro dw_registro
...
integer height = 1108
integer taborder = 30
string dataobject = "d_abc_sic_reg_aplicacion_tbl"
boolean hscrollbar = true
boolean vscrollbar = true
borderstyle borderstyle = styleraised!
end type
...
integer height = 2224
long backcolor = 79741120
string text = "Lavado y Deslatexado~r~nde Closters"
long tabtextcolor = 33554432
long tabbackcolor = 79741120
long picturemaskcolor = 536870912
dw_lavado dw_lavado
...
on tabpage_4.create
this.dw_lavado=create dw_lavado
this.Control[]={this.dw_lavado}
end on

on tabpage_4.destroy
destroy(this.dw_lavado)
end on

type dw_lavado from u_dw_abc within tabpage_4
integer width = 2153
integer height = 1360
string dataobject = "d_abc_sic_reg_lavado_tbl"
boolean hscrollbar = true
boolean vscrollbar = true
borderstyle borderstyle = styleraised!
end type
...
integer height = 2224
long backcolor = 79741120
string text = "Preparación de Solución~r~nDesinfectante Corona"
long tabtextcolor = 33554432
long tabbackcolor = 79741120
long picturemaskcolor = 536870912
dw_preparacion dw_preparacion
...
this.Control[]={this.dw_preparacion}
end on
...
integer width = 2153
integer height = 1360
string dataobject = "d_abc_sic_reg_preparacion_tbl"
boolean hscrollbar = true
boolean vscrollbar = true
borderstyle borderstyle = styleraised!
end type
	...
	case "und", "und2", "und3"
		...
		if ls_codigo <> '' then
			choose case  lower(as_columna)
				...
				case "und3"
					this.object.und3	[al_row] = ls_codigo
			end choose
...
CHOOSE CASE dwo.name
	...
	case "und2"
		...
		if SQLCA.SQlCode = 100 then
			ROLLBACK;
			MessageBox("Error", "No existe Unidad")
			this.object.und2		[row] = ls_null
			return 1
		end if
		
	this.object.und2	[row] = data
	...
	case "und3"
		...
		if SQLCA.SQlCode = 100 then
			ROLLBACK;
			MessageBox("Error", "No existe Unidad")
			this.object.und3		[row] = ls_null
			return 1
		end if
		
	this.object.und3	[row] = data
...
integer height = 2224
long backcolor = 79741120
string text = "Medición de Cloro, PH"
long tabtextcolor = 33554432
long tabbackcolor = 79741120
long picturemaskcolor = 536870912
dw_medicion dw_medicion
...
on tabpage_6.create
this.dw_medicion=create dw_medicion
this.Control[]={this.dw_medicion}
end on

on tabpage_6.destroy
destroy(this.dw_medicion)
end on

type dw_medicion from u_dw_abc within tabpage_6
integer width = 2153
integer height = 1360
string dataobject = "d_abc_sic_reg_medicion_tbl"
boolean hscrollbar = true
boolean vscrollbar = true
borderstyle borderstyle = styleraised!
end type
