$PBExportHeader$w_cam361_pcm.srw
forward
global type w_cam361_pcm from w_abc_master
end type
type dw_detalle from u_dw_abc within w_cam361_pcm
end type
end forward
...
integer width = 3337
integer height = 3280
string title = "[CAM361] RG 22 PCMs"
string menuname = "m_abc_anular_lista"
dw_detalle dw_detalle
end type
global w_cam361_pcm w_cam361_pcm
...
if is_action = 'new' then
	...
	ls_table = 'LOCK TABLE num_sic_reg_pcm IN EXCLUSIVE MODE'
	EXECUTE IMMEDIATE :ls_table ;
	
	Select ult_nro 
		into :ll_ult_nro 
	from num_sic_reg_pcm
	where cod_origen = :gs_origen;
	
	IF SQLCA.SQLCode = 100 then
		Insert into num_sic_reg_pcm (cod_origen, ult_nro)
			values( :gs_origen, 1);
...
sl_param.dw1    = 'd_list_sic_reg_pcm_tbl'
sl_param.titulo = 'Registro de PCMs'
...
type dw_master from w_abc_master`dw_master within w_cam361_pcm
integer width = 3195
integer height = 768
string dataobject = "d_abc_sic_reg_pcm_cab_ff"
end type
...
integer width = 3136
integer height = 1604
string dataobject = "d_abc_sic_reg_pcm_det_tbl"
