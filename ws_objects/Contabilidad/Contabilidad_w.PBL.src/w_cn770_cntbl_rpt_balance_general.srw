...
try
	...
	invo_wait.of_mensaje("Un momento, se esta procesando el reporte")
	...
	DECLARE USP_CNTBL_RPT_BALANCE PROCEDURE FOR 
		USP_CNTBL_RPT_BALANCE( 	:ln_mes_desde, 
										:ln_mes_hasta, 
										:ln_ano, 
										:ls_moneda, 
										:ln_nivel ) ;
	Execute USP_CNTBL_RPT_BALANCE ;
	
	if gnvo_app.of_existserror( SQLCA, 'Procedure USP_CNTBL_RPT_BALANCE' ) then 
		dw_report.Reset()
		return
	end if
	
	Close USP_CNTBL_RPT_BALANCE;
	...
	dw_report.object.t_titulo.text = 'ESTADO DE SITUACIÓN FINANCIERA'
