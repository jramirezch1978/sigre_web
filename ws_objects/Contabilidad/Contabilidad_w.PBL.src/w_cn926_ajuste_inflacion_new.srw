$PBExportHeader$w_cn926_ajuste_inflacion_new.srw
forward
global type w_cn926_ajuste_inflacion_new from w_prc
end type
type st_2 from statictext within w_cn926_ajuste_inflacion_new
end type
type st_1 from statictext within w_cn926_ajuste_inflacion_new
end type
type sle_ano from singlelineedit within w_cn926_ajuste_inflacion_new
end type
type sle_mes from singlelineedit within w_cn926_ajuste_inflacion_new
end type
type cb_reporte from commandbutton within w_cn926_ajuste_inflacion_new
end type
type cb_cancelar from commandbutton within w_cn926_ajuste_inflacion_new
end type
type cb_aceptar from commandbutton within w_cn926_ajuste_inflacion_new
end type
type st_titulo from statictext within w_cn926_ajuste_inflacion_new
end type
end forward
...
global w_cn926_ajuste_inflacion_new w_cn926_ajuste_inflacion_new
...
String		ls_flag_debhab_rei
String		ls_desc_proceso, ls_win, ls_glosa, ls_temp, ls_cnta_rei, ls_cnta, ls_cnta_ajuste
...
Double		ldb_nro_asiento, ldb_nro_proceso
Integer		li_ano, li_mes, li_mes_sig, li_ano_sig, li_mes_ant, li_ano_ant
Integer		li_nro_libro, li_item, li_libro
...
ll_saldo = lds_saldo.Retrieve(li_mes, li_ano)
...
DECLARE pb_ingreso_asiento PROCEDURE FOR USF_INGRESO_ASIENTO(:gs_origen, :li_libro,
...
FOR ll_x = 1 TO ll_saldo
	...
	ll_rc = lds_as_det.SetItem( ll_row, 'nro_libro', li_nro_libro)
...
//determinar  ls_cnta_rei
	...
	ll_rc = lds_as_det.SetItem( ll_row, 'nro_asiento', ldb_nro_asiento +1)
	ll_rc = lds_as_det.SetItem( ll_row, 'item', ll_row)
	ll_rc = lds_as_det.SetItem( ll_row, 'cnta_ctbl', ls_cnta_rei)
	ll_rc = lds_as_det.SetItem( ll_row, 'fecha_cntbl', ld_final)
	ll_rc = lds_as_det.SetItem( ll_row, 'det_glosa', ls_glosa)
	ll_rc = lds_as_det.SetItem( ll_row, 'flag_gen_aut', '2')
	...
	ll_rc = lds_rpt.SetItem( ll_lin, 'cnta_ctbl', ls_cnta_rei)
	ll_rc = lds_rpt.SetItem( ll_lin, 'factor', ldc_indice)
	ll_rc = lds_rpt.SetItem( ll_lin, 'sldo_actualizado', ldc_actualizado)
	ll_rc = lds_rpt.SetItem( ll_lin, 'ajuste', ldc_ajuste)
...
ELSE
   ...
   FETCH pb_ingreso_asiento INTO :ldb_nro_asiento ;
	CLOSE pb_ingreso_asiento ;
	IF ldb_nro_asiento < 1 THEN
...
SELECT NVL(INDICE,0)
  INTO :adc_indice
  FROM INDICE_AJUSTE
 WHERE ANO = :ai_ano AND MES = :ai_mes;
...
on w_cn926_ajuste_inflacion_new.create
int iCurrent
call super::create
...
on w_cn926_ajuste_inflacion_new.destroy
call super::destroy
