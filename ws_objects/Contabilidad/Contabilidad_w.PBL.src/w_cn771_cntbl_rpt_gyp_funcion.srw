$PBExportHeader$w_cn771_cntbl_rpt_gyp_funcion.srw
forward
global type w_cn771_cntbl_rpt_gyp_funcion from w_report_smpl
end type
type sle_ano from singlelineedit within w_cn771_cntbl_rpt_gyp_funcion
end type
type sle_mes_desde from singlelineedit within w_cn771_cntbl_rpt_gyp_funcion
end type
type cb_1 from commandbutton within w_cn771_cntbl_rpt_gyp_funcion
end type
type st_3 from statictext within w_cn771_cntbl_rpt_gyp_funcion
end type
type st_4 from statictext within w_cn771_cntbl_rpt_gyp_funcion
end type
type ddlb_2 from dropdownlistbox within w_cn771_cntbl_rpt_gyp_funcion
end type
type st_2 from statictext within w_cn771_cntbl_rpt_gyp_funcion
end type
type st_7 from statictext within w_cn771_cntbl_rpt_gyp_funcion
end type
type st_1 from statictext within w_cn771_cntbl_rpt_gyp_funcion
end type
type sle_mes_hasta from singlelineedit within w_cn771_cntbl_rpt_gyp_funcion
end type
type em_nivel from editmask within w_cn771_cntbl_rpt_gyp_funcion
end type
type cbx_1 from checkbox within w_cn771_cntbl_rpt_gyp_funcion
end type
type rb_1 from radiobutton within w_cn771_cntbl_rpt_gyp_funcion
end type
type rb_2 from radiobutton within w_cn771_cntbl_rpt_gyp_funcion
end type
type rb_3 from radiobutton within w_cn771_cntbl_rpt_gyp_funcion
end type
type gb_1 from groupbox within w_cn771_cntbl_rpt_gyp_funcion
end type
type gb_2 from groupbox within w_cn771_cntbl_rpt_gyp_funcion
end type
type gb_3 from groupbox within w_cn771_cntbl_rpt_gyp_funcion
end type
end forward
...
integer width = 3397
integer height = 1624
string title = "(CN771) Ganancias y Pérdidas por Función"
...
global w_cn771_cntbl_rpt_gyp_funcion w_cn771_cntbl_rpt_gyp_funcion

on w_cn771_cntbl_rpt_gyp_funcion.create
int iCurrent
call super::create
...
on w_cn771_cntbl_rpt_gyp_funcion.destroy
call super::destroy
...
string  ls_moneda, ls_mensaje, ls_nom_empresa
...
if not rb_3.checked then
	DECLARE USP_CNTBL_RPT_GYP_FUNCION PROCEDURE FOR 
		USP_CNTBL_RPT_GYP_FUNCION( :ln_mes_desde, 
											:ln_mes_hasta, 
											:ln_ano, 
											:ls_moneda, 
											:ln_nivel ) ;
	Execute USP_CNTBL_RPT_GYP_FUNCION ;
	
	if SQLCA.SQLCode < 0 then
		ls_mensaje = SQLCA.SQlErrText
		ROLLBACK;
		MessageBox('Error en USP_CNTBL_RPT_GYP_FUNCION', ls_mensaje)
		return
	end if
	
	CLOSE USP_CNTBL_RPT_GYP_FUNCION;
else
	//de lo contrario es un comparativo anual
	/*
	  begin
		  pkg_contabilidad.usp_cntbl_rpt_gyp_funcion(ani_year => :ani_year,
																	asi_moneda => :asi_moneda,
																	ani_nivel => :ani_nivel);
		end;
	*/
	DECLARE usp_cntbl_rpt_gyp_funcion_anual PROCEDURE FOR 
		pkg_contabilidad.usp_cntbl_rpt_gyp_funcion( 	:ln_ano, 
																	:ls_moneda, 
																	:ln_nivel ) ;
	Execute usp_cntbl_rpt_gyp_funcion_anual ;
	
	if SQLCA.SQLCode < 0 then
		ls_mensaje = SQLCA.SQlErrText
		ROLLBACK;
		MessageBox('Error en pkg_contabilidad.usp_cntbl_rpt_gyp_funcion', ls_mensaje)
		return
	end if
	
	CLOSE usp_cntbl_rpt_gyp_funcion_anual;
end if

if rb_1.checked then
	dw_report.dataObject = 'd_rpt_gyp_funcion_tbl'
elseif rb_2.checked then
	dw_report.dataObject = 'd_rpt_gyp_funcion_comparativo_tbl'
elseif rb_3.checked then
	dw_report.dataObject = 'd_rpt_gyp_funcion_anual_tbl'
end if
...
if dw_report.of_existstext( "t_empresa2") then
...
if dw_report.of_existecampo( "t_nombre") then
...
IF gs_empresa = 'FISHOLG' then
	dw_report.object.t_14.text = 'ESTADO DE RESULTADOS POR FUNCIÓN'
end if
end event

type dw_report from w_report_smpl`dw_report within w_cn771_cntbl_rpt_gyp_funcion
integer x = 0
integer y = 288
integer width = 3291
integer height = 1068
integer taborder = 70
string dataobject = "d_rpt_gyp_funcion_tbl"
...
event clicked;if this.checked then
	sle_mes_desde.enabled = true
	sle_mes_hasta.enabled = true
end if
end event
...
string facename = "Arial"
long backcolor = 67108864
string text = "Comparativo Anual"
end type

event clicked;if this.checked then
	sle_mes_desde.enabled = false
	sle_mes_hasta.enabled = false
end if
end event
...
string facename = "Arial"
long backcolor = 67108864
string text = "Opciones del reporte"
end type
