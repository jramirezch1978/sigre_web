...
public function integer of_create_log (u_dw_abc adw_master, ref datastore ads_log, string as_colname[], string as_coltype[], string as_user, string as_table)
public subroutine of_dw_map (datawindow adw_1, ref string as_colname[], ref string as_coltype[])
end prototypes
...
public function integer of_create_log (u_dw_abc adw_master, ref datastore ads_log, string as_colname[], string as_coltype[], string as_user, string as_table);Integer			li_totcol, li_y, li_key, li_rc
...
//Obtengo la fecha y hora del servidor
ldt_fec_registro = gnvo_app.of_fecha_Actual()

//Creo el datastore para actualizar los datos
...
FOR ll_x = 1 TO lds_del.RowCount()              // deletes
	...
	ads_log.SetItem(ll_row,'fecha',ldt_fec_registro)
...
FOR ll_x = 1 TO adw_master.RowCount()
	...
	ELSEIF ldis_status = DataModified! THEN       // modified
			...
			IF ldis_status = DataModified! THEN // solo debe proceder si la columna ha sido modificada
				...
				CHOOSE CASE ls_type
					...
					CASE 'datetime'
						...
						ls_nuevo  = String(adw_master.GetItemDateTime(ll_x,ls_campo,Primary!,FALSE), 'dd/mm/yyyy hh:mm:ss')
					CASE 'decimal'
						ls_actual = String(adw_master.GetItemDecimal(ll_x,ls_campo,Primary!,TRUE), "###,##0.0000000")
						ls_nuevo  = String(adw_master.GetItemDecimal(ll_x,ls_campo,Primary!,FALSE), "###,##0.0000000")
					CASE 'char'
				...
				IF (IsNull(ls_actual) and not isNull(ls_nuevo)) or (not isNull(ls_actual) and isNull(ls_nuevo)) or (ls_actual <> ls_nuevo) THEN
					ll_row = ads_log.InsertRow(0) // agregar fila al dw log
...
public subroutine of_dw_map (datawindow adw_1, ref string as_colname[], ref string as_coltype[]);Integer			li_x, li_totcol
Long				ll_row
