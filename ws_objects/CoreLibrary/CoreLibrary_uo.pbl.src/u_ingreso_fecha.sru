...
event ue_validacion(date ad_enter, integer ai_cal);Integer li_rc = 1

IF ad_enter < id_inicio THEN
	MessageBox("Error", "Fecha por debajo del limite", StopSign!)
	return
end if

IF ad_enter > id_fin THEN
	MessageBox("Error", "Fecha sobre el limite", StopSign!)
	return
END IF

PARENT.post EVENT ue_output()
...
event modified;
post EVENT ue_validacion(Date(em_1.text), 0)
