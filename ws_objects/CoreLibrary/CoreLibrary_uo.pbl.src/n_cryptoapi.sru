...
public function unsignedlong of_random ()
end prototypes
...
// ----------  --------		-----------------------------------------------------
// 01/18/2011	RolandS		Initial Coding
// 06/07/2015	RolandS		Changed to use EncodingAnsi! instead of iEncoding
// -----------------------------------------------------------------------------
...
// Hash the password
lblob_data = Blob(as_password, iEncoding)
...
lblob_buffer = ablob_data + Blob(Space(lul_DataLen))
...
Return String(lblob_encrypted, iEncoding)
...
//
// ARGUMENTS:  aul_ProviderId	- Array of provider ids ( by ref )
//					as_Provider		- Array of provider names ( by ref )
//					aul_ProvType	- Array of provider types ( by ref )
//
...
// ----------  --------		-----------------------------------------------------
// 01/18/2011	RolandS		Initial Coding
// 06/11/2013	RolandS		Added aul_ProviderId and aul_ProvType
// -----------------------------------------------------------------------------
...
do while CryptEnumProviders(lul_dwIndex, 0, 0, &
	...
	If CryptEnumProviders(lul_dwIndex, 0, 0, &
		...
		aul_ProviderId[li_index] = lul_dwIndex
		as_Provider[li_index] = ls_provider
		aul_ProvType[li_index] = lul_dwType
	Else
		of_GetLastError(lul_error, ls_msgtext)
		SignalError(lul_error, &
...
public function unsignedlong of_random ();// -----------------------------------------------------------------------------
// SCRIPT:     n_cryptoapi.of_Random
//
// PURPOSE:    This function will return a random number.
//
// RETURN:		ULong containing the random number
//
// DATE        PROG/ID		DESCRIPTION OF CHANGE / REASON
// ----------  --------		-----------------------------------------------------
// 02/04/2016	RolandS		Initial Coding
// -----------------------------------------------------------------------------

String ls_null, ls_msgtext
ULong lul_hProv, lul_error, lul_random
...
If Not CryptGenRandom(lul_hProv, 4, lul_random) Then
	of_GetLastError(lul_error, ls_msgtext)
	SignalError(lul_error, "CryptGenRandom:~r~n~r~n" + ls_msgtext)
End If
