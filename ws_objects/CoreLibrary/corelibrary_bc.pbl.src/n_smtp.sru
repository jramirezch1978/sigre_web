...
public function boolean of_send_dll ()
end prototypes
...
if IsNull(this.is_body) or trim(this.is_body) = '' then
	ls_msg = "Cuerpo del Correo esta NULO o VACIO, por favor corrija!!!" + CRLF 
	of_SetLastError(ls_msg)
	of_LogError(iERROR, ls_msg)
	return false
end if
...
// log start of SMTP conversation
ldt_current = gnvo_app.of_fecha_Actual()
of_LogFile("--------------------------------------------" +CRLF)
...
If Len(as_email) > 254 Then
	as_errormsg = "Direccion de Email no puede ser mayor a 254 caracteres!"
	Return False
End If

li_pos = Pos(as_email, "@")
If li_pos < 2 Then
	as_errormsg = "Direccion de Email debe contener el caracter @!"
	Return False
End If

If LastPos(as_email, "@") > Pos(as_email, "@") Then
	as_errormsg = "Direccion de Email no puede tener mas de un caracter @!"
	Return False
End If

li_pos = Pos(as_email, " ")
If li_pos > 0 Then
	as_errormsg = "Direccion Email no puede contener espacios!"
	Return False
End If
...
For li_idx = 1 To li_max
	...
	choose case li_asc
		...
		case 46
			// Period
		case else
			as_errormsg = "La parte del mailbox de la direccion Email contiene caracteres invalidos!"
			Return False
	end choose
Next
...
public function boolean of_send_dll ();// -----------------------------------------------------------------------------
// SCRIPT:     n_smtp.of_send_dll
//
// PURPOSE:    Envia correo usando el DLL SigreWebServiceWrapper.dll
//             Esta funcion usa la configuracion establecida en el objeto
...
// DATE        PROG/ID		DESCRIPTION OF CHANGE / REASON
// ----------  --------		-----------------------------------------------------
// 01/14/2026	JRamirez		Initial coding - Usa DLL C++ nativo
// -----------------------------------------------------------------------------

n_cst_api_sigre_dll lnvo_api
String ls_remitente, ls_destinatarios, ls_cc, ls_cco
String ls_adjuntos, ls_resultado, ls_msg
Integer li_idx, li_max, li_esHtml
DateTime ldt_current

try
    // log start
    ldt_current = DateTime(Today(), Now())
    of_LogFile("of_send_dll Start: " + String(ldt_current) + CRLF)
    
    // Crear instancia del DLL
    lnvo_api = create n_cst_api_sigre_dll
    
    // *** Preparar remitente: "email, nombre" ***
    ls_remitente = istr_From.Email
    if istr_From.Name <> "" then
        ls_remitente += ", " + istr_From.Name
    end if
    of_LogFile("From: " + ls_remitente)
    
    // *** Preparar destinatarios: "email, nombre; email2, nombre2;" ***
    ls_destinatarios = ""
    li_max = UpperBound(istr_Address)
    for li_idx = 1 to li_max
        ls_destinatarios += istr_Address[li_idx].Email
        if istr_Address[li_idx].Name <> "" then
            ls_destinatarios += ", " + istr_Address[li_idx].Name
        end if
        ls_destinatarios += "; "
    next
    of_LogFile("To: " + ls_destinatarios)
    
    // *** Preparar CC: "email, nombre; email2, nombre2;" ***
    ls_cc = ""
    li_max = UpperBound(istr_CC)
    for li_idx = 1 to li_max
        ls_cc += istr_CC[li_idx].Email
        if istr_CC[li_idx].Name <> "" then
            ls_cc += ", " + istr_CC[li_idx].Name
        end if
        ls_cc += "; "
    next
    if ls_cc <> "" then
    ...
    // *** Preparar BCC: "email, nombre; email2, nombre2;" ***
    ls_cco = ""
    li_max = UpperBound(istr_Bcc)
    for li_idx = 1 to li_max
        ls_cco += istr_Bcc[li_idx].Email
        if istr_Bcc[li_idx].Name <> "" then
            ls_cco += ", " + istr_Bcc[li_idx].Name
        end if
        ls_cco += "; "
    next
    if ls_cco <> "" then
        of_LogFile("BCC: " + ls_cco)
    end if
    ...
    for li_idx = 1 to li_max
        ...
        ls_adjuntos += istr_Attach[li_idx].FileName
    next
    if ls_adjuntos <> "" then
        of_LogFile("Attachments: " + ls_adjuntos)
    end if
    ...
    of_LogFile("Subject: " + is_subject)
    of_LogFile("Sending via DLL...")
    ...
    of_LogFile("Result: " + ls_resultado)
    
    // *** Procesar resultado (JSON) ***
    if Pos(ls_resultado, '"exitoso":true') > 0 then
        ldt_current = DateTime(Today(), Now())
        of_LogFile(CRLF + "of_send_dll End: " + String(ldt_current) + CRLF)
        return True
    else
        // Error
        ls_msg = "Error enviando correo: " + ls_resultado
        of_SetLastError(ls_msg)
        of_LogError(iERROR, ls_msg)
        return False
    end if
    ...
    ls_msg = "Exception en of_send_dll: " + ex.GetMessage()
    of_SetLastError(ls_msg)
    of_LogError(iERROR, ls_msg)
    return False
