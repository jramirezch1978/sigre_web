$PBExportHeader$w_selec_dato.srw
forward
global type w_selec_dato from window
end type
type cb_listo from commandbutton within w_selec_dato
end type
type dw_new from uo_datawindow within w_selec_dato
end type
type st_2 from statictext within w_selec_dato
end type
type st_1 from statictext within w_selec_dato
end type
type cbx_1 from checkbox within w_selec_dato
end type
type st_3 from statictext within w_selec_dato
end type
type cb_cancelar from commandbutton within w_selec_dato
end type
type cb_detener from commandbutton within w_selec_dato
end type
type cb_iniciar from commandbutton within w_selec_dato
end type
type sle_descripcion from singlelineedit within w_selec_dato
end type
type ddlb_seleccion from dropdownlistbox within w_selec_dato
end type
type mle_1 from multilineedit within w_selec_dato
end type
end forward
...
global w_selec_dato w_selec_dato
...
if ll_pos = 0 then
	MessageBox('Aviso wf_condicion_sql()', 'No se encontro: [' + 'AS ' + ls_cadena + '] en sentencia sql: ~r~n' + ls_sql, StopSign!)
	return ls_sql
end if
