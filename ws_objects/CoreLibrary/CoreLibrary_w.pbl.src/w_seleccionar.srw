...
type pb_up_down from picturebutton within w_seleccionar
end type
type cb_ok from commandbutton within w_seleccionar
end type
...
type cb_buscar from commandbutton within w_seleccionar
end type
...
integer 		    	ii_sort = 0, ii_down_up = 0
...
try 
	...
	MessageBox('Error', 'Ha ocurrido una excepcion: ' + ex.getMessage() + ', por favor verifique!', StopSign!)
...
//Reemplazo las tabulaciones con espacios en blanco
ls_where 	= invo_util.of_replace( ls_sql, '~t', ' ')

ls_cadena   = trim(UPPER(ddlb_seleccion.text))

//Hay que obtener el campo, asi que busco de todas maneras la clausula AS
ll_pos      = Pos(ls_where,' AS ' + ls_cadena, 1) 
if ll_pos = 0 then
	MessageBox('Aviso of_new_sql()', 'No se encontro:[' + ' AS ' + ls_cadena + '] en sentencia sql: ' + ls_sql, Exclamation!)
	return ls_sql
end if

ls_where    = Mid(ls_where, 1, ll_pos)

//Luego tengo que buscar si hay otro AS
ll_pos_two  = LastPos(ls_where, ' AS ')
if ll_pos_two > 0 then
	ls_where = trim(Mid(ls_where, ll_pos_two + 2 ))
end if


// Si hubiera una coma al comienzo entonces la quito, pero no debe tener SELECT
if Pos(ls_where, 'SELECT') = 0 then
	ll_pos_two  = Pos(ls_where, ',')
	if ll_pos_two > 0 then
		ls_where = trim(Mid(ls_where, ll_pos_two + 1 ))
	end if
end if
...
IF ISNULL(sle_descripcion.text) OR  TRIM(sle_descripcion.text) = '' THEN 
	ls_descripcion = "'%'"
ELSE 
   ls_descripcion = "upper('"+TRIM(sle_descripcion.text)+'%'+"')"
END IF

ls_where = 'upper(to_char(' + trim(ls_where) + ')) LIKE ' + ls_descripcion
...
//Quito de la sentencia ls_sql el Order by
ls_sql = Mid( ls_sql, 1, ll_pos - 1 )

// Busco si han puesto la clausula GROUP BY en la sentencia SQL
ll_pos	= LastPos(ls_sql,'GROUP BY',1) 

if ll_pos > 0 then
	ls_group_by = Mid( ls_sql, ll_pos )
else
	ls_group_by = ''
end if
...
ll_pos = Pos(UPPER(ls_new_sql), 'GROUP BY', 1)
if ll_pos > 0 then
	ls_new_sql = Trim(Mid(ls_new_sql, 1, ll_pos - 1))
end if

IF lb_where = FALSE THEN
   ls_new_sql = ls_new_sql + ' WHERE ( '+ ls_where +' ) ' + ls_group_by  + ' ' + ls_order_by 
ELSE
	ls_new_sql = ls_new_sql + ' AND ( ' + ls_where + ' ) ' + ls_group_by  + ' ' + ls_order_by
END IF
...
this.pb_up_down=create pb_up_down
...
string picturename = "C:\SIGRE\resources\PNG\down.png"
alignment htextalign = left!
boolean map3dcolors = true
end type

event clicked;if ii_down_up = 0 then
	parent.height = 2140
	ii_down_up = 1
	this.PictureName = 'C:\SIGRE\resources\PNG\up.png'
else
	parent.height = dw_new.y + dw_new.height + 120
	ii_down_up = 0
	this.PictureName = 'C:\SIGRE\resources\PNG\down.png'
end if
end event
...
event ue_column_sort ( string as_columna )
integer x = 14
integer y = 424
integer width = 2674
integer height = 1220
...
event ue_keydwn;if key = KeyEnter! then
	IF cb_ok.enabled = true AND dw_new.rowcount() > 0 THEN
		cb_ok.triggerevent('clicked')
	END IF
end if
end event

event ue_column_sort(string as_columna);Integer li_pos, li_len
String  ls_setsort, ls_name

IF ii_sort = 0 THEN
	ii_sort = 1
	ls_setsort = trim(as_columna) + ' A'
ELSE
	ii_sort = 0
	ls_setsort = trim(as_columna) + ' D'
END IF
...
event doubleclicked;call super::doubleclicked;String  ls_column, ls_name

ls_column = lower(dwo.name)
ls_name = mid(ls_column, 1, len(ls_column) - 2)

IF Lower(Right(ls_column, 2)) = '_t' or row = 0 THEN
	this.event ue_column_sort(ls_name)
	RETURN
END IF

IF cb_ok.enabled = true AND row > 0 THEN
	cb_ok.triggerevent('clicked')
END IF
