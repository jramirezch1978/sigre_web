$PBExportHeader$w_pr756_resumen_pptt.srw
forward
global type w_pr756_resumen_pptt from w_report_smpl
end type
type st_1 from statictext within w_pr756_resumen_pptt
end type
type uo_rango from ou_rango_fechas within w_pr756_resumen_pptt
end type
type cb_proc from commandbutton within w_pr756_resumen_pptt
end type
type cbx_categorias from checkbox within w_pr756_resumen_pptt
end type
type sle_categoria from singlelineedit within w_pr756_resumen_pptt
end type
type st_desc_categoria from statictext within w_pr756_resumen_pptt
end type
type cbx_articulos from checkbox within w_pr756_resumen_pptt
end type
type sle_cod_art from singlelineedit within w_pr756_resumen_pptt
end type
type st_desc_art from statictext within w_pr756_resumen_pptt
end type
type cbx_almacenes from checkbox within w_pr756_resumen_pptt
end type
type sle_almacen from singlelineedit within w_pr756_resumen_pptt
end type
type st_desc_almacen from statictext within w_pr756_resumen_pptt
end type
type rb_1 from radiobutton within w_pr756_resumen_pptt
end type
type rb_2 from radiobutton within w_pr756_resumen_pptt
end type
type gb_1 from groupbox within w_pr756_resumen_pptt
end type
end forward

global type w_pr756_resumen_pptt from w_report_smpl
integer width = 3995
integer height = 1356
string title = "[PR756] Resumen de productos terminados"
string menuname = "m_reporte"
event ue_query_retrieve ( )
st_1 st_1
uo_rango uo_rango
cb_proc cb_proc
cbx_categorias cbx_categorias
sle_categoria sle_categoria
st_desc_categoria st_desc_categoria
cbx_articulos cbx_articulos
sle_cod_art sle_cod_art
st_desc_art st_desc_art
cbx_almacenes cbx_almacenes
sle_almacen sle_almacen
st_desc_almacen st_desc_almacen
rb_1 rb_1
rb_2 rb_2
gb_1 gb_1
end type
global w_pr756_resumen_pptt w_pr756_resumen_pptt

event ue_query_retrieve();this.event ue_retrieve()
end event

on w_pr756_resumen_pptt.create
int iCurrent
call super::create
if this.MenuName = "m_reporte" then this.MenuID = create m_reporte
this.st_1=create st_1
this.uo_rango=create uo_rango
this.cb_proc=create cb_proc
this.cbx_categorias=create cbx_categorias
this.sle_categoria=create sle_categoria
this.st_desc_categoria=create st_desc_categoria
this.cbx_articulos=create cbx_articulos
this.sle_cod_art=create sle_cod_art
this.st_desc_art=create st_desc_art
this.cbx_almacenes=create cbx_almacenes
this.sle_almacen=create sle_almacen
this.st_desc_almacen=create st_desc_almacen
this.rb_1=create rb_1
this.rb_2=create rb_2
this.gb_1=create gb_1
iCurrent=UpperBound(this.Control)
this.Control[iCurrent+1]=this.st_1
this.Control[iCurrent+2]=this.uo_rango
this.Control[iCurrent+3]=this.cb_proc
this.Control[iCurrent+4]=this.cbx_categorias
this.Control[iCurrent+5]=this.sle_categoria
this.Control[iCurrent+6]=this.st_desc_categoria
this.Control[iCurrent+7]=this.cbx_articulos
this.Control[iCurrent+8]=this.sle_cod_art
this.Control[iCurrent+9]=this.st_desc_art
this.Control[iCurrent+10]=this.cbx_almacenes
this.Control[iCurrent+11]=this.sle_almacen
this.Control[iCurrent+12]=this.st_desc_almacen
this.Control[iCurrent+13]=this.rb_1
this.Control[iCurrent+14]=this.rb_2
this.Control[iCurrent+15]=this.gb_1
end on

on w_pr756_resumen_pptt.destroy
call super::destroy
if IsValid(MenuID) then destroy(MenuID)
destroy(this.st_1)
destroy(this.uo_rango)
destroy(this.cb_proc)
destroy(this.cbx_categorias)
destroy(this.sle_categoria)
destroy(this.st_desc_categoria)
destroy(this.cbx_articulos)
destroy(this.sle_cod_art)
destroy(this.st_desc_art)
destroy(this.cbx_almacenes)
destroy(this.sle_almacen)
destroy(this.st_desc_almacen)
destroy(this.rb_1)
destroy(this.rb_2)
destroy(this.gb_1)
end on

event ue_open_pre;call super::ue_open_pre;//ii_lec_mst = 0

ib_preview = true

this.event ue_preview( )
end event

event ue_retrieve;call super::ue_retrieve;Date		ld_fecha1, ld_fecha2
String 	ls_categoria, ls_articulo, ls_almacen

if cbx_categorias.checked then
	ls_categoria = '%%'
else
	if trim(sle_Categoria.text) = '' then
		gnvo_app.of_mensaje_Error("Debe seleccionar una categoria antes de dar el reporte")
		sle_categoria.setFocus()
		return
	end if
	
	ls_categoria = trim(sle_categoria.text) + '%'
end if

if cbx_articulos.checked then
	ls_articulo = '%%'
else
	if trim(sle_cod_art.text) = '' then
		gnvo_app.of_mensaje_Error("Debe seleccionar un ARTICULO antes de dar el reporte")
		sle_cod_art.setFocus()
		return
	end if
	
	ls_articulo = trim(sle_cod_art.text) + '%'
end if

if cbx_almacenes.checked then
	ls_almacen = '%%'
else
	if trim(sle_almacen.text) = '' then
		gnvo_app.of_mensaje_Error("Debe seleccionar un ALMACEN antes de dar el reporte")
		sle_almacen.setFocus()
		return
	end if
	
	ls_almacen = trim(sle_almacen.text) + '%'
end if

ld_fecha1 = date(uo_rango.of_get_fecha1( ))
ld_fecha2 = date(uo_rango.of_get_fecha2( ))

//Papel A-4 Apaisado
//this.dw_report.Object.DataWindow.Print.Paper.Size = 256 
//this.dw_report.Object.DataWindow.Print.CustomPage.Width = 297
//this.dw_report.Object.DataWindow.Print.CustomPage.Length = 210

if rb_2.checked then
	dw_report.DataObject = 'd_rpt_kardex_resumido_nro_vale_tbl'
else
	dw_report.DataObject = 'd_rpt_kardex_resumido_tbl'
end if

dw_report.settransobject( sqlca )
dw_report.retrieve(ld_fecha1, ld_fecha2, ls_categoria, ls_articulo, ls_almacen )

dw_report.object.p_logo.filename = gs_logo
dw_report.object.t_usuario.text 	= gs_user
dw_report.object.t_empresa.text 	= gs_empresa
dw_report.object.t_desde.text 	= string(ld_fecha1, 'dd/mm/yyyy')
dw_report.object.t_hasta.text 	= string(ld_fecha2, 'dd/mm/yyyy')


end event

type dw_report from w_report_smpl`dw_report within w_pr756_resumen_pptt
integer x = 0
integer y = 384
integer width = 3817
integer height = 764
integer taborder = 10
string dataobject = "d_rpt_kardex_resumido_tbl"
string is_dwform = ""
end type

type st_1 from statictext within w_pr756_resumen_pptt
integer y = 24
integer width = 494
integer height = 56
boolean bringtotop = true
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long textcolor = 8388608
long backcolor = 67108864
string text = "Rango de Fechas:"
boolean focusrectangle = false
end type

type uo_rango from ou_rango_fechas within w_pr756_resumen_pptt
integer x = 485
integer y = 16
integer taborder = 20
boolean bringtotop = true
end type

on uo_rango.destroy
call ou_rango_fechas::destroy
end on

type cb_proc from commandbutton within w_pr756_resumen_pptt
integer x = 1755
integer width = 343
integer height = 100
integer taborder = 30
boolean bringtotop = true
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
string text = "Procesar"
boolean default = true
end type

event clicked;parent.event ue_retrieve()
end event

type cbx_categorias from checkbox within w_pr756_resumen_pptt
integer x = 18
integer y = 108
integer width = 626
integer height = 80
boolean bringtotop = true
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long textcolor = 8388608
long backcolor = 67108864
string text = "Todas las Categorias"
boolean checked = true
end type

event clicked;If this.Checked Then
	sle_categoria.enabled = false
	sle_categoria.text=''
	st_desc_categoria.text = ''
Else
	sle_categoria.enabled = true
	sle_categoria.text=''
End If
end event

type sle_categoria from singlelineedit within w_pr756_resumen_pptt
event dobleclick pbm_lbuttondblclk
integer x = 667
integer y = 108
integer width = 357
integer height = 80
integer taborder = 140
boolean bringtotop = true
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
string pointer = "H:\SOURCE\CUR\taladro.cur"
long textcolor = 33554432
boolean enabled = false
integer limit = 8
borderstyle borderstyle = stylelowered!
end type

event dobleclick;boolean lb_ret
string ls_codigo, ls_data, ls_sql

ls_sql = "select a.cat_art as codigo_categoria, " &
		 + "a.desc_categoria as descripcion_Categoria " &
		 + "from articulo_Categ a " &
		 + "where a.flag_estado = '1'"
				 

			
if gnvo_app.of_lista(ls_sql, ls_codigo, ls_data, '2') then
	this.text= ls_codigo
	st_desc_categoria.text = ls_data
end if
end event

event modified;String ls_codigo, ls_desc

ls_codigo = this.text
if ls_codigo = '' or IsNull(ls_codigo) then
	MessageBox('Aviso', 'Debe Ingresar una TAREA')
	return
end if

SELECT desc_categoria
	INTO :ls_desc
FROM articulo_Categ
WHERE cat_art = :ls_codigo
  and flag_estado = '1';

IF SQLCA.SQLCode = 100 THEN
	Messagebox('Aviso', 'Codigo de Categoria ' + ls_codigo + ' no existe o no está activo')
	return
end if

st_desc_categoria.text = ls_desc
end event

type st_desc_categoria from statictext within w_pr756_resumen_pptt
integer x = 1038
integer y = 108
integer width = 1285
integer height = 80
boolean bringtotop = true
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long textcolor = 8388608
long backcolor = 67108864
boolean enabled = false
boolean border = true
boolean focusrectangle = false
end type

type cbx_articulos from checkbox within w_pr756_resumen_pptt
integer x = 18
integer y = 196
integer width = 626
integer height = 80
boolean bringtotop = true
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long textcolor = 8388608
long backcolor = 67108864
string text = "Todas las Articulos"
boolean checked = true
end type

event clicked;If this.Checked Then
	sle_cod_art.enabled = false
	sle_cod_art.text=''
	st_desc_art.text = ''
Else
	sle_cod_art.enabled = true
	st_desc_art.text=''
End If
end event

type sle_cod_art from singlelineedit within w_pr756_resumen_pptt
event dobleclick pbm_lbuttondblclk
integer x = 667
integer y = 196
integer width = 357
integer height = 80
integer taborder = 150
boolean bringtotop = true
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
string pointer = "H:\SOURCE\CUR\taladro.cur"
long textcolor = 33554432
boolean enabled = false
integer limit = 8
borderstyle borderstyle = stylelowered!
end type

event dobleclick;boolean lb_ret
string ls_codigo, ls_data, ls_sql, ls_categoria

if cbx_categorias.checked then
	ls_categoria = '%%'
else
	if trim(sle_categoria.text) = '' then
		MEssageBox('Error', 'Debe elegir primero la cateoria. Por favor corrija!', StopSign!)
		sle_categoria.setFocus()
		return
	end if
	
	ls_categoria = trim(sle_categoria.text) + '%'
	
end if

ls_sql = "select distinct a.cod_art as codigo_articulo, " &
		 + "a.desc_art as descripcion_articulo " &
		 + "from vale_mov vm, " &
		 + "     articulo_mov am, " &
		 + "     articulo     a, " &
		 + "      articulo_sub_categ a2 " &
		 + "where vm.nro_vale = am.nro_Vale " &
		 + "  and am.cod_art  = a.cod_Art " &
		 + "  and a.sub_cat_art = a2.cod_sub_cat " &
		 + "  and vm.flag_estado <> '0' " &
		 + "   and am.flag_estado <> '0' " &
		 + "   and a2.cat_art like '" + ls_categoria + "'"
				 

			
if gnvo_app.of_lista(ls_sql, ls_codigo, ls_data, '2') then
	this.text= ls_codigo
	st_desc_art.text = ls_data
end if
end event

event modified;String ls_codigo, ls_desc

ls_codigo = this.text
if ls_codigo = '' or IsNull(ls_codigo) then
	MessageBox('Aviso', 'Debe Ingresar una TAREA')
	return
end if

SELECT desc_categoria
	INTO :ls_desc
FROM articulo_Categ
WHERE cat_art = :ls_codigo
  and flag_estado = '1';

IF SQLCA.SQLCode = 100 THEN
	Messagebox('Aviso', 'Codigo de Categoria ' + ls_codigo + ' no existe o no está activo')
	return
end if

st_desc_categoria.text = ls_desc
end event

type st_desc_art from statictext within w_pr756_resumen_pptt
integer x = 1038
integer y = 196
integer width = 1285
integer height = 80
boolean bringtotop = true
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long textcolor = 8388608
long backcolor = 67108864
boolean enabled = false
boolean border = true
boolean focusrectangle = false
end type

type cbx_almacenes from checkbox within w_pr756_resumen_pptt
integer x = 18
integer y = 284
integer width = 626
integer height = 80
boolean bringtotop = true
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long textcolor = 8388608
long backcolor = 67108864
string text = "Todas las Almacenes"
boolean checked = true
end type

event clicked;If this.Checked Then
	sle_almacen.enabled 	= false
	sle_almacen.text		= ''
	st_desc_almacen.text = ''
Else
	sle_almacen.enabled = true
	st_desc_almacen.text=''
End If
end event

type sle_almacen from singlelineedit within w_pr756_resumen_pptt
event dobleclick pbm_lbuttondblclk
integer x = 667
integer y = 284
integer width = 357
integer height = 80
integer taborder = 160
boolean bringtotop = true
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
string pointer = "H:\SOURCE\CUR\taladro.cur"
long textcolor = 33554432
boolean enabled = false
integer limit = 8
borderstyle borderstyle = stylelowered!
end type

event dobleclick;boolean lb_ret
string ls_codigo, ls_data, ls_sql, ls_categoria, ls_cod_art

if cbx_categorias.checked then
	ls_categoria = '%%'
else
	if trim(sle_categoria.text) = '' then
		MEssageBox('Error', 'Debe elegir primero la CATEGORIA. Por favor corrija!', StopSign!)
		sle_categoria.setFocus()
		return
	end if
	
	ls_categoria = trim(sle_categoria.text) + '%'
	
end if

if cbx_articulos.checked then
	ls_cod_art = '%%'
else
	if trim(sle_cod_art.text) = '' then
		MEssageBox('Error', 'Debe elegir primero el ARTICULO. Por favor corrija!', StopSign!)
		sle_cod_art.setFocus()
		return
	end if
	
	ls_cod_art = trim(sle_cod_art.text) + '%'
	
end if

ls_sql = "select distinct vm.almacen as almacen, " &
		 + "       al.desc_almacen as desc_almacen " &
		 + "from vale_mov vm, " &
		 + "     articulo_mov am," &
		 + "     almacen      al, " &
		 + "     articulo     a, " &
		 + "     articulo_sub_categ a2 " &
		 + "where vm.nro_vale = am.nro_vale " &
		 + "  and vm.almacen  = al.almacen " &
		 + "  and am.cod_Art  = a.cod_art " &
		 + "  and a.sub_cat_art = a2.cod_sub_cat " &
		 + "  and vm.flag_estado <> '0' " &
		 + "  and am.flag_estado <> '0'" & 
		 + "  and a2.cat_art like '" + ls_categoria + "'" &
		 + "  and am.cod_art like '" + ls_Cod_art + "'"
				 

			
if gnvo_app.of_lista(ls_sql, ls_codigo, ls_data, '2') then
	this.text= ls_codigo
	st_Desc_almacen.text = ls_data
end if
end event

event modified;String ls_codigo, ls_desc

ls_codigo = this.text
if ls_codigo = '' or IsNull(ls_codigo) then
	MessageBox('Aviso', 'Debe un codigo de aLMACEN')
	return
end if

SELECT desc_almacen
	INTO :ls_desc
FROM almacen
WHERE almacen = :ls_codigo
  and flag_estado = '1';

IF SQLCA.SQLCode = 100 THEN
	Messagebox('Aviso', 'Codigo de ALMACEN ' + ls_codigo + ' no existe o no está activo')
	return
end if

st_desc_almacen.text = ls_desc
end event

type st_desc_almacen from statictext within w_pr756_resumen_pptt
integer x = 1038
integer y = 284
integer width = 1285
integer height = 80
boolean bringtotop = true
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long textcolor = 8388608
long backcolor = 67108864
boolean enabled = false
boolean border = true
boolean focusrectangle = false
end type

type rb_1 from radiobutton within w_pr756_resumen_pptt
integer x = 2409
integer y = 104
integer width = 379
integer height = 72
boolean bringtotop = true
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long textcolor = 8388608
long backcolor = 67108864
string text = "Por pallet"
boolean checked = true
end type

type rb_2 from radiobutton within w_pr756_resumen_pptt
integer x = 2409
integer y = 188
integer width = 379
integer height = 72
boolean bringtotop = true
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long textcolor = 8388608
long backcolor = 67108864
string text = "Por Nro Vale"
end type

type gb_1 from groupbox within w_pr756_resumen_pptt
integer x = 2345
integer width = 494
integer height = 368
integer taborder = 30
integer textsize = -8
integer weight = 700
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
string facename = "Arial"
long textcolor = 8388608
long backcolor = 67108864
string text = "Tipo de reporte"
end type

