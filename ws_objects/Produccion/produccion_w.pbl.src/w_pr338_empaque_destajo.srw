$PBExportHeader$w_pr338_empaque_destajo.srw
forward
global type w_pr338_empaque_destajo from w_abc_master_smpl
end type
type cb_buscar from commandbutton within w_pr338_empaque_destajo
end type
type uo_fechas from u_ingreso_rango_fechas within w_pr338_empaque_destajo
end type
type cb_procesar from commandbutton within w_pr338_empaque_destajo
end type
type st_1 from statictext within w_pr338_empaque_destajo
end type
type st_rows from statictext within w_pr338_empaque_destajo
end type
type st_nro from statictext within w_pr338_empaque_destajo
end type
type sle_origen from u_sle_codigo within w_pr338_empaque_destajo
end type
type cb_1 from commandbutton within w_pr338_empaque_destajo
end type
type gb_2 from groupbox within w_pr338_empaque_destajo
end type
end forward
...
integer width = 4695
integer height = 2292
string title = "[PR338] Generacion Partes Empaque - Destajo"
...
global w_pr338_empaque_destajo w_pr338_empaque_destajo
...
try 
	...
	if MessageBox('Pregunta', 'Desea realizar el proceso de consolidación de Partes de ' &
					+ 'EMPAQUE?, ojo que los datos se eliminaran si no se ha proceso', &
	...
	/*
	  pkg_produccion.sp_insert_empaque_dstj(asi_origen => :asi_origen,
	...
	*/
	...
	DECLARE sp_insert_empaque_dstj PROCEDURE FOR 
			  pkg_produccion.sp_insert_empaque_dstj(:ls_origen,
			  													 :gnvo_app.empresa.is_empresa,
																 :ld_fecha1,
																 :ld_Fecha2,
																 :gs_user);
			  
	EXECUTE sp_insert_empaque_dstj ;
	
	IF SQLCA.SQLCode = -1 THEN 
		ls_mensaje = SQLCA.SQLErrText
		Rollback ;
		Messagebox('Error', 'Error en procedure pkg_produccion.sp_insert_empaque_dstj(), Mensaje: ' + ls_mensaje, StopSign!)
		Return
	end if
	
	CLOSE sp_insert_empaque_dstj ;
...
try 
	...
	if MessageBox('Pregunta', 'Desea realizar el proceso de APROBACION de Partes de ' &
					+ 'EMPAQUE - DESTAJO?, Ojo una vez procesados, ya no se podrán hacer modificaciones al respecto', &
					Information!, Yesno!, 2) = 2 then
		return
	end if
	...
	/*
		  -- Call the procedure
		  pkg_produccion.sp_procesa_empaque_dstjo(asi_origen => :asi_origen,
																adi_fecha1 => :adi_fecha1,
																adi_fecha2 => :adi_fecha2,
																asi_usuario => :asi_usuario);
	*/
	
	DECLARE sp_procesa_empaque_dstjo PROCEDURE FOR 
			  pkg_produccion.sp_procesa_empaque_dstjo(:ls_origen,
																 	:ld_fecha1,
																 	:ld_Fecha2,
																 	:gs_user);
			  
	EXECUTE sp_procesa_empaque_dstjo ;
	
	IF SQLCA.SQLCode = -1 THEN 
		ls_mensaje = SQLCA.SQLErrText
		Rollback ;
		Messagebox('Error', 'Error en procedure pkg_produccion.sp_procesa_empaque_dstjo(), Mensaje: ' + ls_mensaje, StopSign!)
		Return
	end if
	
	CLOSE sp_procesa_empaque_dstjo ;
...
on w_pr338_empaque_destajo.create
int iCurrent
call super::create
...
on w_pr338_empaque_destajo.destroy
call super::destroy
...
type dw_master from w_abc_master_smpl`dw_master within w_pr338_empaque_destajo
...
integer width = 4123
integer height = 1752
string dataobject = "d_abc_empaque_destajo_tbl"
end type
...
fontfamily fontfamily = swiss!
string facename = "Arial"
string text = "Procesar Partes de Empaque"
end type
...
fontfamily fontfamily = swiss!
string facename = "Arial"
string text = "Aprobar Partes de Empaque"
end type
