$PBExportHeader$w_pr330_parte_recepcion.srw
forward
global type w_pr330_parte_recepcion from w_abc_master_smpl
end type
type cb_buscar from commandbutton within w_pr330_parte_recepcion
end type
type uo_fechas from u_ingreso_rango_fechas within w_pr330_parte_recepcion
end type
type cb_procesar from commandbutton within w_pr330_parte_recepcion
end type
type hpb_progreso from hprogressbar within w_pr330_parte_recepcion
end type
type gb_2 from groupbox within w_pr330_parte_recepcion
end type
end forward
...
integer width = 4695
integer height = 2292
string title = "[PR330] Parte de Recepcion - PRODUCCION"
...
global w_pr330_parte_recepcion w_pr330_parte_recepcion
...
try 
	...
	gnvo_app.of_Catch_Exception(ex, 'Error al procesar al parte de Recepcion')
...
on w_pr330_parte_recepcion.create
int iCurrent
call super::create
...
on w_pr330_parte_recepcion.destroy
call super::destroy
...
w_pr332_parte_recepcion_popup lw_1
...
type dw_master from w_abc_master_smpl`dw_master within w_pr330_parte_recepcion
event ue_display ( string as_columna,  long al_row )
event ue_print_recepcion ( long al_row )
integer y = 192
integer width = 4123
integer height = 1752
string dataobject = "d_abc_parte_recepcion_tbl"
end type
...
event dw_master::ue_print_recepcion(long al_row);// vista previa de mov. almacen
...
try 
	...
	lstr_rep.dw1 		= 'd_rpt_codigos_recepcion_pptt_lbl'
...
choose case lower(dwo.name)
	...
	case "b_print_recepcion"
		this.event dynamic ue_print_recepcion(row)
...
if this.object.flag_estado [this.getRow()] = '0' then 
	gnvo_app.of_mensaje_Error("No se puede anular el parte de recepcion porque esta anulado")
	return
end if

ls_nro_parte = this.object.nro_parte [this.getRow()]

if MessageBox('PRODUCCIÓN','¿Esta seguro de ANULAR el Parte de RECEPCION ' + ls_nro_parte + ' ?',Question!,yesno!) = 2 then
		return
End if

//begin
//  -- Call the procedure
//  pkg_produccion.sp_anular_parte_recepcion(asi_nro_parte => :asi_nro_parte);
//end;
  
DECLARE 	sp_anular_parte_recepcion PROCEDURE FOR
			pkg_produccion.sp_anular_parte_recepcion(:ls_nro_parte) ;
			
EXECUTE 	sp_anular_parte_recepcion ;

IF SQLCA.sqlcode = -1 THEN
	ls_mensaje = "Error al ejecutar pkg_produccion.sp_anular_parte_recepcion. Mensaje: " + SQLCA.SQLErrText
	Rollback ;
	MessageBox('SQL error', ls_mensaje, StopSign!)	
	return
END IF
...
CLOSE sp_anular_parte_recepcion;
