$PBExportHeader$w_pr329_parte_empaque.srw
forward
global type w_pr329_parte_empaque from w_abc_master_smpl
end type
type cb_buscar from commandbutton within w_pr329_parte_empaque
end type
type uo_fechas from u_ingreso_rango_fechas within w_pr329_parte_empaque
end type
type cb_procesar from commandbutton within w_pr329_parte_empaque
end type
type st_1 from statictext within w_pr329_parte_empaque
end type
type st_rows from statictext within w_pr329_parte_empaque
end type
type hpb_progreso from hprogressbar within w_pr329_parte_empaque
end type
type gb_2 from groupbox within w_pr329_parte_empaque
end type
end forward
...
integer width = 4695
integer height = 2292
string title = "[PR329] Parte de Empaque - PRODUCCION"
...
global w_pr329_parte_empaque w_pr329_parte_empaque
...
on w_pr329_parte_empaque.create
int iCurrent
call super::create
...
on w_pr329_parte_empaque.destroy
call super::destroy
...
//El resto de campos los bloqueo
...
type dw_master from w_abc_master_smpl`dw_master within w_pr329_parte_empaque
...
choose case lower(as_columna)
	...
	case 'nro_tunel'
		ls_sql = "select nro_tunel as nro_tunel," &
				 + "       desc_tunel as descripcion_tunel " &
				 + "from tg_tuneles_frio " &
				 + "where flag_estado = '1'"
				 
		if gnvo_app.of_lista(ls_sql, ls_return1, ls_return2, '2') then 
			
			this.object.nro_tunel		[al_row] = ls_return1
			this.object.desc_tunel		[al_row] = ls_return2
			this.ii_update = 1
	...
	case 'nro_ot'
		...
		if lb_ret and ls_return1 <>''then 
			...
			of_nro_trazabilidad(al_row)
			
			this.ii_update = 1
		end if
		
	case "cod_art_cavalier"
		...
		if gnvo_app.of_lista(ls_sql, ls_return1, ls_return2, '2') then
			...
			this.object.cod_art_cavalier		[al_row] = ls_return1
			this.object.desc_cavalier			[al_row] = ls_return2
			this.ii_update = 1
	...
	case "cod_art_pptt"
		...
		if lb_ret and ls_return1 <> '' then
			...
			this.object.flag_und2			[al_row] = ls_return5
			this.object.factor_conv_und	[al_row] = Dec(ls_return6)
			this.object.org_amp_ing			[al_row] = ls_return7
			this.object.nro_amp_ing			[al_row] = Long(ls_return8)
			
			this.object.total_caja			[al_row] = 0
...
event dw_master::itemchanged;call super::itemchanged;string 	ls_nro_trazabilidad, ls_jefe_turno, ls_grupo_empaque, ls_flag_tipo_proceso, &
			...
			ls_almacen, ls_juliano, ls_year, ls_flag_und2, ls_cod_art, ls_mensaje
...
Date		ld_fecha, ld_hoy, ld_fec_limite
Integer	li_nro_dias
...
choose case lower(dwo.name)
	...
	case 'jefe_turno', 'grupo_empaque', 'flag_tipo_proceso', 'fec_empaque', 'fec_produccion'
		...
		if lower(dwo.name) = 'fec_produccion' then
			ld_fecha = Date(this.object.fec_produccion [row])
			...
			if ld_fecha > ld_hoy then
				MessageBox('Error', 'La fecha de produccion no puede ser mayor que la fecha actual' &
										...
										+ '~r~nFecha Produccion: ' + string(ld_Fecha, 'dd/mm/yyyy') &
...
//				from dual;
//
//				this.object.fec_produccion [row] = ld_fecha
//				return 1
//			end if
			
		end if
		
		//Ahora valido la fecha de empaque
		if lower(dwo.name) = 'fec_empaque' then
			ld_fecha = Date(this.object.fec_empaque [row])
			
			//Valido que la fecha de produccion no puede ser mayor que la fecha de hoy día
			if ld_fecha > ld_hoy then
				MessageBox('Error', 'La fecha de empaque no puede ser mayor que la fecha actual' &
										+ '~r~nFecha Actual: ' + string(ld_hoy, 'dd/mm/yyyy') &
										+ '~r~nFecha Empaque: ' + string(ld_Fecha, 'dd/mm/yyyy') &
										+ '~r~nPor favor verifique!', StopSign!)
...
//			end if
//			
//			if ld_fecha < ld_fec_limite then
...
//			end if
		...
		if of_nro_trazabilidad(row) then
			this.ii_update = 1
		end if

	case 'fec_reproceso'
		
		//Ahora valido la fecha de REPROCESO
		ld_fecha = Date(this.object.fec_reproceso [row])
			
		//Valido que la fecha de produccion no puede ser mayor que la fecha de hoy día
		if ld_fecha > ld_hoy then
			MessageBox('Error', 'La fecha de REPROCESO no puede ser mayor que la fecha actual' &
									+ '~r~nFecha Actual: ' + string(ld_hoy, 'dd/mm/yyyy') &
									+ '~r~nFecha REPROCESO: ' + string(ld_Fecha, 'dd/mm/yyyy') &
									+ '~r~nPor favor verifique!', StopSign!)
									
			this.object.fec_reproceso [row] = ld_hoy
			return 1
		end if
...
//									+ '~r~nPor favor verifique!', StopSign!)
//									
//			this.object.fec_reproceso [row] = ld_fec_limite
//			return 1
//		end if
	...
	case 'total_caja'
		...
		if ls_flag_und2 = '1' and (IsNull(ldc_factor_conv_und) or ldc_factor_conv_und = 0) then
			gnvo_app.of_mensaje_error("No se ha especificado el factor de conversion, por favor corrija" &
											+ "~r~nCod Art: " + this.object.cod_art_pptt [row])
			this.object.cant_producida [row] = 0.00
			this.object.total_caja 		[row] = 0.00
			return 1
		end if
...
choose case lower(dwo.name)
	...
	case 'b_new_pallet'
		...
		if not IsNull(ls_nro_pallet) and trim(ls_nro_pallet) <> '' then
			gnvo_app.of_mensaje_error("No puede generar otro nro de pallet, porque ya existe uno")
			return
		end if
		...
		if not invo_nro.of_nro_pallet( ls_origen, ls_nro_parte, ls_nro_pallet) then return
	...
	case 'b_crear_cu'
		...
		if MessageBox('Aviso', 'Desea generar los códigos CU por cada caja?', Information!, YesNo!, 2) = 2 then return
		
		//Genero un nuevo nro de pallet
		if not invo_nro.of_crear_cajas( ls_origen, ls_nro_parte) then return
	...
	case 'b_procesar'
		...
		if MessageBox('Aviso', 'Desea generar el Vale de Ingreso del registro ' + ls_nro_parte + ' ?', Information!, YesNo!, 2) = 2 then return
	...
	case "b_print_cu"
		...
		this.event dynamic ue_print_cus(row)
