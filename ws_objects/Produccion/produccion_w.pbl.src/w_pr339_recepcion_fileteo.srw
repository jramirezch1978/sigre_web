$PBExportHeader$w_pr339_recepcion_fileteo.srw
forward
global type w_pr339_recepcion_fileteo from w_abc_master_smpl
end type
type cb_buscar from commandbutton within w_pr339_recepcion_fileteo
end type
type uo_fechas from u_ingreso_rango_fechas within w_pr339_recepcion_fileteo
end type
type cb_procesar from commandbutton within w_pr339_recepcion_fileteo
end type
type st_1 from statictext within w_pr339_recepcion_fileteo
end type
type st_rows from statictext within w_pr339_recepcion_fileteo
end type
type st_nro from statictext within w_pr339_recepcion_fileteo
end type
type sle_origen from u_sle_codigo within w_pr339_recepcion_fileteo
end type
type cb_1 from commandbutton within w_pr339_recepcion_fileteo
end type
type gb_2 from groupbox within w_pr339_recepcion_fileteo
end type
end forward
...
integer width = 4695
integer height = 2292
string title = "[PR339] Generacion Partes Recepcion - Fileteo"
...
global w_pr339_recepcion_fileteo w_pr339_recepcion_fileteo
...
try 
	...
	/*
	  pkg_produccion.sp_proc_recepcion_fileteo(asi_origen => :asi_origen,
	...
	*/
	...
	DECLARE sp_proc_recepcion_fileteo PROCEDURE FOR 
			  pkg_produccion.sp_proc_recepcion_fileteo(:ls_origen,
			  													 	 :gnvo_app.empresa.is_empresa,
																 	 :ld_fecha1,
																 	 :ld_Fecha2,
																 	 :gs_user);
			  
	EXECUTE sp_proc_recepcion_fileteo ;
	
	IF SQLCA.SQLCode = -1 THEN 
		ls_mensaje = SQLCA.SQLErrText
		Rollback ;
		Messagebox('Error', 'Error en procedure pkg_produccion.sp_proc_recepcion_fileteo(), Mensaje: ' + ls_mensaje, StopSign!)
		Return
	end if
	
	CLOSE sp_proc_recepcion_fileteo ;
...
try 
	...
	/*
		begin
		  -- Call the procedure
		  pkg_produccion.sp_aprueba_recepcion_fileteo(asi_origen => :asi_origen,
																	 asi_empresa => :asi_empresa,
																	 adi_fecha1 => :adi_fecha1,
																	 adi_fecha2 => :adi_fecha2,
																	 asi_usuario => :asi_usuario);
	...
	*/
	...
	DECLARE sp_aprueba_recepcion_fileteo PROCEDURE FOR 
			  pkg_produccion.sp_aprueba_recepcion_fileteo(:ls_origen,
			  															 :gnvo_app.empresa.is_empresa,
																 		 :ld_fecha1,
																 		 :ld_Fecha2,
																 		 :gs_user);
			  
	EXECUTE sp_aprueba_recepcion_fileteo ;
	
	IF SQLCA.SQLCode = -1 THEN 
		ls_mensaje = SQLCA.SQLErrText
		Rollback ;
		Messagebox('Error', 'Error en procedure pkg_produccion.sp_aprueba_recepcion_fileteo(), Mensaje: ' + ls_mensaje, StopSign!)
		Return
	end if
	
	CLOSE sp_aprueba_recepcion_fileteo ;
	...
	gnvo_app.of_Catch_Exception(ex, 'Error al procesar pkg_produccion.sp_aprueba_recepcion_fileteo()')
finally
...
on w_pr339_recepcion_fileteo.create
int iCurrent
call super::create
...
on w_pr339_recepcion_fileteo.destroy
call super::destroy
...
type dw_master from w_abc_master_smpl`dw_master within w_pr339_recepcion_fileteo
...
integer width = 4123
integer height = 1752
string dataobject = "d_abc_recepcion_fileteo_tbl"
end type
