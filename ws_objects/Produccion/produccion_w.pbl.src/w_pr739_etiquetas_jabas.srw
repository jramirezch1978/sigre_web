$PBExportHeader$w_pr739_etiquetas_jabas.srw
forward
global type w_pr739_etiquetas_jabas from w_report_smpl
end type
type cb_reporte from commandbutton within w_pr739_etiquetas_jabas
end type
type st_1 from statictext within w_pr739_etiquetas_jabas
end type
type st_2 from statictext within w_pr739_etiquetas_jabas
end type
type cbx_todos from checkbox within w_pr739_etiquetas_jabas
end type
type sle_jabas from singlelineedit within w_pr739_etiquetas_jabas
end type
type sle_desde from singlelineedit within w_pr739_etiquetas_jabas
end type
type sle_hasta from singlelineedit within w_pr739_etiquetas_jabas
end type
type cb_1 from commandbutton within w_pr739_etiquetas_jabas
end type
type gb_5 from groupbox within w_pr739_etiquetas_jabas
end type
type gb_3 from groupbox within w_pr739_etiquetas_jabas
end type
end forward
...
integer width = 3506
integer height = 1636
string title = "(PR739) Etiquetas para Jabas de Campo"
string menuname = "m_reporte"
long backcolor = 79741120
event ue_crear_jabas ( )
...
global w_pr739_etiquetas_jabas w_pr739_etiquetas_jabas
...
event ue_crear_jabas();//create or replace procedure USP_PROD_CREAR_JABAS(
...
if li_jabas <= 0 then
	MessageBox('Error', 'Debe indicar un numero de jabas positivo')
	return
end if

DECLARE USP_PROD_CREAR_JABAS PROCEDURE FOR
	USP_PROD_CREAR_JABAS( :gs_origen, 
								 :li_jabas );
								 
EXECUTE USP_PROD_CREAR_JABAS;

IF SQLCA.sqlcode = -1 THEN
	ls_mensaje = "PROCEDURE USP_PROD_CREAR_JABAS: " + SQLCA.SQLErrText
	Rollback ;
	MessageBox('SQL error', ls_mensaje, StopSign!)	
	return
END IF

CLOSE USP_PROD_CREAR_JABAS;
...
select min(nro_jaba), max(nro_jaba)
...
on w_pr739_etiquetas_jabas.create
int iCurrent
call super::create
...
on w_pr739_etiquetas_jabas.destroy
call super::destroy
...
type dw_report from w_report_smpl`dw_report within w_pr739_etiquetas_jabas
integer x = 9
integer y = 220
integer width = 3451
integer height = 1216
integer taborder = 70
string dataobject = "d_etiquetas_jabas2_lbl"
end type
...
event clicked;Parent.Event ue_crear_jabas()
of_limites( )
...
string facename = "Arial"
long backcolor = 67108864
string text = "Rango de Etiquetas para Jabas"
end type
...
string facename = "Arial"
long backcolor = 67108864
string text = "Generar Nuevas Jabas"
end type
