$PBExportHeader$w_pr341_envasado_lavado.srw
forward
global type w_pr341_envasado_lavado from w_abc_master_smpl
end type
type cb_buscar from commandbutton within w_pr341_envasado_lavado
end type
type uo_fechas from u_ingreso_rango_fechas within w_pr341_envasado_lavado
end type
type cb_procesar from commandbutton within w_pr341_envasado_lavado
end type
type st_1 from statictext within w_pr341_envasado_lavado
end type
type st_rows from statictext within w_pr341_envasado_lavado
end type
type st_nro from statictext within w_pr341_envasado_lavado
end type
type sle_origen from u_sle_codigo within w_pr341_envasado_lavado
end type
type cb_1 from commandbutton within w_pr341_envasado_lavado
end type
type gb_2 from groupbox within w_pr341_envasado_lavado
end type
end forward
...
integer width = 4695
integer height = 2292
string title = "[PR341] Generacion Partes Envasado - Lavado"
...
global w_pr341_envasado_lavado w_pr341_envasado_lavado
...
try 
	...
	if MessageBox('Pregunta', 'Desea realizar el proceso de consolidación de Partes de ' &
					+ 'ENVASADO?, ojo que los datos se eliminaran si no se ha proceso', &
	...
	/*
	  pkg_produccion.sp_proc_envasado_lavado(asi_origen => :asi_origen,
	...
	*/
	...
	DECLARE sp_proc_envasado_lavado PROCEDURE FOR 
			  pkg_produccion.sp_proc_envasado_lavado(:ls_origen,
			  													  :gnvo_app.empresa.is_empresa,
																  :ld_fecha1,
																  :ld_Fecha2,
																  :gs_user);
			  
	EXECUTE sp_proc_envasado_lavado ;
	
	IF SQLCA.SQLCode = -1 THEN 
		ls_mensaje = SQLCA.SQLErrText
		Rollback ;
		Messagebox('Error', 'Error en procedure pkg_produccion.sp_proc_envasado_lavado(), Mensaje: ' + ls_mensaje, StopSign!)
		Return
	end if
	
	CLOSE sp_proc_envasado_lavado ;
...
try 
	...
	if MessageBox('Pregunta', 'Desea realizar el proceso de APROBACION de Partes de ' &
					+ 'ENVASADO - LAVADO?, Ojo una vez procesados, ya no se podrán hacer modificaciones al respecto', &
					Information!, Yesno!, 2) = 2 then
		return
	end if
	...
	/*
		  -- Call the procedure
		  pkg_produccion.sp_aprueba_envasado_lavado(asi_origen => :asi_origen,
																adi_fecha1 => :adi_fecha1,
																adi_fecha2 => :adi_fecha2,
																asi_usuario => :asi_usuario);
	*/
	
	DECLARE sp_aprueba_envasado_lavado PROCEDURE FOR 
			  pkg_produccion.sp_aprueba_envasado_lavado(:ls_origen,
																 	:ld_fecha1,
																 	:ld_Fecha2,
																 	:gs_user);
			  
	EXECUTE sp_aprueba_envasado_lavado ;
	
	IF SQLCA.SQLCode = -1 THEN 
		ls_mensaje = SQLCA.SQLErrText
		Rollback ;
		Messagebox('Error', 'Error en procedure pkg_produccion.sp_aprueba_envasado_lavado(), Mensaje: ' + ls_mensaje, StopSign!)
		Return
	end if
	
	CLOSE sp_aprueba_envasado_lavado ;
	...
	gnvo_app.of_Catch_Exception(ex, 'Error al procesar al parte de DESTAJO')
finally
...
on w_pr341_envasado_lavado.create
int iCurrent
call super::create
...
on w_pr341_envasado_lavado.destroy
call super::destroy
...
type dw_master from w_abc_master_smpl`dw_master within w_pr341_envasado_lavado
...
integer width = 4123
integer height = 1752
string dataobject = "d_abc_envasado_lavado_tbl"
end type
...
event dw_master::ue_display;String	ls_sql, ls_codigo, ls_data, ls_turno, ls_cuadrilla, ls_especie, &
			...
			ls_desc_operacion, ls_tarea, ls_desc_labor
...
fontfamily fontfamily = swiss!
string facename = "Arial"
string text = "Procesar Partes de Envasado"
end type
...
fontfamily fontfamily = swiss!
string facename = "Arial"
string text = "Aprobar Envasado - Lavado"
end type
