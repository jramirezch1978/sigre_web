...
String	ls_find, ls_trabajador
u_dw_abc	ldw_detail

ldw_detail = ist_datos.dw_d

FOR ll_j = 1 TO dw_2.RowCount()								
	ls_trabajador = dw_2.object.cod_trabajador [ll_j]
	
	ls_find = "cod_trabajador = '" + ls_trabajador + "'"
	
	ll_find = ldw_detail.Find(ls_find, 0, ldw_detail.RowCount())
	...
	IF ll_row > 0 THEN
		...
		ldw_detail.Object.nom_trabajador	 [ll_row] = dw_2.Object.nom_trabajador	[ll_j]
		ldw_detail.object.cant_destajada	 [ll_row] = 0
		
		ist_datos.w1.dynamic of_modify()
...
public function boolean of_opcion3 ();// Solo para consumos internos 
Long   	ll_j, ll_row, ll_find
String	ls_supervisor, ls_cosechador1, ls_cosechador2

delete tt_datos_campo;

FOR ll_j = 1 TO dw_2.RowCount()								
	ls_supervisor = dw_2.object.supervisor [ll_j]
	ls_cosechador1 = dw_2.object.cosechador1 [ll_j]
	ls_cosechador2 = dw_2.object.cosechador2 [ll_j]
	
	insert into tt_datos_campo(supervisor, cosechador1, cosechador2)
	values(:ls_supervisor, :ls_cosechador1, :ls_cosechador2);
...
CHOOSE CASE ist_datos.opcion
	...
	CASE 2 //Reporte de trabajadores
		if of_opcion2() then 
			ist_datos.titulo = 's'
		else
			return
	...
	CASE 3 //Reporte de trabajadores
		if of_opcion3() then 
			ist_datos.titulo = 's'
		else
			return
