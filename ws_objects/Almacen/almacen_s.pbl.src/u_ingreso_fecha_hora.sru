$PBExportHeader$u_ingreso_fecha_hora.sru
$PBExportComments$Custom object para ingreso de una fecha
forward
global type u_ingreso_fecha_hora from userobject
end type
type cb_1 from commandbutton within u_ingreso_fecha_hora
end type
type em_1 from editmask within u_ingreso_fecha_hora
end type
end forward
...
global u_ingreso_fecha_hora u_ingreso_fecha_hora

type variables
DateTime idt_inicio, idt_fin
end variables

forward prototypes
public subroutine of_set_label (string as_label)
public function datetime of_get_fecha ()
...
public function datetime of_get_fecha ();DateTime	ldt_fecha
Integer	li_status

//ldt_fecha = DateTime(em_1.text)
em_1.GetData(ldt_fecha)
...
on u_ingreso_fecha_hora.create
...
on u_ingreso_fecha_hora.destroy
destroy(this.cb_1)
destroy(this.em_1)
end on
...
event ue_validacion(datetime adt_enter, integer ai_cal);Integer li_rc = 1

IF adt_enter < idt_inicio THEN
	MessageBox("Error", "Fecha por debajo del limite")
	li_rc = -1
ELSE
	IF adt_enter > idt_fin THEN
...
event modified;
THIS.EVENT ue_validacion(DateTime(em_1.text), 0)
