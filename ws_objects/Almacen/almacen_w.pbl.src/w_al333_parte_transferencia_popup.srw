$PBExportHeader$w_al333_parte_transferencia_popup.srw
forward
global type w_al333_parte_transferencia_popup from w_abc_mastdet_smpl
end type
type cb_grabar from commandbutton within w_al333_parte_transferencia_popup
end type
type cb_cancelar from commandbutton within w_al333_parte_transferencia_popup
end type
type cb_1 from commandbutton within w_al333_parte_transferencia_popup
end type
end forward
...
integer width = 3552
integer height = 2784
string title = "[AL333] Parte de Transferencia Interna"
...
global w_al333_parte_transferencia_popup w_al333_parte_transferencia_popup
...
on w_al333_parte_transferencia_popup.create
int iCurrent
call super::create
...
on w_al333_parte_transferencia_popup.destroy
call super::destroy
...
type dw_master from w_abc_mastdet_smpl`dw_master within w_al333_parte_transferencia_popup
integer x = 0
integer y = 0
integer width = 3008
integer height = 944
...
string 	ls_codigo, ls_data, ls_sql, ls_almacen_org, ls_almacen_dst, &
			ls_anaquel, ls_fila, ls_columna, ls_nro_pallet

choose case lower(as_columna)
		
	case "almacen_org"

		ls_sql = "select distinct vw.almacen as almacen, " &
				 + "vw.desc_almacen as desc_almacen " &
				 + " from vw_alm_saldo_pallet vw, " &
				 + "      (select distinct almacen " &
				 + "         from vw_alm_saldo_pallet " &
				 + "       minus " &
				 + "       select distinct te.almacen_pptt " &
				 + "         from tg_parte_empaque te" &
				 + "        where te.flag_estado = '1') s " &
				 + " where vw.almacen = s.almacen " &
				 + "   and vw.saldo > 0 " &
				 + "   and vw.flag_tipo_almacen = 'T' " &
				 + " order by vw.almacen "
	...
	case 'anaquel_org'
		...
		ls_sql = "select distinct vw.anaquel as anaquel, " &
				 + "       vw.fila as fila, " &
				 + "       vw.columna as columna, " &
				 + "       vw.nro_pallet as nro_pallet " &
				 + " from vw_alm_saldo_pallet vw, " &
				 + "      (select distinct almacen " &
				 + "         from vw_alm_saldo_pallet " &
				 + "       minus " &
				 ...
				 + "   and vw.saldo > 0 " &
				 + "   and vw.flag_tipo_almacen = 'T' " &
				 + "   and vw.almacen = '" + ls_almacen_org + "'" &
				 + "  and vw.saldo > 0 " 
				 
		if gnvo_app.of_lista(ls_sql, ls_anaquel, ls_fila, ls_columna, ls_nro_pallet, '1') then
...
type dw_detail from w_abc_mastdet_smpl`dw_detail within w_al333_parte_transferencia_popup
integer x = 0
integer y = 968
integer width = 3470
integer height = 1600
