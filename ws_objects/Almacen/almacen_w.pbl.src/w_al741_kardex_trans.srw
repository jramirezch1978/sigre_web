$PBExportHeader$w_al741_kardex_trans.srw
forward
global type w_al741_kardex_trans from w_rpt_list
end type
type gb_fechas from groupbox within w_al741_kardex_trans
end type
type uo_fecha from u_ingreso_rango_fechas_v within w_al741_kardex_trans
end type
type rb_detalle from radiobutton within w_al741_kardex_trans
end type
type rb_resumen from radiobutton within w_al741_kardex_trans
end type
type cb_seleccionar from commandbutton within w_al741_kardex_trans
end type
type st_1 from statictext within w_al741_kardex_trans
end type
type dw_text from datawindow within w_al741_kardex_trans
end type
type sle_almacen from singlelineedit within w_al741_kardex_trans
end type
type sle_descrip from singlelineedit within w_al741_kardex_trans
end type
type gb_tipo_reporte from groupbox within w_al741_kardex_trans
end type
type gb_1 from groupbox within w_al741_kardex_trans
end type
end forward
...
integer width = 3621
integer height = 2516
string title = "Kardex en tránsito (AL741)"
...
global w_al741_kardex_trans w_al741_kardex_trans
...
on w_al741_kardex_trans.create
int iCurrent
call super::create
...
this.Control[iCurrent+10]=this.gb_tipo_reporte
this.Control[iCurrent+11]=this.gb_1
end on

on w_al741_kardex_trans.destroy
call super::destroy
...
type dw_report from w_rpt_list`dw_report within w_al741_kardex_trans
...
type dw_1 from w_rpt_list`dw_1 within w_al741_kardex_trans
integer x = 32
integer y = 400
integer width = 1669
integer height = 1416
...
type pb_1 from w_rpt_list`pb_1 within w_al741_kardex_trans
integer x = 1728
integer y = 876
...
type pb_2 from w_rpt_list`pb_2 within w_al741_kardex_trans
integer x = 1728
integer y = 1044
...
type dw_2 from w_rpt_list`dw_2 within w_al741_kardex_trans
integer x = 1915
integer y = 400
integer width = 1669
integer height = 1416
...
type cb_report from w_rpt_list`cb_report within w_al741_kardex_trans
integer x = 3081
integer y = 112
integer width = 366
integer height = 84
...
//	create or replace procedure USP_ALM_KARDEX_DET_TRANS(
//		ad_del  in date,
//		ad_al   in date,
//		as_alm  in vale_mov.almacen%type
//	) is	
	DECLARE USP_ALM_KARDEX_DET_TRANS PROCEDURE FOR 
			USP_ALM_KARDEX_DET_TRANS(:ld_desde, :ld_hasta, :is_almacen);
			
  	EXECUTE USP_ALM_KARDEX_DET_TRANS;
	
	If sqlca.sqlcode = -1 then
		ls_mensaje = SQLCA.sqlerrtext
		ROLLBACK;
		messagebox("Error en USP_ALM_KARDEX_DET_TRANS", ls_mensaje)
		return 0
	end if
		
	close USP_ALM_KARDEX_DET_TRANS;
	
	dw_report.dataobject = 'd_rpt_kardex_art_det_trans_tbl'
else		

	//create or replace procedure USP_ALM_KARDEX_TRANS(
	//       ad_del in date, 
	//       ad_al  in date,
	//       as_alm in vale_mov.almacen%type
	//) is	
	 DECLARE USP_ALM_KARDEX_TRANS PROCEDURE FOR 
	 		USP_ALM_KARDEX_TRANS (:ld_desde, :ld_hasta, :is_almacen);
	execute USP_ALM_KARDEX_TRANS;
	
	If sqlca.sqlcode = -1  then
		ls_mensaje = SQLCA.sqlerrtext
		ROLLBACK;
		messagebox("Error en USP_ALM_KARDEX_TRANS", ls_mensaje)
		return 0
	end if
	
	close USP_ALM_KARDEX_TRANS;
	
	dw_report.dataobject = 'd_rpt_kardex_art_res_trans_tbl'
end if
