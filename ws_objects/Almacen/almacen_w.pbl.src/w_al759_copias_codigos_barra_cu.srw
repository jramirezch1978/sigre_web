$PBExportHeader$w_al759_copias_codigos_barra_cu.srw
forward
global type w_al759_copias_codigos_barra_cu from w_report_smpl
end type
type cb_1 from commandbutton within w_al759_copias_codigos_barra_cu
end type
type sle_almacen from singlelineedit within w_al759_copias_codigos_barra_cu
end type
type sle_descrip from singlelineedit within w_al759_copias_codigos_barra_cu
end type
type st_2 from statictext within w_al759_copias_codigos_barra_cu
end type
type cbx_almacen from checkbox within w_al759_copias_codigos_barra_cu
end type
type hpb_1 from hprogressbar within w_al759_copias_codigos_barra_cu
end type
type cbx_articulos from checkbox within w_al759_copias_codigos_barra_cu
end type
type cb_articulos from commandbutton within w_al759_copias_codigos_barra_cu
end type
type st_1 from statictext within w_al759_copias_codigos_barra_cu
end type
type em_copias from editmask within w_al759_copias_codigos_barra_cu
end type
type rb_1 from radiobutton within w_al759_copias_codigos_barra_cu
end type
type rb_2 from radiobutton within w_al759_copias_codigos_barra_cu
end type
type cb_2 from commandbutton within w_al759_copias_codigos_barra_cu
end type
type gb_1 from groupbox within w_al759_copias_codigos_barra_cu
end type
end forward
...
global w_al759_copias_codigos_barra_cu w_al759_copias_codigos_barra_cu
...
on w_al759_copias_codigos_barra_cu.create
int iCurrent
call super::create
...
on w_al759_copias_codigos_barra_cu.destroy
call super::destroy
...
event ue_retrieve;call super::ue_retrieve;String 	ls_almacen, LS_TIPO
...
if rb_1.checked then
	dw_report.dataobject = 'd_rpt_codigos_cu_pptt_lbl'
	ls_tipo = 'P'
else
	dw_report.dataobject = 'd_rpt_codigoqr_grande_lbl'
	ls_tipo = 'G'
end if

dw_report.settransobject(Sqlca)
gnvo_app.almacen.of_rpt_codigos_cu(dw_report, hpb_1, ls_almacen, ldc_nro_copias, ls_tipo)
...
try 
	...
	ldc_cantidad = gnvo_app.of_get_parametro_dec("NRO_COPIAS_CU_PPTT", 2.0)
...
type dw_report from w_report_smpl`dw_report within w_al759_copias_codigos_barra_cu
integer x = 0
integer y = 304
integer width = 3753
integer height = 988
string dataobject = "d_rpt_codigos_cu_pptt_lbl"
end type
...
lstr_param.dw1		= 'd_lista_articulos_almacen_cu_tbl'
...
long textcolor = 33554432
long backcolor = 67108864
string text = "CUs Pequeños"
boolean checked = true
end type

event clicked;DECIMAL ldc_cantidad
TRY
IF RB_1.CHecked THEN
	ldc_cantidad = gnvo_app.of_get_parametro_dec("NRO_COPIAS_CU_PPTT", 2.0)	
	em_copias.text = string(ldc_cantidad)
...
IF RB_2.CHecked THEN
	ldc_cantidad = gnvo_app.of_get_parametro_dec("NRO_COPIAS_CU_PPTT_GRANDES", 1.0)	
	em_copias.text = string(ldc_cantidad)
END IF
CATCH(EXCEPTION E)
	THROW E
...
fontfamily fontfamily = swiss!
string facename = "MS Sans Serif"
string text = "Genera Cus SKU"
end type

event clicked;integer ln_cantidad, ln_generados
boolean lb_flag
string ls_cod_art, ls_cu, ls_almacen, ls_cu_tmp

ls_almacen = upper(sle_almacen.text)

declare tmp_cu cursor for
select descripcion cu from TT_ALM_CURVAS;

declare temporal cursor for
select trim(cod_art) cod_art, cu, cantidad from tt_art;
ln_generados=0
lb_flag = false
...
FETCH temporal INTO :ls_cod_art, :ls_cu, :ln_cantidad;
	if SQLCA.sqlcode < 0 then 
		MessageBox("Fetch Error",SQLCA.sqlerrtext) 
	elseif SQLCA.sqlcode = 0 then 
		if isnull(ls_cu) OR TRIM(LS_CU)= '' then
			lb_flag= true
			DECLARE usp_alm_genera_parteIng_olds PROCEDURE FOR 
				usp_alm_genera_parteIng_olds( :ls_almacen, 
														 :ln_cantidad,
														 :ls_cod_art) ;											 
			EXECUTE usp_alm_genera_parteIng_olds ;
			
			DELETE FROM  TT_ART WHERE COD_ART = :ls_cod_art AND cu IS NULL;
			...
			DO WHILE SQLCA.sqlcode = 0 
			FETCH tmp_cu INTO :ls_cu_tmp;
				if SQLCA.sqlcode < 0 then 
					MessageBox("Fetch Error",SQLCA.sqlerrtext) 
				elseif SQLCA.sqlcode = 0 then 
					insert into tt_art (cod_art, cu, cantidad)
					values (:ls_cod_art, :ls_cu_tmp, 1);
					ln_generados = ln_generados +1
				end if;
			loop;
...
if lb_flag then
	f_mensaje ('Se generaron ' + string (ln_generados) + ' cus',''); 
else
	messagebox ('', 'No se generaron Cus');
end if
