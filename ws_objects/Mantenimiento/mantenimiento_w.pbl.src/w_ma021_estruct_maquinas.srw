$PBExportHeader$w_ma021_estruct_maquinas.srw
forward
global type w_ma021_estruct_maquinas from w_abc
end type
type cb_refrescar from commandbutton within w_ma021_estruct_maquinas
end type
type dw_3 from datawindow within w_ma021_estruct_maquinas
end type
type st_campo from statictext within w_ma021_estruct_maquinas
end type
type cb_colapse from commandbutton within w_ma021_estruct_maquinas
end type
type cb_expand from commandbutton within w_ma021_estruct_maquinas
end type
type tv_estructura from u_tv_estructura within w_ma021_estruct_maquinas
end type
type dw_list from u_dw_list_tbl within w_ma021_estruct_maquinas
end type
end forward
...
integer width = 3502
integer height = 1776
string title = "Estructura de Máquinas (MA021)"
...
global w_ma021_estruct_maquinas w_ma021_estruct_maquinas
...
on w_ma021_estruct_maquinas.create
int iCurrent
call super::create
...
on w_ma021_estruct_maquinas.destroy
call super::destroy
...
if tv_estructura.FindItem (ChildTreeItem!, ll_handle_hijo) <> -1 then 
	...
	if MessageBox('Aviso', 'Desea usted hacer una eliminacion en cascada ?', &
		...
		for ll_i = 1 to lds_data.RowCount()
			ls_padre = lds_data.object.maq_padre[ll_i]
			ls_hijo 	= lds_data.object.maq_hijo	[ll_i]
...
If source = THIS THEN 
	...
	from maq_estructura
	where maq_padre 	= :ls_padre
	  and maq_hijo		= :is_cod_maq;
	
	if ll_count > 0 then
		ROLLBACK;
		MessageBox('Error', 'Codigo de maquina hijo ya pertenece al Padre')
		return
	end if

	update maq_estructura
	   set maq_padre = :ls_padre
    where maq_padre = :is_old_padre
	   and maq_hijo  = :ls_hijo;
	
	if SQLCA.SQlCode < 0 then
		ls_mensaje = SQLCA.SQLErrText
		ROLLBACK;
		MessageBox('Error al momento de mover una maquina', ls_mensaje)
		RETURN
	end if
	...
	select count(*)
		into :ll_count
	from maq_estructura t
	START WITH maq_padre = :is_padre
	...
	INSERT INTO MAQ_ESTRUCTURA ( 
		MAQ_PADRE, MAQ_HIJO, CANTIDAD, Flag_replicacion )  
	VALUES ( 
...
ls_cod_maq 		= trim(ids_Data.Object.maq_hijo		[ai_Row])
ls_desc			= trim(ids_Data.Object.desc_maq		[ai_Row])
li_cantidad		= Int(ids_Data.Object.cantidad		[ai_Row])
li_icono			= Int(ids_Data.Object.icono			[ai_Row])
...
string dataobject = "d_maquinas_tbl"
end type
...
if row > 0 then
	is_cod_maq 	= this.object.cod_maquina  [row]
	is_desc_maq = this.object.desc_maq 		[row]
	il_DragSource = 0
	il_DragParent = 0
	il_dropTarget = 0
else
	MessageBox('Error', 'No existen maquinas')
end if
...
If row = 0 then 
	...
	is_padre 	= trim(this.object.cod_maquina 	[row])
	is_desc_maq = trim(this.object.desc_maq 		[row])
	il_icono		= Long(this.object.icono			[row])
	
	tv_estructura.of_clear()
	tv_estructura.EVENT ue_populate(is_padre, is_desc_maq)
end if
