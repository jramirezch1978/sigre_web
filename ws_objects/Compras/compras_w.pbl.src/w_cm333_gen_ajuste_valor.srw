$PBExportHeader$w_cm333_gen_ajuste_valor.srw
forward
global type w_cm333_gen_ajuste_valor from w_abc
end type
type cb_1 from commandbutton within w_cm333_gen_ajuste_valor
end type
type cbx_1 from checkbox within w_cm333_gen_ajuste_valor
end type
type uo_fecha from u_ingreso_rango_fechas within w_cm333_gen_ajuste_valor
end type
type st_3 from statictext within w_cm333_gen_ajuste_valor
end type
type st_2 from statictext within w_cm333_gen_ajuste_valor
end type
type dw_oc_det from u_dw_abc within w_cm333_gen_ajuste_valor
end type
type dw_detail from u_dw_abc within w_cm333_gen_ajuste_valor
end type
type dw_master from u_dw_abc within w_cm333_gen_ajuste_valor
end type
end forward
...
integer width = 2839
integer height = 2916
string title = "Generación de Mov de Ajuste (CM933)"
...
global w_cm333_gen_ajuste_valor w_cm333_gen_ajuste_valor

type variables
string is_doc_oc, is_salir
m_generar_ajuste im_p1
...
event ue_generar();Long 		ll_row, ll_item_os
...
Do While ll_row <> 0
	...
	//create or replace procedure USP_CMP_GEN_MOV_AJUSTE(
	...
	DECLARE USP_CMP_GEN_MOV_AJUSTE PROCEDURE FOR
		USP_CMP_GEN_MOV_AJUSTE( :ls_org_os, 
									 	:ls_nro_os, 
									 	:ll_item_os,
									 	:gs_user);
	
	EXECUTE USP_CMP_GEN_MOV_AJUSTE;
	
	IF SQLCA.sqlcode = -1 THEN
		ls_mensaje = "PROCEDURE USP_CMP_GEN_MOV_AJUSTE:" &
				  + SQLCA.SQLErrText
		Rollback;
...
on w_cm333_gen_ajuste_valor.create
int iCurrent
call super::create
...
on w_cm333_gen_ajuste_valor.destroy
call super::destroy
...
event ue_anular;call super::ue_anular;Long 		ll_row, ll_item_os
string	ls_nro_os, ls_org_os, ls_mensaje
...
Do While ll_row <> 0
	...
	//create or replace procedure USP_CMP_ANULAR_MOV_AJUSTE(
	//       asi_org_os    in orden_servicio_det.cod_origen%TYPE,
	//       asi_nro_os    in orden_servicio_det.nro_os%TYPE,
	//       ani_item_os   in orden_servicio_det.nro_item%TYPE
	//) is

	DECLARE USP_CMP_ANULAR_MOV_AJUSTE PROCEDURE FOR
		USP_CMP_ANULAR_MOV_AJUSTE( :ls_org_os, 
									 		:ls_nro_os, 
									 		:ll_item_os);
	
	EXECUTE USP_CMP_ANULAR_MOV_AJUSTE;
	
	IF SQLCA.sqlcode = -1 THEN
		ls_mensaje = "PROCEDURE USP_CMP_ANULAR_MOV_AJUSTE:" &
				  + SQLCA.SQLErrText
		Rollback;
		MessageBox('Aviso', ls_mensaje)
		Return
	...
	CLOSE USP_CMP_ANULAR_MOV_AJUSTE;
...
long textcolor = 33554432
long backcolor = 67108864
string text = "Mov de Almacen Generados"
boolean focusrectangle = false
end type
...
integer height = 724
integer taborder = 30
string dataobject = "d_abc_am_os_det_tbl"
...
integer height = 724
integer taborder = 20
string dataobject = "d_abc_asign_os_oc_det2_tbl"
boolean hscrollbar = true
boolean vscrollbar = true
borderstyle borderstyle = styleraised!
end type
...
integer width = 2565
integer height = 668
string dataobject = "d_abc_asign_os_oc_cab2_tbl"
boolean hscrollbar = true
boolean vscrollbar = true
end type
