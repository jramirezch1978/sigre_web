...
type dw_datos_transporte from u_dw_abc within tabpage_22
end type
type tabpage_22 from userobject within tab_1
dw_datos_transporte dw_datos_transporte
end type
...
type shl_1 from statichyperlink within w_cm002_proveedores
end type
type shl_buscar_ruc from statichyperlink within w_cm002_proveedores
end type
end forward
...
integer width = 5486
integer height = 3000
string title = "Proveedores [CM002]"
...
u_dw_abc  	idw_nave     , idw_direccion, idw_telefonos, &
				...
				idw_basc_subcontra, idw_lineas_credito, idw_datos_transporte
...
if is_Action = 'new' then
	...
	for ll_i=1 to idw_datos_transporte.RowCount()
		idw_datos_transporte.object.proveedor [ll_i] = ls_nro
	next
...
idw_datos_transporte = tab_1.tabpage_22.dw_datos_transporte
end subroutine
...
idw_datos_transporte.retrieve(as_proveedor)
...
idw_datos_transporte.ii_protect = 0
...
idw_datos_transporte.of_protect()
...
idw_datos_transporte.ResetUpdate()
...
this.shl_buscar_ruc=create shl_buscar_ruc
iCurrent=UpperBound(this.Control)
this.Control[iCurrent+1]=this.st_1
this.Control[iCurrent+2]=this.shl_1
this.Control[iCurrent+3]=this.shl_buscar_ruc
end on
...
tab_1.tabpage_14.tab_2.tabpage_15.dw_6.enabled = true

ib_log = TRUE			

//m_mantto_smpl.m_reportes.m_basc.m_formatodevisistaacliente.toolbaritemvisible = true
//m_mantto_smpl.m_reportes.m_basc.m_formatodevisistaacliente.toolbaritemname = 'Report!'
//v.m_reportes.m_basc.m_formatodevisistaacliente.toolbaritemorder = 990

m_mantto_smpl.m_file.m_printer.m_print1.enabled = true
m_mantto_smpl.m_file.m_printer.m_print1.toolbaritemvisible = true
m_mantto_smpl.m_file.m_printer.m_print1.toolbaritemorder = 980
...
IF li_share_status <> 1 THEN
	messagebox("Error en Sharedata","Error al compartir con DW6",exclamation!)
	RETURN
END IF
end event
...
idw_datos_transporte.AcceptText()
...
if not gnvo_app.of_row_Processing( idw_datos_transporte ) then return 
...
idw_datos_transporte.of_Set_flag_replicacion()
...
try 
	...
	IF ib_log THEN
		...
		idw_datos_transporte.of_create_log()
	END IF
	...
	IF idw_datos_transporte.ii_update = 1 THEN
		IF idw_datos_transporte.Update(true, false) = -1 then		// Grabacion del detalle
			lbo_ok = FALSE
			messagebox("Error en Grabacion dw_datos_transporte","Se ha procedido al rollback",exclamation!)
		END IF
	END IF
	...
	IF ib_log THEN
		...
		IF lbo_ok THEN
			lbo_ok = idw_datos_transporte.of_save_log()
		END IF
	...
	IF lbo_ok THEN
		...
		idw_basc_subcontra.ii_update = 0
		idw_basc_progvis.ii_update = 0
		idw_datos_transporte.ii_update = 0
...
idw_datos_transporte.Width = tab_1.tabpage_22.width - idw_datos_transporte.x - 20
idw_datos_transporte.height= tab_1.tabpage_22.height - idw_datos_transporte.y - 20
...
this.object.flag_personeria	[al_row] = 'N'
...
event dw_1::doubleclicked;call super::doubleclicked;string ls_columna, ls_string, ls_evaluate
...
event dw_1::ue_display;call super::ue_display;boolean lb_ret
string ls_codigo, ls_data, ls_sql
choose case lower(as_columna)
	case "cod_entidad_autorizada"
		ls_sql = "select t.codigo as codigo, " &
				 + "       t.descripcion as descripcion " &
				 + "from SUNAT_CATALOGO_D37 t " &
				 + "where t.flag_estado = '1'"
				 
		lb_ret = gnvo_app.of_lista(ls_sql, ls_codigo, ls_data, '1')
			
		if ls_codigo <> '' then
			this.object.cod_entidad_autorizada	[al_row] = ls_codigo
			this.object.desc_entidad_autorizada	[al_row] = ls_data
			this.ii_update = 1
		end if
end choose
end event
...
string text = "Vehiculos~r~nTransporte"
long tabtextcolor = 33554432
long tabbackcolor = 12632256
long picturemaskcolor = 536870912
string powertiptext = "Inserte los vehiculos de transporte que tiene el proveedor"
dw_datos_transporte dw_datos_transporte
end type

on tabpage_22.create
this.dw_datos_transporte=create dw_datos_transporte
this.Control[]={this.dw_datos_transporte}
end on

on tabpage_22.destroy
destroy(this.dw_datos_transporte)
end on
...
integer height = 1444
boolean bringtotop = true
string dataobject = "d_abc_proveedor_transporte_tbl"
borderstyle borderstyle = styleraised!
end type
...
Choose CASE dwo.name
	case "tipo_carroceria"
		...
		select t.desc_carroceria
		  into :ls_data
		from tg_tipo_carroceria t 
		where tipo_carroceria = :data 
		  and t.flag_estado = '1';

		// Verifica que articulo solo sea de reposicion		
		if SQLCA.SQLCode = 100 then
			this.object.tipo_carroceria	[row] = gnvo_app.is_null
			this.object.desc_carroceria	[row] = gnvo_app.is_null
			MessageBox('Error', 'TIPO DE CARROCERIA no existe o no esta activo, por favor verifique', StopSign!)
			return 1
		end if

		this.object.desc_carroceria			[row] = ls_data	
	...
	case "color"
		...
		if SQLCA.SQLCode = 100 then
			...
			MessageBox('Error', 'CODIGO DE COLOR no existe o no esta activo, por favor verifique', StopSign!)
			return 1
		end if

		this.object.desc_color			[row] = ls_data
...
event ue_insert_pre;call super::ue_insert_pre;this.object.proveedor		[al_row] = dw_master.Object.proveedor[1]
this.object.flag_estado 	[al_row] = '1'
...
choose case lower(as_columna)
	case "tipo_carroceria"
		ls_sql = "select t.tipo_carroceria as tipo_carroceria, " &
				 + "t.desc_carroceria as descripcion_carroceria " &
				 + "  from tg_tipo_carroceria t " &
				 + " where t.flag_estado = '1'"
		...
		if ls_codigo <> '' then
			this.object.tipo_carroceria	[al_row] = ls_codigo
...
datetime value = DateTime(Date("2025-10-08"), Time("17:22:36.000000"))
integer textsize = -8
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = swiss!
...
boolean enabled = false
string text = "CODIGO DE RELACION (PROVEEDORES / CLIENTES)"
...
long textcolor = 134217856
long backcolor = 67108864
string text = "Buscar RUC en SUNAT >>"
boolean focusrectangle = false
end type
