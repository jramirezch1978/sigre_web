...
String 	is_col = '', is_tipo, is_type, is_oper_ing_oc, is_doc_sc, &
			...
			is_comprador, is_nro_cotizacion, is_flag, is_FLAG_CARGA_OC_PRESUP
...
public function integer of_opcion21 ()
end prototypes
...
string 	ls_descripcion, ls_obs, ls_moneda_org, ls_moneda_dst, ls_mensaje
decimal	ldc_importe
Date		ld_fecha
u_dw_abc ldw_master, ldw_detail
...
ls_moneda_dst = ldw_master.object.cod_moneda[ll_row_mas]
ld_fecha  = Date(ldw_master.object.fec_registro[ll_row_mas])
...
For ll_j = 1 to dw_2.RowCount()
	...
	if ll_row > 0 then	
		...
		//Ahora el importe aprobado
		ldc_importe = Dec(dw_2.object.importe_aprobado[ll_j])
		ls_moneda_org = dw_2.object.cod_moneda[ll_j]
		
		select usf_fl_conv_mon(:ldc_importe, :ls_moneda_org, :ls_moneda_dst, :ld_fecha)
		  into :ldc_importe
		from dual;		  
		...
		if SQLCA.SQLCode < 0 then
			...
			ROLLBACK;
			gnvo_app.of_mensaje_error("error en la conversion de moneda. Mensaje : " + ls_mensaje)
			return 0
		end if
		
		ldw_detail.object.importe_aprobado	[ll_row] = ldc_importe
		ldw_detail.object.importe				[ll_row] = ldc_importe
		ldw_detail.object.cod_moneda			[ll_row] = ls_moneda_dst
...
public function integer of_get_param ();select oper_ing_oc, doc_sc
	into :is_oper_ing_oc, :is_doc_sc
from logparam
where reckey = '1';
...
Decimal 	ldc_precio, ldc_tasa_impuesto1, ldc_tasa_impuesto2, ldc_impuesto1, ldc_impuesto2, ldc_cant_proyect
u_dw_abc ldw_master, ldw_detail
...
For ll_j = 1 to dw_2.RowCount()
	...
	if ll_row > 0 then	
		...
		if is_FLAG_CARGA_OC_PRESUP = '1' then
			//Si esta activo el flag entonces coloco el Fondo de COmpras
			//deacuerdo al OT_ADM
			ldw_detail.object.cencos			[ll_row] = dw_2.object.cencos			[ll_j]
			ldw_detail.object.cnta_prsp		[ll_row] = dw_2.object.cnta_prsp		[ll_j]
		...
		lw_1.function dynamic of_update_imp1 (ll_row)
		lw_1.function dynamic of_update_imp2 (ll_row)
		lw_1.function dynamic of_precio_total(ll_row)
...
Long 		ll_find
String	ls_cadena, ls_req, ls_Cod_art, ls_almacen
Decimal	ldc_saldo_almacen
u_dw_abc ldw_master, ldw_detail
...
For ll_j = 1 to dw_2.RowCount()
	...
	if ll_row > 0 then	
		...
		if ldw_detail.of_ExisteCampo("saldo_almacen") then
			ls_cod_art 	= dw_2.object.cod_art			[ll_j]
			ls_almacen	= dw_2.object.almacen			[ll_j]
			
			select nvl(sum(aa.sldo_total),0)
				into :ldc_saldo_almacen
			from articulo_almacen aa
			where cod_Art = :ls_cod_art
			  and almacen = :ls_almacen;
			 
			ldw_detail.object.saldo_almacen [ll_row] = ldc_saldo_almacen
...
public function integer of_opcion21 ();Long 		ll_row_mas, ll_row, ll_j, ll_count
Integer	li_year
Date 		ld_fec_reg
string 	ls_cod_art, ls_proveedor, ls_almacen, ls_moneda, ls_ot_adm, &
			ls_cencos, ls_cnta_prsp, ls_null
...
ls_proveedor 	= ldw_master.object.proveedor				[1]
ld_fec_reg 		= Date(ldw_master.object.fec_registro	[1])
ls_moneda		= ldw_master.object.cod_moneda			[1]
...
For ll_j = 1 to dw_2.RowCount()
	...
	if ll_row > 0 then	
		...
		if SQLCA.SQlCode = -1 then
			ROLLBACK;
			MessageBox('Error en funcion usf_cmp_prec_compra_artic', SQLCA.SQlErrText, StopSign!)
			return 0
		end if
		
		if IsNull(ldc_precio) then ldc_precio = 0
		
		ldw_detail.object.fec_proyect		[ll_row] = dw_2.object.fec_emision		[ll_j]
		ldw_detail.object.cod_art			[ll_row] = dw_2.object.cod_art			[ll_j]
		ldw_detail.object.desc_art			[ll_row] = dw_2.object.desc_art			[ll_j]
		ldw_detail.object.und				[ll_row] = dw_2.object.und					[ll_j]
		
		ldw_detail.object.cant_proyect	[ll_row] = dw_2.object.pend_compra		[ll_j]
		ldw_detail.object.cant_pendiente	[ll_row] = dw_2.object.pend_compra		[ll_j]
		ldw_detail.object.almacen			[ll_row] = dw_2.object.almacen			[ll_j]
		ldw_detail.object.precio_unit		[ll_row] = ldc_precio
		ldw_detail.object.dias_reposicion[ll_row] = 7
		ldw_detail.object.dias_rep_import[ll_row] = 7
		ldw_detail.object.cod_usr			[ll_row] = gs_user
		ldw_detail.object.saldo_almacen	[ll_row] = dw_2.object.saldo_almacen	[ll_j]
		ldw_detail.object.flag_almacen	[ll_row] = ls_null
		ldw_detail.object.centro_benef	[ll_row] = dw_2.object.centro_benef		[ll_j]
		
		ldw_detail.object.nro_proforma	[ll_row] = dw_2.object.nro_proforma		[ll_j]
		ldw_detail.object.item_proforma	[ll_row] = dw_2.object.nro_item			[ll_j]
	...
	CHOOSE CASE is_tipo
		...
		CASE 'NRO_DOC'
		   ...
		   is_nro_doc	  	 			= ist_datos.string3
			...
			is_FLAG_CARGA_OC_PRESUP = ist_datos.FLAG_CARGA_OC_PRESUP
...
string	ls_sol_cmp, ls_origen, ls_cod_art, ls_oper_sec, &
			ls_nro_mov, ls_nro_programa, ls_org_amp, ls_col
...
choose case ist_datos.opcion
	...
	case 11  //Orden de servicios a partir de Orden de Trabajo
		...
		if ll_find > 0 then
			...
			MessageBox('Aviso', "El OperSec " + ls_oper_sec + " ya ha sido seleccionada", Information!)
			ib_process = false
			return
		end if 		
...
FOR li_x = 1 to ll_count
	ls_col = this.describe(string(li_x) + ".Name")
...
Choose case ist_datos.opcion
	...
	CASE 10 // Orden de trabajo
		...
		For j = 1 to dw_2.RowCount()
			if ist_datos.dw_d.triggerevent ("ue_insert") > 0 then	
				...
				ist_datos.dw_d.object.cod_origen		[j] 		 = gs_origen
	...
	CASE 21  //Proforma de Ventas
		if of_opcion21() = 0 then return
...
if ist_datos.opcion = 3 then
	
	dw_1.Retrieve(this.object.grupo[al_row])
	return
	
elseif ist_datos.opcion = 21 then
	
	dw_1.retrieve( Date(this.object.fec_emision[al_row]), this.object.cod_moneda[al_row])
	return
 ...
 if ib_sel = false then
	if al_row > 0 then
		// Verifica que registro no este anulado
		if this.of_existeCampo('flag_estado') then
			if this.object.flag_estado[al_row] = '0' then
				Messagebox( "Atencion", "Registro esta anulado", Exclamation!)
				Return
			end if
		end if
		...
		Choose case ist_datos.opcion 
			...
			Case 20 // Ordenes de Servicio con flag de Acta de conformidad
				dw_1.retrieve( this.object.nro_os[al_row])
