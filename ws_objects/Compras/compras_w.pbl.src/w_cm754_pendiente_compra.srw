$PBExportHeader$w_cm754_pendiente_compra.srw
forward
global type w_cm754_pendiente_compra from w_report_smpl
end type
type cb_lectura from commandbutton within w_cm754_pendiente_compra
end type
type uo_fecha from u_ingreso_rango_fechas within w_cm754_pendiente_compra
end type
type st_1 from statictext within w_cm754_pendiente_compra
end type
type st_registros from statictext within w_cm754_pendiente_compra
end type
type rb_tipo_art_todos from radiobutton within w_cm754_pendiente_compra
end type
type rb_tipo_art_pedido from radiobutton within w_cm754_pendiente_compra
end type
type rb_tipo_art_reposicion from radiobutton within w_cm754_pendiente_compra
end type
type em_ot_adm from singlelineedit within w_cm754_pendiente_compra
end type
type em_descripcion from singlelineedit within w_cm754_pendiente_compra
end type
type cbx_todos from checkbox within w_cm754_pendiente_compra
end type
type sle_almacen from singlelineedit within w_cm754_pendiente_compra
end type
type sle_descrip from singlelineedit within w_cm754_pendiente_compra
end type
type cbx_todos_ot_adm from checkbox within w_cm754_pendiente_compra
end type
type gb_1 from groupbox within w_cm754_pendiente_compra
end type
type gb_tipo_art from groupbox within w_cm754_pendiente_compra
end type
end forward
...
integer width = 5285
integer height = 2152
string title = "Req de Articulos Pendiente de Compra (CM754)"
...
cbx_todos_ot_adm cbx_todos_ot_adm
gb_1 gb_1
gb_tipo_art gb_tipo_art
end type
global w_cm754_pendiente_compra w_cm754_pendiente_compra
...
on w_cm754_pendiente_compra.create
int iCurrent
call super::create
...
this.cbx_todos_ot_adm=create cbx_todos_ot_adm
...
this.Control[iCurrent+13]=this.cbx_todos_ot_adm
this.Control[iCurrent+14]=this.gb_1
this.Control[iCurrent+15]=this.gb_tipo_art
end on

on w_cm754_pendiente_compra.destroy
call super::destroy
...
String	ls_st_origen, ls_st_tipo_art, ls_alm_text, ls_ot_adm
...
// Determinar Tipo de Almacen
IF cbx_todos.checked THEN
	ls_almacen = '%%'
	ls_alm_text = 'Todos los Almacenes'
ELSE
	ls_almacen = trim(sle_almacen.text) + '%'
	ls_alm_text = sle_descrip.text
END IF


if cbx_todos_ot_adm.checked = true then
	ls_ot_adm = '%%'
else
	if trim(em_ot_adm.text) = '' then
		MessageBox('Compras', 'Debe indicar una OT_ADM',StopSign!)
		return
	end if
	ls_ot_adm = trim(em_ot_adm.text) + '%'
end if

idw_1.Retrieve(is_doc_ot, is_doc_oc, is_oper_cons, uo_fecha.of_get_fecha1(), uo_fecha.of_get_fecha2(), ls_almacen, ls_tipo_art, ls_ot_adm) 
...
idw_1.object.t_subtitulo_2.text =  ls_alm_text + '  ' + 	ls_st_tipo_art	
...
type dw_report from w_report_smpl`dw_report within w_cm754_pendiente_compra
...
integer width = 2807
integer height = 1272
string dataobject = "d_art_mov_proy_pend_comp_sug_log_tbl"
end type
...
ls_sql = "SELECT O.OT_ADM as COD_OT_ADM, O.DESCRIPCION AS DESCRIPCION " &
