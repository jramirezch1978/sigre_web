...
integer width = 4169
integer height = 5628
string title = "Programa de Compras [CM305]"
...
public subroutine of_saldo_almacen (long al_row)
end prototypes
...
if is_Action = 'new' then
   ...
   ls_ult_nro = TRIM(gs_origen) + f_llena_caracteres('0',String(ll_ult_nro),ll_long) 		
...
if SQLCA.SQLCode < 0 then
	ls_mensaje = SQLCA.SQLErrText
	ROLLBACK;
	MessageBox('Error', 'Error al actualizar cant_ingresada. Mensaje: ' + ls_mensaje, StopSign!)
	return
end if
...
public subroutine of_saldo_almacen (long al_row);String	ls_cod_art, ls_almacen
decimal	ldc_saldo_total

ls_cod_art 	= dw_detail.object.cod_art [al_row]
ls_almacen	= dw_detail.object.almacen	[al_row]
...
dw_detail.object.saldo_Almacen [al_row] = ldc_saldo_total
end subroutine
...
CHOOSE CASE dwo.name
	CASE 'cencos'
		...
		if SQLCA.SQLCode = 100 then
			...
			MessageBox('Error', 'Codigo de centros de Costo no existe o no esta activo, por favor verifique')
			return 1
		end if
...
this.object.fec_requerida		[al_row] = Date(ldt_now)
this.object.flag_estado			[al_row] = '1'
...
this.object.cant_ingresada		[al_row] = 0
this.object.comprador			[al_row] = gs_user
...
choose case lower(as_columna)
	case "cod_art"
		...
		if lstr_articulo.b_Return then
			...
			this.object.comprador			[al_row] = gs_user
			this.object.almacen				[al_row] = of_set_almacen_tacito(lstr_articulo.cod_art)
			
			of_saldo_almacen(al_row)
			this.ii_update = 1
...
event getfocus;call super::getfocus;if not gnvo_app.of_row_processing( dw_master ) then dw_master.SetFocus()
