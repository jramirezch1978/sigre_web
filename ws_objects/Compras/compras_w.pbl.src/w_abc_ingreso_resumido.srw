...
public function boolean of_procesar (long al_row);string 	ls_cod_marca, ls_cod_sub_linea, ls_cod_suela, ls_cod_acabado, ls_color1, &
			...
			ls_und, ls_und_art, ls_cod_sku, ls_cod_Clase, ls_flag_nuevo
...
decimal 	ldc_talla_min, ldc_talla_max, ldc_talla, ldc_incremento, ldc_precio_vta_unidad, &
			ldc_precio_compra_old, ldc_precio_compra_new
...
public function boolean of_sugerencias (long al_row);string 	ls_cod_marca, ls_cod_sub_linea, ls_estilo, ls_cod_sub_cat
...
ids_sugerencias.Retrieve(ls_cod_sub_cat, ls_cod_marca, ls_cod_sub_linea, ls_estilo)
...
public function boolean of_sub_linea (long al_row);String 	ls_cod_sub_linea, ls_desc_sub_linea, ls_cod_linea
...
if not gnvo_app.of_prompt_string("Indique la nueva Sub Línea", ls_desc_sub_linea) then return false
...
if ll_count > 0 then
	MessageBox('Error', 'la Sub Linea ' + ls_desc_sub_linea + ' ya existe, por favor verifique!', StopSign!)
	return false
end if
...
insert into zc_sub_linea(cod_linea, cod_sub_linea, desc_sub_linea, flag_estado)
values(:ls_cod_linea, :ls_cod_sub_linea, :ls_desc_sub_linea, '1');
...
dw_master.object.desc_sub_linea	[1] = ls_desc_sub_linea
...
this.Control[iCurrent+1]=this.shl_sugerencias
...
Long 				ll_rows, ll_i, ll_ult_nro, ll_count, ll_row
Decimal			ldc_Cantidad, ldc_precio_compra_new, ldc_base, ldc_igv, ldc_talla, &
...
for ll_i = 1 to dw_detail.RowCount()
	...
	if ldc_cantidad > 0 then
		...
		//Ahora inserto en el detalle de la orden de compra
		ll_row = idw_detail.event ue_insert()
		if ll_row > 0 then
			
			//Obtengo el almacen tacito
			select almacen
				into :ls_almacen
			from almacen_tacito
			...
			if SQLCA.SQLCode = 100 then
				//Busco el primer almacen activo
				select almacen
					into :ls_almacen
					from almacen
				where flag_estado = '1'
			...
			ldc_base = ldc_precio_compra_new / 1.18
			ldc_igv	= ldc_base * 0.18
			
			idw_detail.object.cod_art			[ll_row] = ls_cod_art
			idw_detail.object.desc_Art			[ll_row] = ls_Desc_art
			idw_detail.object.cod_sku			[ll_row] = ls_cod_sku
			idw_detail.object.almacen			[ll_row] = ls_almacen
			idw_detail.object.fec_proyect		[ll_row] = Date(gnvo_app.of_Fecha_actual())
			idw_detail.object.cant_proyect	[ll_row] = ldc_cantidad	
			idw_detail.object.precio_unit		[ll_row] = ldc_base
			idw_detail.object.tipo_impuesto1	[ll_row] = gnvo_app.finparam.is_igv
			idw_detail.object.impuesto1		[ll_row] = ldc_igv * ldc_cantidad
		end if
...
try 
	...
	if idw_master.RowCount() = 0 then
		MessageBox('Error', 'La orden de compra debe tener una cabecera por favor verifique!', StopSign!)
	...
	if IsNull(is_proveedor) or trim(is_proveedor) = '' then
		MessageBox('Error', 'La orden de compra debe tener un proveedor, por favor verifique!', StopSign!)
		this.event post close()
		return
	end if
...
lstr_param.string3 = ls_cod_sub_linea
lstr_param.string4 = ls_estilo
