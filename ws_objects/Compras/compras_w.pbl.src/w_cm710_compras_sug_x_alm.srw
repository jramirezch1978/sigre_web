$PBExportHeader$w_cm710_compras_sug_x_alm.srw
forward
global type w_cm710_compras_sug_x_alm from w_rpt
end type
type rb_1 from radiobutton within w_cm710_compras_sug_x_alm
end type
type rb_2 from radiobutton within w_cm710_compras_sug_x_alm
end type
type rb_3 from radiobutton within w_cm710_compras_sug_x_alm
end type
type st_2 from statictext within w_cm710_compras_sug_x_alm
end type
type st_almacen from statictext within w_cm710_compras_sug_x_alm
end type
type sle_almacen from singlelineedit within w_cm710_compras_sug_x_alm
end type
type rb_res from radiobutton within w_cm710_compras_sug_x_alm
end type
type rb_det from radiobutton within w_cm710_compras_sug_x_alm
end type
type dw_report from u_dw_rpt within w_cm710_compras_sug_x_alm
end type
type cb_1 from commandbutton within w_cm710_compras_sug_x_alm
end type
type uo_fecha from u_ingreso_fecha within w_cm710_compras_sug_x_alm
end type
type gb_1 from groupbox within w_cm710_compras_sug_x_alm
end type
end forward
...
global w_cm710_compras_sug_x_alm w_cm710_compras_sug_x_alm

on w_cm710_compras_sug_x_alm.create
int iCurrent
call super::create
...
this.Control[iCurrent+5]=this.st_almacen
this.Control[iCurrent+6]=this.sle_almacen
this.Control[iCurrent+7]=this.rb_res
this.Control[iCurrent+8]=this.rb_det
...
on w_cm710_compras_sug_x_alm.destroy
call super::destroy
...
string	ls_almacen, ls_mensaje, ls_est_amp
...
IF rb_det.checked = true then
	...
	IF SQLCA.SQLCODE = -1 THEN
		...
		ROLLBACK;
		Messagebox( "Error USP_CMP_SUGERIDAS_DET_X_ALM", ls_mensaje)
		Return
	END IF
	CLOSE USP_PROC_D;
...
ELSEIF rb_res.checked = true then
	...
	DECLARE USP_PROC_R PROCEDURE FOR 
    		USP_CMP_SUGERIDAS_RES_X_ALM( :ld_fecha, 
			 									  :gs_origen, 
												  :ls_almacen,
												  :ls_est_amp);
	EXECUTE USP_PROC_R;
	...
	IF SQLCA.SQLCODE = -1 THEN
		ls_mensaje = SQLCA.SQLErrText
		ROLLBACK;
		Messagebox( "Error USP_CMP_SUGERIDAS_RES_X_ALM", ls_mensaje)
		Return
	END IF
	CLOSE USP_PROC_R;
END IF
...
idw_1.object.t_almacen.text = ls_almacen + ': ' + st_almacen.text
