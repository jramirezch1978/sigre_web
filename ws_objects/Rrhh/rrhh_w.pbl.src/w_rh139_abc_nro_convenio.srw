$PBExportHeader$w_rh139_abc_nro_convenio.srw
forward
global type w_rh139_abc_nro_convenio from w_abc_master_smpl
end type
end forward
...
integer width = 1783
integer height = 664
string title = "(RH139) Ultimo Número de Convenio"
string menuname = "m_master_simple"
long backcolor = 67108864
end type
global w_rh139_abc_nro_convenio w_rh139_abc_nro_convenio

on w_rh139_abc_nro_convenio.create
call super::create
if this.MenuName = "m_master_simple" then this.MenuID = create m_master_simple
end on

on w_rh139_abc_nro_convenio.destroy
call super::destroy
...
event ue_open_pre;call super::ue_open_pre;//  Centra la ventana dentro de la pantalla
...
integer ln_nro_convenio, li_row
...
if li_row > 0 then
	...
	if isnull(ls_llave) or ls_llave = ' ' then
		dw_master.ii_update = 0
		messagebox("Validación","Ingrese llave del registro")
		dw_master.SetColumn("cod_origen")
		dw_master.SetFocus()
		return
	end if
	ln_nro_convenio = dw_master.GetItemNumber(li_row,"ult_nro")
	if isnull(ln_nro_convenio) then
		dw_master.ii_update = 0
...
type dw_master from w_abc_master_smpl`dw_master within w_rh139_abc_nro_convenio
integer x = 59
integer y = 52
integer width = 1623
integer height = 376
string dataobject = "d_nro_convenio_ff"
...
choose case dwo.name 
	case 'cod_origen'
		...
		if ls_reckey = ' ' then
			messagebox("Validación","Ingrese llave primaria del registro")
			dw_master.SetColumn("cod_origen")
			dw_master.SetFocus()
			return 1
		end if 
