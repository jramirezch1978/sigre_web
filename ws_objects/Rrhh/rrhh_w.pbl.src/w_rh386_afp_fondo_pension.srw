$PBExportHeader$w_rh386_afp_fondo_pension.srw
$PBExportComments$abc Maestro detalle con pop window para la busqueda del maestro, ff para el Maestro, tbl para el detalle
forward
global type w_rh386_afp_fondo_pension from w_abc
end type
type cb_3 from commandbutton within w_rh386_afp_fondo_pension
end type
type rb_regenera from radiobutton within w_rh386_afp_fondo_pension
end type
type rb_recupera from radiobutton within w_rh386_afp_fondo_pension
end type
type sle_desc_origen from singlelineedit within w_rh386_afp_fondo_pension
end type
type sle_desc_afp from singlelineedit within w_rh386_afp_fondo_pension
end type
type em_afp from editmask within w_rh386_afp_fondo_pension
end type
type em_origen from editmask within w_rh386_afp_fondo_pension
end type
type em_ano from editmask within w_rh386_afp_fondo_pension
end type
type dw_master from u_dw_abc within w_rh386_afp_fondo_pension
end type
type dw_detail from u_dw_abc within w_rh386_afp_fondo_pension
end type
type st_3 from statictext within w_rh386_afp_fondo_pension
end type
type st_2 from statictext within w_rh386_afp_fondo_pension
end type
type st_1 from statictext within w_rh386_afp_fondo_pension
end type
type cb_1 from commandbutton within w_rh386_afp_fondo_pension
end type
type cb_2 from commandbutton within w_rh386_afp_fondo_pension
end type
type gb_1 from groupbox within w_rh386_afp_fondo_pension
end type
end forward
...
integer width = 3451
integer height = 2220
string title = "[RH386] Aportes y retencions de AFP - Fondo de pensiones"
...
global w_rh386_afp_fondo_pension w_rh386_afp_fondo_pension
...
IF as_flag_recupera = '1' THEN 
	...
	delete from rrhh_formato_afp r WHERE r.ano=:ai_ano AND r.cod_origen=:as_cod_origen AND 
													 r.cod_afp = :as_cod_afp and tipo_aporte_afp='1';
	// Inserta registros de historico
	INSERT INTO rrhh_formato_afp(ano, mes, cod_origen, cod_trabajador, concep, cod_afp, 
										  flag_manual, tipo_aporte_afp, valor_digitado_pension, 
										  valor_planilla_pension ) 
	SELECT to_number(to_char(hc.fec_calc_plan,'yyyy')), 
			 ...
			 hc.concep, 
			 hc.cod_afp, 
			 'A', '1', 0, Sum(hc.imp_soles)
	  FROM historico_calculo hc, maestro m
	 WHERE ( hc.cod_trabajador = m.cod_trabajador ) and
				( hc.concep in (select gc.concepto_gen from grupo_calculo gc
									where gc.grupo_calculo  in (select v.afp_jubilacion from rrhhparam_cconcep v where v.reckey = '1')) ) and
...
on w_rh386_afp_fondo_pension.create
int iCurrent
call super::create
...
on w_rh386_afp_fondo_pension.destroy
call super::destroy
...
integer taborder = 80
boolean bringtotop = true
string dataobject = "d_fondo_pensiones_afp_grd"
boolean vscrollbar = true
end type
...
this.Object.tipo_aporte_afp[al_row]='1'
this.Object.valor_planilla_pension[al_row]=0
