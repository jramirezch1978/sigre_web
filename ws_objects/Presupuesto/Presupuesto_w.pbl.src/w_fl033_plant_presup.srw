...
type cb_1 from commandbutton within w_fl033_plant_presup
end type
type sle_1 from singlelineedit within w_fl033_plant_presup
end type
type st_1 from statictext within w_fl033_plant_presup
end type
...
event ue_display(string as_columna, long al_row);boolean lb_ret
string 	ls_cnta_prsp, ls_data, ls_sql, &
			ls_plantilla, ls_cencos, ls_codigo
long ll_count
integer li_i
str_seleccionar lstr_seleccionar
...
choose case lower(as_columna)
	...
	case "cencos"
		...
		lb_ret = f_lista(ls_sql, ls_cencos, ls_data, '1')
...
this.object.flag_ingreso_egreso[al_row] = 'E'
...
choose case lower(dwo.name)
	...
	case 'tipo_prtda_prsp'
		...
		if sqlca.sqlcode = 100 then
			Messagebox( "Error", "Tipo de Cuenta Presupuestal no existe o no esta activo", Exclamation!)		
