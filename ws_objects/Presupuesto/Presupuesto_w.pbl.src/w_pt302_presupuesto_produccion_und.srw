$PBExportHeader$w_pt302_presupuesto_produccion_und.srw
forward
global type w_pt302_presupuesto_produccion_und from w_abc_master
end type
type st_1 from statictext within w_pt302_presupuesto_produccion_und
end type
type em_year from editmask within w_pt302_presupuesto_produccion_und
end type
type st_2 from statictext within w_pt302_presupuesto_produccion_und
end type
type em_cencos from editmask within w_pt302_presupuesto_produccion_und
end type
type st_3 from statictext within w_pt302_presupuesto_produccion_und
end type
type em_cod_art from editmask within w_pt302_presupuesto_produccion_und
end type
type cb_cencos from commandbutton within w_pt302_presupuesto_produccion_und
end type
type cb_cnta from commandbutton within w_pt302_presupuesto_produccion_und
end type
type cb_1 from commandbutton within w_pt302_presupuesto_produccion_und
end type
end forward
...
integer width = 2491
integer height = 2036
string title = "Presupuesto produccion (PT302)"
...
global w_pt302_presupuesto_produccion_und w_pt302_presupuesto_produccion_und
...
public subroutine of_retrieve (long an_ano, string as_cencos, string as_cod_art)
end prototypes
...
public subroutine of_retrieve (long an_ano, string as_cencos, string as_cod_art);Long ll_row

ll_row = dw_master.retrieve(an_ano, as_cencos, as_cod_art)
...
on w_pt302_presupuesto_produccion_und.create
int iCurrent
call super::create
...
on w_pt302_presupuesto_produccion_und.destroy
call super::destroy
...
sl_param.dw1 = "d_lista_presupuesto_produccion_und"
sl_param.titulo = "Presupuesto produccion"
...
if sl_param.titulo <> 'n' then
	of_retrieve(LONG( sl_param.field_ret[1]), sl_param.field_ret[2], sl_param.field_ret[3])
END IF
end event
...
type dw_master from w_abc_master`dw_master within w_pt302_presupuesto_produccion_und
event ue_display ( string as_columna,  long al_row )
integer x = 14
integer y = 112
integer width = 2405
integer height = 1720
string dataobject = "d_abc_presupuesto_produccion_und"
...
choose case lower(as_columna)
	...
	case "plantilla"
		...
		ls_sql = "select cod_plantilla as codigo_plantilla, " &
				 + "descipcion as descripcion_plantilla, " &
...
parent.of_retrieve(Long(em_year.text), em_cencos.text, em_cod_art.text)
of_limpia_datos()
