...
ls_where = "UPPER(" + ls_where + ") LIKE upper(" + ls_descripcion + ")"
...
do while ll_pos1 <> 0
	...
	ddlb_seleccion.InsertItem (trim(lower(ls_columna)), ll_i )
	...
	if string(ll_i) = istr_seleccionar.s_column then
		ddlb_seleccion.Text = trim(lower(ls_columna))
	end if
...
String 	ls_report_type, ls_style, ls_sql_syntax, ls_dw_err, &
			ls_dw_syntax, ls_type_font, ls_old_str, ls_new_str
Long		ll_long, ll_start_pos
...
//Rellenar los tabs por espacioes en blanco
ls_sql_syntax = mle_1.text

// Find the first occurrence of old_str.
ll_start_pos = Pos(ls_sql_syntax, ls_old_str, ll_start_pos)
// Only enter the loop if you find old_str.
DO WHILE ll_start_pos > 0
    // Replace old_str with new_str.
    ls_sql_syntax = Replace(ls_sql_syntax, ll_start_pos, 1, ls_new_str)
    // Find the next occurrence of old_str.
    ll_start_pos = Pos(ls_sql_syntax, ls_old_str, ll_start_pos + 1)
LOOP
