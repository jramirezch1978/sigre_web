...
$PBExportComments$ddplb que se carga desde datawindow
forward
global type u_ddplb from dropdownpicturelistbox
end type
end forward
...
Datastore     ids_data
Any               ia_key[], ia_id
Integer          ii_lc1, ii_lc2, ii_cn1, ii_cn2, ii_ck, ii_cp
String            is_dataobject, is_pic[], is_directory, is_def_pic
...
event ue_open_pre;//is_dataobject = 'd_product_list2'
//is_directory = 'c:\Program Files\Sybase\PB6\DemoDB\'  // Directorio de imagenes
//is_def_pic = 'capb.bmp'        // Imagen x defecto
...
event ue_item_add();Integer	li_index, li_x, li_pic
Long     ll_rows
Any  		la_id
String	ls_item, ls_pic
...
FOR li_x = 1 TO ll_rows
	...
	ls_pic = ids_data.object.data.primary.current[li_x, ii_cp]
	IF IsNull(ls_pic) THEN ls_pic = is_def_pic
	ls_pic = is_directory + ls_pic
	li_pic = THIS.AddPicture(ls_pic)
	li_index = THIS.AddItem(ls_item, li_pic)
...
event selectionchanged;IF index = 0 OR index > UpperBound(ia_key) THEN RETURN
