$PBExportHeader$n_cst_numlet.sru
$PBExportComments$Convierte numeros a literales
forward
global type n_cst_numlet from nonvisualobject
end type
end forward

global type n_cst_numlet from nonvisualobject
end type
global n_cst_numlet n_cst_numlet
...
public function string of_numfechlar (date ad_fecha);String	ls_fecha, ls_mes, ls_dia, ls_ano, ls_de, ls_temp
Integer	li_mes, li_pos, li_len
Decimal	ldc_dia, ldc_ano

li_mes = Month(ad_fecha)
ldc_dia = DAY(ad_fecha)
ldc_ano = Year(ad_fecha)
...
// convertir año
ls_ano = of_numlet(String(ldc_ano) + '.00')
li_pos = POS(ls_ano, ' y')
li_len = Len(ls_ano)
ls_ano = Left(ls_ano, li_pos -1)
IF Right(ls_ano, 2) = 'UN' THEN ls_ano = ls_ano + 'O'
	
IF ldc_ano >= 2000 THEN
	ls_de = ' del '
ELSE
	ls_de = ' de '
END IF

ls_fecha = ls_dia + ' de ' + ls_mes + ls_de + ls_ano
...
CHOOSE CASE li_numero
	...
	CASE 31 TO 39
		ls_resultado = "TREINTA Y " + of_numlet0(ls_digito)	
	CASE 40
		ls_resultado = "CUARENTA"
	CASE 41 TO 49
		ls_resultado = "CUARENTA Y " + of_numlet0(ls_digito)
	CASE 50
		ls_resultado = "CINCUENTA"
	CASE 51 TO 59
		ls_resultado = "CINCUENTA Y " + of_numlet0(ls_digito)
	CASE 60
		ls_resultado = "SESENTA"
	CASE 61 TO 69
		ls_resultado = "SESENTA Y " + of_numlet0(ls_digito)
	CASE 70
		ls_resultado = "SETENTA"
	CASE 71 TO 79
		ls_resultado = "SETENTA Y " + of_numlet0(ls_digito)
	CASE 80
		ls_resultado = "OCHENTA"
	CASE 81 TO 89
		ls_resultado = "OCHENTA Y " + of_numlet0(ls_digito)
	CASE 90
		ls_resultado = "NOVENTA"
	CASE 91 TO 99
		ls_resultado = "NOVENTA Y " + of_numlet0(ls_digito)
END CHOOSE
...
public function string of_numfech (date ad_fecha);String	ls_fecha, ls_mes, ls_del
...
ls_fecha = String(li_dia) + ' de ' + ls_mes + ls_del + String(li_ano)
...
public function integer of_check_separador ();Integer 	li_rc = 0, li_pos
Real		lr_test
String	ls_test

lr_test = (3/2) -1

ls_test = String(lr_test, '###,##0.00')

li_pos = Pos(ls_test, '.')

IF li_pos = 0 THEN
	li_rc = -1
	MessageBox('Error', 'Error en Configuracion Regional Separador de Decimales = ' &
		+ Mid(ls_test, Len(ls_test) - 2,1))
END IF
...
public function string of_numlet (string as_numero);String ls_resultado, ls_result1, ls_result2, ls_result3, ls_punto
String ls_millon, ls_mil, ls_resto, ls_entero, ls_dec
Integer li_punto, li_len, li_len_numero

if this.of_check_separador( ) = -1 then return 'ERROR'

as_numero = String(Round(Dec(as_numero), 2))
li_len_numero = Len(as_numero)

ls_punto = MID(as_numero, li_len_numero - 2, 1)  // Menos 2, por los 2 decimales. 

IF ls_punto <> '.' THEN 
 	ls_resultado = 'DEFINA SU CONFIGURACION REGIONAL' 
...
on n_cst_numlet.create
call super::create
TriggerEvent( this, "constructor" )
end on

on n_cst_numlet.destroy
