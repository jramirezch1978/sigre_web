...
FOR li_orig = 1 TO li_olen
	li_ktemp = asc(right(is_key, li_klen - li_kind + 1))	// tomar char del key
	li_work = asc(right(is_original, li_olen - li_orig + 1)) // tomar char del valor original
	li_work += li_ktemp
...
FOR li_orig = 1 to li_olen
	li_ktemp = asc(right(is_key, len(is_key) - li_kind + 1))
	li_work = asc(right(is_encriptado, len(is_encriptado) - li_orig + 1))
	li_work -= li_ktemp
