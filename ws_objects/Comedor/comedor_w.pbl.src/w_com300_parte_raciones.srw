$PBExportHeader$w_com300_parte_raciones.srw
forward
global type w_com300_parte_raciones from w_abc
end type
type st_4 from statictext within w_com300_parte_raciones
end type
type pb_1 from picturebutton within w_com300_parte_raciones
end type
type em_zona from singlelineedit within w_com300_parte_raciones
end type
type em_ot_adm from singlelineedit within w_com300_parte_raciones
end type
type hpb_1 from hprogressbar within w_com300_parte_raciones
end type
type dw_carga_ratios from datawindow within w_com300_parte_raciones
end type
type hpb_2 from hprogressbar within w_com300_parte_raciones
end type
type dw_carga_parte from datawindow within w_com300_parte_raciones
end type
type st_3 from statictext within w_com300_parte_raciones
end type
type pb_2 from picturebutton within w_com300_parte_raciones
end type
type st_campo from statictext within w_com300_parte_raciones
end type
type dw_text from datawindow within w_com300_parte_raciones
end type
type st_master from statictext within w_com300_parte_raciones
end type
type dw_ratios from u_dw_abc within w_com300_parte_raciones
end type
type sle_ot from sle_text within w_com300_parte_raciones
end type
type st_2 from statictext within w_com300_parte_raciones
end type
type cb_2 from commandbutton within w_com300_parte_raciones
end type
type st_ratios from statictext within w_com300_parte_raciones
end type
type dw_operaciones from u_dw_abc within w_com300_parte_raciones
end type
type sle_parte from singlelineedit within w_com300_parte_raciones
end type
type st_1 from statictext within w_com300_parte_raciones
end type
type cb_1 from commandbutton within w_com300_parte_raciones
end type
type st_operaciones from statictext within w_com300_parte_raciones
end type
type dw_detail from u_dw_abc within w_com300_parte_raciones
end type
type st_detail from statictext within w_com300_parte_raciones
end type
type dw_master from u_dw_abc within w_com300_parte_raciones
end type
type gb_1 from groupbox within w_com300_parte_raciones
end type
type gb_2 from groupbox within w_com300_parte_raciones
end type
type r_1 from rectangle within w_com300_parte_raciones
end type
type r_2 from rectangle within w_com300_parte_raciones
end type
end forward
...
integer width = 4274
integer height = 2172
string title = "Parte de Raciones Comedor (COM300)"
...
global w_com300_parte_raciones w_com300_parte_raciones
...
string				is_nro_parte, is_table, is_col, is_action, &
...
public function boolean of_llena_ratios (string as_nro_parte, string as_proveedor, string as_origen)
end prototypes
...
for ll_i = 1 to ll_count
    ...
    if isnull(ls_oper_sec)  or ls_oper_sec = '' or &
	 ...
	 Messagebox('Comedores','Campo requerido en OT/OperSec')
	 lb_resul = false
	 Exit
    End if
Next
...
IF is_action = 'new' THEN
	...
	IF ll_count = 0 THEN
		ls_lock_table = 'LOCK TABLE num_parte_rac IN EXCLUSIVE MODE'
		...
		INSERT INTO num_parte_rac(origen, ult_nro)
		VALUES( :gs_origen, 1);
...
DECLARE USP_PR_CARGA_RATIOS_C PROCEDURE FOR
	     USP_PR_CARGA_RATIOS_C(:as_nro_parte, :as_proveedor, :as_origen);

EXECUTE USP_PR_CARGA_RATIOS_C;

IF sqlca.sqlcode = -1 THEN
		messagebox( "Comedores", sqlca.sqlerrtext)
		 lb_result = false
		 return lb_result
END IF

CLOSE   USP_PR_CARGA_RATIOS_C;
...
on w_com300_parte_raciones.create
int iCurrent
call super::create
...
this.Control[iCurrent+3]=this.em_zona
this.Control[iCurrent+4]=this.em_ot_adm
this.Control[iCurrent+5]=this.hpb_1
this.Control[iCurrent+6]=this.dw_carga_ratios
this.Control[iCurrent+7]=this.hpb_2
this.Control[iCurrent+8]=this.dw_carga_parte
...
on w_com300_parte_raciones.destroy
call super::destroy
...
ib_log = TRUE
is_tabla_m = 'COM_PARTE_RAC'
is_tabla_d = 'COM_PARTE_RAC_DET'
is_tabla_r = 'COM_PARTE_RAC_RATIO'
...
if li_count > 0 then
	...
	Messagebox("Modulo de Comedores","No puede modificar un parte que ya ha sido amarrado a una Orden de Servicio")
	return
end if
else
...
CHOOSE CASE idw_1
	...
	CASE ELSE
		...
		IF li_estado = 2 THEN
		Messagebox("Modulo de Comedores","No puede hacer modificaciones, debido a que los costos de este parte ya han sido prorrateados")
		return
...
//Long 		ll_row_det, ll_row_mas, li_count
//Boolean	lb_ret
//string   ls_proveedor, ls_confin
...
//
//	if li_count = 0 then
//	Messagebox("Modulo de Comedores","El parte Diario debe de tener al menos un registro en en detalle. Por favor verifique")
//	return
//	end if
//
//end if
...
Boolean lbo_ok = TRUE
string  ls_nro_parte, ls_origen, ls_codigo
long    ll_master
dw_master.AcceptText()
dw_detail.AcceptText()
...
IF lbo_ok THEN
	...
	if is_action = 'new' then
		...
		ls_origen    = dw_master.object.origen_solicitud [ll_master]
		ls_codigo    = dw_master.object.proveedor        [ll_master]
	
		if of_llena_ratios(ls_nro_parte, ls_codigo, ls_origen) = false then 
			messagebox('Aviso', 'Problemas con el ingreso automatico de ratios')
			return
		end if
	
	   dw_ratios.retrieve(ls_nro_parte)
...
sl_param.dw1    = 'ds_parte_raciones_grid'
sl_param.titulo = "Partes Diario de Raciones"
sl_param.field_ret_i[1] = 1

sl_param.tipo    = '1SQL'
sl_param.string1 =  " WHERE SUBSTR(PARTE_RACION,1,2) = '"+ gs_origen + "'" 



OpenWithParm( w_lista, sl_param)
//OpenWithParm( w_lista_2, sl_param)
...
//		Messagebox("Modulo de Comedores","El parte de Comedores tiene registros en el detalle")
//	Return
//   end if
//end if
//
...
if ll_row_d > 0 then
	messagebox('Comedores', 'El PARTE YA TIENE ASIGNADO UN DETALLE, NO PUEDE PROCEDER')
	return
end if


if ll_row_r > 0 then
	messagebox('Comedores', 'El PARTE YA TIENE ASIGNADO RATIOS, NO PUEDE PROCEDER')
	return
end if
...
if ls_nro_parte_new = '' or IsNull(ls_nro_parte_new) then
	MessageBox('Aviso', 'Debe indicar un Nro. De parte')
	return
end if
...
DECLARE USP_PR_CARGA_PARTE_C PROCEDURE FOR
	     USP_PR_CARGA_PARTE_C(:ls_parte, :ls_nro_parte_new);

EXECUTE USP_PR_CARGA_PARTE_C;
...
IF ll_rac_ant >=1 THEN 
	...
	FOR ll_rec = 1 TO ll_rac_ant
		...
		is_parte_r				= dw_carga_ratios.object.parte_racion   	[1]
		is_tipo_r		 	   = dw_carga_ratios.object.flag_tipo_racion [1]
		is_ratio_r			 	= dw_carga_ratios.object.ratio         	[1]
   ...
   if dw_ratios.rowcount( ) <> ll_rac_ant then 
		messagebox(parent.title, "No se han copiado correctamente los registros del Ratio. Vuelva a intentarlo")
		dw_ratios.reset( )
	end if
...
IF ll_parte_ant >=1 THEN 
	...
	FOR ll_rec 					 = 1 TO ll_parte_ant
		...
		is_tipo_comedor		= dw_carga_parte.object.flag_tipo_rac     [1]
		ii_nro_raciones		= dw_carga_parte.object.nro_raciones      [1]
		is_nro_orden   		= dw_carga_parte.object.nro_orden         [1]
		is_oper_sec				= dw_carga_parte.object.oper_sec          [1]
		is_confin				= dw_carga_parte.object.confin            [1]
		is_desc_confin			= dw_carga_parte.object.descripcion       [1]
   ...
   if dw_detail.rowcount( ) <> ll_parte_ant then 
		messagebox(parent.title, "No se han copiado correctamente los registros del Parte. Vuelva a intentarlo")
		dw_detail.reset( )
	end if
...
ls_sql = "SELECT DISTINCT C.CENCOS AS CODIGO_CENCOS, " &
				  ...
				  + "FROM CENTROS_COSTO C, COM_PARTE_RAC_DET B " &
				  + "WHERE C.FLAG_ESTADO = '1' " &
				  + "AND C.CENCOS = B.CENCOS " &
				  + "AND B.PARTE_RACION= '" + ls_nro_parte + "'"
...
ls_flag_tipo	= dw_master.object.tipo_comedor [ll_row]
...
ls_sql = "SELECT C.PARTE_RACION AS NRO_PARTE, to_char(C.FECHA_PARTE_RAC, 'DD/MM/YYYY') AS FECHA_DE_PARTE, C.TIPO_COMEDOR " &
				  + "FROM COM_PARTE_RAC C " &
				  + "WHERE C.ORIGEN_SOLICITUD = '" + gs_origen + "'" &
				  + "AND C.TIPO_COMEDOR = '" + ls_flag_tipo + "'"
...
integer taborder = 30
boolean titlebar = true
string title = "Cargando Detalle del Parte de Piso..."
...
long textcolor = 16777215
long backcolor = 134217729
string text = "Cabecera del Parte de Raciones"
boolean border = true
borderstyle borderstyle = styleraised!
boolean focusrectangle = false
end type
...
integer height = 356
integer taborder = 30
string dataobject = "d_parte_racion_ratio_grid"
...
if ii_copia = 1 then
	...
	this.object.parte_racion			[al_row] = is_parte_r
	this.object.tipo_racion				[al_row] = is_tipo_r
	this.object.ratio						[al_row] = is_ratio_r
...
ELSE
	...
	this.object.parte_racion[al_row] = ls_nro_parte
	this.object.ratio			[al_row] = 1.00
...
choose case upper(dwo.name)
	case "TIPO_RACION"
		...
		ls_codigo = this.object.tipo_racion[row]
		
		if ls_codigo = '' or IsNull(ls_codigo) then
			SetColumn('tipo_racion')
			return 1
		end if
		
		ll_row_find = this.find("tipo_racion = '" + ls_codigo + "' and campo2 = ", 1, this.RowCount() )
		
		if ll_row_find > 0 and ll_row_find <> row then
			SetNull(ls_codigo)
			MessageBox('Aviso', 'Tipo de Ración ya ha sido ingresado', StopSign!)
			this.object.tipo_racion[row] = ls_codigo
			setColumn('tipo_racion')
			return 1
		end if
...
long textcolor = 16777215
long backcolor = 134217729
string text = "Ratios del Tipo de Ración"
boolean border = true
borderstyle borderstyle = styleraised!
boolean focusrectangle = false
end type
...
integer height = 996
integer taborder = 20
string dataobject = "d_parte_raciones_detalle_tbl"
boolean hscrollbar = true
boolean vscrollbar = true
borderstyle borderstyle = styleraised!
end type
...
choose case upper(as_columna)
	...
	case "OPER_SEC"
		...
		ls_sql = "SELECT OPER_SEC AS CODIGO, " &
				 ...
				 + "FROM OPERACIONES " &
				 + "WHERE FLAG_ESTADO IN ('1','3') " &
				 + "AND SERVICIO = (SELECT y.servicio_com FROM comedor_param y) " &
...
ll_row 			= dw_master.GetRow()
ls_nro_parte	= dw_master.object.nro_parte[ll_row]
ls_nro_orden	= dw_master.object.nro_orden[ll_row]
...
this.object.nro_orden	[al_row]	= ls_nro_orden
   ...
   this.object.zona_proceso 	[al_row] = is_zona
	...
	this.object.tipo_racion	   [al_row] = is_tipo_comedor
	this.object.nro_raciones 	[al_row] = ii_nro_raciones
	this.object.nro_orden	 	[al_row] = is_nro_orden
	this.object.oper_sec		 	[al_row] = is_oper_sec
	this.object.confin		 	[al_row] = is_confin
...
this.object.nro_parte	[al_row] = ls_nro_parte
this.object.nro_orden	[al_row]	= ls_nro_orden
this.object.nro_raciones[al_row] = 0
...
if source = dw_operaciones then
	...
	if ll_row_det = 0 then
		MessageBox('COMEDORES', 'DEBE ELEGIR UN REGISTRO PARA EFECTUAR ESTA OPERACION',StopSign!)
		return
	end if
...
choose case upper(dwo.name)
	...
	case "TIPO_RACION"
		...
		ll_row_find = dw_ratios.find( "tipo_racion = '" + ls_codigo + "'", 1, dw_ratios.RowCount() )
		
		if ll_row_find = 0 then
			SetNull(ls_codigo)
			MessageBox('Aviso', 'No ha ingresado el Ratio para este Tipo de Ración', StopSign!)
			this.object.tipo_racion[row] = ls_codigo
			SetColumn('tipo_racion')
			return 1
		end if
	...
	case "NRO_ORDEN"
		...
		if ll_count = 0 then 
			Messagebox('COMEDORES', "ORDEN DE TRABAJO NO EXISTE O NO ESTA ACTIVO", StopSign!)
			SetNull(ls_codigo)
	...
	case "OPER_SEC"
		...
		if ll_count = 0 then 
			Messagebox('COMEDORES', "OPER_SEC NO EXISTE, NO ESTA ACTIVO O NO CORRESPONDE A LA O.T.", StopSign!)
			SetNull(ls_codigo)
		return 1
		else
...
long textcolor = 16777215
long backcolor = 134217729
string text = "Detalles de las Raciones Diarias"
boolean border = true
borderstyle borderstyle = styleraised!
boolean focusrectangle = false
end type
...
integer width = 1335
integer height = 656
string dataobject = "d_parte_raciones_ff"
borderstyle borderstyle = styleraised!
end type

event ue_display(string as_columna, long al_row);boolean lb_ret
string ls_codigo, ls_data, ls_sql, ls_nro_parte, ls_origen
str_seleccionar lstr_seleccionar
...
choose case upper(as_columna)
	...
	case "TIPO_COMEDOR"
		...
		ls_sql = "SELECT TIPO_COMEDOR AS CODIGO, " &
				  + "DESCR_COMEDOR AS DESCRIPCION " &
				  + "FROM COM_TIPO_COMED " &
				  + "WHERE FLAG_ESTADO = '1'"
		...
		if ls_codigo <> '' then
			this.object.tipo_comedor[al_row] = ls_codigo
			this.object.desc_tipo_comedor	[al_row] = ls_data
			this.ii_update = 1
		end if
	...
	CASE "NRO_SOLICITUD"
		...
		ls_sql = "SELECT NRO_SOLICITUD AS CODIGO, " &
				 + "to_char(FECHA_REGISTRO, 'dd/mm/yyyy') AS FECHA_REGISTRO, " &
				 + "to_char(FECHA_EMISION, 'dd/mm/yyyy') AS FECHA_EMISION, " &
				 + "DESCRIPCION AS DESC_SOL_GIRO, " &
				 + "IMPORTE AS IMP_SOL_GIRO " &
				 + "FROM SOLICITUD_GIRO " &
				 + "WHERE FLAG_ESTADO IN ('1','2') " &
		...
		if ls_codigo <> '' then
			this.object.nro_solicitud[al_row] = ls_codigo
			this.ii_update = 1
		end if
	...
	case "PROVEEDOR"
		...
		ls_sql = "SELECT PROVEEDOR AS CODIGO, " &
				  ...
				  + "FROM PROVEEDOR " &
				  + "WHERE FLAG_ESTADO = '1' AND PROVEEDOR IN (SELECT P.PROVEEDOR FROM com_ratio_proveedor P)"
		...
		ls_nro_parte = this.object.nro_parte [al_row]
		
		if isnull(ls_nro_parte) or ls_nro_parte = '' then
			
			messagebox('Comedores', 'No se ha generado en Nro. Del parte. Tiene actualizaciones Pendientes') 
		
		else
			
			ls_nro_parte = this.object.nro_parte	     [al_row]
			ls_origen    = this.object.origen_solicitud [al_row]
...
this.object.origen_solicitud	[al_row] = gs_origen
...
if lb_ret = FALSE then
	SetNull(li_ano)
	SetNull(li_semana)
	MessageBox('COMEDORES', 'NO EXISTE FECHA EN MAESTRO DE SEMANAS DE PRODUCCIÓN',StopSign!)
end if
...
event itemchanged;call super::itemchanged;string 	ls_codigo, ls_data, ls_nro_parte, ls_origen
Date 		ld_fecha
Integer	li_ano, li_semana
Long		ll_count
Boolean	lb_ret
...
choose case upper(dwo.name)
	CASE "FECHA_PARTE"
		...
		if lb_ret = FALSE then
			SetNull(li_ano)
			SetNull(li_semana)
			MessageBox('COMEDORES', 'NO EXISTE FECHA EN MAESTRO DE SEMANAS',StopSign!)
		end if
	...
	case "TIPO_COMEDOR"
		...
		ls_codigo = this.object.tipo_comedor[row]

		SetNull(ls_data)
		select descr_comedor
			into :ls_data
		from com_tipo_comed
		where tipo_comedor = :ls_codigo
		  and flag_estado = '1';
		
		if IsNull(ls_data) or ls_data = "" then
			Messagebox('COMEDORES', "TIPO DE COMEDOR NO EXISTE O NO ESTA ACTIVO", StopSign!)
			SetNull(ls_codigo)
			this.object.tipo_comedor 	  [row] = ls_codigo
			this.object.desc_tipo_comedor[row] = ls_codigo
			return 1
		end if

		this.object.desc_tipo_comedor[row] = ls_data
	...
	case "NRO_ORDEN"
		...
		if ll_count = 0 then
			MessageBox('COMEDORES', 'NRO DE ORDEN NO EXISTE, POR FAVOR VERIFIQUE',StopSign!)
			SetNull(ls_codigo)
			this.object.nro_orden[row] = ls_codigo
			return 1
		end if
		...
		if isnull(ls_nro_parte) or ls_nro_parte = '' or ii_update = 1 then
			
			messagebox('Comedores', 'No se ha generado en Nro. Del parte. Tiene actualizaciones Pendientes')
			
			ls_nro_parte = this.object.nro_parte	     [row]
			ls_origen    = this.object.origen_solicitud [row]
...
event buttonclicked;call super::buttonclicked;u_dw_abc	ldw_1
string	ls_cencos_com, ls_desc_cen, ls_nro_ot,  &
			ls_ot_tipo, ls_ot_adm, ls_desc_ot_tipo, &
			ls_desc_ot_adm, ls_desc_ot
Long		ll_row, ll_row_mas, ll_row_this, ll_count
...
choose case lower(dwo.name)
	case 'b_lanz'
		ll_row_this = This.GetRow()
		
		if ll_row_this <= 0 then return
		
		ls_nro_ot = this.object.nro_orden[ll_row_this]
		
		if ls_nro_ot <> '' and Not Isnull(ls_nro_ot) then
			if MessageBox('COMEDORES', 'EL PARTE YA TIENE ASIGNADO UNA O.T., AUN DESEA GENERAR LA O.T.?', &
		...
		li_ano 	 = Integer(dw_master.object.ano[ll_row_mas])
		li_semana = Integer(dw_master.object.semana[ll_row_mas])
		
		select trunc(fecha_inicio), trunc(fecha_fin)
			into :ld_fecha1, :ld_fecha2
		from vw_pr_semanas_prod
		where ano = :li_ano
		  and semana = :li_semana;

		OpenSheet(w_com901_generar_ot_com, w_main, 0, Original!)
		
		
		ldw_1 	= w_com901_generar_ot_com.dw_master
		ll_row	= ldw_1.GetRow()
		...
		select desc_cencos
			into :ls_desc_cen
		from centros_costo
		where cencos = :ls_cencos_com;
		
		// Descripción de la O.T.
		ls_desc_ot = 'ORDEN DE TRABAJO DE COMEDORES, AÑO: ' &
					  + string(li_ano) + ' SEMANA: ' &
					  + string(li_semana)
					  
		// Busco el ot_tipo y el ot_adm de la ultima
		// OT que se asigno a un parte de raciones
		
		lb_ret = of_ot_tipo_adm_default(ls_ot_tipo, &
					ls_desc_ot_tipo, ls_ot_adm, 		  &
					ls_desc_ot_adm )
		
		ldw_1.SetRedraw(false)
		ldw_1.object.cencos_slc			[ll_row] = ls_cencos_com
		ldw_1.object.desc_cencos_slc	[ll_row] = ls_desc_cen
		ldw_1.object.cencos_rsp			[ll_row] = ls_cencos_com
		ldw_1.object.desc_cencos_rsp	[ll_row] = ls_desc_cen
		ldw_1.object.fec_desde			[ll_row] = ld_fecha1
		ldw_1.object.fec_hasta			[ll_row] = ld_fecha2
		ldw_1.object.descripcion		[ll_row] = ls_desc_ot
		if lb_Ret = true then
			ldw_1.object.ot_tipo			[ll_row] = ls_ot_tipo
			ldw_1.object.desc_ot_tipo	[ll_row] = ls_desc_ot_tipo
			ldw_1.object.ot_adm			[ll_row] = ls_ot_adm
			ldw_1.object.desc_ot_adm	[ll_row] = ls_desc_ot_adm
		end if
		...
		case 'b_importar'
			...
			string ls_parte_i
					 ...
					 ls_parte_i = this.object.parte [this.getrow()]
					 
					 messagebox('parte_impor', ls_parte_i)
			
			dw_detail.retrieve(ls_parte_i)
