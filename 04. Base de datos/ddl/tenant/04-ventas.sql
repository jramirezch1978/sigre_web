-- ============================================================
-- SIGRE ERP - Tenant DB - ventas schema
...
--   zona → auth.sucursal
--   mesa → zona
--   pedido_mesa → auth.sucursal, mesa (sesión de atención en salón/POS; no es OV comercial)
--   Flujo típico POS: pedido_mesa + comanda/comanda_det (cocina) → luego factura/boleta (fs_factura_simpl)
...
COMMENT ON TABLE ventas.comanda IS
    'Pedido que atiende cocina/barra (ítems a preparar). Se asocia a mesa/punto de venta; suele ser base para liquidar en boleta o factura electrónica (fs_factura_simpl).';
...
COMMENT ON TABLE ventas.pedido_mesa IS
    'Atención por mesa en salón/POS: apertura/cierre de la sesión en mesa. '
...
-- Orden de venta comercial (cabecera SIGRE ORDEN_VENTA / ERP). Distinta de pedido_mesa (POS/comanda).
...
COMMENT ON TABLE ventas.orden_venta IS
    'Orden de venta comercial (SIGRE ORDEN_VENTA): cliente, precios, despacho B2B. No es pedido_mesa ni comanda POS.';
