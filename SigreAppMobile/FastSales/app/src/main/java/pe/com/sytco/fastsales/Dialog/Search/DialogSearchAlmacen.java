...
public class DialogSearchAlmacen extends DialogSearchAncestor {
    private List<BeanAlmacen> listadoAlmacenes = null;

    private DialogSearchAlmacen(){

    }

    public DialogSearchAlmacen(Context value) {
        this.context = value;
    }
    ...
    protected void AsignarEventos() {
        ...
        lvListadoItems.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                try {
                    BeanAlmacen item = listadoAlmacenes.get(position);

                    if(bean instanceof BeanParteIngreso){
                        BeanParteIngreso obj = (BeanParteIngreso) bean;
                        obj.setAlmacen(item.getAlmacen());
                        obj.setDescAlmacen(item.getDescAlmacen());
    ...
    public void Filtrar() {
        ...
        new LoadAlmacenesTask(lsFiltro).execute();

    }


    //Clase Asincrona para tareas en segundo plano
    private class LoadAlmacenesTask extends AsyncTask<Integer, Void, Boolean> {
        ...
        private LoadAlmacenesTask()
        {

        }

        public LoadAlmacenesTask(String value){
            _filtro = value;
        }
        ...
        protected void onPreExecute() {
            ...
            pDialog.setMessage("Cargando Almacenes, por favor espere...");
        ...
        protected Boolean doInBackground(Integer... params) {
            try {
                ...
                listadoAlmacenes = implAlmacen.getAlmacenByUsuario(userLogin.getUsuario(), _filtro);
        ...
        protected void onPostExecute(Boolean result) {
            try {
                ...
                if (listadoAlmacenes.size() == 0) {
                    MessageBox.AlertDialog("No existen Almacen alguno que cumpla elfiltro ", "Error", context);
                    return;
                }

                adaptador = new ListViewSearchAdapter(context, BeanItemSearch.createListFromAlmacenes(listadoAlmacenes));
