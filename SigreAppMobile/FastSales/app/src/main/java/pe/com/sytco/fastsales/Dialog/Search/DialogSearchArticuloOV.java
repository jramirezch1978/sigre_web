...
public class DialogSearchArticuloOV extends DialogSearchAncestor {
    private List<BeanArticuloMovProy> listadoAMP = null;

    private EditText etOrdenVenta, etAlmacen, etArticulo;
    private TextView tvDescArticulo, tvOrgAMP, tvNroAMP;

    private DialogSearchArticuloOV(){

    }

    public DialogSearchArticuloOV(Context value) {
        this.context = value;
    }
    ...
    protected void AsignarEventos() {
        ...
        lvListadoItems.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                try {
                    BeanArticuloMovProy item = listadoAMP.get(position);
                    ...
                    ex.printStackTrace();
                    MessageBox.AlertDialog("Ha ocurrido un error al seleccionar un item del Detalle de ORDEN DE VENTA. Exception: " + ex.getMessage(),
    ...
    public void LoadData() {
        ...
        if (etOrdenVenta.getText() == null || etOrdenVenta.getText().toString().trim().equals("")){
            UTIL.SonidoError(context);
            MessageBox.AlertDialog(context, "Error", "No ha seleccionado una ORDEN DE VENTA, verifique!", false);
            etOrdenVenta.requestFocus();
            dialogMain.dismiss();
            return;
        }

        tvTitle.setText("DETALLE DE ORDEN DE VENTA");
        etBusqueda.setText("");
    ...
    public void Filtrar() {
        ...
        new LoadDetalleOVTask(etAlmacen.getText().toString(), etOrdenVenta.getText().toString(), lsFiltro).execute();
    ...
    private class LoadDetalleOVTask extends AsyncTask<Integer, Void, Boolean> {
        ...
        protected void onPostExecute(Boolean result) {
            try {
                ...
                adaptador = new ListViewSearchAdapter(context, BeanItemSearch.createListFromAMP(listadoAMP));
