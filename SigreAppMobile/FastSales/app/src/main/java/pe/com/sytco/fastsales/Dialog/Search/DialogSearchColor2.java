...
import pe.com.sytco.fastsales.Controller.Compras.ImplColor;
...
public class DialogSearchColor2  extends DialogSearchAncestor {

    private List<BeanColor> listadoColor = null;

    private EditText etColor2;
    private TextView tvDescColor2;

    private DialogSearchColor2(){

    }

    public DialogSearchColor2(View pLayoutReference) {
    ...
    protected void AsignarEventos() {
        ...
        lvListadoItems.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                try {
                    BeanColor item = listadoColor.get(position);

                    etColor2.setText(item.getCodColor());
                    tvDescColor2.setText(item.getDescColor());
                    ...
                    ex.printStackTrace();
                    MessageBox.AlertDialog("Ha ocurrido un error al seleccionar un item de COLORES. Exception: " + ex.getMessage(),
    ...
    public void LoadData() {
        ...
        tvTitle.setText("BUSQUEDA DE COLORES");
        etBusqueda.setText("");

        new LoadColoresTask("%%").execute();
    ...
    public void Filtrar() {
        ...
        new LoadColoresTask(lsFiltro).execute();

    }


    //Clase Asincrona para tareas en segundo plano
    private class LoadColoresTask extends AsyncTask<Integer, Void, Boolean> {

        private String mensaje, _filtro;
        private ImplColor implColor = null;
        private ArrayAdapter adaptador;

        private ProgressDialog pDialog;

        private LoadColoresTask()
        {

        }

        public LoadColoresTask(String value){
            _filtro = value;
        }
        ...
        protected void onPreExecute() {
            ...
            pDialog.setMessage("Cargando COLORES, por favor espere...");
        ...
        protected Boolean doInBackground(Integer... params) {
            try {
                ...
                implColor = new ImplColor(ImplEmpresa.empresaDefault.getCodigo());
                listadoColor = implColor.getActivosByFiltro(_filtro);
                ...
                mensaje = "Error al cargar las COLORES. Mensaje: " + ex.getMessage();
                ex.printStackTrace();
                return false;
            } finally {
                implColor = null;
        ...
        protected void onPostExecute(Boolean result) {
            try {
                ...
                if (listadoColor.size() == 0) {
                    UTIL.SonidoError(context);
                    MessageBox.AlertDialog("No existen MARCAS alguno que cumpla elfiltro ", "Error", context);
                    return;
                }

                adaptador = new ListViewSearchAdapter(context, BeanItemSearch.createListFromColores(listadoColor));
