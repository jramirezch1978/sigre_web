...
@DisplayName("Pruebas de Integración - PeriodoCtsController")
class PeriodoCtsControllerIntegrationTest {
    ...
    void flujoCRUD_completo() throws Exception {
        String crearJson = """
                {
                    "codigo": "X",
                    "nombre": "Período CTS Test"
                }
                """;

        String response = mockMvc.perform(post("/api/rrhh/periodos-cts")
                        .header("Authorization", authToken)
                ...
                .andExpect(jsonPath("$.data.nombre").value("Período CTS Test"))
                .andReturn().getResponse().getContentAsString();

        Long id = objectMapper.readTree(response).get("data").get("id").asLong();

        mockMvc.perform(get("/api/rrhh/periodos-cts/{id}", id)
                        .header("Authorization", authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
        ...
        String actualizarJson = """
                {
                    "nombre": "Período CTS Test Actualizado"
                }
                """;

        mockMvc.perform(put("/api/rrhh/periodos-cts/{id}", id)
                        .header("Authorization", authToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(actualizarJson))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.nombre").value("Período CTS Test Actualizado"));

        mockMvc.perform(patch("/api/rrhh/periodos-cts/{id}/desactivar", id)
                        .header("Authorization", authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
    }
    ...
    @Test
    @DisplayName("POST /api/rrhh/periodos-cts con código duplicado -> 409")
    void crear_codigoDuplicado_retorna409() throws Exception {
        String json = """
                {
                    "codigo": "DUP",
                    "nombre": "Período CTS Duplicado"
                }
                """;
        ...
        mockMvc.perform(post("/api/rrhh/periodos-cts")
                ...
                .andExpect(jsonPath("$.errorCode").value("RH-PC-001"));
    }

    @Test
    @DisplayName("GET /api/rrhh/periodos-cts/9999 -> 404")
    void obtenerPorId_idInexistente_retorna404() throws Exception {
        mockMvc.perform(get("/api/rrhh/periodos-cts/{id}", 9999)
                        .header("Authorization", authToken))
    ...
    @Test
    @DisplayName("PUT /api/rrhh/periodos-cts/9999 -> 404")
    void actualizar_idInexistente_retorna404() throws Exception {
        ...
        mockMvc.perform(put("/api/rrhh/periodos-cts/{id}", 9999)
                        .header("Authorization", authToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(json))
                .andExpect(status().isNotFound())
    ...
    @Test
    @DisplayName("DELETE /api/rrhh/periodos-cts/9999 -> 404")
    void eliminar_idInexistente_retorna404() throws Exception {
        mockMvc.perform(patch("/api/rrhh/periodos-cts/{id}/desactivar", 9999)
                        .header("Authorization", authToken))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.success").value(false));
    }
