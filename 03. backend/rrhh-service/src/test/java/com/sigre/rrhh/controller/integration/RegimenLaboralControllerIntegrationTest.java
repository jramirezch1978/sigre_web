...
@DisplayName("Pruebas de Integración - RegimenLaboralController")
class RegimenLaboralControllerIntegrationTest {
    ...
    void flujoCRUD_completo() throws Exception {
        ...
        String response = mockMvc.perform(post("/api/rrhh/regimenes-laborales")
                        .header("Authorization", authToken)
                ...
                .andExpect(jsonPath("$.data.nombre").value("Regimen Test"))
                .andReturn().getResponse().getContentAsString();

        Long id = objectMapper.readTree(response).get("data").get("id").asLong();

        mockMvc.perform(get("/api/rrhh/regimenes-laborales/{id}", id)
                        .header("Authorization", authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
        ...
        String actualizarJson = """
                {
                    "nombre": "Regimen Test Actualizado"
                }
                """;

        mockMvc.perform(put("/api/rrhh/regimenes-laborales/{id}", id)
                        .header("Authorization", authToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(actualizarJson))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.nombre").value("Regimen Test Actualizado"));

        mockMvc.perform(patch("/api/rrhh/regimenes-laborales/{id}/desactivar", id)
                        .header("Authorization", authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
    }
    ...
    @Test
    @DisplayName("POST /api/rrhh/regimenes-laborales con código duplicado -> 409")
    void crear_codigoDuplicado_retorna409() throws Exception {
        String json = """
                {
                    "codigo": "DUP",
                    "nombre": "Regimen Duplicado"
                }
                """;
        ...
        mockMvc.perform(post("/api/rrhh/regimenes-laborales")
                ...
                .andExpect(jsonPath("$.errorCode").value("RH-RL-001"));
    }

    @Test
    @DisplayName("GET /api/rrhh/regimenes-laborales/9999 -> 404")
    void obtenerPorId_idInexistente_retorna404() throws Exception {
        mockMvc.perform(get("/api/rrhh/regimenes-laborales/{id}", 9999)
                        .header("Authorization", authToken))
    ...
    @Test
    @DisplayName("PUT /api/rrhh/regimenes-laborales/9999 -> 404")
    void actualizar_idInexistente_retorna404() throws Exception {
        ...
        mockMvc.perform(put("/api/rrhh/regimenes-laborales/{id}", 9999)
                        .header("Authorization", authToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(json))
                .andExpect(status().isNotFound())
    ...
    @Test
    @DisplayName("DELETE /api/rrhh/regimenes-laborales/9999 -> 404")
    void eliminar_idInexistente_retorna404() throws Exception {
        mockMvc.perform(patch("/api/rrhh/regimenes-laborales/{id}/desactivar", 9999)
                        .header("Authorization", authToken))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.success").value(false));
    }
