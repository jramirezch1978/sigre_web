...
@DisplayName("Pruebas de Integración - VacacionController")
class VacacionControllerIntegrationTest {
    ...
    @Test
    @DisplayName("GET /api/rrhh/vacaciones -> retorna lista")
    ...
    @Test
    @DisplayName("POST -> GET -> PUT -> solicitar goce -> aprobar flujo completo")
    void flujoCRUD_completo() throws Exception {
        ...
        String response = mockMvc.perform(post("/api/rrhh/vacaciones")
                        .header("Authorization", authToken)
                ...
                .andExpect(jsonPath("$.data.periodoAnio").value(2026))
                .andExpect(jsonPath("$.data.diasDerecho").value(30))
                .andExpect(jsonPath("$.data.diasPendientes").value(30))
                .andReturn().getResponse().getContentAsString();

        Long id = objectMapper.readTree(response).get("data").get("id").asLong();

        mockMvc.perform(get("/api/rrhh/vacaciones/{id}", id)
                        .header("Authorization", authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
        ...
        mockMvc.perform(put("/api/rrhh/vacaciones/{id}", id)
                        .header("Authorization", authToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(actualizarJson))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.diasDerecho").value(15));
    }

    @Test
    @DisplayName("Flujo solicitar goce -> aprobar -> reprogramar")
    void flujoGoceVacacional() throws Exception {
        String crearJson = """
                {
                    "trabajadorId": %d,
                    "periodoAnio": 2026,
        ...
        mockMvc.perform(post("/api/rrhh/vacaciones/{id}/solicitar-goce", id)
                        .header("Authorization", authToken)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        mockMvc.perform(post("/api/rrhh/vacaciones/{id}/aprobar", id)
                        .header("Authorization", authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));

        mockMvc.perform(post("/api/rrhh/vacaciones/{id}/reprogramar", id)
                        .header("Authorization", authToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(solicitarJson))
                .andExpect(status().isOk())
    ...
    @Test
    @DisplayName("GET /api/rrhh/vacaciones/9999 -> 404")
    void obtenerPorId_idInexistente_retorna404() throws Exception {
        mockMvc.perform(get("/api/rrhh/vacaciones/{id}", 9999)
                        .header("Authorization", authToken))
