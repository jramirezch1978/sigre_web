...
@DisplayName("Pruebas Unitarias - SexoController")
class SexoControllerTest {
    ...
    @Test
    @DisplayName("GET /api/rrhh/sexos -> retorna lista paginada")
    void listar_retornaListaPaginada() throws Exception {
        Page<SexoResponse> page = new PageImpl<>(List.of(
                RrhhTestFixtures.sexoResponse(1L, "M", "Masculino")
        ));
        when(service.listar(any(), any(), any(), any())).thenReturn(page);

        mockMvc.perform(get("/api/rrhh/sexos")
                        .param("page", "0")
                        .param("size", "10"))
                ...
                .andExpect(jsonPath("$.data.content[0].codigo").value("M"));

        verify(service).listar(any(), any(), any(), any());
    }

    @Test
    @DisplayName("GET /api/rrhh/sexos con filtros -> aplica filtros")
    void listar_conFiltros_aplicaFiltros() throws Exception {
        Page<SexoResponse> page = new PageImpl<>(List.of(
                RrhhTestFixtures.sexoResponse(1L, "M", "Masculino")
        ));
        when(service.listar(eq("M"), eq("Masculino"), eq("1"), any())).thenReturn(page);

        mockMvc.perform(get("/api/rrhh/sexos")
                        .param("codigo", "M")
                        .param("nombre", "Masculino")
                        .param("flagEstado", "1")
                        .param("page", "0")
                        .param("size", "10"))
                .andExpect(status().isOk())
        ...
        verify(service).listar(eq("M"), eq("Masculino"), eq("1"), any());
    }

    @Test
    @DisplayName("GET /api/rrhh/sexos/{id} -> retorna entidad")
    void obtenerPorId_idExistente_retornaEntidad() throws Exception {
        when(service.obtenerPorId(1L)).thenReturn(
                RrhhTestFixtures.sexoResponse(1L, "M", "Masculino")
        );

        mockMvc.perform(get("/api/rrhh/sexos/{id}", 1L))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.id").value(1))
                .andExpect(jsonPath("$.data.codigo").value("M"))
                .andExpect(jsonPath("$.data.nombre").value("Masculino"));

        verify(service).obtenerPorId(1L);
    }

    @Test
    @DisplayName("POST /api/rrhh/sexos -> crea exitosamente")
    void crear_datosValidos_creaExitosamente() throws Exception {
        ...
        mockMvc.perform(post("/api/rrhh/sexos")
    ...
    @Test
    @DisplayName("PUT /api/rrhh/sexos/{id} -> actualiza exitosamente")
    void actualizar_datosValidos_actualizaExitosamente() throws Exception {
        ...
        when(service.actualizar(eq(1L), any())).thenReturn(
                RrhhTestFixtures.sexoResponse(1L, "F", "Femenino")
        );

        mockMvc.perform(put("/api/rrhh/sexos/{id}", 1L)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.nombre").value("Femenino"));

        verify(service).actualizar(eq(1L), any());
    }

    @Test
    @DisplayName("PATCH /api/rrhh/sexos/{id}/desactivar -> desactiva lógicamente")
    void desactivar_idExistente_desactiva() throws Exception {
        when(service.desactivar(1L)).thenReturn(
                RrhhTestFixtures.sexoResponse(1L, "M", "Masculino")
        );

        mockMvc.perform(patch("/api/rrhh/sexos/{id}/desactivar", 1L))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
    ...
    @Test
    @DisplayName("PATCH /api/rrhh/sexos/{id}/activar -> activa entidad")
    void activar_idExistente_activaExitosamente() throws Exception {
        when(service.activar(1L)).thenReturn(
                RrhhTestFixtures.sexoResponse(1L, "M", "Masculino")
        );

        mockMvc.perform(patch("/api/rrhh/sexos/{id}/activar", 1L))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
    ...
    @Test
    @DisplayName("GET /api/rrhh/sexos/activos -> retorna lista activos")
    void listarActivos_retornaLista() throws Exception {
        when(service.listarActivos()).thenReturn(List.of(
                RrhhTestFixtures.sexoResponse(1L, "M", "Masculino")
        ));

        mockMvc.perform(get("/api/rrhh/sexos/activos"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
