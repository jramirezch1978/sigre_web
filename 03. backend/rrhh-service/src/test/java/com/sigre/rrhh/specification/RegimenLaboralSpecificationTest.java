...
@DisplayName("RegimenLaboralSpecification - Pruebas Unitarias")
class RegimenLaboralSpecificationTest {

    @Mock private CriteriaBuilder cb;
    @Mock private CriteriaQuery<?> query;
    @Mock private Root<RegimenLaboral> root;

    @Test void filtros_todosNull_cbAndVacio() {
        Specification<RegimenLaboral> spec = RegimenLaboralSpecification.filtros(null, null, null);
    ...
    @Test void filtros_conCodigo_cbLike() {
        Specification<RegimenLaboral> spec = RegimenLaboralSpecification.filtros("COD", null, null);
    ...
    @Test void filtros_conNombre_cbLike() {
        Specification<RegimenLaboral> spec = RegimenLaboralSpecification.filtros(null, "Laboral", null);
        when(cb.lower(any())).thenReturn(mock(Expression.class));
        when(cb.like(any(), anyString())).thenReturn(mock(Predicate.class));
        when(cb.and(any(Predicate[].class))).thenReturn(mock(Predicate.class));
        assertThat(spec.toPredicate(root, query, cb)).isNotNull();
        verify(cb).lower(root.get("nombre"));
        verify(cb).like(any(), eq("%laboral%"));
    }

    @Test void filtros_conFlagEstado_cbEqual() {
        Specification<RegimenLaboral> spec = RegimenLaboralSpecification.filtros(null, null, "1");
    ...
    @Test void filtros_conTodos_todosPredicados() {
        Specification<RegimenLaboral> spec = RegimenLaboralSpecification.filtros("COD", "Laboral", "1");
        when(cb.lower(any())).thenReturn(mock(Expression.class));
        when(cb.like(any(), anyString())).thenReturn(mock(Predicate.class));
        when(cb.equal(root.get("flagEstado"), "1")).thenReturn(mock(Predicate.class));
        when(cb.and(any(Predicate[].class))).thenReturn(mock(Predicate.class));
    ...
    @Test void filtros_conFlagEstadoVacio_noAgrega() {
        Specification<RegimenLaboral> spec = RegimenLaboralSpecification.filtros(null, null, "");
        when(cb.and(any(Predicate[].class))).thenReturn(mock(Predicate.class));
        assertThat(spec.toPredicate(root, query, cb)).isNotNull();
        verify(cb).and(any(Predicate[].class));
    }
