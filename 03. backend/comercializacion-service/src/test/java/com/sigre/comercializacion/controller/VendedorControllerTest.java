...
class VendedorControllerTest {
    ...
    void findAll() {
        when(service.findAllWithFilters(any(), any(), any(), any(Pageable.class)))
                .thenReturn(new PageImpl<>(List.of(new Vendedor())));
        when(mapper.toResponseList(any())).thenReturn(List.of(new VendedorResponse()));
        assertThat(controller.findAll(Pageable.unpaged(), null, null, null).isSuccess()).isTrue();
    }

    @Test
    void findById() {
        when(service.findById(1L)).thenReturn(new Vendedor());
        when(mapper.toResponse(any())).thenReturn(new VendedorResponse());
    ...
    void create_update_delete_activate_deactivate() {
        ...
        when(mapper.toResponse(entity)).thenReturn(new VendedorResponse());
