...
public interface AsistenciaHt580Repository extends JpaRepository<AsistenciaHt580, String> {
    ...
    @Query(value = """
        SELECT
            ROW_NUMBER() OVER (ORDER BY a01.fec_marcacion, m.cod_trabajador) AS nro,
            m.cod_trabajador,
            ...
            a08.fec_marcacion AS hora_salida_produccion,
            a02.fec_marcacion AS hora_salida_planta,
            CASE
                WHEN a08.fec_marcacion IS NOT NULL AND a07.fec_marcacion IS NOT NULL
                THEN ROUND(EXTRACT(EPOCH FROM (a08.fec_marcacion - a07.fec_marcacion)) / 3600.0, 2)
            END AS horas_efectivas_produccion,
            CASE
                WHEN a02.fec_marcacion IS NOT NULL AND a01.fec_marcacion IS NOT NULL
                THEN ROUND(EXTRACT(EPOCH FROM (a02.fec_marcacion - a01.fec_marcacion)) / 3600.0, 2)
            END AS horas_total_planta,
            CASE
                WHEN a08.fec_marcacion IS NOT NULL AND a07.fec_marcacion IS NOT NULL
                     AND a02.fec_marcacion IS NOT NULL AND a01.fec_marcacion IS NOT NULL
                THEN ROUND(
                    EXTRACT(EPOCH FROM (a02.fec_marcacion - a01.fec_marcacion)) / 3600.0
                    - EXTRACT(EPOCH FROM (a08.fec_marcacion - a07.fec_marcacion)) / 3600.0
                , 2)
        ...
        WHERE TRIM(a01.flag_in_out) = '1'
          AND a01.fec_movimiento BETWEEN :fechaInicio AND :fechaFin
          AND a01.cod_origen = :codOrigen
          AND a07.reckey IS NOT NULL
        ORDER BY a01.fec_marcacion, m.cod_trabajador
        """,
        nativeQuery = true)
