...
public class Maestro {
    ...
    // Relación muchos a uno con CentrosCosto
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "CENCOS", referencedColumnName = "CENCOS", insertable = false, updatable = false)
    private CentrosCosto centroCostoEntity;
    
    // Relación uno a muchos con RrhhAsignaTrjtReloj
    @OneToMany(mappedBy = "trabajador", fetch = FetchType.LAZY)
    private List<RrhhAsignaTrjtReloj> tarjetasAsignadas;
    
    // Relación uno a muchos con RacionesSeleccionadas
    @OneToMany(mappedBy = "trabajador", fetch = FetchType.LAZY)
    private List<RacionesSeleccionadas> racionesSeleccionadas;
    ...
    // Método helper para verificar si marca reloj
    public boolean isMarcaReloj() {
        return "1".equals(flagMarcaReloj);
    }
}