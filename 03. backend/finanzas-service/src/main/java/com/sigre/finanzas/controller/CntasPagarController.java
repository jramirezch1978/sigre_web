...
public class CntasPagarController {
    ...
    /**
     * Maneja excepciones de comunicación con contabilidad-service.
     * Extrae el código de error y mensaje específico del response body.
     * 
     * @param ex Excepción de Feign al comunicarse con contabilidad-service
     * @return Response con el error específico de contabilidad-service
     */
    @ExceptionHandler(feign.FeignException.class)
    public ResponseEntity<ApiResponse<Void>> handleFeignException(feign.FeignException ex) {
        log.error("Error en comunicación con contabilidad-service: status={}, message={}", ex.status(), ex.getMessage());
        
        String errorMessage = "Error de comunicación con contabilidad-service";
        String errorCode = FinanzasErrorCodes.ERROR_COMUNICACION_CONTABILIDAD;
        HttpStatus httpStatus = HttpStatus.INTERNAL_SERVER_ERROR;
        
        try {
            String responseBody = ex.contentUTF8();
            log.error("Response body de contabilidad-service: {}", responseBody);
            ...
            if (responseBody != null && !responseBody.isEmpty()) {
                ...
                log.warn("Error de contabilidad-service capturado: code={}, message={}", errorCode, errorMessage);
            }
        } catch (Exception parseEx) {
            log.error("Error al parsear response de contabilidad-service", parseEx);
    ...
    public ApiResponse<PendientesPagarSimpleResponse> listarPendientesPorPagarSimple(
        ...
        log.info("GET /api/finanzas/cuentas-pagar/pendientes/simple - sucursal: {}, proveedor: {}, fechas: {} - {}", 
                sucursalId, proveedorId, fechaDesde, fechaHasta);
