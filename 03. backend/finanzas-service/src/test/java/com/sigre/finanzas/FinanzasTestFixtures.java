...
import com.sigre.finanzas.entity.*;
import com.sigre.finanzas.dto.request.*;
...
/**
 * Fixtures estáticas en RAM para pruebas unitarias y bodies JSON en integration tests.
 * Rol <b>C</b> según el estándar TEST_STANDARDS.md — sin acceso a BD ni Spring context.
 *
 * <p>Uso en unit tests:</p>
 * <pre>
 * CajaBancos entity = FinanzasTestFixtures.cajaBancos(1L, "C");
 * when(repository.findById(1L)).thenReturn(Optional.of(entity));
 * </pre>
 *
 * <p>Uso en integration tests (solo para bodies JSON):</p>
 * <pre>
 * String body = new ObjectMapper().writeValueAsString(
 *     FinanzasTestFixtures.crearCajaBancosRequestCobro());
 * </pre>
 *
 * <p>Migración: reemplaza progresivamente a {@code testutil.TestDataFactory} (legacy).
 * Ambos conviven durante la transición; no es big-bang.</p>
 *
 * @see com.sigre.finanzas.testdata.FinanzasTestDataFactory (B — JDBC, @Component en main)
 */
public final class FinanzasTestFixtures {

    private FinanzasTestFixtures() {
    ...
    public static CajaBancos cajaBancos(Long id, String flagTipoTransaccion) {
    ...
    public static CajaBancosDet cajaBancosDet(Long id, CajaBancos cajaBancos) {
    ...
    public static List<CajaBancosDet> listaCajaBancosDet(CajaBancos cajaBancos, int cantidad) {
        List<CajaBancosDet> detalles = new ArrayList<>();
        for (int i = 1; i <= cantidad; i++) {
            detalles.add(cajaBancosDet((long) i, cajaBancos));
        }
        return detalles;
    }

    public static CntasPagar cntasPagar(Long id) {
    ...
    public static ConceptoFinanciero conceptoFinanciero(Long id) {
    ...
    public static List<ConceptoFinanciero> listaConceptoFinanciero() {
        List<ConceptoFinanciero> conceptos = new ArrayList<>();
        conceptos.add(conceptoFinanciero(1L));
        conceptos.add(conceptoFinanciero(2L));
        conceptos.add(conceptoFinanciero(3L));
        conceptos.add(conceptoFinanciero(4L));
        conceptos.add(conceptoFinanciero(5L));
        conceptos.add(conceptoFinanciero(6L));
        return conceptos;
    }
