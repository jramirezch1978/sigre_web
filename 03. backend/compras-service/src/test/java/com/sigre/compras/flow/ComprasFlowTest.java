package com.sigre.compras.flow;
...
import com.sigre.compras.service.*;
import com.sigre.compras.service.impl.*;
...
class ComprasFlowTest {
    ...
    // ══════════════════════════════════════════════════════════════════════
    // VALIDACIÓN: No enviar a aprobación si el flujo no está activo
    // ══════════════════════════════════════════════════════════════════════

    @Test
    @DisplayName("No se puede enviar a aprobación si COMPRA_APROBACION_OC=0")
    void noEnviaAprobacionSiFlujoInactivo() {
        mockAprobacionRequerida(false);

        assertThatThrownBy(() -> ocService.enviarAprobacion(1L))
                .isInstanceOf(BusinessException.class)
                .hasMessageContaining("flujo de aprobación no está activo");
    }
    ...
    void recepcionEnAlmacenInvocaFeign() {
        ...
        lenient().when(almacenClient.recepcionOrdenCompra(any(IntegracionRecepcionOcRequest.class)))
                .thenReturn(com.sigre.common.dto.ApiResponse.ok(movResp));
    ...
    private void mockAprobacionRequerida(boolean requerida) {
        ConfiguracionRef conf = mock(ConfiguracionRef.class);
        when(conf.getValorTexto()).thenReturn(requerida ? "1" : "0");
        when(configuracionRefRepository.findFirstByParametro("COMPRA_APROBACION_OC")).thenReturn(Optional.of(conf));
    }
}
