...
@Component({
  selector: 'app-semester-picker',
  templateUrl: './semester-picker.component.html',
  styleUrls: ['./semester-picker.component.scss'],
  standalone: false
})
export class SemesterPickerComponent implements OnInit {
  ...
  @Input() placeholder: string = 'Seleccionar semestre';
  @Input() disabled: boolean = false;
  @Input() position: 'left' | 'center' | 'right' | 'top-left' = 'left';
  @Output() semesterYearChange = new EventEmitter<{startMonth: number, endMonth: number, year: number}>();
  ...
  selectMonth(month: number) {
    ...
    const endMonth = month + 5 > 12 ? month + 5 - 12 : month + 5;
    this.semesterYearChange.emit({
      startMonth: month,
      endMonth: endMonth,
  ...
  isMonthSelected(month: number): boolean {
    ...
    // Marcar el mes seleccionado y los 5 siguientes
    const endMonth = this.selectedStartMonth + 5;
    if (endMonth <= 12) {
      return month >= this.selectedStartMonth && month <= endMonth;
    } else {
      // Si cruza el año (ej: Ago, Set, Oct, Nov, Dic, Ene)
  ...
  getDisplayText(): string {
    if (this.selectedStartMonth && this.selectedYear) {
      ...
      const endMonth = this.selectedStartMonth + 5 > 12 ? this.selectedStartMonth + 5 - 12 : this.selectedStartMonth + 5;
