import { Injectable, inject, effect } from '@angular/core';
import { ToastService } from 'src/app/ui/services/toast.service';
import { NumTrasladoStore } from '../store/num-traslado.store';

/**
 * Effects de feedback para Numerador de Traslados.
 * Muestra toasts de éxito/error tras cada operación CRUD.
 */
@Injectable()
export class NumTrasladoFeedbackEffects {
  private readonly store        = inject(NumTrasladoStore);
  private readonly toastService = inject(ToastService);

  constructor() {
    // Feedback guardar
    effect(() => {
      const result = this.store.resultGuardar();
      if (result?.success) {
        this.toastService.success('Numerador de traslado guardado correctamente');
      }
    });

    effect(() => {
      const error = this.store.errorGuardar();
      if (error) {
        this.toastService.danger(error);
      }
    });

    // Feedback actualizar
    effect(() => {
      const result = this.store.resultActualizar();
      if (result?.success) {
        this.toastService.success('Numerador de traslado actualizado correctamente');
      }
    });

    effect(() => {
      const error = this.store.errorActualizar();
      if (error) {
        this.toastService.danger(error);
      }
    });

    // Feedback eliminar
    effect(() => {
      const result = this.store.resultEliminar();
      if (result?.success) {
        this.toastService.success('Numerador de traslado eliminado correctamente');
      }
    });

    effect(() => {
      const error = this.store.errorEliminar();
      if (error) {
        this.toastService.danger(error);
      }
    });
  }
}
