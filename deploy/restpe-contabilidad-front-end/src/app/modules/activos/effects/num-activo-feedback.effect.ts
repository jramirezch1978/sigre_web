import { Injectable, inject, effect } from '@angular/core';
import { ToastService } from 'src/app/ui/services/toast.service';
import { NumActivoStore } from '../store/num-activo.store';

/**
 * Effects de feedback para Numerador de Activos.
 * Muestra toasts de éxito/error tras cada operación CRUD.
 */
@Injectable()
export class NumActivoFeedbackEffects {
  private readonly store        = inject(NumActivoStore);
  private readonly toastService = inject(ToastService);

  constructor() {
    // Feedback guardar
    effect(() => {
      const result = this.store.resultGuardar();
      if (result?.success) {
        this.toastService.success('Numerador de activo guardado correctamente');
      }
    });

    effect(() => {
      const error = this.store.errorGuardar();
      if (error) {
        this.toastService.danger(error);
      }
    });

    // Feedback actualizar
    effect(() => {
      const result = this.store.resultActualizar();
      if (result?.success) {
        this.toastService.success('Numerador de activo actualizado correctamente');
      }
    });

    effect(() => {
      const error = this.store.errorActualizar();
      if (error) {
        this.toastService.danger(error);
      }
    });

    // Feedback eliminar
    effect(() => {
      const result = this.store.resultEliminar();
      if (result?.success) {
        this.toastService.success('Numerador de activo eliminado correctamente');
      }
    });

    effect(() => {
      const error = this.store.errorEliminar();
      if (error) {
        this.toastService.danger(error);
      }
    });
  }
}
