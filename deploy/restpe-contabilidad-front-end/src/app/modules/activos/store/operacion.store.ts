...
import { OperacionState, initialOperacionState } from './operacion.state';
import { OperacionEntity } from '../domain/models/operacion.entity';
import { ApiResponse } from '../../../shared/models/api-response.model';

@Injectable()
export class OperacionStore {
  private readonly _state = signal<OperacionState>(initialOperacionState);

  // ── Selectores ──────────────────────────────────────────────────────────────
  readonly operaciones              = computed(() => this._state().operaciones);
  readonly operacionSeleccionada    = computed(() => this._state().operacionSeleccionada);
  ...
  // ── Setters ─────────────────────────────────────────────────────────────────
  setOperaciones(items: OperacionEntity[])                { this._state.update(s => ({ ...s, operaciones: items })); }
  setOperacionSeleccionada(item: OperacionEntity | null)  { this._state.update(s => ({ ...s, operacionSeleccionada: item })); }
  ...
  reset() { this._state.set(initialOperacionState); }
}
