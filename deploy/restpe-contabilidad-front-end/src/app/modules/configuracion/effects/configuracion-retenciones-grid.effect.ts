...
/**
 * @summary Effects para sincronización de grilla de retenciones
 * @description
 * Maneja los efectos secundarios reactivos para la grilla AG-Grid de retenciones:
 * - Actualiza rowData cuando cambian las retenciones en el store
 ...
 */
@Injectable()
export class ConfiguracionRetencionesGridEffects {

  /** @summary Store reactivo con el estado de configuración (retenciones + loading). */
  ...
  setGridApi(api: GridApi): void {
    ...
    const loading = this.store.loadingRetenciones();
    if (loading) {
   ...
   */
  setRowData(data: RetencionEntity[]): void {
    this.rowData = data;
    if (this.gridApi) {
  ...
  /**
   * @summary Effect: actualiza rowData cuando cambian las retenciones en el store.
   * @description
   * Observa el signal `retenciones()` del store y actualiza automáticamente
   * la grilla cuando los datos cambian. No requiere suscripciones manuales.
   */
  private actualizarRowDataEffect(): void {
    effect(() => {
      const retenciones = this.store.retenciones();
      this.rowData = retenciones;
   ...
   * @description
   * Observa el signal `loadingRetenciones()` y muestra/oculta
