...
/**
 * @summary Effects para sincronización de grilla de compañías
 * @description
 * Maneja los efectos secundarios reactivos para la grilla AG-Grid de compañías:
 * - Actualiza rowData cuando cambian las compañías en el store
 ...
 */
@Injectable()
export class ConfiguracionCompaniasGridEffects {

  /** @summary Store reactivo con el estado de configuración (compañías + loading). */
  ...
  setGridApi(api: GridApi): void {
    ...
    const loading = this.store.loadingCompanias();
    if (loading) {
   ...
   */
  setRowData(data: CompaniaEntity[]): void {
    this.rowData = data;
    if (this.gridApi) {
  ...
  /**
   * @summary Effect: actualiza rowData cuando cambian las compañías en el store.
   * @description
   * Observa el signal `companias()` del store y actualiza automáticamente
   * la grilla cuando los datos cambian. No requiere suscripciones manuales.
   */
  private actualizarRowDataEffect(): void {
    effect(() => {
      const companias = this.store.companias();
      this.rowData = companias;
   ...
   * @description
   * Observa el signal `loadingCompanias()` y muestra/oculta
