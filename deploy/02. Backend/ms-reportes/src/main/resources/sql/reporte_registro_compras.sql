...
cp_base AS (
    SELECT
        ...
        CASE
            WHEN dt.codigo IN ('NCP', 'NDP', 'CNC', 'CDB') THEN -1
            ELSE 1
        END AS flag_signo,
        CASE
...
cp_refs AS (
    ...
    WHERE cpd.flag_estado <> '0'
      ...
      AND dt_nc.codigo IN ('NCP', 'NDP', 'CNC', 'CDB')
...
SELECT
    ...
    COALESCE((SELECT SUM((CASE WHEN cp.moneda_id = pen.moneda_pen_id THEN cpi.importe ELSE cpi.importe * cp.tasa_cambio_efectiva END)
        ...
        WHERE cpd.cntas_pagar_id = cp.id AND cpd.flag_estado <> '0'
          ...
          AND ti.flag_igv = '1' AND ti.id NOT IN (SELECT id_isc FROM imp UNION SELECT id_perc FROM imp)), 0) AS ln_igv01,
    COALESCE((SELECT SUM((CASE WHEN cp.moneda_id = pen.moneda_pen_id THEN cpd.monto ELSE cpd.monto * cp.tasa_cambio_efectiva END) * cp.flag_signo)
        FROM finanzas.cntas_pagar_det cpd CROSS JOIN pen
        WHERE cpd.cntas_pagar_id = cp.id AND cpd.flag_estado <> '0'
    ...
    COALESCE((SELECT SUM((CASE WHEN cp.moneda_id = pen.moneda_pen_id THEN cpi.importe ELSE cpi.importe * cp.tasa_cambio_efectiva END)
        ...
        WHERE cpd.cntas_pagar_id = cp.id AND cpd.flag_estado <> '0'
          ...
          AND ti.flag_igv = '1' AND ti.id NOT IN (SELECT id_isc FROM imp UNION SELECT id_perc FROM imp)), 0) AS ln_igv02,
    COALESCE((SELECT SUM((CASE WHEN cp.moneda_id = pen.moneda_pen_id THEN cpd.monto ELSE cpd.monto * cp.tasa_cambio_efectiva END) * cp.flag_signo)
        FROM finanzas.cntas_pagar_det cpd CROSS JOIN pen
        WHERE cpd.cntas_pagar_id = cp.id AND cpd.flag_estado <> '0'
    ...
    COALESCE((SELECT SUM((CASE WHEN cp.moneda_id = pen.moneda_pen_id THEN cpi.importe ELSE cpi.importe * cp.tasa_cambio_efectiva END)
        ...
        WHERE cpd.cntas_pagar_id = cp.id AND cpd.flag_estado <> '0'
          ...
          AND ti.flag_igv = '1' AND ti.id NOT IN (SELECT id_isc FROM imp UNION SELECT id_perc FROM imp)), 0) AS ln_igv03,
    COALESCE((SELECT SUM((CASE WHEN cp.moneda_id = pen.moneda_pen_id THEN cpd.monto ELSE cpd.monto * cp.tasa_cambio_efectiva END) * cp.flag_signo)
        FROM finanzas.cntas_pagar_det cpd CROSS JOIN pen
        WHERE cpd.cntas_pagar_id = cp.id AND cpd.flag_estado <> '0'
    ...
    + COALESCE((SELECT SUM((CASE WHEN cp.moneda_id = pen.moneda_pen_id THEN cpi.importe ELSE cpi.importe * cp.tasa_cambio_efectiva END)
        ...
        WHERE cpd.cntas_pagar_id = cp.id AND cpd.flag_estado <> '0'
          ...
          AND ti.flag_igv = '1' AND ti.id NOT IN (SELECT id_isc FROM imp UNION SELECT id_perc FROM imp)), 0) AS ln_base04,
    ...
    COALESCE((SELECT SUM((CASE WHEN cp.moneda_id = pen.moneda_pen_id THEN cpi.importe ELSE cpi.importe * cp.tasa_cambio_efectiva END)
        ...
        JOIN core.tipos_impuesto ti ON ti.id = cpi.tipos_impuesto_id
        CROSS JOIN pen
        WHERE cpd.cntas_pagar_id = cp.id AND cpd.flag_estado <> '0' AND ti.id = (SELECT id_isc FROM imp)), 0) AS ln_isc,
    ...
    COALESCE((SELECT SUM((CASE WHEN cp.moneda_id = pen.moneda_pen_id THEN cpi.importe ELSE cpi.importe * cp.tasa_cambio_efectiva END)
        ...
        JOIN core.tipos_impuesto ti ON ti.id = cpi.tipos_impuesto_id
        CROSS JOIN pen
        WHERE cpd.cntas_pagar_id = cp.id AND cpd.flag_estado <> '0' AND ti.id = (SELECT id_perc FROM imp)), 0) AS ln_percepcion,
    ...
    COALESCE((SELECT SUM((CASE WHEN cp.moneda_id = pen.moneda_pen_id THEN cpi.importe ELSE cpi.importe * cp.tasa_cambio_efectiva END)
        ...
        CROSS JOIN pen
        WHERE cpd.cntas_pagar_id = cp.id AND cpd.flag_estado <> '0'
          AND ti.flag_igv = '0' AND ti.id NOT IN (SELECT id_isc FROM imp UNION SELECT id_perc FROM imp)), 0) AS ln_otros,

    COALESCE((SELECT SUM((CASE WHEN cp.moneda_id = pen.moneda_pen_id THEN cpd.monto ELSE cpd.monto * cp.tasa_cambio_efectiva END) * cp.flag_signo)
        FROM finanzas.cntas_pagar_det cpd CROSS JOIN pen WHERE cpd.cntas_pagar_id = cp.id AND cpd.flag_estado <> '0'), 0)
    + COALESCE((SELECT SUM((CASE WHEN cp.moneda_id = pen.moneda_pen_id THEN cpi.importe ELSE cpi.importe * cp.tasa_cambio_efectiva END)
            * CASE WHEN ti.signo = '-' THEN -1 ELSE 1 END * cp.flag_signo)
        FROM finanzas.cntas_pagar_det cpd
        JOIN finanzas.cntas_pagar_det_imp cpi ON cpi.cntas_pagar_det_id = cpd.id
