...
/**
 * Tests unitarios para CajaBancosController usando MockitoExtension.
 * Enfoque híbrido: usa mocks para service pero TestDataFactory para datos realistas.
 * 
 * @author Equipo de Desarrollo
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("Pruebas Unitarias - CajaBancosController")
class CajaBancosControllerTest {
    ...
    void setUp() {
        ...
        // Configurar datos de prueba usando TestDataFactory
        setupTestData();
    }
    ...
    private void setupTestData() {
        ...
        // No hay método setObservación en CajaBancosResponse
        ...
        cajaBancosDetalleResponse.setObservacion("Cobro de prueba");
        ...
        // No hay método setMontoEjecutado en EjecutarMovimientoResponse
    ...
    @Test
    @DisplayName("Debe filtrar movimientos por cuenta bancaria")
    void listar_ConFiltroBancoCntaId_DebeRetornarFiltrados() throws Exception {
        // Arrange
        Page<CajaBancosResponse> page = new PageImpl<>(List.of(cajaBancosResponse));
        when(service.listar(any(), eq(1L), any(), any(), any(), any(), any())).thenReturn(page);
        ...
        mockMvc.perform(get("/api/finanzas/caja-bancos")
                ...
                .andExpect(jsonPath("$.data.content").isArray())
                .andExpect(jsonPath("$.data.content.length()").value(1))
                .andExpect(jsonPath("$.data.content[0].bancoCntaId").value(1));

        verify(service).listar(any(), eq(1L), any(), any(), any(), any(), any());
    }

    @Test
    @DisplayName("Debe filtrar movimientos por fecha")
    void listar_ConFiltroFecha_DebeRetornarFiltrados() throws Exception {
        // Arrange
        Page<CajaBancosResponse> page = new PageImpl<>(List.of(cajaBancosResponse));
        String fechaHoy = java.time.LocalDate.now().toString();
        when(service.listar(any(), any(), eq(java.time.LocalDate.now()), eq(java.time.LocalDate.now()), any(), any(), any())).thenReturn(page);
        ...
        verify(service).listar(any(), any(), eq(java.time.LocalDate.now()), eq(java.time.LocalDate.now()), any(), any(), any());
    }
    ...
    void actualizar_ConIdExistente_DebeActualizarYRetornar() throws Exception {
        // Arrange
        CajaBancosRequest request = TestDataFactory.crearCajaBancosRequestCobro();
        // No hay método setObservación en CajaBancosDetalleResponse
