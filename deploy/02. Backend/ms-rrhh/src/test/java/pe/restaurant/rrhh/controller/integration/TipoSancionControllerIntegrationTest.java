...
@DisplayName("Pruebas de Integración - TipoSancionController")
class TipoSancionControllerIntegrationTest {
    ...
    void flujoCRUD_completo() throws Exception {
        ...
        String response = mockMvc.perform(post("/api/rrhh/tipos-sancion")
                ...
                .andExpect(jsonPath("$.data.nombre").value("Sanción Test"))
                .andReturn().getResponse().getContentAsString();

        Long id = objectMapper.readTree(response).get("data").get("id").asLong();

        mockMvc.perform(get("/api/rrhh/tipos-sancion/{id}", id)
                        .header("Authorization", authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
        ...
        String actualizarJson = """
                {
                    "nombre": "Sanción Actualizada"
                }
                """;

        mockMvc.perform(put("/api/rrhh/tipos-sancion/{id}", id)
                        .header("Authorization", authToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(actualizarJson))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.nombre").value("Sanción Actualizada"));

        mockMvc.perform(patch("/api/rrhh/tipos-sancion/{id}/desactivar", id)
                        .header("Authorization", authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
    }
    ...
    @Test
    @DisplayName("POST /api/rrhh/tipos-sancion con código duplicado -> 409")
    void crear_codigoDuplicado_retorna409() throws Exception {
        String json = """
                {
                    "codigo": "DUP",
                    "nombre": "Sanción Duplicada"
                }
                """;
        ...
        mockMvc.perform(post("/api/rrhh/tipos-sancion")
                ...
                .andExpect(jsonPath("$.errorCode").value("RH-TS-002"));
    }

    @Test
    @DisplayName("GET /api/rrhh/tipos-sancion/9999 -> 404")
    void obtenerPorId_idInexistente_retorna404() throws Exception {
        mockMvc.perform(get("/api/rrhh/tipos-sancion/{id}", 9999)
                        .header("Authorization", authToken))
    ...
    @Test
    @DisplayName("PUT /api/rrhh/tipos-sancion/9999 -> 404")
    void actualizar_idInexistente_retorna404() throws Exception {
        ...
        mockMvc.perform(put("/api/rrhh/tipos-sancion/{id}", 9999)
                        .header("Authorization", authToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(json))
                .andExpect(status().isNotFound())
    ...
    @Test
    @DisplayName("PATCH /api/rrhh/tipos-sancion/9999/desactivar -> 404")
    void desactivar_idInexistente_retorna404() throws Exception {
        mockMvc.perform(patch("/api/rrhh/tipos-sancion/{id}/desactivar", 9999)
                        .header("Authorization", authToken))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.success").value(false));
    }
