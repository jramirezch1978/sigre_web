...
@DisplayName("TipoConceptoCalculoSpecification - Pruebas Unitarias")
class TipoConceptoCalculoSpecificationTest {

    @Mock private CriteriaBuilder cb;
    @Mock private CriteriaQuery<?> query;
    @Mock private Root<TipoConceptoCalculo> root;

    @Test void filtros_todosNull_cbAndVacio() {
        Specification<TipoConceptoCalculo> spec = TipoConceptoCalculoSpecification.filtros(null, null, null);
    ...
    @Test void filtros_conCodigo_cbLike() {
        Specification<TipoConceptoCalculo> spec = TipoConceptoCalculoSpecification.filtros("COD", null, null);
    ...
    @Test void filtros_conNombre_cbLike() {
        Specification<TipoConceptoCalculo> spec = TipoConceptoCalculoSpecification.filtros(null, "Ingreso", null);
        when(cb.lower(any())).thenReturn(mock(Expression.class));
        when(cb.like(any(), anyString())).thenReturn(mock(Predicate.class));
        when(cb.and(any(Predicate[].class))).thenReturn(mock(Predicate.class));
        assertThat(spec.toPredicate(root, query, cb)).isNotNull();
    ...
    @Test void filtros_conFlagEstado_cbEqual() {
        Specification<TipoConceptoCalculo> spec = TipoConceptoCalculoSpecification.filtros(null, null, "1");
    ...
    @Test void filtros_conTodos_todosPredicados() {
        Specification<TipoConceptoCalculo> spec = TipoConceptoCalculoSpecification.filtros("COD", "Ingreso", "1");
        when(cb.lower(any())).thenReturn(mock(Expression.class));
        when(cb.like(any(), anyString())).thenReturn(mock(Predicate.class));
        when(cb.equal(root.get("flagEstado"), "1")).thenReturn(mock(Predicate.class));
        when(cb.and(any(Predicate[].class))).thenReturn(mock(Predicate.class));
    ...
    @Test void filtros_conFlagEstadoVacio_noAgrega() {
        Specification<TipoConceptoCalculo> spec = TipoConceptoCalculoSpecification.filtros(null, null, "");
        when(cb.and(any(Predicate[].class))).thenReturn(mock(Predicate.class));
        assertThat(spec.toPredicate(root, query, cb)).isNotNull();
        verify(cb).and(any(Predicate[].class));
    }
