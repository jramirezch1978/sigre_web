...
class LiquidacionControllerIntegrationTest {
    ...
    @Test
    @DisplayName("GET /api/rrhh/liquidaciones/{id} -> 404 para ID inexistente")
    void obtener_idInexistente_retorna404() throws Exception {
        mockMvc.perform(get("/api/rrhh/liquidaciones/{id}", 9999)
                        .header("Authorization", authToken))
    ...
    @Test
    @Disabled("Requires seed data: trabajador con fecha_cese y contrato activo")
    @DisplayName("POST /api/rrhh/liquidaciones/calcular -> necesita seed data")
    void calcular_requiereSeedData() {
    }
}
