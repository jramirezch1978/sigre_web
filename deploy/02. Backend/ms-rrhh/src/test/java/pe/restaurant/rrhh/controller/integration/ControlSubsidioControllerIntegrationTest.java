...
@DisplayName("Pruebas de Integración - ControlSubsidioController")
class ControlSubsidioControllerIntegrationTest {
    ...
    @Test
    @DisplayName("GET /api/rrhh/subsidios -> retorna lista")
    ...
    @Test
    @DisplayName("POST -> GET -> PUT -> PATCH desactivar flujo completo CRUD")
    void flujoCRUD_completo() throws Exception {
        String crearJson = """
                {
                    ...
                    "fechaFin": "2026-01-15",
                    "dias": 15,
                    "montoSubsidio": 500.0000
                }
                """.formatted(trabajadorId);

        String response = mockMvc.perform(post("/api/rrhh/subsidios")
                        .header("Authorization", authToken)
                ...
                .andExpect(jsonPath("$.data.montoSubsidio").value(500.0))
                .andReturn().getResponse().getContentAsString();

        Long id = objectMapper.readTree(response).get("data").get("id").asLong();

        mockMvc.perform(get("/api/rrhh/subsidios/{id}", id)
                        .header("Authorization", authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
        ...
        String actualizarJson = """
                {
                    ...
                    "fechaFin": "2026-03-15",
                    "dias": 15,
                    "montoSubsidio": 700.0000,
                    "flagEstado": "1"
                }
                """;

        mockMvc.perform(put("/api/rrhh/subsidios/{id}", id)
                        .header("Authorization", authToken)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        mockMvc.perform(patch("/api/rrhh/subsidios/{id}/desactivar", id)
                        .header("Authorization", authToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));
    }
    ...
    @Test
    @DisplayName("GET /api/rrhh/subsidios/9999 -> 404")
    void obtenerPorId_idInexistente_retorna404() throws Exception {
        mockMvc.perform(get("/api/rrhh/subsidios/{id}", 9999)
                        .header("Authorization", authToken))
