...
class AfTrasladoControllerTest {
    ...
    void aprobar() {
        ...
        assertThat(result.getMessage()).contains("aprobado");
    }

    @Test
    void rechazar() {
        when(service.rechazar(1L, "Observacion de rechazo")).thenReturn(entity);
        when(mapper.toResponse(entity)).thenReturn(response);
        var result = controller.rechazar(1L, "Observacion de rechazo");
        assertThat(result.getMessage()).contains("rechazado");
    }
