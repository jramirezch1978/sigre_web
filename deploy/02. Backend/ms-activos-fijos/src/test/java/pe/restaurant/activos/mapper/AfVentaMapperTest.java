package pe.restaurant.activos.mapper;

import org.junit.jupiter.api.Test;
import org.mapstruct.factory.Mappers;
import pe.restaurant.activos.dto.AfVentaRequest;
import pe.restaurant.activos.dto.AfVentaResponse;
import pe.restaurant.activos.entity.AfVenta;

import java.math.BigDecimal;
import java.time.LocalDate;
import java.util.List;

import static org.assertj.core.api.Assertions.assertThat;

class AfVentaMapperTest {

    private final AfVentaMapper mapper = Mappers.getMapper(AfVentaMapper.class);

    @Test
    void toResponse_mapsAllFields() {
        AfVenta entity = new AfVenta();
        entity.setId(1L);
        entity.setAfMaestroId(10L);
        entity.setFechaBaja(LocalDate.of(2025, 6, 30));
        entity.setMotivo("VENTA");
        entity.setValorVenta(new BigDecimal("35000.0000"));
        entity.setComprador("Inversiones ABC SAC");
        entity.setEstado("COMPLETADO");
        entity.setTipoBaja("VENTA");
        entity.setTipoDocumentoVenta("FACTURA");
        entity.setNumeroDocumento("F001-00123");
        entity.setDepreciacionAcumulada(new BigDecimal("15000.0000"));
        entity.setValorNetoContable(new BigDecimal("35000.0000"));
        entity.setResultadoBaja(new BigDecimal("0.0000"));
        entity.setTipoSiniestro("NINGUNO");
        entity.setMontoIndemnizacion(new BigDecimal("0.0000"));
        entity.setMotivoObsolescencia("N/A");
        entity.setDescripcionDetalle("Venta programada de equipo obsoleto");

        AfVentaResponse response = mapper.toResponse(entity);

        assertThat(response.getId()).isEqualTo(1L);
        assertThat(response.getAfMaestroId()).isEqualTo(10L);
        assertThat(response.getFechaBaja()).isEqualTo(LocalDate.of(2025, 6, 30));
        assertThat(response.getMotivo()).isEqualTo("VENTA");
        assertThat(response.getValorVenta()).isEqualByComparingTo(new BigDecimal("35000.0000"));
        assertThat(response.getComprador()).isEqualTo("Inversiones ABC SAC");
        assertThat(response.getEstado()).isEqualTo("COMPLETADO");
        assertThat(response.getTipoBaja()).isEqualTo("VENTA");
        assertThat(response.getTipoDocumentoVenta()).isEqualTo("FACTURA");
        assertThat(response.getNumeroDocumento()).isEqualTo("F001-00123");
        assertThat(response.getDepreciacionAcumulada()).isEqualByComparingTo(new BigDecimal("15000.0000"));
        assertThat(response.getValorNetoContable()).isEqualByComparingTo(new BigDecimal("35000.0000"));
        assertThat(response.getResultadoBaja()).isEqualByComparingTo(new BigDecimal("0.0000"));
        assertThat(response.getTipoSiniestro()).isEqualTo("NINGUNO");
        assertThat(response.getMontoIndemnizacion()).isEqualByComparingTo(new BigDecimal("0.0000"));
        assertThat(response.getMotivoObsolescencia()).isEqualTo("N/A");
        assertThat(response.getDescripcionDetalle()).isEqualTo("Venta programada de equipo obsoleto");
    }

    @Test
    void toResponse_nullReturnsNull() {
        assertThat(mapper.toResponse(null)).isNull();
    }

    @Test
    void toResponseList_mapsAll() {
        AfVenta e1 = new AfVenta();
        e1.setId(1L);
        e1.setMotivo("VENTA");
        AfVenta e2 = new AfVenta();
        e2.setId(2L);
        e2.setMotivo("SINIESTRO");

        List<AfVentaResponse> result = mapper.toResponseList(List.of(e1, e2));

        assertThat(result).hasSize(2);
        assertThat(result.get(0).getMotivo()).isEqualTo("VENTA");
        assertThat(result.get(1).getMotivo()).isEqualTo("SINIESTRO");
    }

    @Test
    void toEntity_mapsAllFields() {
        AfVentaRequest request = new AfVentaRequest();
        request.setAfMaestroId(10L);
        request.setFechaBaja(LocalDate.of(2025, 6, 30));
        request.setMotivo("VENTA");
        request.setValorVenta(new BigDecimal("35000.0000"));
        request.setComprador("Inversiones ABC SAC");
        request.setTipoBaja("VENTA");
        request.setTipoDocumentoVenta("FACTURA");
        request.setNumeroDocumento("F001-00123");
        request.setTipoSiniestro("NINGUNO");
        request.setMontoIndemnizacion(new BigDecimal("0.0000"));
        request.setMotivoObsolescencia("N/A");
        request.setDescripcionDetalle("Venta programada");

        AfVenta entity = mapper.toEntity(request);

        assertThat(entity.getAfMaestroId()).isEqualTo(10L);
        assertThat(entity.getFechaBaja()).isEqualTo(LocalDate.of(2025, 6, 30));
        assertThat(entity.getMotivo()).isEqualTo("VENTA");
        assertThat(entity.getValorVenta()).isEqualByComparingTo(new BigDecimal("35000.0000"));
        assertThat(entity.getComprador()).isEqualTo("Inversiones ABC SAC");
        assertThat(entity.getTipoBaja()).isEqualTo("VENTA");
        assertThat(entity.getTipoDocumentoVenta()).isEqualTo("FACTURA");
        assertThat(entity.getNumeroDocumento()).isEqualTo("F001-00123");
        assertThat(entity.getTipoSiniestro()).isEqualTo("NINGUNO");
        assertThat(entity.getMontoIndemnizacion()).isEqualByComparingTo(new BigDecimal("0.0000"));
        assertThat(entity.getMotivoObsolescencia()).isEqualTo("N/A");
        assertThat(entity.getDescripcionDetalle()).isEqualTo("Venta programada");
        assertThat(entity.getId()).isNull();
        assertThat(entity.getEstado()).isNotNull();
        assertThat(entity.getDepreciacionAcumulada()).isNull();
        assertThat(entity.getValorNetoContable()).isNull();
        assertThat(entity.getResultadoBaja()).isNull();
    }

    @Test
    void toEntity_nullReturnsNull() {
        assertThat(mapper.toEntity(null)).isNull();
    }

    @Test
    void updateEntity_mapsFieldsExceptIgnored() {
        AfVenta entity = new AfVenta();
        entity.setId(99L);
        entity.setEstado("EN_PROCESO");
        entity.setDepreciacionAcumulada(new BigDecimal("10000.0000"));
        entity.setValorNetoContable(new BigDecimal("40000.0000"));
        entity.setResultadoBaja(new BigDecimal("5000.0000"));
        entity.setAfMaestroId(1L);

        AfVentaRequest request = new AfVentaRequest();
        request.setAfMaestroId(50L);
        request.setFechaBaja(LocalDate.of(2026, 2, 28));
        request.setMotivo("OBSOLESCENCIA");
        request.setValorVenta(new BigDecimal("5000.0000"));
        request.setComprador("Recicladora Perú SRL");
        request.setTipoBaja("OBSOLESCENCIA");
        request.setTipoDocumentoVenta("BOLETA");
        request.setNumeroDocumento("B001-00456");
        request.setTipoSiniestro(null);
        request.setMontoIndemnizacion(null);
        request.setMotivoObsolescencia("Tecnología superada");
        request.setDescripcionDetalle("Equipo ya no cumple especificaciones");

        mapper.updateEntity(request, entity);

        assertThat(entity.getId()).isEqualTo(99L);
        assertThat(entity.getEstado()).isEqualTo("EN_PROCESO");
        assertThat(entity.getDepreciacionAcumulada()).isEqualByComparingTo(new BigDecimal("10000.0000"));
        assertThat(entity.getValorNetoContable()).isEqualByComparingTo(new BigDecimal("40000.0000"));
        assertThat(entity.getResultadoBaja()).isEqualByComparingTo(new BigDecimal("5000.0000"));
        assertThat(entity.getAfMaestroId()).isEqualTo(50L);
        assertThat(entity.getFechaBaja()).isEqualTo(LocalDate.of(2026, 2, 28));
        assertThat(entity.getMotivo()).isEqualTo("OBSOLESCENCIA");
        assertThat(entity.getValorVenta()).isEqualByComparingTo(new BigDecimal("5000.0000"));
        assertThat(entity.getComprador()).isEqualTo("Recicladora Perú SRL");
        assertThat(entity.getTipoBaja()).isEqualTo("OBSOLESCENCIA");
        assertThat(entity.getTipoDocumentoVenta()).isEqualTo("BOLETA");
        assertThat(entity.getNumeroDocumento()).isEqualTo("B001-00456");
        assertThat(entity.getMotivoObsolescencia()).isEqualTo("Tecnología superada");
        assertThat(entity.getDescripcionDetalle()).isEqualTo("Equipo ya no cumple especificaciones");
    }
}
