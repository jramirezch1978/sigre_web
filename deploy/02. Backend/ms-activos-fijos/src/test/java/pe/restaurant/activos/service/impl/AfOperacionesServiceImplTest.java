...
class AfOperacionesServiceImplTest {
    ...
    @Mock
    private AfTipoOperacionRepository tipoOperacionRepository;
    @InjectMocks
    ...
    void createSavesEntity() {
        try (MockedStatic<TenantContext> tc = mockStatic(TenantContext.class)) {
            ...
            entity.setAfTipoOperacionId(3L);
            ...
            when(tipoOperacionRepository.findById(3L)).thenReturn(Optional.of(tipo));
    ...
    void createThrowsWhenActivoNotFound() {
        ...
        entity.setAfTipoOperacionId(null);
    ...
    @Test
    void createThrowsWhenTipoOperacionInactivo() {
        AfMaestro maestro = new AfMaestro();
        maestro.setId(10L);

        AfTipoOperacion tipo = new AfTipoOperacion();
        tipo.setId(3L);
        tipo.setFlagEstado("0");
        ...
        BusinessException ex = assertThrows(BusinessException.class, () -> service.create(entity));
        assertEquals(ActivosErrorCodes.TIPO_OPERACION_NO_ENCONTRADO, ex.getErrorCode());
    }

    @Test
    void createWithNullTipoOperacionSkipsValidation() {
        try (MockedStatic<TenantContext> tc = mockStatic(TenantContext.class)) {
            tc.when(TenantContext::getUsuarioId).thenReturn(5L);
            ...
            verify(tipoOperacionRepository, never()).findById(anyLong());
        }
    }
    ...
    void updateModifiesEntity() {
        try (MockedStatic<TenantContext> tc = mockStatic(TenantContext.class)) {
            ...
            updateData.setAfTipoOperacionId(4L);
            ...
            when(tipoOperacionRepository.findById(4L)).thenReturn(Optional.of(tipo));
