...
public class ContabilidadIntegracionServiceImpl implements ContabilidadIntegracionService {
    ...
    private final AfTrasladoRepository trasladoRepository;
    ...
    public IntegracionContabilidadResult contabilizarVenta(Long ventaId) {
        ...
        if (venta.getCntblAsientoId() != null) {
            return resultadoExistente(AfIntegracionContableModulo.MODULO, ventaId, venta.getCntblAsientoId(),
                    correlacionVenta(venta));
        }
        ...
        if (existente != null) {
            venta.setCntblAsientoId(existente.getAsientoId());
        ...
        venta.setCntblAsientoId(asientoId);
        venta.setUpdatedBy(TenantContext.getUsuarioId());
        ventaRepository.save(venta);
    ...
    public IntegracionContabilidadResult contabilizarAdaptacion(Long adaptacionId) {
        ...
        if (!"CAPITALIZADO".equals(adaptacion.getEstado())) {
            throw new BusinessException(
                    "La adaptación debe estar capitalizada antes de contabilizar",
                    HttpStatus.BAD_REQUEST,
                    ActivosErrorCodes.ADAPTACION_NO_CAPITALIZADA);
        }

        if (adaptacion.getCntblAsientoId() != null) {
            return resultadoExistente(AfIntegracionContableModulo.MODULO, adaptacionId,
                    adaptacion.getCntblAsientoId(), correlacionAdaptacion(adaptacion));
        }
        ...
        if (existente != null) {
            adaptacion.setCntblAsientoId(existente.getAsientoId());
        ...
        adaptacion.setCntblAsientoId(asientoId);
        adaptacion.setUpdatedBy(TenantContext.getUsuarioId());
        adaptacionRepository.save(adaptacion);
    ...
    @Override
    @Transactional
    public IntegracionContabilidadResult contabilizarTraslado(Long trasladoId) {
        verificarContabilidadHabilitada();
        AfTraslado traslado = trasladoRepository.findById(trasladoId)
                .orElseThrow(() -> new ResourceNotFoundException("Traslado", trasladoId));

        if (!"EJECUTADO".equals(traslado.getEstado())) {
            throw new BusinessException(
                    "Solo se contabilizan traslados ejecutados (actual: " + traslado.getEstado() + ")",
                    HttpStatus.BAD_REQUEST,
                    ActivosErrorCodes.TRASLADO_ESTADO_INVALIDO);
        }

        if (traslado.getCntblAsientoId() != null) {
            return resultadoExistente(AfIntegracionContableModulo.MODULO, trasladoId,
                    traslado.getCntblAsientoId(), correlacionTraslado(traslado));
        }

        Long ccOrigen = traslado.getCentroCostoOrigenId();
        Long ccDestino = traslado.getCentroCostoDestinoId();
        if (ccOrigen == null || ccDestino == null || ccOrigen.equals(ccDestino)) {
            throw new BusinessException(
                    "El traslado debe tener centros de costo origen y destino distintos",
                    HttpStatus.BAD_REQUEST,
                    ActivosErrorCodes.TRASLADO_SIN_CAMBIO_CC);
        }
        ...
        BigDecimal valorNeto = obtenerValorNetoActivo(activo.getId(), activo.getValorAdquisicion());
        if (valorNeto.compareTo(BigDecimal.ZERO) <= 0) {
            throw new BusinessException(
                    "Valor neto del activo es cero; no aplica asiento de traslado",
                    HttpStatus.BAD_REQUEST,
                    ActivosErrorCodes.TRASLADO_VALOR_NETO_CERO);
        }

        String correlacion = correlacionTraslado(traslado);
        IntegracionContabilidadResult existente = resolverIdempotencia(
                AfIntegracionContableModulo.MODULO, trasladoId, correlacion);
        if (existente != null) {
            traslado.setCntblAsientoId(existente.getAsientoId());
        ...
        List<AsientoDetalleRequest> detalles = List.of(
                detalle(matriz.getCuentaActivoId(), ccDestino, valorNeto, BigDecimal.ZERO,
                        "Traslado CC destino", correlacion),
                detalle(matriz.getCuentaActivoId(), ccOrigen, BigDecimal.ZERO, valorNeto,
                        "Traslado CC origen", correlacion));

        Long asientoId = crearAsientoRemoto(
                AfIntegracionContableModulo.TRASLADO,
                trasladoId,
                traslado.getFechaEjecucion() != null ? traslado.getFechaEjecucion() : LocalDate.now(),
                "Traslado activo " + activo.getCodigo() + " CC " + ccOrigen + " → " + ccDestino,
                detalles);

        traslado.setCntblAsientoId(asientoId);
        traslado.setUpdatedBy(TenantContext.getUsuarioId());
        trasladoRepository.save(traslado);
        registrarHistorialContable(activo.getId(), "TRASLADO_CONTABILIZADO", asientoId, correlacion);

        return IntegracionContabilidadResult.builder()
                .asientoId(asientoId)
                .moduloOrigen(AfIntegracionContableModulo.MODULO)
                .documentoOrigenId(trasladoId)
                .correlacion(correlacion)
                .yaExistia(false)
                .build();
    }
    ...
    private static String correlacionTraslado(AfTraslado t) {
        return "AF|MAESTRO=" + t.getAfMaestroId() + "|TRASLADO=" + t.getId();
    }
