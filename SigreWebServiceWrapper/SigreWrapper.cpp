    ...
    const wchar_t* fecha)
{
    ...
    if (fecha && wcslen(fecha) > 0) {
        ...
        // Detectar formato dd/MM/yyyy (10 chars, con /)
    ...
    // Construir path: /tipo-cambio/YYYY-MM-DD.json
    wchar_t path[256];
    ...
    // Llamada directa a la API publica (HTTPS, sin JWT)
    if (HttpRequest(L"free.e-api.net.pe", 443, path, L"GET",
                    L"Accept: application/json\r\n", NULL, response, sizeof(response), TRUE))
    {
        // Envolver la respuesta en formato consistente con el resto de la DLL
        // La API retorna: {"fecha":"2026-04-13","sunat":3.382,"compra":3.379,"venta":3.385}
        if (wcsstr(response, L"\"fecha\"") != NULL && wcsstr(response, L"\"sunat\"") != NULL) {
            ...
            swprintf_s(g_result, 16384, 
                L"{\"success\":false,\"mensaje\":\"Respuesta inesperada de la API\",\"raw\":%s}", response);
